...
sap.ui.define([
...
], function (library, Core,LocaleData, AxisTimeStrategyBase, Format, TimeHorizon, CalendarWeekNumbering, CalendarUtils, GanttChartConfigurationUtils, WeekPattern, DayInterval, SkipInterval,BaseRectangle,GanttQUnitUtils) {
	"use strict";
    ...
    QUnit.module("test time format", {
        beforeEach: function () {
            var oShape = [
                new BaseRectangle({
                    shapeId: "0",
                    time: Format.abapTimestampToDate("20181002140000"),
                    endTime: Format.abapTimestampToDate("20181022140000"),
                    height: 20
                })];
    ...
    QUnit.test("Test 12hour and 24hour format", function (assert) {
        var done = assert.async();
        GanttQUnitUtils.waitForGanttRendered(this.gantt).then(function(){
            var oShape = this.gantt.getTable().getRows()[0].getAggregation("_settings").getShapes1()[0];
            var oFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({ pattern: "HH:mm" }, new sap.ui.core.Locale("en"));
            assert.strictEqual(oFormat.format(oShape.getTime()), "14:00", "Test 24hour format");
            oFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({ pattern: "hh:mm a" }, new sap.ui.core.Locale("en"));
            assert.strictEqual(oFormat.format(oShape.getTime()), "02:00 PM", "Test 12hour format");
            done();
        }.bind(this));
    });
