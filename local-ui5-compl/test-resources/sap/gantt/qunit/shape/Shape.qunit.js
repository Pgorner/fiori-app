...
sap.ui.define([
...
], function (Shape, ShapeConfig, TimeHorizon, GanttChart, Parameters, GanttChartConfigurationUtils) {
	"use strict";
	...
	QUnit.module("Create Shape directly and test Private methods._formatting()", {
		beforeEach: function () {
			...
			sap.gantt.misc.Format._oDefaultDateTimeFormat = sap.ui.core.format.DateFormat.getDateTimeWithTimezoneInstance({
				showTimezone: false
			}, oLocale);
			this.oShape.mShapeConfig = this.oShapeConfig;
			this.oShape.mChartInstance = new GanttChart({
	...
	QUnit.test("Shape.getStyle default value", function (assert) {
		var expectedResult = {
			"stroke": this.sColorValue,
			"stroke-width": 1
		};
		assert.deepEqual(this.oShape.getStyle(), expectedResult, "default style is correct");
		this.oShape.setStroke(null);
		expectedResult = {
			"stroke": undefined,
			"stroke-width": 1
		};
		assert.deepEqual(this.oShape.getStyle(), expectedResult, "invalid style is removed");
		this.oShape.setStroke(undefined);
		expectedResult = {
			"stroke": undefined,
			"stroke-width": 1
		};
		assert.deepEqual(this.oShape.getStyle(), expectedResult, "getStyle has correct inline value");
	});
	QUnit.test("LESS parameter cached test", function (assert) {
		...
		assert.strictEqual(sValue, Parameters.get("sapUiChart1"), "@sapUiChart1 has correct cached value");
	});
});
