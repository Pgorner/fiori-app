sap.ui.define([
	...
	"sap/gantt/simple/PrintConfig"
],  function (Controller, JSONModel, GanttChartWithTable, StepwiseZoomStrategy, TimeHorizon, TreeTable, Column, Label, Text, GanttChartContainer,
		    ...
		    GanttPrinting, CalendarWeekNumbering, Core, AutoRowMode, GanttChartConfigurationUtils, Lib,PrintDialogTemplate,Button,PrintConfig) {
	"use strict";

	var TimeUnit = GanttLibrary.config.TimeUnit;

	var oRb = Lib.getResourceBundleFor("sap.gantt");
	var oLocaleData = LocaleData.getInstance(GanttChartConfigurationUtils.getFormatLanguageTag());
	...
	return Controller.extend("sap.gantt.sample.GanttPrinting.Gantt", {
		onInit: function () {
			...
			this.oGantt = new GanttChartWithTable({
				...
				printingBatchSize: 75
			});

			var oContainer = this.getView().byId("ganttContainer");
			oContainer.setStatusBarDatePattern(sCalendarWeekPattern);
		...
		onExportPDF: function () {
			...
			if (!this.printConfig){
				//Creates a simple print configuration for SAP Gantt.
			...
			this.oGanttPrinting.setPrintData(this.printConfig);
			this.oGanttPrinting.open(fnUpdateTable,this.footerButtons);
		},
		onExportSinglePage: function () {
			...
			if (!this.printConfigSinglePage){
				//Creates a simple printConfiguration for SAP Gantt.
				this.printConfigSinglePage = new PrintConfig({
					pageConfig: {
						paperHeight: 11,
						paperWidth: 8,
						paperSize: "A4",
			...
			this.oGanttPrintingSinglePage.setPrintData(this.printConfigSinglePage);
			this.oGanttPrintingSinglePage.openPdfPreview(fnUpdateTable,this.footerButtons);
		},
