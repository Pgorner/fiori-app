sap.ui.define([
	...
	"sap/ui/core/Theming",
	"sap/m/Popover"
], function (Controller, MockServer, ODataModel, Relationship, AdhocLine, Utility, Element, Theming, Popover) {
	"use strict";
	...
	return Controller.extend("sap.gantt.sample.GanttChart2OData.GanttChart2OData", {
		...
		onShapeContextMenu: function(oEvent) {
			var oShape = oEvent.getParameter("shape");
			var oJQueryEvent = oEvent.getParameter("originEvent");
			...
			if (oShape instanceof Relationship) {
				...
				oContextMenu.openAsContextMenu(oJQueryEvent, oShape);
			}
		},
		...
		handlePopoverOnHover: function (oEvent) {
			var oShape = oEvent.getParameter("shape");
			var iPageX = oEvent.getParameter("pageX");
			var iPageY = oEvent.getParameter("pageY");
			if (oShape.isA('sap.gantt.simple.Relationship') && !oEvent.getParameter("isPinConnector")){
				this.oPopover = new Popover({
					showHeader:false,
					content: [new sap.m.Text({ text: oShape.getType() })]
				});
		...
		handlePopoverClose: function (oEvent) {
			this.oPopover && this.oPopover.close();
		},
