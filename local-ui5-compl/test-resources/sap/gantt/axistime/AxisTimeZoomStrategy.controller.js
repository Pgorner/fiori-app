sap.ui.define([
	...
	], async function(jQuery, Controller, JSONModel, settingsModel, ganttConfig, Format, StepwiseZoomStrategy, FullScreenStrategy, TimeHorizon, Fragment, Element, XMLView) {
	"use strict";
	...
	var Controller = Controller.extend("test.sap.gantt.axistime.AxisTimeZoomStrategy", {
		...
		onInit: function(){
			...
			var oStrategyDemoData = new JSONModel(sap.ui.require.toUrl("local" + sDataFilePath));
			...
			this._oControlTypeSelect = Element.getElementById("idView--zoomControlTypeSelect");
			this._oStrategyTypeSelect = Element.getElementById("idView--zoomStrategyTypeSelect");
			Fragment.load({
				definition: jQuery(document.getElementById("fragment1")).html()
			}).then(function(oHelp){
				this._oTypeHelpPopover = oHelp;
				oView.addDependent(this._oTypeHelpPopover);
			}.bind(this));

			Fragment.load({
				definition: jQuery(document.getElementById("fragment2")).html()
			}).then(function(oHelp){
				this._oControlTypeHelpPopover = oHelp;
	  ...
	  var myView = await XMLView.create({id: "idView", definition: jQuery('#view1').html() });
