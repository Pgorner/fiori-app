sap.ui.define("STTA_MP.ext.controller.DetailsExtension", [
	...
	return {
		...
		onPageDataLoaded: function (oEvent) {
			var oValues = oEvent.context.getObject();
			var price = oValues.Price;
			if (price > 500) {
				var oMsg = {
					message: "Sample showing custom message strip on OP header. Message shown if price per unit > 500.",
					type: MessageType.Warning
				};
				this.extensionAPI.setCustomMessage(oMsg);
			} else {
				this.extensionAPI.setCustomMessage(null);
			}
		},
		...
		onLeaveAppExtension: function(bIsDestroyed) {
			var oCommunicationObject = this.extensionAPI.getCommunicationObject();
			var aCommunicationObjectEntries = oCommunicationObject.entries;
			var fnReactivate = function(){
                aCommunicationObjectEntries.forEach(function (oRIDefault) {
					oRIDefault.setValue(1);
				});
			};
	        return fnReactivate;
		},
		...
		onBeforeRebindTableExtension: function (oEvent) {
			var customFilter = Element.getElementById("STTA_MP::sap.suite.ui.generic.template.ObjectPage.view.Details::STTA_C_MP_Product--CustomFilter-Language");
			if (customFilter) {
