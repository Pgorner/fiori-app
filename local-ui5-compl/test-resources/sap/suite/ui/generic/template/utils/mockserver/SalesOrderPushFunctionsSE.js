sap.ui.define([
	...
	function (Utils, deepExtend, AjaxHelper, Button, Dialog, MessageBox, MessageToast) {
		var seSmartField = seCheckbox = seValueHelp = sNavTarget = sRefreshEntity = sStructuralChange = 1;
		...
		function fnSave(aRequests, oMockServer) {
			...
			//Side effect is configured for the field OpportunityID , target is BusinessPartnerID
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrder_WD_20(.*)\\?.*\\$select=BusinessPartnerID"),
				response: function (oXhr, SalesOrder) {
					...
					var sMsgStringSmartField = seSmartField + " call Side effect message:SmartField as source";
					seSmartField = seSmartField + 1;
					if (oResponse.success) {
						oXhr.respond(200)

						setTimeout(function () {
							MessageBox.alert(sMsgStringSmartField);
						}, 2000);
			...
			//Side effect is configured for the field EnabledStatus , target is CurrencyCode
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrder_WD_20(.*)\\?.*\\$select=CurrencyCode"),
				...
				response: function (oXhr, SalesOrder) {
					...
					var sMsgStringcheckBox = seCheckbox + " call Side effect message:CheckBox as source";
					seCheckbox = seCheckbox + 1;

					if (oResponse.success) {
						oXhr.respond(200)
						setTimeout(function () {
							MessageBox.alert(sMsgStringcheckBox);
						}, 2000);
			...
			//Side effect is configured for the field CurrencyCode , target is SalesOrder
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrder_WD_20(.*)\\?.*\\$select=SalesOrder"),
				response: function (oXhr, SalesOrder) {
					//TODO Use response object
					var sSalesOrderID = SalesOrder.split("'")[1].split("'")[0];
					...
					var sMsgStringValueHelp = seValueHelp + " call Side effect value help message:SmartField as source";
					seValueHelp = seValueHelp + 1;
					if (oResponse.success) {
						oXhr.respond(200)

						setTimeout(function () {
							MessageBox.alert(sMsgStringValueHelp);
						}, 1500);
			...
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrderItem_WD_20(.*)\\?.*\\$select=to_Item&\\$expand=to_Item"),

				response: function (oXhr, SalesOrder) {

					var sSalesOrderID = SalesOrder.split("'")[1].split("'")[0];
					var oResponse = AjaxHelper.sjax({
						url: sRootUri + "C_STTA_SalesOrderItem_WD_20?$filter=SalesOrder eq '" + sSalesOrderID +
							"'",
						dataType: "json"
					});

					var sMsgStringnavtarget = sNavTarget + " call Side effect message:Refresh the navigation target to_Item";
					sNavTarget = sNavTarget + 1;

					if (oResponse.success) {
						oXhr.respond(200)
						setTimeout(function () {
							MessageBox.alert(sMsgStringnavtarget);
						}, 2000);
			...
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrder_WD_20(.*)\\?.*\\$select=\\*"),
				...
				response: function (oXhr, SalesOrder) {
					...
					var sMsgStringEntity = sRefreshEntity + " call Side effect message:whole entity refresh";
					sRefreshEntity = sRefreshEntity + 1;

					if (oResponse.success) {
						oXhr.respond(200)
						setTimeout(function () {
							MessageBox.alert(sMsgStringEntity);
						}, 2500);
			...
			//NavigationPropertyPath is to_Item and target property is NetAmount
			aRequests.push({
				method: 'GET',
				path: new RegExp("C_STTA_SalesOrder_WD_20(.*)\\?.*\\$select=NetAmount"),
				...
				response: function (oXhr, SalesOrder) {
					...
					var sMsgstructuralchange = sStructuralChange + " call Side effect message:Side effect on structural changes of a 1:n association";
					sStructuralChange = sStructuralChange + 1;

					if (oResponse.success) {
						oXhr.respond(200)
						setTimeout(function () {
							MessageBox.alert(sMsgstructuralchange);
						}, 1500);
			...
			aRequests.push({
				...
				response: function (oXhr, SalesOrder, DraftUUID, IsActiveEntity) {
					...
					var sMsgString = "Preparation call message";
					if (oResponse.success) {
						oXhr.respond(200)
							MessageToast.show(sMsgString);
						return true;
					}
				}.bind(this)
			});
			...
			aRequests.push({
				method: 'POST',
				path: new RegExp("C_STTA_SalesOrder_WD_20Showsinglemsg\\?SideEffectsQualifier=(.*)&SalesOrder=(.*)&DraftUUID=(.*)&IsActiveEntity=(.*)"),
				response: function (oXhr, SalesOrder, DraftUUID, IsActiveEntity) {
					//TODO Use response object
					var oResponse = AjaxHelper.sjax({
						url: sRootUri + "C_STTA_SalesOrder_WD_20?$filter=SalesOrder eq '" + sSalesOrderID + "'",
					...
					var sMsgTriggerAction = "Trigger Action Called";
					if (oResponse.success) {
						oXhr.respond(200)

						setTimeout(function () {
							MessageBox.confirm(sMsgTriggerAction);
						}, 2000);
			...
			aRequests.push({
				...
				response: function (oXhr, SalesOrder, DraftUUID, IsActiveEntity) {
					...
					if (oResponse && oResponse.data && oResponse.data.d.results[0] && (oRecord && oRecord.IsActiveEntity.toString() === "false")) {
						oResponse.data.d.results[0].__metadata = {
							"id": sRootUriRelative + "C_STTA_SalesOrder_WD_20(SalesOrder='" + sSalesOrderID +
								"',DraftUUID=guid'00000000-0000-0000-0000-000000000000',IsActiveEntity=true)",
							"type": "STTA_SALES_ORDER_WD_20_SRV.C_STTA_SalesOrder_WD_20Type",
