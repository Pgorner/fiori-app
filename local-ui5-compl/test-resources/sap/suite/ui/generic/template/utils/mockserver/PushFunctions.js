sap.ui.define([
	...
	function(Utils, deepExtend, AjaxHelper, MockServer) {
		"use strict";
		...
		function _handleCreateRequest(oXhr, sUrlParams, oMockserver, oMockServerStandard) {
			...
			if(sObjectPageEntitySet.indexOf("}")!= -1)
			{
				 sObjectPageEntitySet = oXhr.requestBody.split(".")[1].split(",")[0].replace('Type"}','');	
			}
			var sServiceName = oXhr.requestBody.split(".")[0].split(":").slice(-1)[0].replace('"','');
			...
			var aData = MockServer.prototype.getEntitySetData.apply(oMockserver, [sEntitySetForMetadataSetup]);
		...
		function _handleEditRequest(oXhr, oMockserver) {
			...
			var oEntry = oResponse.data && oResponse.data.d.results[0];
			...
			var aData = MockServer.prototype.getEntitySetData.apply(oMockserver, [sEntitySet]);
