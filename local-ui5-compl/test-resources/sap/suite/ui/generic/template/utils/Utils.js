sap.ui.define(["sap/suite/ui/generic/template/genericUtilities/AjaxHelper"], function (AjaxHelper) {
	"use strict";

	var Utils = {};
	var oURLParsing = null;

	// Note: This method should be invoked to initialize the UShell services before invoking any other methods in this class
	Utils.initShellServices = function (UShellContainer) {
		var oURLParsingPromise = UShellContainer && UShellContainer.getServiceAsync("URLParsing");
		return oURLParsingPromise.then(function (URLParsing) {
			oURLParsing = URLParsing;
		});
	} 
	/*
	...
	Utils.getAppInfo = function (sDemoApp, sCalledFromOpt) {
		...
		switch (sDemoApp) {
			case "demoFilterSettings":
				sModulePath = "./demoFilterSettings/webapp";
				break;
			case "demoObjectPageSections":
				sModulePath = "./demoObjectPageSections/webapp";
				break;
			case "demoObjectPagePathSupport":
					sModulePath = "./demoObjectPagePathSupport/webapp";
				break;
			...
			case "sttasalesorderwklt":
				...
				break;
			case "sttasalesordersmartlist":
				sModuleName = "SalesOrderSmartList";
				sModulePath = "./sample.stta.sales.order.smartlist/webapp";
				break;
			...
			case "alpWithParams":
				...
				break;
			case "lrWithParams":
				sModuleName = "sample.stta.with.params";
				sModulePath = "./sample.stta.with.params/webapp";
				break;
			case "analytics3":
			...
			case "analytics5":
				...
				break;
			case "analytics6":
				sModuleName = "analytics6";
				sModulePath = "./sample.analytical.list.page.multitable/webapp";
				break;
			case "products":
	...
	Utils.getFlpConfiguredAppInfo = function (sSemanticObjectAction) {
		...
		var aConfiguredFlpSandboxApps =
			...
			window["sap-ushell-config"].services.CommonDataModel &&
			window["sap-ushell-config"].services.CommonDataModel.adapter &&
			window["sap-ushell-config"].services.CommonDataModel.adapter.config &&
			window["sap-ushell-config"].services.CommonDataModel.adapter.config.siteData &&
			window["sap-ushell-config"].services.CommonDataModel.adapter.config.siteData.applications;
		if (aConfiguredFlpSandboxApps){
			var oConfiguredFlpSandboxApp = aConfiguredFlpSandboxApps[sSemanticObjectAction];
			
			sModuleName = sSemanticObjectAction;
			sModulePath = oConfiguredFlpSandboxApp &&
				oConfiguredFlpSandboxApp["sap.platform.runtime"] && 
				oConfiguredFlpSandboxApp["sap.platform.runtime"].componentProperties && 
				oConfiguredFlpSandboxApp["sap.platform.runtime"].componentProperties.url;
		}
		return {
