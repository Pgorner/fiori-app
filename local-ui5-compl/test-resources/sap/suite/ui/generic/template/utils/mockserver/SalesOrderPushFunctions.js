sap.ui.define([
	...
	function (Utils, deepExtend, AjaxHelper, MessageBox) {
		...
		function fnSalesOrderPush(aRequests, oMockserver) {
			...
			aRequests.push({
				...
				response: function (oXhr, sUrlParams) {
					MessageBox.success("FI Action without parameter triggered", {});
					oXhr.respond(204);
					return true;
				}.bind(this)
			});
			aRequests.push({
				method: "POST",
				path: new RegExp(oMockserver._oDraftMetadata.draftRootName+"Create_simple_param"+"(.*)"),
				response: function (oXhr, sUrlParams) {
					MessageBox.success("FI Action with parameter triggered", {});
					oXhr.respond(204);
					return true;
				}.bind(this)
			});
		...
		function fnSave(aRequests, oMockServer) {
			...
			aRequests.push({
				...
				response: function (oXhr, SalesOrder) {
					var sSalesOrderID = SalesOrder.split("'")[1].split("'")[0];
					if (oXhr.requestHeaders.Prefer === 'handling=strict' && sSalesOrderID === "500000012") {
							// SalesOrder "500000012" deleteion will respond with 412 code 
							oXhr.respondJSON(412, {
								...
								"error": {
									"innererror": {
										"errordetails": [{
											"code": "001",
											"message": "412 code sent Warning Transient Message",
											"severity": "warning",
											"preference-applied": "handling=strict",
											"type": "warning",
											"transient": true,
			...
			//Set Opprtunity ID Function Import
			aRequests.push({
				method: 'POST',
				path: new RegExp("C_STTA_SalesOrder_WD_20Setopportunityid(.*)"),
				response: function (oXhr, SalesOrder) {
					var sSalesOrderID = SalesOrder.split("'")[1].split("'")[0];
					if (oXhr.requestHeaders.Prefer === 'handling=strict' && sSalesOrderID === "500000012") {
						oXhr.respondJSON(412, {
						...
						var opportunityID = SalesOrder.split("'")[5].split("'")[0];
						var oResponse = AjaxHelper.sjax({
							url: sRootUri + "C_STTA_SalesOrder_WD_20?$filter=SalesOrder eq '" + sSalesOrderID +
								"'",
							dataType: "json"
						...
						if (oResponse && oResponse.data && oResponse.data.d.results[0]) {
							oResponse.data.d.results[0].OpportunityID = opportunityID;
							oResponse.data.d.results[0].__metadata = {
								"id": sRootUriRelative + "C_STTA_SalesOrder_WD_20(SalesOrder='" + sSalesOrderID +
									"',DraftUUID=guid'00000000-0000-0000-0000-000000000000',IsActiveEntity=true)",
								"type": "STTA_SALES_ORDER_WD_20_SRV.C_STTA_SalesOrder_WD_20Type",
						...
						oMockServer.setEntitySetData("C_STTA_SalesOrder_WD_20_Sibling", oResponse);
						if (oResponse) {
							oXhr.respondJSON(200, {}, JSON.stringify({
								d: oResponse.data.d
							}));
