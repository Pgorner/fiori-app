sap.ui.define([
...
], function (JSONModel, Graph, Node, Line, Group, Attribute, SvgBase, ActionButton, GraphMap, Status, QUnitUtils, jQuery, FlexBox, Dialog, nextUIUpdate) {
	var GraphTestUtils = {};

	GraphTestUtils.NODE_EXPAND_STATE_ATTR = "_oExpandState";
	GraphTestUtils.NODE_HIDDEN_STATE_ATTR = "_bIsHidden";

	GraphTestUtils.checkGraphFiresEvent = async function (assert, oGraph, sEventName) {
	...
	GraphTestUtils.checkGraphDataFiresEvent = async function (assert, oData, sEventName) {
		await GraphTestUtils.checkGraphFiresEvent(
			assert,
	...
	GraphTestUtils.buildGraph = function (oData, oGraph, bBindActionButtontoLine, bAttachShowDetail, bLeftActionButton) {
		...
		var oNode = new Node({
			...
			oActionButton = new ActionButton({
				...
				position: bLeftActionButton ? "Left" : "Right"
			}),
			oStatus = new Status({
				key: "{key}",
				title: "{title}",
		...
		if (bBindActionButtontoLine) {
			oLine.addActionButton(new ActionButton({
				icon: "sap-icon://bubble-chart"
			}));
		}
		if (bAttachShowDetail) {
			oGroup.attachShowDetail(function(oEvent) {
				oEvent.cancelBubble();
				oEvent.preventDefault();
				this.oDialog = new Dialog({
					title: "A Custom Dialog"
				});
				this.addDependent(this.oDialog);
				this.oDialog.open();
			});
		}
	...
	GraphTestUtils.runAsyncAssert = async function (oGraph, fnAssert, fnDone, bIsAsynchronousAssert) {
		oGraph.attachGraphReady(function () {
			if (bIsAsynchronousAssert) {
				fnAssert()
					.then(function() {
						fnDone();
						oGraph.destroy();
	...
	GraphTestUtils.runAsyncActionAssertChain = async function (oGraph, mAsyncChain, fnDone) {
		var iIteration = 0;

		oGraph.attachEvent(mAsyncChain.eventName, async function () {
			iIteration++;
	...
	GraphTestUtils.runSyncActionAssertChain = async function (oGraph, mSyncChain, fnDone) {
	...
	GraphTestUtils.performeActionButtonTest = async function (mOptions) {
		var oGraph,
