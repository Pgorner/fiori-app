sap.ui.define([
...
], function (GraphTestUtils, SwimLaneChainLayout, nextUIUpdate) {
	"use strict";
	...
	QUnit.test("Custom status box shape.", async function (assert) {
		...
		oGraph.attachEvent("graphReady", function () {
			...
			oNode._onClick();
			fnBoxCheckColors(assert, oNode, {
				headerContent: Color.DefaultSelectedColor,
				background: Color.DefaultSelectedBgColor,
				border: Color.DefaultSelectedBorderColor
			}, "Selected");
	...
	QUnit.test("Selected nodes with custom status.", function (assert) {
		...
		oGraph.attachEvent("graphReady", function () {
			...
			fnBoxCheckColors(assert, oNode, {
				...
				background: Color.Yellow,
				border: Color.Yellow,
				rows: [Color.White, Color.DefaultSelectedColor, Color.Red]
			}, "Selected-Default");
	...
	QUnit.test("Custom Group status.", function (assert) {
		...
		oGraph.attachEvent("graphReady", function () {
			...
			assert.ok(aGroupMenuButton[1].style.color, "Color is set for Menu button.");
			assert.ok(aGroupMenuButton[0].style.color, "Color is set for Expand/Collapse button.");
			assert.ok(oGroupIcon.style.color, "Color is set for Group Icon button.");
	...
	QUnit.test("Custom status legend color.", function (assert) {
		...
		oGraph.attachEvent("graphReady", function () {
			...
			assert.equal(hexToRgb(jQuery($items[0]).css("background-color")), "rgb(255, 192, 203)", "Arrow stroke");
			assert.equal(hexToRgb(jQuery($items[1]).css("background-color")), "rgb(255, 0, 0)", "Arrow stroke");
	...
	QUnit.test("Custom status focus color.", function (assert) {
		...
		oGraph.attachEvent("graphReady", function () {
			...
			oNode1._onClick();
			assert.equal(hexToRgb($focus.css("border-left-color")), Color.DefaultSelectedColor, "focus 2");

			oNode1._onClick();
			// same as hover (specialcase)
			assert.equal(hexToRgb($focus.css("border-left-color")), Color.Black, "focus 2");
