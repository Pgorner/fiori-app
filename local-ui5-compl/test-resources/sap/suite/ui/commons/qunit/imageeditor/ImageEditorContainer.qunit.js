...
sap.ui.define([
	...
	"sap/ui/core/Element"
], function(library, ImageEditor, ImageEditorContainer, createAndAppendDiv, Input, coreLibrary, nextUIUpdate, Element) {
	"use strict";
	...
	QUnit.test("getToolbarIds", function(assert) {
		...
		Object.keys(oIds).forEach(function(sKey) {
			...
			assert.ok(Element.getElementById(oIds[sKey]), "object exists");
		}, this);
	});
	...
	QUnit.test("zoomToFit called on img load", async function(assert) {
		var that = this,
			fnDone = assert.async();

		await render(this.oImageEditorContainer);
	...
	QUnit.test("zoomToFit called on rendering", function(assert) {
		...
		this.oImageEditor.attachLoaded(async function() {
			that.oSandbox.spy(that.oImageEditor, "zoomToFit");
			await render(that.oImageEditorContainer);
	...
	QUnit.test("zoomToFit called on setImageEditor", async function(assert) {
		var that = this,
			fnDone = assert.async();
	...
	QUnit.module("ImageEditorContainer with rendering", {
		beforeEach: function(assert) {
			...
			this.oImageEditor.attachEventOnce("loaded", async function() {
				await render(that.oImageEditorContainer);
				fnDone();
			});
	...
	QUnit.test("customToolbarContent", async function(assert) {
