...
sap.ui.define([
	...
	FileUtil, createAndAppendDiv, nextUIUpdate, Parameters) {
	"use strict";
	...
	QUnit.test("_getFileFormat", function (assert) {
		var oFile, oBlob, fnDone = assert.async();
	...
	QUnit.module("ImageEditor with image, with rendering", {
		beforeEach: async function (assert) {
			...
			await render(this.oImageEditor);
			this.oSandbox = sinon.sandbox.create();
		},
		afterEach: function () {
			this.oSandbox.restore();
	...
	QUnit.test("applyVisibleCrop", async function (assert) {
		this.oSandbox.spy(this.oImageEditor, "_cancelPreview");
	...
	QUnit.test("Default mode re-rendering", function (assert) {
		...
		function checkIfOk() {
			that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").focus();
			...
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(Parameters.get("sapUiContentFocusStyle")), "Focus Style applied.");
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(Parameters.get("sapUiContentFocusColor")), "Focus Color applied.");
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(Parameters.get("sapUiContentFocusWidth")), "Focus Width applied.");
		}
	...
	QUnit.test("Resize mode re-rendering", async function (assert) {
		var that = this;
	...
	QUnit.test("Rectangle crop mode re-rendering", async function (assert) {
		var that = this,
			fnDone = assert.async(),
			$CropArea;
		...
		var afterRenderDelegate = {
			onAfterRendering: function () {
				if (that.oImageEditor && that.oImageEditor.$()) {
					assert.ok(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").is(":visible"), "inner container visible");
					assert.ok(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvas").is(":visible"), "canvas visible");
				...
				if (count === 1) {
					that.oImageEditor.removeEventDelegate(afterRenderDelegate);
					that.oImageEditor.destroy();
					fnDone();
				}
				count++;
			}
		};
		that.oImageEditor.addEventDelegate(afterRenderDelegate);
	...
	QUnit.test("Ellipse crop mode re-rendering", async function (assert) {
		var that = this,
			fnDone = assert.async(),
			$CropArea;
	...
	QUnit.test("No image loaded text", function (assert) {
		...
		this.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorNoData").focus();
		assert.ok(this.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorNoData").is(":visible"), "text is visible when no image loaded");
	});

	QUnit.module("ImageEditor without image, with rendering", {
		beforeEach: async function (assert) {
			this.oImageEditor = new ImageEditor();
			await render(this.oImageEditor);
			this.oSandbox = sinon.sandbox.create();
	...
	QUnit.test("Making no data div as a focusable item", function (assert) {
		var oNodataDiv = document.querySelector(".sapSuiteUiCommonsImageEditorNoDataText");
		assert.equal(oNodataDiv.tabIndex,0,"No data div is an focusable item");
	});

	QUnit.module("SetSrc with useExifOrientation");

	QUnit.test("useExifOrientation", function (assert) {
		var fnDone = assert.async();
		this.oSandbox = sinon.sandbox.create();
		this.oImageEditor = new ImageEditor();
		this.oImageEditor.attachEventOnce("loaded", function () {
		assert.ok(!this._handleExifOrientation.calledOnce,"This _handleExifOrientation is not called once");
		fnDone();
	});
		this.oImageEditor.setSrc(sBigImg);

	this.oSandbox.stub(this.oImageEditor, "_handleExifOrientation");
	});
	QUnit.test("useExifOrientation", function (assert) {
		var fnDone = assert.async();
		this.oSandbox = sinon.sandbox.create();
		this.oImageEditor = new ImageEditor({useExifOrientation:true});
		this.ohandleExifOrientationSpy = sinon.spy(this.oImageEditor, "_handleExifOrientation");
		this.oImageEditor.setSrc(sBigImg);
		setTimeout(function(){
			assert.ok(this.ohandleExifOrientationSpy.calledOnce, "_handleExifOrientation called!");
			fnDone();
		}.bind(this), 500);
	});
});
