sap.ui.define([
...
], function (jQuery, suiteLibrary, ProcessFlow, ProcessFlowNode, ProcessFlowLaneHeader, JSONModel, CreateAndAppendDiv, qutils, KeyCodes, nextUIUpdate, Parameters) {
	"use strict";

	var ProcessFlowNodeState = suiteLibrary.ProcessFlowNodeState;
	...
	QUnit.test("Move focus based on arrow - check movement function last element on row", function (assert) {
		...
		assert.ok(getComputedStyle(nodes[17].getDomRef()).outline.indexOf(Parameters.get("sapUiContentFocusStyle")), "Focus Style applied.");
		assert.ok(getComputedStyle(nodes[17].getDomRef()).outline.indexOf(Parameters.get("sapUiContentFocusColor")), "Focus Color applied.");
		assert.ok(getComputedStyle(nodes[17].getDomRef()).outline.indexOf(Parameters.get("sapUiContentFocusWidth")), "Focus Width applied.");
	...
	QUnit.test("Move focus based on arrow - check movement function with highligted nodes", function (assert) {
		...
		var eDelegate = this.oProcessFlowCC2.addEventDelegate({
			onAfterRendering: function() {
				...
				assert.ok(bMovePossible, "Next selection succesfully done.");
				this.oProcessFlowCC2.removeEventDelegate(eDelegate);
				done();
            }.bind(this)
        });
		nodes[0]._setNavigationFocus(true);
	...
	async function buildProcessFlow(idx) {
	...
	QUnit.test("Check the keyboard event on processflow chart - ENTER", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(1);
		...
		qutils.triggerKeydown(aNodes[0].getDomRef(), KeyCodes.ENTER);
		qutils.triggerKeyup(aNodes[0].getDomRef(), KeyCodes.ENTER);
		oProcessFlow.destroy();
	});

	QUnit.test("Check the keyboard event on processflow chart - KeyDown", async function (assert) {
        //Arrange
        var oProcessFlow = await buildProcessFlow(1);
        //Act
        var aNodes = oProcessFlow.getNodes();
        ...
        oProcessFlow._lastNavigationFocusElement = aNodes[0];
        qutils.triggerKeydown(aNodes[0].getDomRef(), KeyCodes.ENTER);
        assert.ok(aNodes[0].getDomRef().getAttribute("class").indexOf("sapSuiteUiCommonsProcessFlowNodeActive") >= 0, "Active Class added");
        assert.ok(!document.getElementsByClassName("sapUiIconTitle")[0].classList.contains("sapSuiteUiCommonsProcessFlowNodeActive"), "Active Class not added");
        oProcessFlow.destroy();
    });

	QUnit.test("Check the keyboard event on processflow chart - mouseEnter", async function (assert) {
        //Arrange
        var oProcessFlow = await buildProcessFlow(1);
        //Act
        var aNodes = oProcessFlow.getNodes();
        aNodes[0]._setNavigationFocus(true);
        oProcessFlow._lastNavigationFocusElement = aNodes[0];
        qutils.triggerMouseEvent(aNodes[0].getDomRef(), "mouseenter");
        assert.ok(aNodes[0].getDomRef().getAttribute("class").indexOf("sapSuiteUiCommonsProcessFlowNodeHover") >= 0, "Active Class added");
        assert.ok(!document.getElementsByClassName("sapUiIconTitle")[0].classList.contains("sapSuiteUiCommonsProcessFlowNodeHover"), "Active Class not added");
        oProcessFlow.destroy();
    });

	QUnit.test("Check the keyboard event on processflow chart - SPACE", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(2);
		...
		qutils.triggerKeydown(aNodes[0].getDomRef(), KeyCodes.SPACE);
		qutils.triggerKeyup(aNodes[0].getDomRef(), KeyCodes.SPACE);
		oProcessFlow.destroy();
	});

	QUnit.test("Check the keyboard event on processflow chart - unfocus navigation element - TAB", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(3);

		//Act
		var aNodes = oProcessFlow.getNodes();
		aNodes[0]._setNavigationFocus(false);
		oProcessFlow._lastNavigationFocusElement = null;
		qutils.triggerKeydown(aNodes[0].getDomRef(), KeyCodes.TAB);
		qutils.triggerKeyup(aNodes[0].getDomRef(), KeyCodes.TAB);
	...
	QUnit.test("Check the keyboard event on processflow chart - TAB", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(4);
	...
	QUnit.test("Check the keyboard event on processflow chart - unfocus navigation element - Shift-TAB", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(5);

		//Act
		var aNodes = oProcessFlow.getNodes();
		oProcessFlow._lastNavigationFocusElement = null;
		qutils.triggerKeydown(aNodes[0].getDomRef(), KeyCodes.TAB, true, false, false);
		qutils.triggerKeyup(aNodes[0].getDomRef(), KeyCodes.TAB, true, false, false);
	...
	QUnit.test("Check the keyboard event on processflow chart - focus navigation element - Shift-TAB", async function (assert) {
		//Arrange
		var oProcessFlow = await buildProcessFlow(6);
	...
	QUnit.test("Check the keyboard event on processflow chart - focus on last button of Hbox - Shift-TAB", async function (assert) {
		...
		oHBox.placeAt("processflowdiv");
		await nextUIUpdate();

		//Arrange
		var oProcessFlow = await buildProcessFlow(7);
		...
		qutils.triggerKeyup(aNodes[0].getDomRef(), KeyCodes.TAB,true, false, false);
		//Assert
		assert.equal(document.activeElement.id, oHBox.getItems()[2].getId(), "Focus on last button");
		oProcessFlow.destroy();
	});
