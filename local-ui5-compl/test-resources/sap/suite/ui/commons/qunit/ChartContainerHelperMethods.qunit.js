sap.ui.define([
	...
	"sap/base/Log"
], function(ChartContainerContent, ChartContainer, VizFrame, ChartContainerToolbarPlaceholder, mobileLibrary, Device,
            Button, Control, ToolbarSpacer, sinon, nextUIUpdate, Popover, Dialog, ResponsivePopover, Log) {
	"use strict";
	...
	QUnit.test("Ensure that navigation open/closes the fullscreen", function(assert) {
	...
	QUnit.test("Ensure that the custom class has been applied to the Dialog", function(assert) {
		this.oChartContainer._toggleFullScreen();
		assert.ok(this.oChartContainer._oFullScreenDialog.getDomRef().querySelector(".sapMDialogScroll").classList.contains('sapMDialogChartContainerScroll'), "Relevant class has been applied to the sap.m.Dialog");
	});

	QUnit.test("Ensure blank value not works for toggleFullScreen function", function(assert) {
		assert.equal(this.oChartContainer.getFullScreen(), false, "fullscreen is off");
		this.oChartContainer._toggleFullScreen();
		assert.equal(this.oChartContainer.getFullScreen(), true, "fullscreen is on");
		this.oChartContainer.getAggregation("content")[0].getContent().setHeight('');
		this.oChartContainer._toggleFullScreen();
		assert.notEqual(this.oChartContainer.getAggregation("content")[0].getContent().getHeight(), this.oChartContainer._sChartOriginalHeight, "set hight didn't executed");
		assert.equal(this.oChartContainer.getFullScreen(), false, "fullscreen is off");
	});
	...
	QUnit.test("_performHeightChanges perform height changes is viz frame", function(assert) {
		...
		var stubRerender = sinon.stub(this.oChartContainer, "invalidate");
	...
	QUnit.test("_performHeightChanges re-renders when content is not a VizFrame", async function(assert) {
		...
		var stubRememberOriginalHeight = sinon.stub(this.oChartContainer, "_rememberOriginalHeight");
		var stubRerender = sinon.stub(this.oChartContainer, "invalidate");
		this.checkRendererLoop = async function () {
			await nextUIUpdate();
	...
	QUnit.test("ChartContainer toggle fullscreen behavior check", function(assert) {
		assert.equal(this.oChartContainer.getFullScreen(), false, "fullscreen is off");
		this.oChartContainer._toggleFullScreen();
		assert.equal(this.oChartContainer.getFullScreen(), true, "fullscreen is on");
		this.oChartContainer._toggleFullScreen();
		...
		var toggleFullScreenBrowserAPIStub = sinon.stub(this.oChartContainer._oFullScreenUtil, "toggleFullScreenBrowserAPI");
		var toggleFullScreenStub = sinon.stub(this.oChartContainer._oFullScreenUtil, "toggleFullScreen");

		this.oChartContainer._toggleFullScreen();
		assert.ok(toggleFullScreenStub.called, "ChartContainer uses default fullscreen");

		this.oChartContainer.setProperty("enableBrowserFullScreen", true, true);
		this.oChartContainer._toggleFullScreen();
		assert.ok(toggleFullScreenBrowserAPIStub.called, "ChartContainer uses Browser fullscreen");
	});

	QUnit.test("ChartContainer addDependent check", function(assert) {
		var oDialog = new Dialog();
		var oPopover = new Popover();
		var oResponsivePopover = new ResponsivePopover();

		var logStub = this.spy(Log, "error");

		this.oChartContainer.addDependent(oDialog);
		assert.equal(this.oChartContainer.getDependents().length, 0, "ChartContainer has no element as dependent");

		this.oChartContainer.setProperty("enableBrowserFullScreen", true, true);
		this.oChartContainer.addDependent(oDialog);
		assert.equal(this.oChartContainer.getDependents().length, 1, "ChartContainer has oDialog element as dependent");

		this.oChartContainer.addDependent(oPopover);
		assert.equal(this.oChartContainer.getDependents().length, 2, "ChartContainer has oPopover element as dependent");

		this.oChartContainer.addDependent(oResponsivePopover);
		assert.equal(this.oChartContainer.getDependents().length, 3, "ChartContainer has oResponsivePopover element as dependent");

		this.oChartContainer.addDependent(new sap.m.Text());
		assert.equal(this.oChartContainer.getDependents().length, 3, "ChartContainer has no Text element as dependent");
		assert.ok(logStub.called, "Log error is displayed");
	});
