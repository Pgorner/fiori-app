...
sap.ui.define([
...
], function(nextUIUpdate, jQuery, ComparisonMicroChart, ComparisonMicroChartData, mobileLibrary, Device, JSONModel, Button, ColumnListItem, Table, Text, GenericTile, TileContent, FlexBox, TooltipBase, microchartLibrary, TestUtils, Mobile, Core, CoreLib,Theming,QUtils,Localization) {
	"use strict";

	var Size = mobileLibrary.Size;
	var oRb = CoreLib.getResourceBundleFor("sap.suite.ui.microchart");
	var ComparisonMicroChartViewType = microchartLibrary.ComparisonMicroChartViewType;
	...
	*/
	QUnit.test("Core not loaded and theme not loaded", function(assert) {
		...
		assert.ok(this.fnStubAttachThemeApplied.calledOnce, "Method Theming.attachApplied has been called once.");
	...
	*/
	...
	QUnit.test("Comparison chart rendered.", function(assert) {
		assert.ok(document.getElementById("comparison-chart"), "Comparison chart was rendered successfully");
		assert.equal(jQuery("#comparison-chart-chart-item-0-title").text(), "Americas", "Title was rendered successfully");
		assert.equal(jQuery("#comparison-chart-chart-item-0-value").text(), "10MM", "Value was rendered successfully");
	});
	...
	QUnit.test("Function _addTitleAttribute adds the tooltip correctly in the fixed size chart", async function(assert) {
		//Arrange
	...
	QUnit.test("Function _removeTitleAttribute removes the tooltip correctly from the fixed size chart", async function(assert) {
		//Arrange
		var sTooltip = "The Tooltip";
		this.oCC.setTooltip(sTooltip);
		await nextUIUpdate();
	 ...
	 * @deprecated Since version 1.58
	 */
	QUnit.test("Function _addTitleAttribute adds the tooltip correctly in the responsive chart", async function(assert) {
		//Arrange
		var sTooltip = "The Tooltip";
		this.oCC.setTooltip(sTooltip);
		this.oCC.setSize(Size.Responsive);
		await nextUIUpdate();
		// Act
		this.oCC._addTitleAttribute();
		//Assert
	 ...
	 * @deprecated Since version 1.58
	 */
	QUnit.test("Function _removeTitleAttribute removes the tooltip correctly from the responsive chart", async function(assert) {
		//Arrange
		var sTooltip = "The Tooltip";
		this.oCC.setTooltip(sTooltip);
		this.oCC.setSize(Size.Responsive);
	...
	QUnit.test("If the tooltip of the bar is set to whitespace, then no tooltip is displayed on the bar level", async function(assert) {
		// Arrange
	...
	QUnit.test("The bar with empty tooltip will not result in an extra line in the composite tooltip of the chart", function(assert) {
		...
		// Assert
		assert.equal(iCountNewlines, 3, "Exactly two newlines has been rendered: before the last entry of the composite tooltip");
	});

	QUnit.test("Test tooltip when color is None", function(assert) {
		// Arrange
		var stooltip = this.oCC.getTooltip_AsString();
		var sExpectedTooltip = 'Comparison Micro Chart\nAmericas 10MM Good\nEMEA 18MM Warning\nEMEA 18MM Warning\nEMEA 18MM ';
		// Act
		// Assert
		assert.equal(stooltip, sExpectedTooltip, "When color is None the tooltip should show no value");
	});
	...
	QUnit.skip("_calculateBarContainerHeight calculates the height as individual item height sum for firefox, non-responsive, normal mode", function(assert) {
		...
		sinon.stub(this.oCC, "_isResponsive").returns(bResponsive);
	...
	QUnit.test("Test press behavior",async function(assert) {
		...
		await nextUIUpdate();
		setTimeout(() => {
			assert.equal(fnCallback.callCount, 3, "Chart press handler called.");
			assert.equal(fnBarCallback.callCount, 1, "Bar press handler called.");
			fnDone();
		},100);
	});

	QUnit.test("Check if the menu open on the right click", function(assert) {
		QUtils.triggerEvent("contextmenu",this.oCC.getDomRef());
		assert.ok(this.oCC._oMenu.isOpen(),"Menu has been opened on right click");
	});
	...
	QUnit.skip("Vertical responsiveness: decreasing fonts", async function(assert) {
		//Arrange
	...
	QUnit.skip("Vertical responsiveness: bars disappears", async function(assert) {
		//Arrange
	...
	QUnit.skip("Horizontal responsiveness: decreasing fonts", async function(assert) {
		//Arrange
		this._oFB.setWidth("16rem");
		this._oFB.setHeight("8rem");
		await nextUIUpdate();
	...
	QUnit.test("All items can be seen when enough space", async function(assert) {
		//Arrange
	...
	QUnit.test("Items get removed when there is not enought vertical space", async function(assert) {
		//Arrange
		this._oFB.setWidth("10.5rem");
	...
	QUnit.test("More items get removed when there is not enought vertical space", async function(assert) {
		//Arrange
		this._oFB.setWidth("10.5rem");
	...
	QUnit.test("All items shown in size of generic tile content because of proportional width resizing", async function(assert) {
		//Arrange
		this._oFB.setWidth("142px");
	...
	QUnit.test("Model exists, control is bound, tooltip aggregataion is not bound", async function(assert) {
		//Arrange
	...
	QUnit.test("Enter button is pressed (bar)", function(assert) {
		//Arrange
		var $FirstBar = this.oChart.$().find(".sapSuiteCpMCChartBar"),
		...
		//Act
		this.oChart.getData()[0].onsapenter(oEventEnter);
		//Assert
	...
	QUnit.test("Space button is pressed (bar)", function(assert) {
		...
		var fnDone = assert.async();
		//Act
		this.oChart.getData()[0].onsapspace(oEventEnter);
		//Assert
		setTimeout(() => {
			assert.equal(this.oChart.firePress.callCount, 0, "Press event is not fired for chart");
			assert.equal(oFirstBar.firePress.callCount, 1, "Press event is fired for the first bar");
	...
	QUnit.test("No data rendered when no data given", function(assert) {
		assert.ok(document.getElementById("comparisonMicroChart"), "Control was rendered");
	...
	QUnit.test("Chart has correct height, all items are visible", async function(assert) {
	...
	QUnit.test("default view with bigger width", async function(assert) {
	...
	QUnit.test("wide view with small width", async function(assert) {
		this.oChart.setView(ComparisonMicroChartViewType.Wide);
	...
	QUnit.test("responsive view with small width", async function(assert) {
	...
	QUnit.test("responsive view with bigger width", async function(assert) {
		this.oChart.setView(ComparisonMicroChartViewType.Responsive);
		this.oChart.setWidth("500px");
	...
	QUnit.module("Theme change handling", {
		...
		afterEach: function(assert) {
			this.oChart.destroy();
			this.oChart = null;
			if (Theming.getTheme() !== this.sStartTheme) {
				this.applyTheme(this.sStartTheme, assert.async());
			}
		},
		assertContents: function(aComparisonChartData, assert) {
			for (var i = 0; i < aComparisonChartData.length; i++){
				if (window.getComputedStyle(aComparisonChartData[i]).border) {
					if (window.getComputedStyle(aComparisonChartData[i]).border.indexOf("1px solid") == 0) {
						assert.ok(window.getComputedStyle(aComparisonChartData[i]).border, "Border has been added.");
					} else {
						assert.ok(window.getComputedStyle(aComparisonChartData[i]).border, "Border is not added.");
					}
				} else {
					assert.equal(window.getComputedStyle(aComparisonChartData[i]).border, "", "Border is not added.");
				}
			}
		}
	});

	QUnit.test("Chart will be rerendered and no ChartData border added for sap_belize", function(assert) {
		//Assert
		var done = assert.async();
		assert.ok(this.oChart.getDomRef(), "The chart is rerendered");
		this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNegNoValue"),assert);
		this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNoValue"),assert);
		this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarValue"),assert);
		done();
	});

	QUnit.test("Chart will be rerendered and no ChartData border added for sap_belize_plus", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_belize_plus", function() {
			assert.ok(this.oChart.getDomRef(), "The chart is rerendered");
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNegNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarValue"),assert);
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_fiori_dark", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_fiori_dark", function() {
			assert.ok(this.oChart.getDomRef(), "The chart is rerendered");
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNegNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarValue"),assert);
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_fiori_3", function(assert) {
		//Assert
		var done = assert.async();
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_fiori_3_hcb", function(assert) {
		//Assert
		var done = assert.async();
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_fiori_3_hcw", function(assert) {
		//Assert
		var done = assert.async();
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_belize_hcw", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_belize_hcw", function() {
			assert.ok(this.oChart.getDomRef(), "The chart is rerendered");
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNegNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarNoValue"),assert);
			this.assertContents(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBarValue"),assert);
	...
	QUnit.test("Chart will be rerendered and no ChartData border added for sap_belize_hcb", function(assert) {
		//Assert
		var done = assert.async();
	...
	QUnit.module("Chinese character adjustments", {
		beforeEach: async function() {
			this.oChart = new ComparisonMicroChart({
				size: "S",
				data: [
	...
	QUnit.test("Small Chart top and margin", function(assert) {

		var sBarTop = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBar")[0]).top;
		var sItemMargin = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartItem")[0]).margin;

		assert.equal(sBarTop,'16px',"Small chart bar top value is correct");
		assert.equal(sItemMargin,'0px 0px 2px',"Small chart Margin-bottom value is correct");
	});
	...
	QUnit.test("Medium Chart top and margin ", function(assert) {

		var sBarTop = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBar")[0]).top;
		var sItemMargin = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartItem")[0]).margin;

		assert.equal(sBarTop,'18px',"Medium chart bar top value is correct");
	    assert.equal(sItemMargin,'0px',"Medium chart Margin-bottom value is correct");
	});
	...
	QUnit.test("Large Chart top and margin ", function (assert) {

		var sBarTop = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartBar")[0]).top;
		var SItemMargin = getComputedStyle(this.oChart.getDomRef().querySelectorAll(".sapSuiteCpMCChartItem")[0]).margin;

		assert.equal(sBarTop, '17px', "Medium chart bar top value is correct");
		assert.equal(SItemMargin, '0px 0px 10px', "Medium chart Margin-bottom value is correct");
	});

	QUnit.test("Appropriate Class has been added if its chinese", async function (assert) {
		Localization.setLanguage("zh");
		this.oChart.setSize(Size.Responsive);
		this.oGenericTile.setSizeBehavior(TileSizeBehavior.Small);
		await nextUIUpdate();
		assert.ok(this.oChart.hasStyleClass("sapCMCEastCharsPresent"),"Chinese responsible class has been added");
	});
});
