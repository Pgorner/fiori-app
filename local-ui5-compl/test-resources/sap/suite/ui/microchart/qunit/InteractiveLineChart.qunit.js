...
sap.ui.define([
	...
	"sap/ui/core/Core",
	"sap/ui/core/Theming"
], function(nextUIUpdate, jQuery, InteractiveLineChart, InteractiveLineChartPoint, mobileLibrary, FlexBox, Label, microchartLibrary, Log, IntervalTrigger, Mobile, Core, Theming) {
	"use strict";
	...
	*/
	...
	QUnit.test("InteractiveLineChart is rendered", async function(assert) {
	...
	QUnit.test("Selected points are rendered", async function(assert) {
		//Arrange
		this.oChart.placeAt("qunit-fixture");
		await nextUIUpdate();
	...
	QUnit.test("Tests property displayedPoints", async function(assert) {
		//Arrange
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "July", selected: false, value: 33.1}));
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "Aug", selected: false, value: 55.2}));
	...
	QUnit.test("Focus test for mouse click event", async function(assert) {
		//Arrange
	...
	QUnit.test("Click event on selection disabled line chart test", async function(assert) {
		//Arrange
		this.oChart.setSelectionEnabled(false);
		this.oChart.attachEvent("selectionChanged", this.fnSelectionChangedHandler);
		await nextUIUpdate();
	...
	QUnit.test("Horizontal responsiveness: show value labels", async function(assert) {
		//Arrange
	...
	QUnit.test("Horizontal responsiveness: show truncated labels", async function(assert) {
		//Arrange
	...
	QUnit.test("Horizontal responsiveness: show labels if no truncation occurs", async function(assert) {
		//Arrange
	...
	QUnit.test("Horizontal responsiveness: hide labels except the edges", async function(assert) {
		//Arrange
		this.oFlexBox.setWidth("120px");
		await nextUIUpdate();
		this.oChart.getPoints()[0].setLabel("large bottom label");
	...
	QUnit.test("Horizontal responsiveness: small font", async function(assert) {
		//Arrange
	...
	QUnit.test("Horizontal responsiveness: non-interactive mode", async function(assert) {
		//Arrange
	...
	QUnit.test("Positive values with 0", async function(assert) {
		//Arrange
	...
	QUnit.test("Negative values without 0", async function(assert) {
		//Arrange
	...
	QUnit.test("Negative values with 0", async function(assert) {
		//Arrange
		this.oChart.getPoints()[0].setValue(0);
		this.oChart.getPoints()[1].setValue(-13.2);
		this.oChart.getPoints()[2].setValue(-33.4);
	...
	QUnit.test("Aria label - semantic color", async function(assert) {
		//Arrange
	...
	QUnit.test("Custom chart tooltip (interactive mode)", async function(assert) {
		...
		assert.equal(sFirstItemTooltip, null, "First item tooltip is removed after mouse leave");
		...
		var sSecondItemTooltip = $InteractionAreas.eq(1).attr("title");
		assert.equal(sSecondItemTooltip, null, "Second item tooltip is removed after mouse leave");
	});
	...
	QUnit.test("Suppress tooltip (non-interactive)", async function(assert) {
		//Arrange
		this.oChart.addPoint(new InteractiveLineChartPoint({label: "July", value: 23.1}));
	...
	QUnit.test("Custom tooltip added to an interaction area (interactive mode)", async function(assert) {
		//Arrange
		var sCustomTooltip = "Cumulative Totals\ncalculated in EURO",
	...
	QUnit.test("Suppress interaction area tooltip (interactive mode)", async function(assert) {
		//Arrange
	...
	QUnit.test("N/A value area tooltip", async function(assert) {
		//Arrange
	...
	QUnit.test("Standard behavior with semantic colors", async function(assert) {
		//Arrange
	...
	QUnit.test("Standard behavior with semantic colors (non-interactive)", async function(assert) {
		//Arrange
		this.oChart.getPoints()[0].setColor(ValueColor.Good);
	...
	QUnit.test("Validate Interactive Line Chart with Dynamic Data", async function(assert) {
		assert.equal(this.oChart.getPoints().length, 0, "Interactive Line Chart has no Points.");
		assert.equal(this.oChart.getDomRef().querySelector(".sapSuiteILCChartCanvas"), null, "Interactive Line Chart Canvas is not created.");
