...
sap.ui.define(["sap/base/util/deepClone", "sap/cards/ap/generator/app/controls/CompactFormatterSelection", "sap/m/CheckBox", "sap/m/Input", "sap/m/Select", "sap/m/StepInput"], function (deepClone, CompactFormatterSelection, CheckBox, Input, Select, StepInput) {
  "use strict";
  ...
  describe("CompactFormatterSelection", () => {
    ...
    test("createControl sets up controls correctly", () => {
      ...
      expect(compactFormatterSelection._selectorControl.getModel().getData()["formattersList"]).toMatchSnapshot();
      type = "Edm.DateTimeOffset";
      compactFormatterSelection.createControl(type);
      expect(compactFormatterSelection._selectorControl.getModel().getData()["formattersList"]).toMatchSnapshot();
      type = "Edm.TimeOfDay";
    ...
    test("_createParametersControl: returns Text control for string propertytype", () => {
      ...
      const context = {
        getProperty: jest.fn().mockReturnValue(property)
      };
    ...
    test("onFormatterSelected: updates control with selected formatter configuration", () => {
      const eventMock = {
        getSource: jest.fn().mockReturnValue({
          getSelectedKey: jest.fn().mockReturnValue("format.date")
        })
      };
      ...
      const targetProperty = compactFormatterSelection.getModel().getProperty("/configuration/advancedFormattingOptions/targetFormatterProperty");
      const formatter = Object.assign({
        property: targetProperty
      }, deepClone(mockGetFormatterConfiguration));
    ...
    test("onFormatterSelected: disable apply formatter button for invalid entry", () => {
      const model = compactFormatterSelection.getModel();
      const eventMock = {
        getSource: jest.fn().mockReturnValue({
    ...
    test("applyFormatter: should update data and fire change event", () => {
      ...
      const propertyValueFormatters = compactFormatterSelection.getModel().getProperty("/configuration/advancedFormattingOptions/propertyValueFormatters");
      expect(compactFormatterSelection.fireEvent).toHaveBeenCalledWith("change", {
...
//# sourceMappingURL=CompactFormatterSelection.spec.js.map