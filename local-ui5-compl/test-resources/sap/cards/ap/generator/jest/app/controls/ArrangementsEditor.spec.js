...
sap.ui.define(["sap/cards/ap/generator/app/controls/ArrangementsEditor", "sap/cards/ap/generator/config/TextArrangementOptions", "sap/m/Button", "sap/m/ComboBox", "sap/m/CustomListItem", "sap/m/List", "sap/m/Text", "sap/ui/core/Element", "sap/ui/core/library", "sap/ui/model/json/JSONModel", "sap/ui/core/date/Gregorian"], function (ArrangementsEditor, sap_cards_ap_generator_config_TextArrangementOptions, Button, ComboBox, CustomListItem, List, Text, CoreElement, sap_ui_core_library, JSONModel, sap_ui_core_date_Gregorian) {
  "use strict";

  const TEXTARRANGEMENT_OPTIONS = sap_cards_ap_generator_config_TextArrangementOptions["TEXTARRANGEMENT_OPTIONS"];
  const ValueState = sap_ui_core_library["ValueState"];
  describe("ArrangementsEditor", () => {
    let arrangementsEditor;
    const settings = {
      mode: "text",
      selectionKeys: {
    ...
    test("onAfterRendering: should update internal model and refresh combo box model after rendering", () => {
      ...
      const internalModel = arrangementsEditor._textArrangementComboBox.getModel("internal");
    ...
    test("setSelectionKeys: setSelectionKeys method", () => {
      ...
      const [, {
        factory: Factoryfn1
      }] = arrangementsEditor._propertyComboBox.bindAggregation.mock.calls[0];
      ...
      const [, {
        factory: Factoryfn2
      }] = arrangementsEditor._uomComboBox.bindAggregation.mock.calls[0];
      ...
      const [, {
        factory: Factoryfn3
      }] = arrangementsEditor._textArrangementComboBox.bindAggregation.mock.calls[0];
    ...
    test.skip("setItems: sets items property and binds controls properly", () => {
      ...
      expect(arrangementsEditor._propertyComboBox.bindProperty).toHaveBeenCalledWith("selectedKey", {
        path: "propertyKeyForId"
      });
      expect(arrangementsEditor._uomComboBox.bindProperty).toHaveBeenCalledWith("selectedKey", {
        path: "propertyKeyForDescription"
      });
      expect(arrangementsEditor._textArrangementComboBox.bindProperty).toHaveBeenCalledWith("selectedKey", {
    ...
    test("_onAddButtonClicked: adds an item to the model", () => {
      ...
      const model = arrangementsEditor.getModel();
...
//# sourceMappingURL=ArrangementsEditor.spec.js.map