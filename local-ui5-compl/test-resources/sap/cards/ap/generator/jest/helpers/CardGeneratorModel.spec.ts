...
import { FormatterConfigurationMap } from "sap/cards/ap/generator/helpers/Formatter";
...
describe("Card generation model creation", () => {
	...
	test("getCardActionInfo: Test the Generation of the Card Action information for CardGenerator Model when there exists actions for entityType", async () => {
		const component = {
			getModel: function () {
				return {
					isA: function () {
		...
		const cardActionsInfo = await getCardActionInfo(component, [], {});
		expect(cardActionsInfo).toMatchSnapshot();
		ApplicationInfo.getInstance()._resetInstance();
		windowSpy1.mockRestore();
	});

	test("getCardActionInfo: Test the Generation of the Card Action information for CardGenerator Model when there does not exist actions for entityType", async () => {
		const component = {
			getModel: function () {
				return {
					isA: function () {
...
describe("getCardGeneratorDialogModel", () => {
	...
	test("updateUnitOfMeasures", () => {
		...
		const expectedResult = updateUnitOfMeasures(unitOfMeasurement, formatterConfigsWithUnit as FormatterConfigurationMap);
