...
import {
	...
	parseCard,
	resolvePropertyLabelFromExpression,
	updateCardGroups,
	updateExistingCardManifest
} from "sap/cards/ap/generator/helpers/IntegrationCardHelper";
...
jest.spyOn(rootComponent, "getModel").mockImplementation((yesOrNo: string) => {
	return {
		isA: () => false,
...
describe("getCriticallityStateForGroup", () => {
	it("should return formatted property when state has a formatter", () => {
		const testCases = [
			{ input: "{= extension.formatters.formatCriticality(${op_id_fc}, 'state') }", expected: "{op_id_fc}" },
			{ input: "Warning", expected: "Critical" },
			{ input: "Success", expected: "Good" },
			{ input: "Error", expected: "Error" },
			{ input: "UnknownState", expected: "Neutral" },
...
describe("createInitialManifest", () => {
	...
	test("Create initial integration card manifest", () => {
		const manifest = createInitialManifest({
				...
				getMetaModel: () => {
					return {
						...
						getODataEntityType: () => {
							return {
								key: {
									propertyRef: [{ name: "node_key" }, { name: "so_id" }]
								},
								property: [
									{
										name: "node_key",
			...
			sapCoreVersionInfo: { version: "1.0.0", buildTimestamp: "2021-09-01T00:00:00Z" },
...
describe("parseCard", () => {
	...
	test("resolvePropertyLabelFromExpression should return the correct label", () => {
		const properties: PropertyInfoMap = [
			{
				type: "Edm.Decimal",
				name: "net_amount",
				label: "Net Amount",
...
describe("enhanceManifestWithConfigurationParameters", () => {
	...
	beforeAll(() => {
		...
		windowSpy.mockImplementation(() => ({
			hasher: {
				getHash: () =>
					"test-intent&/C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)"
			}
		}));
		ApplicationInfo.createInstance(rootComponent);
	});
...
describe("updateCardGroups for group level", () => {
	let manifest: CardManifest, oModel: JSONModel;
	let windowSpy: jest.SpyInstance;
	beforeAll(() => {
		...
		oModel.setProperty("/configuration/groups", [
			{
				title: "Group 1",
				items: [
					{ label: null, value: "{/item/1}" },
					{ name: "Item 2", label: "Label 2", value: "Value 2" }
				]
			}
		]);
...
describe("addQueryParametersToManifest", () => {
	it("should add query parameters when an already saved manifest w/o query parameters is reopened", () => {
		const manifest = {
			"sap.card": {
				...
				data: {
					request: {
						batch: {
							header: { url: "C_STTA_SalesOrder_WD_20({{parameters.contextParameters}})" },
							content: { url: "C_STTA_SalesOrder_WD_20({{parameters.contextParameters}})" }
						}
					}
				}
			}
		...
		const expectedResultHeader =
			"C_STTA_SalesOrder_WD_20({{parameters.contextParameters}})?{{parameters._headerSelectQuery}}{{parameters._headerExpandQuery}}";
		const expectedResultContent =
			"C_STTA_SalesOrder_WD_20({{parameters.contextParameters}})?{{parameters._contentSelectQuery}}{{parameters._contentExpandQuery}}";
	...
	it("should not modify the original manifest and the query parameters should not update", () => {
		...
		const expectedResultHeader =
			"C_STTA_SalesOrder_WD1_20({{parameters.contextParameters}})?{{parameters._headerSelectQuery}}{{parameters._headerExpandQuery}}";
		const expectedResultContent =
			"C_STTA_SalesOrder_WD1_20({{parameters.contextParameters}})?{{parameters._contentSelectQuery}}{{parameters._contentExpandQuery}}";
