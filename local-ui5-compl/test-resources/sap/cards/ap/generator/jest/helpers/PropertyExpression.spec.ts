...
import {
	...
	isExpression,
	isI18nExpression,
	parseFormatterExpression,
	resolvePropertyPathFromExpression
} from "sap/cards/ap/generator/helpers/PropertyExpression";
...
describe("Utility methods", () => {
	...
	test("getExpressionParts: parse expression and extract paths", () => {
		const expressions = [
			'{= format.float(${net_amount}, {"decimals":2,"style":"long"})}',
			'{= format.float(${net_amount}, {"decimals":2,"style":"long"})} {currency_code}',
			'{= format.float(${net_amount}, {"decimals":2,"style":"long"})} ({so_id}) {currency_code}',
			'{so_id} ({= format.float(${net_amount}, {"decimals":2,"style":"long"})}) {currency_code}'
		];
	...
	test("extractPathWithoutTextArrangement: extract path without text arrangement", () => {
		const expressions = [
			...
			'{= format.float(${net_amount}, {"decimals":2,"style":"long"})} ({so_id}) {currency_code}',
			'{so_id} ({= format.float(${net_amount}, {"decimals":2,"style":"long"})}) {currency_code}',
			'{= format.float(${net_amount}, {"decimals":2,"style":"short"})} ({=format.dateTime(${created_at}, {"relative":false,"UTC":true})}) {currency_code}'
		];
			...
			propertyValueFormatters: [
				{
					...
					displayName: "",
					parameters: [
						{ name: "type", displayName: "", type: "string", defaultValue: "", value: "${currency_code}" },
						{
							name: "options",
							displayName: "Options",
							type: "object",
			...
			propertyValueFormatters: [
				{
					...
					formatterName: "format.unit",
					displayName: "",
					parameters: [{ name: "type", displayName: "", type: "string", defaultValue: "", value: "${currency_code}" }],
					type: "numeric",
					visible: false
				}
			]
