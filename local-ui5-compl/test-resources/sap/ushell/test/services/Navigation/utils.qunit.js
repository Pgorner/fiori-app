...
/**
 * @fileOverview QUnit tests for Navigation service utils
 */
sap.ui.define([
    "sap/ushell/services/Navigation/utils",
    "sap/ushell/Container",
    "sap/ushell/TechnicalParameters"
], function (navigationUtils, Container, TechnicalParameters) {
    "use strict";
    ...
    [{
        ...
        QUnit.test("_parseGetLinksParameters when " + oFixture.testDescription, function (assert) {
            assert.deepEqual(
                navigationUtils._parseGetLinksParameters(oFixture.oGetLinksParams).sort(sortByNameField),
    ...
    [{
        ...
        QUnit.test("extractGetLinksParameterDefinition when " + oFixture.testDescription, function (assert) {
            assert.deepEqual(
                navigationUtils.extractGetLinksParameterDefinition(oFixture.oGetLinksParams),
    ...
    QUnit.module("_injectStickyParameter", {
        beforeEach: async function (assert) {
            ...
            this.oInjectParametersStub = sandbox.stub(navigationUtils, "injectParameters");

            await Container.init("local");
            this.AppLifeCycle = await Container.getServiceAsync("AppLifeCycle");
        },
        afterEach: function () {
            sandbox.restore();
            Container.reset();
    ...
    QUnit.test("Returns input parameters if app life cycle is undefined", async function (assert) {
        ...
        // Act
        const vResult = await navigationUtils.injectStickyParameters(oParams);
    ...
    QUnit.test("Calls getParameters of TechnicalParameter", async function (assert) {
        ...
        // Act
        await navigationUtils.injectStickyParameters(oParams);
    ...
    QUnit.test("Calls injectParameters", async function (assert) {
        // Arrange
        var oAppLifeCycle = {
            getCurrentApplication: function () {
                return {
    ...
    QUnit.test("Returns the result of injectParameters", async function (assert) {
        // Arrange
        var oAppLifeCycle = {
            getCurrentApplication: function () {
                return {
        ...
        // Act
        const oResult = await navigationUtils.injectStickyParameters(oParams);
    ...
    QUnit.test("Returns oParams.args when it is undefined", function (assert) {
        ...
        // Action
        var oResult = navigationUtils.injectParameters(oParams);
    ...
    QUnit.test("Self-invoke when oParam.args is like { target: { shellHash : string}}", function (assert) {
        ...
        var oInjectParametersStub = sandbox.stub(navigationUtils, "injectParameters");
        var oParamsToInject = {
            "sap-system": "UR5",
        ...
        // Assert
        assert.strictEqual(oInjectParametersStub.callCount, 2, "injectParameters are called twice");
        assert.strictEqual(oResult.target.shellHash, "MyHash", "The function injectParameters has returned the correct value.");
    });
    ...
    QUnit.module("_createAppStateFromData", {
        beforeEach: function () {
    ...
    QUnit.test("Creates AppState, saves data and resolves the AppState key", function (assert) {
        ...
        var oData = {};
        // Act
        return navigationUtils._createAppStateFromData(oData)
    ...
    QUnit.test("Rejects when the AppState Service is not available", function (assert) {
        ...
        // Act
        return navigationUtils._createAppStateFromData({})
            .then(function () {
    ...
    QUnit.module("injectStickyParameters", {
        beforeEach: function () {
            this.oGetServiceAsyncStub = sandbox.stub(Container, "getServiceAsync");
            ...
            this.oInjectStickyParametersStub = sandbox.stub(navigationUtils, "_injectStickyParameters").returnsArg(0);
        },
        afterEach: function () {
            sandbox.restore();
            Container.reset();
    ...
    QUnit.test("Calls the private method and resolves", function (assert) {
        ...
        var oParams = {};
        // Act
        return navigationUtils.injectStickyParameters(oParams)
    ...
    QUnit.test("Rejects when the AppLifeCycle Service is not available", function (assert) {
        ...
        // Act
        return navigationUtils.injectStickyParameters({})
            .then(function () {
                assert.ok(false, "the promise should have been rejected");
            })
            .catch(function () {
    ...
    QUnit.module("addXAppStateFromParameter", {
        beforeEach: function () {
            this.oGetServiceAsyncStub = sandbox.stub(Container, "getServiceAsync");
            ...
            this._oCreateAppStateFromDataStub = sandbox.stub(navigationUtils, "_createAppStateFromData").resolves("AppStateKey");
            this.oInjectParameterStub = sandbox.stub(navigationUtils, "_injectParameter");
        },
        afterEach: function () {
            sandbox.restore();
            Container.reset();
    ...
    QUnit.test("Resolves if there is no x-app-state data", function (assert) {
        ...
        // Act
        return navigationUtils.addXAppStateFromParameter(oIntent, "sap-xapp-state-data")
            .then(function () {
                // Assert
                assert.strictEqual(this._oCreateAppStateFromDataStub.callCount, 0, "_createAppStateFromData was not called");
            }.bind(this));
    });
    ...
    QUnit.test("Resolves and injects parameter", function (assert) {
        ...
        return navigationUtils.addXAppStateFromParameter(oIntent, "sap-xapp-state-data")
            .then(function () {
                // Assert
                assert.deepEqual(this._oCreateAppStateFromDataStub.getCall(0).args, [oAppStateData], "_createAppStateFromData was called with the correct app state data");
                var aExpectedArgs = [
                    {
                        target: { semanticObject: "AnObject", action: "action" },
                        params: {}
