...
sap.ui.define([
    ...
    [{
        ...
        oInbound: {
            ...
            signature: {
                additionalParameters: "allowed",
                parameters: { IMPORTANT: { required: true } }
            }
        }
    }, {
        ...
        expectedMatch: true, // match occurs but is a potential match...
        expectedMissingUserDefaultRefsOut: { // ... because the value of this default parameter is unknown
            someValidContentProviderId: { "UserDefault.currency": true }
        },
        sShellHash: "#SO-act",
        oKnownUserDefaultRefsIn: {}, // no user default parameters known
        oInbound: {
        ...
        sShellHash: "#SO-act?currency=EUR",
        oKnownUserDefaultRefsIn: {
            someValidContentProviderId: { "UserDefault.currency": "EUR" } // known user default
        },
        oInbound: {
            semanticObject: "SO",
            action: "act",
        ...
        sShellHash: "#SO-act?currency=EUR",
        oKnownUserDefaultRefsIn: {
            someValidContentProviderId: { "UserDefault.currency": "USD" } // NOTE: USD
        },
        oInbound: {
            semanticObject: "SO",
            action: "act",
    ...
    (function () {
        ...
        var oSampleInboundWithParameterMapping = {
            ...
            parameterMapping: { param1: { target: "param2" } },
            title: "Application State Example (Icons)",
            contentProviderId: "someContentProviderId"
        };

        var oSampleInboundWithNoParameterMapping = deepExtend({}, oSampleInboundWithParameterMapping);
        ...
        var oSampleInboundWithParameterMappingAndHideIntentLink = deepExtend({}, oSampleInboundWithParameterMapping);
        ...
        var oSampleInboundWithHideIntentLink = deepExtend({}, oSampleInboundNoPars);
        ...
        [{
            ...
            expectedResult: {
                intentParamsPlusAllDefaults: { P1: ["val1"] },
                genericSO: false,
                defaultedParamNames: [],
                resolutionResult: { contentProviderId: "someContentProviderId" },
                matches: true,
    ...
    [
        ...
        function fnValToString (vVal) {
            ...
            if (vVal === undefined) { return "undefined"; }
            if (vVal === "") { return '""'; }
            return ("" + vVal).replace("|", " or ");
        }
    ...
    [{
        ...
        actual: {},
        signatureParameters: {
            P1: { defaultValue: { value: undefined } }
        },
        expectedDefaultedParameterNames: ["P1"],
    ...
    [{
        ...
        QUnit.test("_matchToInbound: returns expected sort string when " + oFixture.testDescription, function (assert) {
            ...
            var oMatchResult = oSearch.matchOne(oFixture.oParsedShellHash, oSampleInbound, {}, {});
    ...
    QUnit.test("_matchToInbound: returns expected sort string with CURCP=0 when contentProviderId and sap-app-origin-hint are different", function (assert) {
        ...
        this.oParsedShellHash.params = { "sap-app-origin-hint": ["origin1"] };
    ...
    QUnit.test("_matchToInbound: returns sort string with CURCP=1 when contentProviderId and sap-app-origin-hint are both an empty string", function (assert) {
        ...
        this.oParsedShellHash.params = { "sap-app-origin-hint": [""] };
    ...
    QUnit.test("sortMatchingResultsDeterministic: prioritize result with the same origin", function (assert) {
        ...
        var oMatchResult1 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});

        this.oSampleInbound.contentProviderId = "origin1";
        var oMatchResult2 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});

        this.oSampleInbound.contentProviderId = "origin2";
        var oMatchResult3 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});
    ...
    QUnit.module("sap-app-origin", {
        beforeEach: function () {
            ...
            this.oGetLookupStub = sinon.stub().returns(new jQuery.Deferred().resolve(this.oLookup).promise());
        }
    });

    QUnit.test("Not matched when content provider is not matched", function (assert) {
        var fnDone = assert.async();

        var sHash = "#SO-aBC?sap-app-origin=CP2",
            oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Not matched when sap-app-origin is not matched and sap-app-origin-hint is matched", function (assert) {
        var fnDone = assert.async();

        var sHash = "#SO-aBC?sap-app-origin=CP2&sap-app-origin-hint=CP1",
            oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Matched when content provider is matched", function (assert) {
        var fnDone = assert.async();

        var sHash = "#SO-aBC?sap-app-origin=CP1",
            oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
        ...
        oSearch.match(
            ...
            var bMatches = oMatchResults.matchResults.length > 0;
            assert.strictEqual(bMatches, true, "inbound and intent should be matched");
        }, function () {
            assert.ok(false, "promise was resolved");
        }).then(fnDone);
    });

    QUnit.test("Matched when content provider is empty", function (assert) {
        var fnDone = assert.async();

        var sHash = "#SO-aBC?sap-app-origin=",
            oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Matched when sap-app-origin is not provided", function (assert) {
        var fnDone = assert.async();
