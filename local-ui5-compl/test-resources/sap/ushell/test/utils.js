...
sap.ui.define([
    ...
    var fnErrorConstructor = ushellUtils.Error;
    ...
    function createLogMock () {
        ...
        Log = Object.keys(oOriginalLog).reduce(function (oFinalLogMock, sNextProperty) {
    ...
    function getOwnScriptDirectory () {
        ...
        if (!sOwnScriptUrl) {
            ...
            var aScriptElements = window.document.getElementsByTagName("script");
            var oOwnScriptElement = aScriptElements[aScriptElements.length - 1];
    ...
    function prettyCdmSiteDeepEqual (assert, oActualSite, oExpectedSite, sTestDescription) {
        var sMethodPrefix = "[sap.ushell.test.utils#prettyCdmSiteDeepEqual] ";
        var sTestDescriptionPrefix = sMethodPrefix + sTestDescription + ": ";
        var oSiteCloneA = extend({}, oActualSite);
        var oSiteCloneE = extend({}, oExpectedSite);

        // function shortcuts
        var fnDeepEqualSitesPart = deepEqualByNamespace.bind(this, assert, sTestDescriptionPrefix,
            oSiteCloneA, oSiteCloneE);
        var fnPrettyDeepEqualOfObjectMaps = prettyDeepEqualOfObjectMaps.bind(this, assert, sTestDescriptionPrefix);
    ...
    function overrideObject (oObject, oProperties) {
        var oObjectClone = deepExtend({}, oObject);
    ...
    function resetConfigChannel (oUshellConfig) {
        return new Promise(function (resolve, reject) {
            ...
            sap.ui.require([
                ...
                "sap/ushell/Config"
            ], function (CommonCreateConfigcontract, Config) {
    ...
    function registerFakesForRequire (oFakes) {
        ...
        var fnOriginalDefine = sap.ui.define;

        var oRequireStub = sinon.stub(sap.ui, "require").callsFake(function (vPath, fnCallback) {
            // Probing
        ...
        // sap.ui.define
        var oDefineStub = sinon.stub(sap.ui, "define").callsFake(function (aDefineInput, fnDefineCallback) {
            var iOriginalLength = aDefineInput.length;
            var aForwardedInput = [];
            var oStubbedInput = {};
            var aMergedCallbacks = [];

            // Removed items that are stubbed
            for (var i = 0; i < iOriginalLength; i++) {
            ...
            fnOriginalDefine.call(this, aForwardedInput, function () {
                var aForwardedResult = Array.from(arguments);
                for (var count = 0; count < iOriginalLength; count++) {
    ...
    return {
        ...
        waitForEventHubEvent: waitForEventHubEvent,
        waitForEventHubEvents: waitForEventHubEvents,
        onError: onError,
        ...
        registerFakesForRequire: registerFakesForRequire
    };
});
