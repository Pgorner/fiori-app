...
sap.ui.define([
    ...
    QUnit.module("Card CHIP controller", {
        beforeEach: function () {
            ...
            return Controller.create({ name: "sap.ushell.components.cards.Card" }).then(function (oController) {
    ...
    QUnit.test("onPress: navigate to the card tile's navigation target", function (assert) {
        ...
        setTimeout(function () {
            ...
            var oNavigationArguments = oNavigationServiceStub.navigate.getCall(0).args[0];
    ...
    QUnit.module("Card CHIP configuration controller", {
        beforeEach: function () {
            ...
            return Controller.create({ name: "sap.ushell.components.cards.Configuration" }).then(function (oController) {
                this.oController = oController;
                sinon.stub(this.oController, "getView").returns(this.oView);
            }.bind(this));
        },
    ...
    QUnit.test("_checkOriginalLanguage: the user is logged on in the original language of the card", function (assert) {
        ...
        var oModel = new JSONModel({ config: {} });
        this.oView.getModel = sinon.stub().returns(oModel);
