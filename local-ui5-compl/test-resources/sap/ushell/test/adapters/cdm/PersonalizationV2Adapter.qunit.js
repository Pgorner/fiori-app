 ...
 *               sap.ushell.adapters.cdm.PersonalizationV2Adapter
 */
sap.ui.define([
    ...
    "sap/ushell/services/PersonalizationV2/constants.private", // TODO: clarify if private access is OK for adapter
    ...
    oStorageConstants,
    oInternalPersonalizationConstants,
    oTestUtils
) {
    "use strict";
    ...
    QUnit.module("PersonalizationV2Adapter");
    QUnit.test("PersonalizationV2Adapter", function (assert) {
        var oPersAdapter,
        ...
        oPersAdapter = new PersonalizationV2Adapter(oSystem, null, oConfig);
        assert.strictEqual(typeof oPersAdapter, "object", "expected PersonalizationV2Adapter to be an object");
    });
    ...
    QUnit.test("PersonalizationV2Adapter: construct an http client with the expected configuration", function (assert) {
    ...
    [
        ...
        QUnit.test("delAdapterContainer: " + oFixture.testDescription, function (assert) {
            ...
            // 2. It should not throw when a cache is not available.
            oDeletedPromise = oAdapterInternals.delAdapterContainer(
                oFixture.input.adapterConfig,
    ...
    QUnit.test(".getStorageResourceRoot", function (assert) {
        ...
        // -- Test improper usage
        [
            { input: { /* invalid <undefined> config */ } },
            { input: { config: null /* invalid <null> config */ } },
            { input: { config: "" /* invalid <string> config */ } },
            { input: { config: 123 /* invalid <int> config */ } },
            { input: { config: {} /* invalid <empty> config */ } }
        ].forEach(function (oFixture) {
            assert.throws(
                function () {
    ...
    [{
        ...
        testDescription: "container key is prefixed with standard container prefix but exceeds maximum length",
        input: {
    ...
    [{
        ...
        QUnit.test("#save " + oFixture.testDescription, function (assert) {
            ...
            var done = assert.async(),
                sOriginalInputContainerData = extend({}, oFixture.input.containerData, true);
        ...
        var oFixture = {
            input: {
                oContainerData: {
                    ...
                    // ...
                    "item-n": {},
                    // prefixed item should be double prefixed
    ...
    [{
        ...
        QUnit.test("setItemValue:  " + oFixture.testDescription, function (assert) {
            var oContainerData = extend({}, oFixture.input.oContainerData, true);
    ...
    [{
        ...
        input: {
            aHeaders: [
                { name: "x-sap-lrep-time", value: "34ms" },
                { name: "expires", value: "Wed, 19 Dec 2018 08" },
                { name: "cache-control", value: "max-age=31536000, private" },
                { name: "x-csrf-token", value: "MBXUhKZuSSJzV73bSjt3SQ==" },
                { name: "etag", value: "\"98858D1F374710A720C8213DE2181B815610BD76\"" },
                { name: "content-length", value: "932" },
        ...
        description: "header item is twice in the header array",
        input: {
            aHeaders: [
                { name: "cache-control", value: "max-age=31536000, private" },
        ...
        description: "header item is not in the header array",
        input: {
            aHeaders: [
                { name: "x-sap-lrep-time", value: "34ms" },
                { name: "expires", value: "Wed, 19 Dec 2018 08" },
