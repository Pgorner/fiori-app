...
sap.ui.define([
	...
	function commonSetup(assert){
		...
		this.coreApi = new CoreApiDouble({
			instances : {
		...
		this.BindingStub = function BindingStub() {
			that.updateTreeTableCounter = 0;
			this.getFilter = function(){
				return that.selectionFilter || new CoreFilter(that.messageHandler);
			};
		...
		this.configurationTemplate = sampleConfig.getSampleConfiguration();
		this.requestCounter = 0;
		// a Request double, used in the context of this QUnit.module only
		this.RequestStub = function(inject, config) {
		...
		this.configurationFactory = new ConfigurationFactory({
			...
			constructors : {
				Binding: this.BindingStub,
				Hashtable : Hashtable,
				Request: this.RequestStub
			}
		});
		this.configurationFactory.loadConfig(this.configurationTemplate);
		this.generateOdataPathStub = sinon.stub(uriGenerator, "generateOdataPath", function (){
			return "path";
		});
		this.cumulativeFilter = new CoreFilter(this.messageHandler, "FilterableProperty", "EQ", 1); 
		this.cumulativeFilter.addAnd("NotFilterableProperty", "EQ", 1);
		this.cumulativeFilter.addAnd("Parameter", "EQ", 1);
		this.cumulativeFilter2 = new CoreFilter(this.messageHandler, "FilterableProperty2", "EQ", 2); 
	}
	...
	QUnit.test("OData Model", function(assert) {
		...
		this.hStep.update(this.cumulativeFilter, () => { 
			var firstOdataModel = this.updateTreeTableParameters.odataModel;
			assert.ok(firstOdataModel instanceof ODataModelV2, "OData model instantiated");
			...
			this.hStep.update(this.cumulativeFilter2, () => {
	...
	QUnit.test("ControlObject.filters for Treetable if no applicable filter for request", function(assert) {
		const done = assert.async();
		this.hStep.update(new CoreFilter(this.messageHandler), () => {
	...
	QUnit.test("ControlObject constants for Treetable", function(assert) {
		const done = assert.async();
		this.hStep.update(this.cumulativeFilter, () => {
			assert.strictEqual(this.updateTreeTableParameters.controlObject.parameters.operationMode, OperationMode.Server , "OperationMode set correctly");
	...
	QUnit.test("Update tree table is called when parameter has changed", function(assert) {
		...
		this.hStep.update(this.cumulativeFilter, () => {
			...
			this.hStep.update(cumulativeFilterWithAdditionalParameter, () => {
				callback();
	...
	QUnit.test("If hierarchyProperty has no hierarchyAnnotations", async function(assert) {
		this.hStep = this.configurationFactory.createStep("nonHierarchicalStepId");
		await this.hStep.whenMetadataInitialized();
	...
	QUnit.test("Request sent on update call", function(assert) {
		const done = assert.async();
		assert.expect(4);
		this.selectionFilter = new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", "1001");
	...
	QUnit.test("Request not sent when selection is empty", function(assert) {
		const done = assert.async();
		assert.expect(2);
		this.selectionFilter = new CoreFilter(this.messageHandler);
		this.hStep = this.configurationFactory.createStep("hierarchicalStepId");
		this.hStep.update(this.cumulativeFilter, callback.bind(this));
		function callback(){
	...
	QUnit.test("Request sent with only the nodeID from selection", function(assert) {
		...
		this.selectionFilter = new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", "1001");
		this.hStep = this.configurationFactory.createStep("hierarchicalStepId");
		this.hStep.update(new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", "1"), callback.bind(this));
		function callback(){
			assert.strictEqual(this.requestCounter, 1, "One request sent");
			assert.ok(true, "Callback called");
			done();
	...
	QUnit.test("When server answers with values that were previously not selected", function(assert){
		...
		this.selectionFilter = new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", "1001").addAnd("hierarchyPropertyNodeID", "eq", "1002");
		this.hStep = this.configurationFactory.createStep("hierarchicalStepId");
		this.hStep.update(this.cumulativeFilter, callback.bind(this));
		function callback(){
			assert.strictEqual(this.requestCounter, 1, "One request sent");
	...
	QUnit.test("With selectable property and selection", function(assert) {
		this.selectionFilter = new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", 2);
		var hStep = this.configurationFactory.createStep("hierarchicalStepId");
		var newCumulativeFilter = hStep.adjustCumulativeFilter(new CoreFilter(this.messageHandler, "hierarchyPropertyNodeID", "eq", 1).addAnd("Property", "eq", 1));
