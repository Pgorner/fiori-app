sap.ui.define([
	...
	'sap/apf/testhelper/doubles/step',
	...
	// <div id="apf_deployment_information" export_folder="../../../../export/"></div>
	...
	QUnit.test('Load binding fails if a representation has no id', function(assert) {
		assert.throws(function() {
			this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("representationWithoutId"));
	...
	QUnit.test('Load binding fails if representations have duplicated ids', function(assert) { 
		...
		this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("representationsWithDuplicatedtIds"));
	...
	QUnit.test('Set item', function(assert) {
		var oFactory = new ConfigurationFactory({
			instances : {
				messageHandler : this.oMessageHandler,
				coreApi : this.oCoreApi
			}
	...
	QUnit.test('Load config again in order to replace the first config', function(assert) {
		this.oConfigurationFactory.loadConfig(this.oConfigurationTemplate);
		var secondConfig = sampleConfig.getSampleConfiguration();
	...
	QUnit.test('Injects of Step constructor', function(assert) {
		...
		var oStep;
		this.oConfigurationInject.constructors.Step = fnStepConstructor;
		this.oConfigurationFactory.loadConfig(this.oConfigurationTemplate);
	...
	QUnit.test('Get service documents', function(assert) {
		this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("secondServiceDocument"));
	...
	QUnit.test('Representation constructor module path does not contain a function', function(assert) {
		this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("wrongRepresentationConstructor"));
	...
	QUnit.test('Mal formed step assignment throws technical error', function(assert) {
		assert.expect(1);
		try {
			this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("malformedStepAssignmentForCategory"));
		} catch (err) {
	...
	QUnit.test('category assignment for not existing step throws technical error', function(assert) {
		assert.expect(1);
		try {
			this.oConfigurationFactory.loadConfig(sampleConfig.getSampleConfiguration("notExistingStepAssignedToCategory"));
		} catch (err) {
	...
	QUnit.module('Error behavior', {
		beforeEach : function(assert) {
			this.oMessageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().supportLoadConfigWithoutAction().spyPutMessage();
			this.oCoreApi = new CoreApiDouble({
				instances : {
					messageHandler : this.oMessageHandler
				}
			...
			this.oResourcePathHandler = new ResourcePathHandlerDouble({
				instances : {
					messageHandler : this.oMessageHandler
				}
	...
	QUnit.module('SmartFilterBar', {
		beforeEach : function(assert) {
			this.messageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage();
			this.configurationFactory = new ConfigurationFactory({
				instances : {
					messageHandler : this.messageHandler
				}
			});
			this.sampleConfiguration = sampleConfig.getSampleConfiguration();
		}
	});
	...
	QUnit.module('Load facet filter config', {
		beforeEach : function(assert) {
			...
			this.sampleConfiguration = sampleConfig.getSampleConfiguration('addTwoFacetFilters');
		}
	});
	QUnit.test('Configured facet filters create no indicator in idRegistry for facet filter option', function(assert) {
		...
		assert.equal(this.idRegistry.getItem(coreConstants.existsEmptyFacetFilterArray), undefined, 'Indicator for facet filter not set');
	});
	QUnit.test('Configured facet filters are loaded in design time', function(assert) {
		...
		var expectedFilters = deepExtend([], this.sampleConfiguration.facetFilters);
	...
	QUnit.test('Empty array creates indicator in idRegistry', function(assert) {
		...
		this.configurationFactory.loadConfig(this.sampleConfiguration);
		this.configurationFactory.getRegistry();
		assert.equal(this.idRegistry.getItem(coreConstants.existsEmptyFacetFilterArray), true, 'Indicator for facet filter set');
	});
	...
	QUnit.module('Error handling when loading', {
		beforeEach : function(assert) {
			this.messageHandler = new MessageHandlerDouble().putMessageOnCheck().spyPutMessage();
			var metadataFacadeStub = {
					getAllProperties : function(callback) {
						callback( [ 'property1']);
					},
	...
	QUnit.test('Mandatory properties for facet filter object', function(assert) {
		...
		var sampleConfiguration = sampleConfig.getSampleConfiguration();
	...
	QUnit.module("Configuration from modeler", {
		beforeEach : function(assert) {
			...
			this.oCoreApi.createRepresentation = function(sRepresentationConstructorPath, oConfig) {
				var interfaceProxy = new RepresentationInterfaceProxy(this.oCoreApi, {});
			...
			this.oConfigurationTemplate = this.getJSON(
				sap.ui.require.toUrl('sap/apf/testhelper/config/analyticalConfigurationFromModeler1.json'), assert);
			this.oConfigurationFactory = new ConfigurationFactory({
				instances : {
					messageHandler : this.oMessageHandler,
					coreApi : this.oCoreApi
