...
sap.ui.define([
	...
	function setupTestEnvironment(o) {
		function defineFilterOperators() {
			Object.assign(o, coreConstants.FilterOperators);
		}
		o.Filter = CoreFilter;
		defineFilterOperators();
		var oMessageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage();
		o.oMessageHandler = oMessageHandler;
		o.oCoreApi = new CoreApiDouble({
			instances : {
		...
		o.oMetadataDouble = new MetadataDouble({
			instances : {
				messageHandler : oMessageHandler
			},
			constructors : {
	...
	QUnit.module('Check generated URIs', {
		beforeEach : function(assert) {
			...
			this.request = new Request(this.oInject, {
				service : 'dummy',
	...
	QUnit.test('Build uri with filter validation request', function(assert) {
		...
		var selectionFilter =  new CoreFilter(this.oMessageHandler, 'FilterPropertyTwo', this.EQ, '2');
		...
		assert.equal(this.sRequestUri, sExpected, 'Exact URI (case sensitive) expected');
		assert.equal(this.secondRequestUri, "entityTypeWithParams(p_stringParameter=%27defaultString%27,p_int32Parameter=4711)/Results?$select=FilterPropertyTwo&$filter=((FilterPropertyOne%20eq%20%271%27)%20and%20(FilterPropertyTwo%20eq%20%272%27))&$format=json", 'Exact second URI (case sensitive) expected');
	});
