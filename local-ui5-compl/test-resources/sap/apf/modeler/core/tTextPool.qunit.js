...
sap.ui.define([
...
], function(coreConstants, TextPool, MessageHandlerDouble, Hashtable, parseTextPropertyFile, ResourceBundle, DateFormat, sinon){
	'use strict';
	function commonSetupTextPool(assert, context, applicationId, expectedUuid, odataDoChangeOperationsInBatch) {
		...
		context.messageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage();
		context.texts = [ {
		...
		context.textPool = new TextPool(context.inject, context.ApplicationId, context.texts);
	}
	...
	QUnit.test("set empty text and get the special predefined key", function(assert) {
		this.textPool.setTextAsPromise("", {
			...
			assert.equal(textKey, coreConstants.textKeyForInitialText, "Expected Guid");
			var text = this.textPool.get(coreConstants.textKeyForInitialText);
			assert.equal(text.TextElement, coreConstants.textKeyForInitialText, "Correct Key");
	...
	QUnit.test("Parse a valid text file", function(assert) {
		var i;
		var fileString = this.textPool.exportTexts("NameOfAnalyticalConfiguration");
		var textFileInformation = parseTextPropertyFile(fileString, {
			instances : {
	...
	QUnit.test("Parse text property file with missing apf application id", function(assert) {
		...
		var textFileInformation = parseTextPropertyFile(propertyFile, {
			instances : {
				messageHandler : this.messageHandler
			}
		});
	...
	QUnit.module("M: Error missing format during export of textPool", {
		beforeEach : function() {
			...
			this.textPool = new TextPool(this.inject, this.ApplicationId, this.texts);
		}
	});
	QUnit.test("Export Texts", function(assert) {
		var oDateFormat = DateFormat.getDateTimeInstance({
	...
	QUnit.test("remove 2 texts from textPool", function(assert) {
		assert.expect(6);
		var that = this;
