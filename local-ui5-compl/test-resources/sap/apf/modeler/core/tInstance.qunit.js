...
sap.ui.define([
	...
	function createInstancesObject(manifest) {
		var instances = {
			component : {
				getManifest() {
		...
		Object.setPrototypeOf(instances.component, UIComponent.prototype);
		return instances;
	}
	...
	function setupModelerInstance(oContext, assert) {
		...
		var inject = {
			instances : createInstancesObject({}),
			constructors : {
	...
	QUnit.test("WHEN get the representation types", function(assert) {
		...
		var predefinedRepresentationTypes = fnRepresentationTypes();
	...
	QUnit.test("Import from Lrep with existing configId, and renaming configuration", function (assert){
		...
		this.modelerInstance.importConfigurationFromVendorLayer(this.expectedApplicationID, this.configIdForRepeatedImport ,callbackOverwrite,callbackImport.bind(this));
	});
	...
	QUnit.test("WHEN probe injected THEN is active", function(assert) {
		...
		var fnProbe = function(oInternalHandlers) {
			...
			assert.strictEqual(oInternalHandlers.messageHandler instanceof MessageHandler, true, "expected instance type");
			...
			assert.strictEqual(oInternalHandlers.proxyTextHandlerForLocalTexts instanceof ProxyTextHandlerForLocalTexts, true, "THEN default proxy text handler has been injected");
		};
		var persistenceConfiguration = {
			serviceRoot : "/serviceRoot"
		};
	...
	QUnit.module("M: Tests for getUnusedTextKeys - GIVEN prepared and isolated modeler core instance", {
		beforeEach : function() {
			...
			this.GetTextKeysFromAllConfigurations = function() {
				var resultValue = new Hashtable(that.messageHandler);
	...
	QUnit.module("Instantiation of Persistence Proxy", {
		beforeEach: function() {
			this.spyCloudFoundryProxy = sinon.spy(modelerProxy, "ModelerProxy");
	...
	QUnit.test("Injection of AjaxHandler and isUsingCloudFoundry is true", function(assert){
		...
		var AjaxHandlerStub = function(inject) {
			assert.ok(inject.instances.messageHandler instanceof MessageHandler, "THEN messageHandler has been injected");
			assert.ok(typeof inject.functions.coreAjax === 'function', "THEN function has been injected for ajax");
		};
		var inject = {
				instances : createInstancesObject({ type: "manifestOfComponent" }),
				constructors: {
	...
	QUnit.test("Injection of function isUsingCloudFoundryProxy returning TRUE, so that cloud foundry modeler proxy shall be used", function(assert){
		...
		var inject = {
				...
				instances : createInstancesObject(manifestForCloudFoundryProxy),
				functions: {
					isUsingCloudFoundryProxy : function() { return true; }
				}
		};
		...
		assert.ok(proxyInject.instances.ajaxHandler instanceof AjaxHandler, "THEN ajax handler has been injected");
	...
	QUnit.test("Injection of manifest setting activateLrep = true", function(assert){
		...
		var inject = {
				...
				instances : createInstancesObject({ "sap.apf" : { activateLrep : true } }),
				functions: {
					isUsingCloudFoundryProxy : function() { return false; }
				}
		};
	...
	QUnit.test("Injection of function isUsingCloudFoundryProxy returning true wins over manifest setting activateLrep = true", function(assert){
		assert.expect(3);
		var manifest = deepExtend({}, manifestForCloudFoundryProxy, { "sap.apf" : { activateLrep : true }});
		var inject = {
				constructors: {
					OdataProxy : this.spyOdataProxy,
					LayeredRepositoryProxy: this.spyLayeredRepositoryProxy
				},
				instances : createInstancesObject(manifest),
				functions: {
					isUsingCloudFoundryProxy : function() { return true; }
				}
		};
	...
	QUnit.test("WHEN isUsingCloudFoundryProxy returns false AND layered repository is NOT active", function(assert){
		...
		var inject = {
				instances : createInstancesObject({ "sap.apf" : { activateLrep : false } }),
				exit: {
	...
	QUnit.test("WHEN component is defined by injection from ModelerComponent", function(assert) {
		var instances = createInstancesObject(undefined);
		var ComponentStub = instances.component;
		var instance = new ModelerCoreInstance({}, {
			instances: instances
		});
