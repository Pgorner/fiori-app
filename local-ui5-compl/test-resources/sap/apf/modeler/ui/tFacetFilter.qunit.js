...
sap.ui.define([
	...
	var FacetFilterEvents = modelerConstants.events.facetFilter;
	...
	async function _instantiateView(sId, assert) {
		var oFacetFilterController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.facetFilter"
		});
	...
	function _commonTestForInvisibleFilters(assert, idForPreselectionMode) {
		...
		_assertsForConfigListIfValues(assert, oFacetFilterView.byId("idVHNone"), false);
		assert.strictEqual(oFacetFilterView.byId("idUseVHRAsFRRCheckBox").getSelected(), false, "then use same as VHR checkbox is not selected");
		assert.strictEqual(spyFireEvent.calledWith(FacetFilterEvents.ENABLEDISABLEFRRFIELDS), true, "then enableDisableFRRFieldsEvent is fired when facet filter is invisible");
		assert.strictEqual(spyFireEvent.calledWith(FacetFilterEvents.DONOTSHOWATRUNTIME), true, "then doNotShowAtRuntimeEvent is fired when facet filter is invisible");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	}
	...
	async function _placeViewAt(oFacetFilterView) {
	...
	QUnit.module("Test for a facet filter which is new", {
		...
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oTextPool = oModelerInstance.configurationHandler.getTextPool();
				spyOnGetFilterablePropertiesAndParametersAsPromise = sinon.spy(oModelerInstance.configurationEditorForUnsavedConfig, "getFilterablePropertiesAndParametersAsPromise");
				oFacetFilterView = await _instantiateView("newFilter0", assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When facet filter is initialized", async function(assert) {
		// arrange
		await _placeViewAt(oFacetFilterView);
		// assert 
	...
	QUnit.module("Test for a facet filter is visible, multiple selection, has preselection mode as none and value help as none ", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oFacetFilterView = await _instantiateView(oModelerInstance.facetFilterIdUnsaved2, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When preselection mode is automatic values, value help is value help request and use same as VHR checkbox is selected", function(assert) {
		...
		assert.strictEqual(spyFireEvent.calledWith(FacetFilterEvents.USESAMEASVHR), true, "then useSameAsVHREvent is fired when use same as VHR is selected");
		assert.strictEqual(spyFireEvent.calledWith(FacetFilterEvents.ENABLEDISABLEFRRFIELDS), true, "then enableDisableFRRFieldsEvent is fired when use same as VHR is selected");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	});
	QUnit.test("When facet filter is set to invisible", function(assert) {
		//arrange
	...
	QUnit.test("When facet filter's configuration value list is set with values", async function(assert) {
		// action
		await _placeViewAt(oFacetFilterView);
	...
	QUnit.test("When facet filter's configured value list is emptied", async function(assert) {
		//action
		await _placeViewAt(oFacetFilterView);
	...
	QUnit.test("When preselection mode is fixed values and adding more than one default value", async function(assert) {
		//arrange
		await _placeViewAt(oFacetFilterView);
		...
		assert.strictEqual(oFacetFilterView.byId("idPreselectionDefaults").getValueState(), ValueState.None, "No warning displayed when trying to add more than one value");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.called, true, "then configuration editor is set to unsaved");
		//cleanup
		document.body.removeChild(document.getElementById('contentOfFF'));
	});
	QUnit.test("When preselection mode is fixed values and removing the default value", async function(assert) {
		//arrange
		await _placeViewAt(oFacetFilterView);
		var spyOnFFSetPreselectionDefaults = sinon.spy(oModelerInstance.facetFilterUnsaved2, "setPreselectionDefaults");
		//action
	...
	QUnit.test("When facet filters selection mode is changed from multiple to single selection", function(assert) {
		...
		assert.strictEqual(oFacetFilterView.byId("idPreselectionDefaults").getValueState(), ValueState.Warning, "Warning displayed when trying to add more than one value");
	...
	QUnit.test("When preselection mode is fixed values and value help is configuration list of values and changing the value of config list of values", async function(assert) {
		// action
		await _placeViewAt(oFacetFilterView);
		oFacetFilterView.byId("idConfigListOfValuesMultiInput").setValue("token2");
		oFacetFilterView.getController().handleChangeForConfigListOfValues(_eventCreation("token2"));
	...
	QUnit.test("When preselection mode is fixed values and value help is configuration list of values and removing the config list of values", async function(assert) {
		//action
		await _placeViewAt(oFacetFilterView);
		var token1 = oFacetFilterView.byId("idConfigListOfValuesMultiInput").getTokens()[0];
		oFacetFilterView.byId("idConfigListOfValuesMultiInput").removeToken(token1);
	...
	QUnit.test("When label of facet filter is changed", function(assert) {
		//arrange
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.FACETFILTER_LABEL;
	...
	QUnit.test("When suggestions are triggered for label of facet filter", function(assert) {
		//arrange
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.FACETFILTER_LABEL;
		var oSuggestionTextHandler = new textPoolHelper.SuggestionTextHandler(oTextPool);
		var oSuggestionTextHandlerStub = sinon.stub(textPoolHelper, "SuggestionTextHandler", function() {
	...
	QUnit.test("When facet filter view data has to be reset then", function(assert) {
		...
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET), true, "then sub views VHR and FRR are updated with new instances of editor and facet filter objects");
	});
	...
	QUnit.test("When preselection mode is fixed values and changing the default value", async function(assert) {
		//arrange
		await _placeViewAt(oFacetFilterView);
		var spyOnFFSetPreselectionDefaults = sinon.spy(oModelerInstance.facetFilterUnsaved2, "setPreselectionDefaults");
		var spyOnRemovePreselectionDefaults = sinon.spy(oModelerInstance.facetFilterUnsaved2, "removePreselectionDefaults");
		...
		assert.strictEqual(oFacetFilterView.byId("idPreselectionDefaults").getValueState(), ValueState.None, "No Warning displayed when trying to change the default value");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.called, true, "then configuration editor is set to unsaved");
		//cleanup
		document.body.removeChild(document.getElementById('contentOfFF'));
	});
		...
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetFilterablePropertiesAndParametersAsPromise = sinon.stub(oModelerInstance.configurationEditorForUnsavedConfig, "getFilterablePropertiesAndParametersAsPromise", function() {
					return utils.createPromise([ "property1", "property3", "property4" ]);
				});
				oFacetFilterView = await _instantiateView(oModelerInstance.facetFilterIdUnsaved2, assert);
				done();
			});
