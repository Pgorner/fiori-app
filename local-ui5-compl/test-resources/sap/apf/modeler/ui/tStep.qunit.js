...
sap.ui.define([
...
], function(modelerConstants, textPoolHelper, modelerUIHelper, utils, Input, Item, Controller, View, ViewType, nextUIUpdate) {
	'use strict';

	// keep setTimeout working
	sinon.config = {
		useFakeTimers : false
	};
	...
	async function _instantiateView(sId, assert, bIsHierarchicalStep) {
		var oStepController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.step"
		});
		...
		var oView = await View.create({
			...
			viewData : {
				...
				StepPropertyMetadataHandler : _stepPropertyMetadataHandlerStub
			}
		});
	...
	async function _placeViewAt(oStepView) {
	...
	QUnit.module("Test for a step which is new", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepView = await _instantiateView("newStep0", assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When step view is initialized", async function(assert) {
		// arrange
		await _placeViewAt(oStepView);
		//asserts
	...
	QUnit.module("Test for an existing step with filter mapping", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				_commonSpiesInBeforeEach(oModelerInstance);
				oStepView = await _instantiateView(oModelerInstance.stepIdUnsavedWithFilterMapping, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When step view data has to be reset then", function(assert) {
		...
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET), true,
	...
	QUnit.test("When step title is changed", function(assert) {
		//arrange
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.STEP_TITLE;
	...
	QUnit.test("When step long title is changed", function(assert) {
		//arrange
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.STEP_LONG_TITLE;
	...
	QUnit.module("Test for an existing step with an already set top-N value", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				_commonSpiesInBeforeEach(oModelerInstance);
				oStepView = await _instantiateView(oModelerInstance.stepIdUnsavedWithoutFilterMapping, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.module("When a new hierarchical step without filter is created", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				_commonSpiesInBeforeEach(oModelerInstance);
				oStepView = await _instantiateView("newStep1", assert, true);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When hierarchical step view is initialized", async function(assert) {
		// arrange
		await _placeViewAt(oStepView);
		//asserts
		assert.strictEqual(oStepView.byId("idStepTitle").getValue(), "", "then value of step title is displayed");
	...
	QUnit.module("When an existing hierarchical step without required filter is loaded", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				_commonSpiesInBeforeEach(oModelerInstance);
				oStepView = await _instantiateView(oModelerInstance.firstHierarchicalStepId, assert, true);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When existing hierarchical step view is initialized", async function(assert) {
		// arrange
		await _placeViewAt(oStepView);
		//asserts
	...
	QUnit.module("When an existing hierarchical step with required filter is loaded", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				_commonSpiesInBeforeEach(oModelerInstance);
				oStepView = await _instantiateView(oModelerInstance.hierarchicalStepWithFilterId, assert, true);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When existing hierarchical step (with filter) view is initialized", async function(assert) {
		// arrange
		await _placeViewAt(oStepView);
		//asserts
