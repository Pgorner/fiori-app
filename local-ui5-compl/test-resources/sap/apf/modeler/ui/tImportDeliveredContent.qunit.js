sap.ui.define([
...
], function(modelerUIHelper, Table, Fragment, coreLibrary, Controller, View, ViewType, nextUIUpdate, sinon, jQuery) {
	'use strict';
	...
	QUnit.test("When importing delivered content and entering an application name which does not exist in list of applications in vendor layer", async function(assert) {
		//action
		oImportDeliveredContentView.byId("idAppConfigCombobox").setValue("Dummy Application");
		oImportDeliveredContentView.byId("idImportDeliveredContentDialog").getBeginButton().firePress();
		await nextUIUpdate();
		//assertion
		assert.strictEqual(oImportDeliveredContentView.byId("idAppConfigCombobox").getValueState(), ValueState.Error, "then value state of combo box is set to error state");
	...
	QUnit.test("When importing delivered content and selecting an application name from existing list of applications in vendor layer", async function(assert) {
		//action
		oImportDeliveredContentView.byId("idAppConfigCombobox").setSelectedItem(oImportDeliveredContentView.byId("idAppConfigCombobox").getItems()[1]);
		...
		await nextUIUpdate();
		//assertion
		assert.strictEqual(oImportDeliveredContentView.byId("idAppConfigCombobox").getValueState(), ValueState.None, "then value state of combo box is set to None");
	});
	...
	QUnit.test("When importing delivered content and when configuration already exists", function(assert) {
		...
		setTimeout(async function() {
			...
			assert.strictEqual(oOverwriteDialog.getContent()[0].getItems()[2].getItems()[1].getValueState(), ValueState.Error, "then Value state for input field is set to Error since its blank");
	...
	QUnit.test("When importing delivered content and when configuration already exists and on not overwriting existing configuration and not providing title for new config", function(assert) {
		...
		setTimeout(async function() {
			...
			assert.strictEqual(oOverwriteDialog.getContent()[0].getItems()[2].getItems()[1].getValueState(), ValueState.Error, "then Value state for input field is set to Error since title for new config is empty");
			//cleanup
			oOverwriteDialog.getEndButton().firePress();
			await nextUIUpdate();
			done();
	...
	QUnit.test("When import delivered content view is opened twice", async function(assert) {
		for (var i = 0; i < 2; i++) {
			var oImportDeliveredContentController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.importDeliveredContent"
			});
			var oParentControl = new Table();
			var oImportDeliveredContentView = await View.create({
				viewName : "sap.apf.modeler.ui.view.importDeliveredContent",
				type : ViewType.XML,
				controller : oImportDeliveredContentController,
				viewData : {
