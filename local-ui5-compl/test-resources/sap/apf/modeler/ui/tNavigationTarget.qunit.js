...
sap.ui.define(
		...
		function(modelerConstants, textPoolHelper, Input, MessageBox, Item, Controller, View, ViewType, modelerUIHelper, nextUIUpdate, jQuery) {
			"use strict";
			...
			async function _instantiateView(sId, assert) {
				var oNavigationTargetController = await Controller.create({
			...
			QUnit.module("Test for navigation target with semantic object and action from list", {
				beforeEach : function(assert) {
					...
					modelerUIHelper.getModelerInstance(async function(modelerInstance) {
						...
						oNavigationTargetView = await _instantiateView(oModelerInstance.firstNavigationTargetId, assert);
			...
			QUnit.test("When Navigation Target is initialized", async function(assert) {
				//arrange
			...
			QUnit.test("When title of navigation target is changed", function(assert) {
				//arrange
				var oTranslationFormat = textPoolHelper.TranslationFormatMap.NAVTARGET_TITLE;
			...
			QUnit.test("When navigation target view data has to be reset", function(assert) {
				...
				assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET), true, "then navigation target context mapping view is updated with new instances of editor and navigation target objects");
				//cleanup
			...
			QUnit.test("WHEN help button of the dynamic parameters is pressed", function(assert){
				...
				var stub = sinon.stub(MessageBox, "show", function(messageText, settings){
					...
					assert.deepEqual(settings.actions, [MessageBox.Action.CLOSE], "THEN actions as expected");
				});
			...
			QUnit.module("Test for a Navigation Target for which semantic object and action is an user input", {
				beforeEach : function(assert) {
					...
					modelerUIHelper.getModelerInstance(async function(modelerInstance) {
						...
						//instantiate the view
						oNavigationTargetView = await _instantiateView(oModelerInstance.secondNavigationTargetId, assert);
			...
			QUnit.module("Test for a Navigation Target which is new", {
				beforeEach : function(assert) {
					...
					modelerUIHelper.getModelerInstance(async function(modelerInstance) {
						...
						spyOnConfigEditorSetIsUnsaved = sinon.spy(oModelerInstance.configurationEditorForUnsavedConfig, "setIsUnsaved");
						//instantiate the view
						oNavigationTargetView = await _instantiateView("newNavigationTarget", assert);
