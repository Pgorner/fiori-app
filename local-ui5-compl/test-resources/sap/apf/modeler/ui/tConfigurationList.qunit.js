...
sap.ui.define([
	...
	async function _instantiateView(oConfigurationListController) {
	...
	async function _placeViewAt(oConfigurationListView) {
	...
	function _setupForSwitchFromNewConfig(assert) {
		...
		oConfigurationListView.getController().configurationHandler.loadConfiguration(newConfigId, async function(configurationEditor) {
			...
			assert.notEqual(Element.getElementById("idMandatoryValidationDialogFragement--idMandatoryValidationDialog"), undefined, "then mandatory popup dialog is available");
			deferred.resolve();
		});
	...
	async function _setupForSwitchFromNonNewConfig(assert, that) {
		var oSubView;
	...
	async function _setupForSwitchInsideSameConfig(assert) {
		var oSubView;
		_setupForSavedConfig();
		//action
		oSubView = oConfigurationListView.getController()._getSubView();
	...
	function _memorizeConfigurationFn() {
		memorizedEditor = deepExtend({}, oConfigurationListView.getController().configEditor);
	}
	...
	function _pressButtonOnMandatoryPopup(assert, buttonNumber, continuation) {
		...
		var mandatoryPopupDialog = Element.getElementById("idMandatoryValidationDialogFragement--idMandatoryValidationDialog");
		// ensure the async callback afterClose has successfully completed.
		mandatoryPopupDialog.attachAfterClose(function() {
			assert.strictEqual(Element.getElementById("idMandatoryValidationDialogFragement--idMandatoryValidationDialog"), undefined, "then mandatory popup dialog is destroyed");
			continuation();
			done();
		});
	...
	function _pressButtonOnLossOfDataPopup(assert, buttonNumber, continuation) {
		var done = assert.async();
		var lossOfDataDialog = Element.getElementById("idMessageDialogFragment--idMessageDialog");
		// ensure the async callback afterClose has successfully completed.
		lossOfDataDialog.attachAfterClose(function() {
			assert.strictEqual(Element.getElementById("idMessageDialogFragment--idMessageDialog"), undefined, "then loss of data popup dialog is destroyed");
			continuation();
			done();
		});
	...
	QUnit.module("Configuration List Unit Tests", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				sinon.stub(APFRouter, "patternMatch").returns(that);
				APFRouter.params = {
					name : ""
				};
				...
				sinon.stub(UIComponent, "getRouterFor", that.oGetRouterStub);
				...
				that.oConfigView = await View.create({
				...
				that.oByIdStub = function(param) {
					if (param === "configPage") {
						...
						return new APFTree().bindAggregation("items", "/aConfigDetails", oTreeNodeTemplate).setModel(oModel);
					} else if (param === "idConfigTitleMaster") {
						return new Label();
					} else if (param === "idConfigDetailData") {
						return new Grid({
							content : [ that.oConfigView ]
						});
				...
				that.oConfigurationListController = await Controller.create({
		...
		afterEach : function() {
			APFRouter.patternMatch.restore();
			delete APFRouter.params;
			...
			if ((jQuery(".sapMDialog").length) !== 0) {
				//Element.getElementById(jQuery(".sapMDialog")[0].id).getBeginButton().firePress();
				Element.getElementById(jQuery(".sapMDialog")[0].id).destroy();
			}
	...
	QUnit.test("Test for toggle of a tree node", async function(assert) {
		//arrangement
		this.oConfigurationListController.oModel.getData().aConfigDetails = this.oConfigurationListController.oTreeInstance.getModel().getData().aConfigDetails;
	...
	QUnit.test("Test _deleteConfigNavTargetTexts", function(assert) {
		...
		this.oConfigurationListController.navTargetTextsTable = deepExtend({}, navTargetTexts);
	...
	QUnit.test("WHEN the export configuration is called", function(assert) {
		...
		this.sandbox.stub(File, 'save', function(configurationString, configurationName, fileExtension){
	...
	QUnit.test("WHEN the export text is called", function(assert) {
		...
		this.sandbox.stub(File, 'save', function(exportedTexts, configurationName, fileExtension){
	...
	QUnit.module("Configuration List Unit Tests - Memorize and restore configuration", {
		beforeEach : async function(assert) {
			...
			var done = assert.async();
			var oConfigurationListController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.configurationList"
			});
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				UIComponent.extend("sap.apf.modeler.Component", {});
				sinon.stub(UIComponent, "getRouterFor", _oGetRouterStub);
				oConfigurationListView = await _instantiateView(oConfigurationListController);
				await _placeViewAt(oConfigurationListView);
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields and YES on loss of data popup to save current configuration before switch", async function(assert) {
		var that = this;
		oConfigurationListView.getController().oTreeInstance.setMode("SingleSelectMaster");
		oConfigurationListView.getController().oTreeInstance.expandToLevel(0);
		//arrangement
		await _setupForSwitchFromNonNewConfig(assert,this);
		//action
		_pressButtonOnMandatoryPopup(assert, 0, function() {
			assert.notEqual(Element.getElementById("idMessageDialogFragment--idMessageDialog"), undefined, "then loss of data popup dialog is available");
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields and NO on loss of data popup to save current configuration(saved) before switch", async function(assert) {
		oConfigurationListView.getController().oTreeInstance.expandToLevel(0);
		//arrangement
		await _setupForSwitchFromNonNewConfig(assert);
		//action
		_pressButtonOnMandatoryPopup(assert, 0, function() {
			assert.notEqual(Element.getElementById("idMessageDialogFragment--idMessageDialog"), undefined, "then loss of data popup dialog is available");
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields and NO on loss of data popup to not save current configuration(new) before switch", async function(assert) {
		oConfigurationListView.getController().oTreeInstance.setMode("SingleSelectMaster");
		//arrangement
		await _setupForSwitchFromNewConfig(assert);
		//action
		_pressButtonOnMandatoryPopup(assert, 0, function() {
			assert.notEqual(Element.getElementById("idMessageDialogFragment--idMessageDialog"), undefined, "then loss of data popup dialog is available");
			_pressButtonOnLossOfDataPopup(assert, 1, function() {
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields when switching between nodes of same configuration", async function(assert) {
		//arrangement
		await _setupForSwitchInsideSameConfig(assert);
		//action
		_pressButtonOnMandatoryPopup(assert, 0, function() {
			//assert
			assert.strictEqual(restoreMemorizedConfigurationStub.calledOnce, true, "then memorized configuration editor is restored for a reset of mandatory fields");
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press NO on reset mandatory fields when switching between nodes of same configuration", async function(assert) {
		//arrangement
		await _setupForSwitchInsideSameConfig(assert);
		//action
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on loss of data popup when switching between different configurations", async function(assert) {
		//arrangement
		var that = this;
		var oSubView;
		_setupForSavedConfig();
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press NO on loss of data popup when switching between different configurations", async function(assert) {
		//arrangement
		var oSubView;
		_setupForSavedConfig();
		//action
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press CANCEL on loss of data popup when switching between different configurations", async function(assert) {
		//arrangement
		var oSubView;
		_setupForSavedConfig();
		//action
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields and CANCEL on loss of data popup to save current configuration before switch", async function(assert) {
		//arrangement
		await _setupForSwitchFromNewConfig(assert);
		//action
		_pressButtonOnMandatoryPopup(assert, 0, function() {
	...
	QUnit.module("NavBack (Cancel or back button)", {
		beforeEach : async function(assert) {
			...
			var oGetOwnerComponentStub;
			var done = assert.async();
			this.oConfigurationListController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.configurationList"
			});
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;//Modeler instance from callback
				...
				oConfigurationListView = await _instantiateView(this.oConfigurationListController);
				await _placeViewAt(oConfigurationListView);
	...
	QUnit.test("Mandatory fields are not filled, old configuration is restored and saved", async function(assert) {
	...
	QUnit.test("Mandatory fields are not filled, old configuration is restored but not saved", async function(assert) {
		var configurationView = this.oConfigurationListController._getSubView();
		configurationView.byId("idConfigTitle").setValue(""); // setConfigurationTitle to empty so that mandatory field is not filled
		configurationView.byId("idConfigTitle").fireChange();
		await loadedFragment(() => {
	...
	QUnit.test("Config is unsaved, is saved on popup", async function(assert) {
	...
	QUnit.module("Publish Button Text and Visibility", {
		beforeEach : async function() {
			...
			// TODO in a newer sinon, sinon.stub(APFRouter, "patternMatch").value()
			sinon.stub(APFRouter, "patternMatch");
			...
			this.stubShowShareDialog = sinon.stub(uiHandler, "showShareDialog");
		},
		afterEach : function() {
