...
sap.ui.define([
	...
	async function _placeTableAtDom(oDataTableScrollContainer) {
	...
	async function _commonAttachingPagination(tableRepresentation, isAlternateTable) {
		if (!isAlternateTable) {
	...
	function _commonAssertsForTableWithNoFilter(api, tableRepresentation, assert, requiredParameter) {
		...
		// row mode
		assert.strictEqual(oTable.getRowMode(), RowMode.Auto, "then correct row mode is set to the table");
	}
	...
	function getTitleOfTable(oTable) {
		return oTable.getExtension().find((oExt) => oExt.isA("sap.m.HBox"));
	}
	...
	QUnit.test("getPrintContent", function(assert) {
		...
		assert.strictEqual(getTitleOfTable(oTable).getItems()[0].getText(), getTitleOfTable(printTable).getItems()[0].getText(), "PrintTable has same title as normal table");
		assert.equal(oTable.getModel(), printTable.getModel(), "PrintTable has same model as normal table");
		assert.ok(printTable.getRowMode() instanceof FixedRowMode, "PrintTable has a fixed row mode");
		assert.strictEqual(printTable.getRowMode().getRowCount(), 403, "PrintTable has a row count of the data length of the oModel");
	});
	QUnit.test("getPrintContent with selections", async function(assert) {
		var oTable = this.oTableRepresentation.tableControl;
		oTable.addSelectionInterval(1,1);
		oTable.addSelectionInterval(3,3);
		var printTable = this.oTableRepresentation.getPrintContent().oRepresentation;
		printTable.placeAt("testArea");
		await nextUIUpdate(); // render
	...
	QUnit.module("When there is a required filter on the table - Manual selection", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise().done(async function(api) {
				...
				await _placeTableAtDom(mainContent);
				that.oEvent = {
					getSource: function(){
						return {
	...
	function clickRow(tableId, Index){
		jQuery("#" + tableId + "-rowsel" + Index).trigger("tap");
	}
	QUnit.test("When click on row selection element", async function (assert) {
	...
	QUnit.test("When click twice on row selection element (select-deselect)", async function (assert) {
	...
	QUnit.test("When clicking once and calling markSelectionInTable", async function (assert) {
	...
	QUnit.test("When sort property is changed", async function (assert) {
		//arrange
		var done = assert.async();
		var oMainContentViewSetting = _createMainContentAndViewSettingDialog(this.tableRepresentation);
	...
	QUnit.test("When sort property is changed after selection", async function (assert) {
		//arrange
		var done = assert.async();
		var oMainContentViewSetting = _createMainContentAndViewSettingDialog(this.tableRepresentation);
	...
	QUnit.test("When getViewSettingDialog is called for the table and sort property is not available in table column", function (assert) {
		...
		//assert.deepEqual(oMainContentViewSetting.oViewSettingDialog.getSelectedSortItem(), null, "then no item is selected as the sort item");
		assert.deepEqual(oMainContentViewSetting.oViewSettingDialog.getSortDescending(), true, "Descending is selected");
		assert.deepEqual(oMainContentViewSetting.oViewSettingDialog.getSortItems()[0].getText(), BaseUI5ChartRepresentationTestHelper.getSampleMetadata().getPropertyMetadata("BestPossibleDaysSalesOutstndng").label, "then the first sort item is "
			+ oMainContentViewSetting.oViewSettingDialog.getSortItems()[0].getText());
	...
	QUnit.test("Selection before and after pagination", async function (assert) {
		var done = assert.async();
		// arrange stub
		var testEnv = this;
		...
		await _placeTableAtDom(oMainContentViewSetting.mainContent);
		//act
	...
	QUnit.test("When switching between chart and paginated table", async function (assert) {
		assert.expect(6);
		var done = assert.async();
		//arrange stub
		var testEnv = this;
		...
		await _placeTableAtDom(oMainContentViewSettingForTable.mainContent);
		this.tableRepresentation.oPaginationHandler.attachPaginationOnTable(this.tableRepresentation); // attach the pagination event
		//act
	...
	QUnit.test("When an alternate table is initialized", async function (assert) {
		//arrangement
		...
		//act
		await _commonAttachingPagination(tableRepresentation, true);
		//assert
	...
	QUnit.test("When a main table without filter is initialized", async function (assert) {
		//arrangement
		...
		var spyOfAttachPaginationOnTable = sinon.spy(tableRepresentation.oPaginationHandler, "attachPaginationOnTable");
		//act
		await _commonAttachingPagination(tableRepresentation, false);
		//assert
	...
	QUnit.test("When a main table with filter is initialized", async function (assert) {
		//arrangement
	...
	QUnit.module("Table Tests - Export to Excel", {
		beforeEach: function (assert) {
			...
			createUiApiAsPromise().done(function (api) {
				...
				var data = sampleService.get('sampleData');
				var metadataStub = {
	...
	QUnit.test("When getMainContent is called", function (assert) {
		...
		var hbox = getTitleOfTable(table);
	...
	QUnit.test("When topN is configured AND getMainContent is called", function (assert) {
		...
		var hbox = getTitleOfTable(content.getItems()[0]);
		assert.equal(hbox.getMetadata().getName(), "sap.m.HBox", "Then hbox is returned as control, that shows the title line");
		var label = hbox.getItems()[0];
		assert.equal(label.getMetadata().getName(), "sap.m.Title", "Then Title is first item");
		var buttonBox = hbox.getItems()[1];
	...
	QUnit.test("WHEN getMainContent is called", async function(assert){
		var done = assert.async();
