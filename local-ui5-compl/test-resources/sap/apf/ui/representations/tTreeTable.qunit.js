...
sap.ui.define([
	...
	var _mockServer = MockServer(sap.ui.require.toUrl("sap/apf/demokit/"));
	...
	function commonSetup (assert, context, singleSelection, bFilter){
		return new Promise(function(resolve){
			...
			createUiApiAsPromise().done(async function(instances) {
				...
				await _placeTableAtDom(context.mainContent);
			});
		});
	}
	...
	QUnit.module("Tree Table Unit Tests - with a required filter", {
		beforeEach : function(assert) {
			...
			return commonSetup(assert, this, false, true).then(function(){
	...
	QUnit.module("Tree Table Unit Tests - with a required single filter with display option as text", {
		beforeEach : function(assert) {
			...
			return commonSetup(assert, this, true, true);
		},
		afterEach : function() {
			commonTeardown(this);
		}
	});
	QUnit.test("When selection is changed in the treetable - single selection", async function(assert) {
	...
	QUnit.module("Tree Table Unit Tests - without a required filter with display option as key", {
		beforeEach : function(assert) {
			...
			return commonSetup(assert, this, false);
		},
		afterEach : function() {
			commonTeardown(this);
		}
	...
	QUnit.test("When treeTable without a required filter is initialized", function(assert) {
		...
		// row mode
		assert.strictEqual(this.oTreeTableRepresentation.oTreeTable.getRowMode(), RowMode.Auto, "then correct row mode is set to the table");
		...
		function setup(){
			createUiApiAsPromise().done(function(oInstances) {
				...
				api.createStep("hierarchicalStep", function(){
					...
					oModel.attachBatchRequestCompleted(onComplete);
					/* ignore async */ _placeTableAtDom(mainContent);
				}, "treeTable");
			});
		}
	...
	QUnit.done(() => {
		// re-enable scrolling of the page after all tests finished
