...
sap.ui.define([
	...
	function createApplicationComponentWithStubbedApiAsPromise(testModule, createContentF, startParameters, loadApplPathForComponent) {
		 ...
		 * Here, the parent call to sap.apf.Component.init is replaced by a copy of that code in order to inject a probe */
		var ComponentWithStubbedApi = ApfComponent.extend("sap.apf.applComponentWithStubbedApi.Component", {
			metadata : {
				"name" : "Component",
				"version" : "0.0.0"
			},
			init : function() {
						...
						ExternalContext: function() {
							this.getCombinedContext = function() {
								if (testModule.xappStateFilter && testModule.filterFromSmartBusiness) {
									var combinedFilter = new CoreFilter(testModule.messageHandler);
								...
								return jQuery.Deferred().resolve(new CoreFilter(testModule.messageHandler));
							};
						},
				...
				this.oApi = new Api(this, injectConstructors);
		...
		var component = new ComponentWithStubbedApi(oComponentParameter);
	...
	function createComponentLikeDSOAsPromise(testModule, startParameters, loadApplPathForComponent) {
		function createContent(component) {
			...
			// Phase 2
			return ApfComponent.prototype.createContent.apply(component, arguments);
		}
	...
	QUnit.module("Component startup -- Forward Navigation", {
		beforeEach : function() {
			...
			// this.spy.getAllProperties = sinon.stub(MetadataDouble.prototype, "getAllProperties", getAllProperties);
			MetadataDouble.prototype.getAllProperties = sinon.stub();
			MetadataDouble.prototype.getAllProperties.returns(getAllProperties());
			this.filterFromFacetFilterConfiguration = new CoreFilter(this.messageHandler, "CompanyCode", coreConstants.FilterOperators.EQ, "1000");
		},
		addFilterToXappState : function() {
			this.xappStateFilter = new CoreFilter(this.messageHandler, "Customer", coreConstants.FilterOperators.EQ, "SAP");
		},
		addSmartBusinessEvaluation : function() {
			this.filterFromSmartBusiness = new CoreFilter(this.messageHandler, "Smart", coreConstants.FilterOperators.EQ, "Business");
		},
		afterEach : function() {
			this.spy.assert.restore();
			jQuery.ajax.restore();
			MetadataDouble.prototype.getAllProperties = undefined;
			ushellHelper.teardown();
			Element.getElementById('stepList')?.destroy();
		}
	});
	...
	QUnit.test("Consumption of filter from xapp-state, Facet Filter and SmartBusiness", function(assert) {
		...
		this.addFilterToXappState();
		this.addSmartBusinessEvaluation();
		var expectedFilter = new CoreFilter(this.messageHandler);
	...
	//            ushellHelper.setup();
	//            
	//            this.OriginalSessionHandler = sap.apf.core.SessionHandler;
	...
	//            sap.apf.core.Metadata = MetadataDouble;
	//            
	...
	//            jQuery.ajax.restore();
	//            ushellHelper.teardown();
	//            sap.apf.core.SessionHandler = this.OriginalSessionHandler;
	...
	QUnit.test("WHEN calling createApplicationLayout twice THEN the second call is blocked", function(assert) {
		...
		}, sapApfAppConfigPathOfComponent).done(async function(component) {
			var oApi = component.getApi();
			assert.equal(this.spyUiCreateApplicationLayout.called, true, "called by createContent / startApf");
			assert.equal(this.spyGetLayoutView.callCount, 2, "called by createContent and handleStartUp in Ui instance");
			var layout2 = await oApi.createApplicationLayout();
			assert.equal(this.spyGetLayoutView.callCount, 2, "test objective");
