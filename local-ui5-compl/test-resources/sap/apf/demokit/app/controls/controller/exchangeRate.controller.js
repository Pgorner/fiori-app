sap.ui.define([
...
], function(ContextMediator, deepExtend, Device, coreLibrary, Controller, JSONModel) {
	"use strict";
	...
	return Controller.extend("sap.apf.demokit.app.controls.controller.exchangeRate", {
		onInit : function() {
			...
			oApi.getApplicationConfigProperties().done(function(configProperties) {
				...
				mModels = {
					oExchangeRateModel : new JSONModel(mDatasets.oExchangeRateDataset),
					oDateTypeModel : new JSONModel(mDatasets.oDateTypeDataset),
					oDateModel : new JSONModel(mDatasets.oDateDataset)
				};
			...
			var oContextMediator = ContextMediator.getInstance(oApi);
		...
		handleOkPress : function() {
			if (oView.oDatePicker.getValueState() === ValueState.None) {
				_setFilters();
				oView.oDialog.close();
			}
		},
		...
		handleCancelPress : function() {
			oView.oDatePicker.setValueState(ValueState.None);
		...
		_saveInitialState : function() {
			...
			for(i in mModels) {
				if (mModels.hasOwnProperty(i)) {
					var oDataset = deepExtend({}, mModels[i].getData());
