...
sap.ui.define([
...
], function(utils, testhelper, MessageHandlerDouble, MessageHandler, Hashtable, Formatting, Localization, deepExtend, CalendarType){
	'use strict';
	...
	QUnit.module('Miscellaneous helpers', {
		beforeEach : function(assert) {
			this.oMsgHandler = new MessageHandlerDouble().doubleCheckAndMessaging();
		}
	});
	...
	QUnit.test('GIVEN an new (migrated) configuration with steps attribute for categories', function(assert) {
		...
		var expected = deepExtend({}, migratedConfig);
	...
	QUnit.test('createPromise', function(assert){
	...
	QUnit.test('Calling "extractFunctionFromModulePathString" with constructor function', function(assert) {
		...
		assert.equal(typeof oInstance, "function", "instance");
	});
