...
sap.ui.define([
    ...
    QUnit.test("Test Object Stream - optimized updateContent", function (assert) {
        var oData = {
            title: "someTitle",
            content: [1, 2, 3]
        };
        ...
        this.oObjectStream = new ObjectStream({
            ...
            content: {
                ...
                length: 20,
                factory: factorySpy,
                clearQuickViewCard: function () { }
            }
        });

        this.oObjectStream.setModel(oModel);

        assert.equal(factorySpy.callCount, 3, "factory call count");
        assert.equal(setBiningContextSpy.callCount, 3, "setBinding call count");

        oModel.setProperty("/content", [1, 2]);
        assert.equal(factorySpy.callCount, 5, "factory call count");
        assert.equal(setBiningContextSpy.callCount, 5, "setBinding call count");

        oModel.setProperty("/content", [1, 2, 3]);
        assert.equal(factorySpy.callCount, 8, "factory call count");
        assert.equal(setBiningContextSpy.callCount, 8, "setBinding call count");

        var getContextsSpy = sinon.spy(this.oObjectStream.getBinding("content"), "getContexts");
        oModel.refresh(true);

        assert.equal(getContextsSpy.callCount, 1, "getContexts call count");
        assert.equal(getContextsSpy.args[0][1], 20, "getContexts second argument length");
    });

    QUnit.test("Test Object Stream - Screen Reader attribute test", function (assert) {
        var oData = {
            title: "someTitle",
            content: []
        };
        ...
        this.oObjectStream.onAfterRendering = function () {
            ...
            assert.ok(objectStreamHtml.getAttribute("role") == "dialog", "object Stream role is define");
            assert.ok(objectStreamCloseHtml.getAttribute("title") == "Close", "object Stream title is define");
            assert.ok(
                objectStreamCloseHtml.getAttribute("aria-label") == "Close",
                "object Stream close aria-label is define"
            );
            assert.ok(objectStreamScrollHtml.getAttribute("role") == "list", "object Stream content role is define");
            assert.ok(objectStreamItemsHtml.attr("role") == "listitem", "item role is define");
            assert.ok(objectStreamItemsHtml.attr("aria-setsize") == 1, "item size role is define");
            assert.ok(objectStreamItemsHtml.attr("aria-posinset") == 1, "item position is define");
        };
        ...
        assert.ok(keysHandlerStub.calledOnce, "scroll handler was called once after scrolling left");

        this.oObjectStream._startScroll("right");
        assert.ok(keysHandlerStub.calledTwice, "scroll handler was called once more after scrolling right");

        this.oObjectStream.exit();
    });

    QUnit.test("Test Object Stream - Key Down Handle", function (assert) {
        var cardTestData = {};
        var oModel = new JSONModel(cardTestData);
        var setBiningContextSpy = sinon.spy();
        this.factory = function () {
            var btn = new Button();
            btn.setBindingContext = setBiningContextSpy;
        ...
        jQuery('<div id="testContainer" style="display: none;">').appendTo("body");
        ...
        objectStream.onAfterRendering = function () {
            ...
            keyboardNavigation.keydownHandler(event);
            assert.ok(keysHandlerStub.calledOnce, "leftRightHandler was called once after pressing arrow left");

            event.keyCode = 39; //Arrow RIGHT
            keyboardNavigation.keydownHandler(event);
            assert.ok(keysHandlerStub.calledTwice, "leftRightHandler was called once after pressing arrow right");

            keysHandlerStub = sinon.spy(keyboardNavigation, "homeEndHandler");
            event.keyCode = 36; //Home
            keyboardNavigation.keydownHandler(event);
            assert.ok(keysHandlerStub.calledOnce, "homeEndHandler was called once after pressing home button");

            event.keyCode = 35; //End
            keyboardNavigation.keydownHandler(event);
            assert.ok(keysHandlerStub.calledTwice, "homeEndHandler was called once after pressing end button");
            ...
            keyboardNavigation.keydownHandler(event);
            assert.ok(
                keysHandlerStub.calledTwice,
                "Spacebar and enter handler were called once after pressing the respective buttons"
            );
            ...
            keyboardNavigation.keydownHandler(event);
            assert.ok(keysHandlerStub.calledOnce, "F6 handler was called once after pressing page down");
        };
