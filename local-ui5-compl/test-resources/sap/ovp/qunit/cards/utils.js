...
"sap/ui/Device"], function (ODataModel, CoreComponent, Device) {
    ...
    utils.createCardModel = function (cardData) {
        var oModel = new ODataModel(cardData.dataSource.rootUri, {
            ...
            .then(function () {
                return CoreComponent.create({
                    ...
                    componentData: {
                        ...
                        template: cardData.card.template,
    ...
    utils.isValidHeaderExtension = function (cardTestData, cardXml) {
        ...
        if (!expectedHeaderRes.headerExtension) {
            ...
            if (headerExtension.length === 1) {
                return utils.validateXMLValue(
                    headerExtension[0].getAttribute("text"),
                    expectedHeaderRes.headerExtension
                );
            }
        }
        return false;
    ...
    utils._validateObjectListItemsXmlValues = function (cardXml, expectedListRes) {
        ...
        if (
            !utils.validateXMLValue(
                status1Xml.getAttribute("state"),
                expectedListRes.ListItem.ObjectStatus[1].state
            )
        ) {
            return false;
        }
        if (
            !utils.validateXMLValue(
                objAttrsXML.getAttribute("text"),
                expectedListRes.ListItem.ObjectAttribute[0].text
            )
        ) {
            return false;
        }
        if (
            !utils.validateXMLValue(
                objAttrs1XML.getAttribute("text"),
                expectedListRes.ListItem.ObjectAttribute[1].text
            )
        ) {
            return false;
        }
    ...
    utils._validateExtendedBarListItemsXmlValues = function (cardXml, expectedListRes) {
        ...
        if (
            !utils.validateXMLValue(
                firstDataFiledXml.getAttribute("text"),
                expectedListRes.CustomListItem.firstDataFiled
            )
        ) {
            return false;
        }
        if (
            !utils.validateXMLValue(
                secondDataFiledXml.getAttribute("text"),
                expectedListRes.CustomListItem.secondDataFiled
            )
        ) {
            return false;
        }
        if (
            oProgressIndicatorXML && 
            !utils.validateXMLValue(
                oProgressIndicatorXML.getAttribute("percentValue"),
                expectedListRes.CustomListItem.progressIndicator
            )
        ) {
            return false;
        }
        if (
            firstDataPointXml && 
            !utils.validateXMLValue(
                firstDataPointXml.getAttribute("number"),
                expectedListRes.CustomListItem.firstDataPoint
            )
        ) {
            return false;
        }
        if (
            !utils.validateXMLValue(
                secondDataPointXml.getAttribute("number"),
                expectedListRes.CustomListItem.secondDataPoint
            )
        ) {
            return false;
        }
        if (
            thirdDataPointXml &&
            !utils.validateXMLValue(
                thirdDataPointXml.getAttribute("number"),
                expectedListRes.CustomListItem.thirdDataPoint
            )
        ) {
            return false;
        }
    ...
    utils._validateCondensedBarListItemsXmlValues = function (cardXml, expectedListRes) {
        ...
        if (
            oFirstDataPointXML &&
            !utils.validateXMLValue(
                oFirstDataPointXML.getAttribute("text"),
                expectedListRes.CustomListItem.firstDataPoint
            )
        ) {
            return false;
        ...
        if (
            oObjNumberXML &&
            !utils.validateXMLValue(
                oObjNumberXML.getAttribute("number"),
                expectedListRes.CustomListItem.SecondDataPoint
            )
        ) {
            return false;
        ...
        if (footerXML) {
            ...
            if (actions && actions.length > 0) {
                ...
                for (var i = 0; i < actions.length; i++) {
                    ...
                    if (
                        !utils.validateXMLValue(
                            currentData.semanticObject,
                            expectedFooterRes.actions[i].semanticObj
                        )
                    ) {
                        return false;
                    }
    ...
    utils.validateQuickviewXmlValues = function (cardXml, expectedquickviewRes) {
        ...
        if (quickviewXML) {
            if (
                !utils.validateXMLValue(
                    quickviewXML.getAttribute("header"),
                    expectedquickviewRes.QuickViewPage.header
                )
            ) {
                return false;
            }
            if (
                !utils.validateXMLValue(
                    quickviewXML.getAttribute("title"),
                    expectedquickviewRes.QuickViewPage.title
                )
            ) {
                return false;
            }
            ...
            if (groupsXml.length == expectedquickviewRes.QuickViewPage.groups.length) {
                for (var gIndex = 0; gIndex < groupsXml.length; gIndex++) {
                    if (
                        !utils.validateXMLValue(
                            groupsXml[gIndex].getAttribute("heading"),
                            expectedquickviewRes.QuickViewPage.groups[gIndex].header
                        )
                    ) {
                        return false;
                    }
                    ...
                    if (propertiesXML) {
                        ...
                        if (propertiesXML.length == propsExpected.length) {
                            for (var pIndex = 0; pIndex < propertiesXML.length; pIndex++) {
                                if (
                                    !utils.validateXMLValue(
                                        propertiesXML[pIndex].getAttribute("label"),
                                        propsExpected[pIndex].label
                                    )
                                ) {
                                    return false;
                                }
                                if (
                                    !utils.validateXMLValue(
                                        propertiesXML[pIndex].getAttribute("value"),
                                        propsExpected[pIndex].value
                                    )
                                ) {
                                    return false;
                                }
                                if (
                                    !utils.validateXMLValue(
                                        propertiesXML[pIndex].getAttribute("type"),
                                        propsExpected[pIndex].type
                                    )
                                ) {
                                    return false;
                                }
                            ...
                            //#of props not as expected
                            return false;
                        }
                    }
                }
            } else {
                //#of groups not as expected
                return false;
            }
        }
        return true;
    ...
    utils.validateTableSemanticObjectValues = function (cardXml, cardCfg, expectedTableRes) {
        ...
        if (firstColumnCell) {
            ...
            actualValueToCheck = expectedTableRes.items.ColumnListItem.cells[0].semanticObject === undefined
                    ? undefined
                    : firstColumnCell.getAttribute("semanticObject");
        ...
        if (SecondColumnCell) {
            ...
            actualValueToCheck = expectedTableRes.items.ColumnListItem.cells[1].semanticObject === undefined
                    ? undefined
                    : SecondColumnCell.getAttribute("semanticObject");
        ...
        if (thirdColumnCell) {
            ...
            actualValueToCheck = expectedTableRes.items.ColumnListItem.cells[2].semanticObject === undefined
                    ? undefined
                    : thirdColumnCell.getAttribute("semanticObject");
    ...
    utils.validateTableContactAnnotationValues = function (cardXml, cardCfg, expectedTableRes) {
        ...
        if (
            !utils.validateXMLValue(
                actualValueToCheck,
                expectedTableRes.items.ColumnListItem.cells[0].quickViewElement
            )
        ) {
            return false;
        }
    ...
    utils.validateBarListSmartLinkValues = function (cardXml, cardCfg, expectedTableRes) {
        ...
        var barListVBoxLeft = cellsXml && cellsXml.children[0];
        var barListVBoxRight = cellsXml && cellsXml.children[1];
        var actualFirstSemanticObject = barListVBoxLeft && barListVBoxLeft.children[0].getAttribute("semanticObject");
        var actualSecondSemanticObject = barListVBoxLeft && barListVBoxLeft.children[1].getAttribute("semanticObject");
        ...
        if (
            !utils.validateXMLValue(
                actualFirstSemanticObject,
                expectedTableRes.CustomListItem.firstDataFieldSemanticObject
            )
        ) {
            return false;
        }
        ...
        if (
            !utils.validateXMLValue(
                actualSecondSemanticObject,
                expectedTableRes.CustomListItem.secondDataFieldSemanticObject
            )
        ) {
            return false;
        }
        ...
        if (
            !utils.validateXMLValue(
                ActualContactAnnotation,
                expectedTableRes.CustomListItem.firstContactAnnotationQuickViewElement
            )
        ) {
            return false;
        }
    ...
    utils.validateOvpKPIHeader = function (xml, expectedHeaderResult) {
        ...
        if (bExpectedNumber) {
            ...
            if (oNumberAggregateNumberContentNode && oNumberAggregateNumberContentNode.length != 0) {
                ...
                if (bExpectedFilter && aResults && aResults.length > 0) {
    ...
    utils.validateDropDown = function (xml, expectedResult) {
        if (Device.browser.msie === true || Device.browser.edge === true) {
            return true;
        }
