...
sap.ui.require([
	...
	QUnit.test("single", function (assert) {
		var that = this;
		var done = assert.async();
		this.oContactDetailsController.getBindingPath4ContactAnnotation("/ProductCollection('38094020.0')", "to_Supplier").then(async function (sPath) {
				...
				events: {
					change: function () {
						...
						setTimeout(function () {
							...
							assert.equal(oContainer.$().find("span")[0].textContent, Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_CONTACT_SECTION_ADR"));
							assert.equal(oContainer.$().find("span")[1].textContent, "Los Angeles");
							assert.equal(oContainer.$().find("span")[2].textContent, Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_CONTACT_SECTION_ADR"));
	...
	QUnit.test("formatting all", function (assert) {
		var that = this;
		var done = assert.async();
		this.oContactDetailsController.getBindingPath4ContactAnnotation("/ProductCollection('38094020.0')", "").then(async function (sPath) {
			// act 'street'
			that.aExpands = [];
			that.aSelects = [];
			that.aContent = [];
