...
sap.ui.define([
	...
	"sap/m/Title"
], function(Log, FakeFlpConnector, Library, UIComponent, Control, SmartLink, NavigationPopoverHandler, SemanticObjectController, LinkData, SmartTable, nextUIUpdate, Table, Column, Button, SimpleForm, Title, Text, VBox, Label, MessageToast, jQuery, MTitle) {
	"use strict";
	...
	QUnit.test("enableAvailableActionsPersonalization: true", async function(assert) {
		// system under test
		...
		var fnAfterOpen = function(oEvent) {
			...
			oPopover._getContentContainer().$().find("span").each(function(iIndex) {
				if (oPopover._getContentContainer().$().find("span")[iIndex].textContent === Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_DEFINE_LINKS")) {
					bFound = true;
				}
			});
			assert.ok(bFound);
	...
	QUnit.test("enableAvailableActionsPersonalization: false", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("enableAvailableActionsPersonalization: false changing to true", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler._getPopover().then(async function(oPopover) {
			oPopover.attachAfterOpen(fnAfterOpen);

			// act
			oNavigationPopoverHandler.setEnableAvailableActionsPersonalization(true);
			oNavigationPopoverHandler._oPopover._getContentContainer().invalidate();
			await nextUIUpdate();
	...
	QUnit.skip("semanticObject", async function(assert) {
	...
	QUnit.test("_createPopover: without context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler._getPopover().then(function(oPopover) {
			...
			assert.equal(oPopover._getContentContainer().getMainNavigation().getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
			...
			assert.equal(oPopover._getContentContainer().getModel("$sapuicompNavigationContainer").getProperty("/availableActions/0/internalHref"), "?TestObject#/dummyLink2");
			...
			assert.equal(oPopover._getContentContainer().getModel("$sapuicompNavigationContainer").getProperty("/mainNavigationLink/title"), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
			...
			assert.equal(oPopover._getContentContainer().getModel("$sapuicompNavigationContainer").getProperty("/mainNavigationLink/internalHref"), "?TestObject#/dummyLink1");
	...
	QUnit.test("_createPopover: field not in context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("_createPopover: field in context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler._getPopover().then(function(oPopover) {
			...
			assert.deepEqual(oPopover.getModel("$sapuicompNavigationPopover").getProperty("/availableActions"), []);
			assert.equal(oPopover.getModel("$sapuicompNavigationPopover").getProperty("/mainNavigationLink/title"), undefined);
			assert.equal(oPopover.getTitle(), "valueA");
	...
	QUnit.test("_getPopover: 1 mainNavigation and 11 availableAction", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler._getPopover().then(function(oPopover) {
			...
			assert.equal(oPopover._getContentContainer().getAvailableActions()[0].getVisible(), true);
			assert.equal(oPopover._getContentContainer().getAvailableActions()[1].getText(), "List2");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[1].getHref(), "?TestObject12#/dummyLink2");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[1].getVisible(), true);
			assert.equal(oPopover._getContentContainer().getAvailableActions()[2].getText(), "List3");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[2].getHref(), "?TestObject12#/dummyLink3");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[2].getVisible(), true);
			assert.equal(oPopover._getContentContainer().getAvailableActions()[3].getText(), "List4");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[3].getHref(), "?TestObject12#/dummyLink4");
			assert.equal(oPopover._getContentContainer().getAvailableActions()[3].getVisible(), false);
			assert.equal(oPopover._getContentContainer().getMainNavigation().getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
	...
	QUnit.test("openPopover: empty content", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			assert.equal(oNavigationPopoverHandler._oPopover.getAriaLabelledBy(), oNavigationPopoverHandler._oPopover.getContent()[0]._oHeaderArea.getItems()[0].getContent().getId(), "Correct ariaLabelledBy returned for popover.");
	...
	QUnit.test("openPopover: 'beforePopoverOpens' handler", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover - 'beforePopoverOpens' handler with modifying data, no context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.attachBeforePopoverOpens(function(oEvent) {
			...
			oEvent.getParameter("setAppStateKey")("MyAppKey");
			oEvent.getParameter("setSemanticAttributes")({
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			oNavigationPopoverHandler._oPopover._getContentContainer().$().find("span").each(function(iIndex) {
				if (oNavigationPopoverHandler._oPopover._getContentContainer().$().find("span")[iIndex].textContent === Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_DEFINE_LINKS")) {
					bFound = true;
				}
			});
			assert.ok(bFound);
			...
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().getMainNavigation().getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
	...
	QUnit.test("openPopover - 'beforePopoverOpens' handler with modifying data, with context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover - called twice sequential", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover - called twice hierarchical", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover: field in context", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		var fnAfterOpen = function(oEvent) {
			...
			assert.equal(oPopover._getContentContainer().$().find("button")[0].textContent, Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_DEFINE_LINKS"));
	...
	QUnit.test("openPopover: 'navigationTargetsObtained' handler", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover: 'navigationTargetsObtained' handler with modifying data", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			assert.deepEqual(oNavigationPopoverHandler._oPopover.getSource(), oButton.getId());
			assert.deepEqual(oNavigationPopoverHandler._oPopover.getExtraContent(), null);
			assert.equal(oNavigationPopoverHandler._oPopover.getAriaLabelledBy(), oNavigationPopoverHandler._oPopover._getContentContainer().getExtraContent(), "Correct ariaLabelledBy returned for popover.");
			assert.deepEqual(oNavigationPopoverHandler._oPopover.getComponent(), null);
			assert.deepEqual(oNavigationPopoverHandler._oPopover.getModel("$sapuicompNavigationPopover").getProperty("/availableActions"), []);
			assert.equal(oNavigationPopoverHandler._oPopover.getModel("$sapuicompNavigationPopover").getProperty("/mainNavigationLink/title"), undefined);
	...
	QUnit.test("openPopover: 'navigationTargetsObtained' with 3 parameters - modify case", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover: 'navigationTargetsObtained' with 3 parameters - delete case", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("openPopover: 'navigationTargetsObtained' with 3 parameters - leave it as it is", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().getAvailableActions()[0].getText(), "List");
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().getAvailableActions()[0].getHref(), "?TestObject#/dummyLink2");
			assert.deepEqual(oNavigationPopoverHandler._oPopover._getContentContainer().getMainNavigation().getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
	...
	QUnit.test("'openPopover' called twice from different NavigationPopoverHandlers with same SemanticObject", async function(assert) {
		var done = assert.async();
		var oButton1 = new Button({
			text: "button1"
		});
		var oFirstNavigationPopoverHandler = new NavigationPopoverHandler({
			semanticObject: "TestObject",
			fieldName: "fieldA",
			control: oButton1
		});

		fPrepareBinding(oFirstNavigationPopoverHandler, {
			fieldA: "valueA"
		});
		...
		var oSecondNavigationPopoverHandler = new NavigationPopoverHandler({
			semanticObject: "TestObject",
			fieldName: "fieldA",
			control: oButton2
		});

		fPrepareBinding(oSecondNavigationPopoverHandler, {
			fieldA: "valueB"
		});

		sinon.stub(oFirstNavigationPopoverHandler, "getNavigationPopoverStableId").returns("sapuicompnavpopoverNavigationPopover---TestObject");
		sinon.stub(oSecondNavigationPopoverHandler, "getNavigationPopoverStableId").returns("sapuicompnavpopoverNavigationPopover---TestObject");

		oButton1.placeAt("qunit-fixture");
		oButton2.placeAt("qunit-fixture");
		await nextUIUpdate();

		oFirstNavigationPopoverHandler._getPopover().then(function(oPopover) {
			var fnPopoverClosedSpy = sinon.spy(oPopover, "close");
			var oNavigationContainer = oPopover.getContent()[0];

			assert.ok(fnPopoverClosedSpy.notCalled, "Popover 'close' not called");

			oSecondNavigationPopoverHandler.openPopover().then(function() {
				assert.ok(fnPopoverClosedSpy.calledOnce, "Popover 'close' called");
				assert.ok(oNavigationContainer.isDestroyed(), "NavigationContainer got destroyed");
				done();
			});
		});
	});
	...
	QUnit.test("Title of Mainlink: enabled sap.m.Link, text = 'fieldName' value of bindingContext", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("Title of Mainlink: disabled sap.m.Link, text = 'fieldName' value of bindingContext", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("Title of Mainlink: enabled sap.m.Link, text = 'Display Fact Sheet'", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].text, Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
			assert.ok(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].href.indexOf("?TestObject#/dummyLink1") > -1);
			assert.equal(jQuery(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).control()[0].getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
			assert.equal(jQuery(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).control()[0].getHref(), "?TestObject#/dummyLink1");
			assert.equal(jQuery(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).control()[0].getEnabled(), true);
	...
	QUnit.test("Subtitle of Mainlink", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			// ---- NavigationContainer ------------
			const oNavigationContainer = oNavigationPopoverHandler._oPopover._getContentContainer();
			assert.ok(oNavigationContainer.getDomRef());
			assert.equal(oNavigationContainer.getMainNavigationId(), "valueA");
			assert.equal(fnGetTitleLink(oNavigationContainer).getText(), "valueA");
			assert.equal(fnGetTitleLink(oNavigationContainer).getHref(), "?TestObject#/dummyLink1");
			assert.equal(fnGetTitleLink(oNavigationContainer).getEnabled(), true);
			assert.equal(fnGetSubTitle(oNavigationContainer).getText(), "MySubtitle", "The subtitle text should be correct.");
	...
	QUnit.skip("_updateSemanticObjectController: SmartLink", function(assert) {
		// system under test
		var oSmartLink = new SmartLink();
	...
	QUnit.test("fireInnerNavigate without handler", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.openPopover().then(function() {
			...
			oNavigationPopoverHandler._oPopover._getContentContainer().fireNavigate({
				text: "New Link",
				href: sBaseUrl + "#newLink",
				internalHref: sBaseUrl + "#newLink"
			});

			// assert
			assert.ok(fnFireInnerNavigateSpy.calledOnce);
			assert.equal(fnFireInnerNavigateSpy.args[0][0].text, "New Link");
			assert.equal(fnFireInnerNavigateSpy.args[0][0].href, sBaseUrl + "#newLink");
	...
	QUnit.test("fireInnerNavigate with handler", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		var oNavigationPopoverHandler = new NavigationPopoverHandler({
			...
			innerNavigate: function(oEvent) {
				...
				// assert
				assert.equal(Object.keys(oEvent.getParameters()).length, 6);
				assert.equal(oEvent.getParameters().text, "New Link");
				assert.equal(oEvent.getParameters().href, sBaseUrl + "#newLink");
				assert.equal(oEvent.getParameters().internalHref, sBaseUrl + "#newLink");
				assert.equal(oEvent.getParameters().originalId, oButton.getId());
				assert.equal(oEvent.getParameters().semanticObject, "TestObject");
				assert.deepEqual(oEvent.getParameters().semanticAttributes, {
					TestObject: "valueA"
	...
	QUnit.test("fireInnerNavigate", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
		...
		oNavigationPopoverHandler.attachInnerNavigate(function(oEvent) {
			...
			assert.equal(oEvent.getParameters().internalHref, this.sIntent);
			assert.equal(oEvent.getParameters().originalId, oButton.getId());
	...
	QUnit.test("StableID: after direct link navigation the popover should be destroyed", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("with 3 parameters: oAdditionalContent", async function(assert) {
		// system under test
		var oButton = new Button({
			text: "button"
		});
	...
	QUnit.test("Event navigate", async function(assert) {
		// system under test
		var oButton;
		var oSemanticObjectController;
		var oSmartTable = new SmartTable({
	...
	QUnit.test("open messageToast before direct navigation", async function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
	...
	QUnit.test("wait 1000ms before popover navigation", async function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
	...
	QUnit.test("wait 1000ms before no popover navigation", async function(assert) {
		FakeFlpConnector.enableFakeConnector({
			TestObjectDirect: {
				links: [
					{
	...
	QUnit.module("sap.ui.comp.navpopover.NavigationPopoverHandler: _convertToExternal", {
		beforeEach: function() {
			this.oNavigationPopoverHandler = new NavigationPopoverHandler();
		},
		afterEach: function() {
	...
	QUnit.test("without CrossApplicationNavigation", function(assert) {
		var done = assert.async();
		var oConvertToExternal = this.oNavigationPopoverHandler._convertToExternal("Test");

		assert.ok(oConvertToExternal instanceof Promise, "Promise returned");

		oConvertToExternal.then(function(sConvertedHref) {
			assert.equal(sConvertedHref, "Test", "Same href returned again");
			done();
		});
	});

	QUnit.test("with CrossApplicationNavigation", function(assert) {
		FakeFlpConnector.enableFakeConnector({});
	...
	QUnit.module("sap.ui.comp.navpopover.NavigationPopoverHandler: _fireNavigationTargetsObtained", {
		beforeEach: function() {
	...
	QUnit.test("should not fire event when no eventhandling is provided", async function(assert) {
		...
		const fnFireNavigationTargetsObtainedSpy = sinon.spy(oNavigationPopoverHandler, "fireNavigationTargetsObtained");

		const sMainNavigationId = "mainNavigationId",
			sSemanticObjectDefault = "",
			oSemanticAttributes = {},
			sId = oNavigationPopoverHandler.getId(),
			aForms = [],
			oNavigationTargets = this.oNavigationTargets;

		const oResult = await oNavigationPopoverHandler._fireNavigationTargetsObtained(sMainNavigationId, sSemanticObjectDefault, oSemanticAttributes, sId, aForms, oNavigationTargets, undefined);
	...
	QUnit.test("should fire event when eventhandling is provided and 'navigationTargetsObtainedCallback' is not set", async function(assert) {
		...
		oNavigationPopoverHandler.attachNavigationTargetsObtained((oEvent) => {
	...
	QUnit.test("should also fire event when 'navigationTargetsObtainedCallback' is set and log an warning when show is called", function(assert) {
		let bEventHandlingCalled = false;
		const fnNavigationTargetsObtainedCallbackSpy = sinon.spy();
		...
		const oNavigationPopoverHandler = new NavigationPopoverHandler({
			...
			navigationTargetsObtainedCallback: fnNavigationTargetsObtainedCallbackSpy
		});
		...
		oNavigationPopoverHandler._fireNavigationTargetsObtained(sMainNavigationId, sSemanticObjectDefault, oSemanticAttributes, sId, aForms, oNavigationTargets, undefined);

		const sExpectedWarningMessage = `sap.ui.comp.navpopover.NavigationPopoverHandler: "navigationTargetsObtained" event handling will be ignored as "navigationTargetsObtainedCallback" property is set.`;

		assert.ok(bEventHandlingCalled, "Eventhandling called");
		assert.ok(fnFireNavigationTargetsObtainedSpy.calledOnce, "'fireNavigationTargetsObtained' called once");
		assert.ok(fnNavigationTargetsObtainedCallbackSpy.calledOnce, "'navigationTargetsObtainedCallback' called once");
	...
	QUnit.test("should return results of 'navigationTargetsObtainedCallback' when set", async function(assert) {
		...
		oNavigationPopoverHandler.setNavigationTargetsObtainedCallback((oCallbackNavigationTargets) => {
