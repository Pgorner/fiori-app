...
sap.ui.define([
	"sap/ui/comp/odata/MetadataAnalyser", "sap/ui/model/odata/v2/ODataModel", "sap/ui/model/odata/ODataMetaModel", "sap/ui/core/util/MockServer", "sap/base/util/merge", 'sap/ui/comp/odata/CalendarMetadata'
], function(MetadataAnalyser, ODataModel, ODataMetaModel, MockServer, merge, CalendarMetadata) {
	"use strict";
	...
	QUnit.test("getAllFilterableFieldsByEntitySetName should call _mapSeparateNavigationProperties", function(assert) {
		var	aSeparateNavigationProperties = ["to_CompanyCode/CompanyCode", "to_CompanyCode/CompanyCodeName"];
		...
		this.oModel.getMetaModel().loaded().then(function() {
			// Arrange
			var oMetadataAnalyser = new MetadataAnalyser(this.oModel),
			fnSpy = sinon.spy(oMetadataAnalyser, "_mapSeparateNavigationProperties");

			// Act
			oMetadataAnalyser.getAllFilterableFieldsByEntitySetName("TestItems", false, aSeparateNavigationProperties);

			// Assert
			assert.equal(fnSpy.calledOnce, true, "_mapSeparateNavigationProperties is correctly called");
	...
	QUnit.test("getAllFilterableFieldsByEntitySetName shall return an array of the field names", function(assert) {
		var aFieldGroup,
			aFields,
			aSeparateNavigationProperties = ["to_CompanyCode/CompanyCode", "to_CompanyCode/CompanyCodeName"],
			aFieldsFromEntityDefinitions = [
				{name: "CompanyCode"},
				{name: "Supplier"},
				{name: "Customer"},
		...
		this.oModel.getMetaModel().loaded().then(function() {
			// Arrange
			var oMetadataAnalyser = new MetadataAnalyser(this.oModel),
				fnStubGetEntityDefinition = sinon.stub(oMetadataAnalyser, "_getEntityDefinition").returns({}),
				fnStubGetFilterableAssociations = sinon.stub(oMetadataAnalyser, "_getFilterableAssociations").returns({"to_CompanyCode": "ZEPM_C_SALESORDERITEMQUERY_CDS.I_CompanyCodeType"}),
				fnStubGetODataAssociationSetEnd = sinon.stub(oMetadataAnalyser._oMetaModel, "getODataAssociationSetEnd").returns({"entitySet": "to_CompanyCode"}),
				fnStubGetODataEntitySet = sinon.stub(oMetadataAnalyser._oMetaModel, "getODataEntitySet").returns({"entitySet": "to_CompanyCode"}),
				fnStubGetFilterableFieldsFromEntityDefinition = sinon.stub(oMetadataAnalyser, "_getFilterableFieldsFromEntityDefinition");

			fnStubGetFilterableFieldsFromEntityDefinition.withArgs({}, "to_CompanyCode", {"entitySet": "to_CompanyCode"}).returns({fields: aFieldsFromEntityDefinitions});

			// Act
			aFieldGroup = oMetadataAnalyser.getAllFilterableFieldsByEntitySetName("TestItems", false, aSeparateNavigationProperties);

			// Assert
			assert.ok(aFieldGroup);
			assert.equal(aFieldGroup.length, 2);
			aFields = aFieldGroup[1].fields;
			assert.ok(aFields);
			assert.equal(aFields.length, 2);
			assert.equal(aFields[0].name, "CompanyCode");
			assert.equal(aFields[1].name, "CompanyCodeName");

			// Cleanup
			oMetadataAnalyser.destroy();
			fnStubGetEntityDefinition.restore();
			fnStubGetFilterableAssociations.restore();
			fnStubGetODataAssociationSetEnd.restore();
			fnStubGetODataEntitySet.restore();
			fnStubGetFilterableFieldsFromEntityDefinition.restore();
			done();
		}.bind(this));
	});
	...
	QUnit.test("getValueListAnnotationLazy shall return Promise that would be resolved once the annotation is loaded", function(assert) {
		...
		this.oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		this.oMetadataAnalyser._oMetaModel.createBindingContext.returns({});
	...
	QUnit.test("_enrichAnnotationWithUIDataField", function(assert) {
		// Arrange
		var oMetadataAnalyser = this.oMetadataAnalyser;
		var oAnnotation = {};
		...
		var aRecords = [{
			...
			"com.sap.vocabularies.UI.v1.Hidden":{"Bool":"false"},
			"com.sap.vocabularies.UI.v1.Importance":{"EnumMember":"com.sap.vocabularies.UI.v1.ImportanceType/High"},
			"Value":{"Path":"Name1"},
			"RecordType":"com.sap.vocabularies.UI.v1.DataField",
			"EdmType":"Edm.String"
			...
			"com.sap.vocabularies.UI.v1.Importance":{"EnumMember":"com.sap.vocabularies.UI.v1.ImportanceType/Medium"},
			"Value":{"Path":"Hwaer"},
			"RecordType":"com.sap.vocabularies.UI.v1.DataField",
			"EdmType":"Edm.String"
		}, {
			"com.sap.vocabularies.UI.v1.Importance":{"EnumMember":"com.sap.vocabularies.UI.v1.ImportanceType/Low"},
			"Value":{"Path":"MyBoolean"},
			"RecordType":"com.sap.vocabularies.UI.v1.DataField",
			"EdmType":"Edm.Boolean"
		}, {
			"com.sap.vocabularies.UI.v1.Importance":{"EnumMember":"com.sap.vocabularies.UI.v1.ImportanceType/Low"},
		...
		// Act
		oMetadataAnalyser._enrichAnnotationWithUIDataField(oAnnotation, aRecords);

		// Assert
		assert.deepEqual(oAnnotation.fields, ["Bukrs", "Gjahr", "Kunnr", "Name1", "Dmbtr", "Hwaer",	"MyBoolean", "ExpirationDate"]);
		assert.deepEqual(oAnnotation.urlInfo, {});
		assert.deepEqual(oAnnotation.labels, {"Bukrs": "my Bukrs"});
		assert.deepEqual(oAnnotation.hidden, {"Gjahr": true, "Kunnr": true});
		assert.deepEqual(oAnnotation.importance, {"Bukrs": "High", "Dmbtr": "High", "ExpirationDate": "Low", "Gjahr": "High", "Hwaer": "Medium",
												"Kunnr": "High", "MyBoolean": "Low", "Name1": "High"});
		assert.deepEqual(oAnnotation.criticality, {});
		assert.deepEqual(oAnnotation.width, {});
	});
	...
	QUnit.test("getValueListAnnotationLazy shall return all additional annotations including those with a PresentationVariantQualifier", function(assert) {
		var done = assert.async();
		var oPromise;
		var oVLA = {
			"": {
		...
		oPromise.then(function(mAnnotation) {
			...
			// only 6 additional annotations found
			assert.strictEqual(mAnnotation.additionalAnnotations.length, 2);
			assert.strictEqual(mAnnotation.additionalAnnotationsWithPVQualifier.length, 1);

			assert.strictEqual(mAnnotation.additionalAnnotations[0].valueListEntitySetName, "VL_SH_GL_ACCT_CC_TEXT");
			assert.strictEqual(mAnnotation.additionalAnnotations[0].valueListTitle, "G/L account description in company code");
			assert.strictEqual(mAnnotation.additionalAnnotations[0].isSearchSupported, false);
			assert.strictEqual(mAnnotation.additionalAnnotations[0].qualifier, "GL_ACCT_CC_TEXT");

			assert.strictEqual(mAnnotation.additionalAnnotations[1].valueListEntitySetName, "VL_SH_GL_ACCT_CC_ALTERNATIV_NO");
			assert.strictEqual(mAnnotation.additionalAnnotations[1].valueListTitle, "Alternative account number");
			assert.strictEqual(mAnnotation.additionalAnnotations[1].isSearchSupported, false);
			assert.strictEqual(mAnnotation.additionalAnnotations[1].qualifier, "GL_ACCT_CC_ALTERNATIV_NO");

			assert.strictEqual(mAnnotation.additionalAnnotationsWithPVQualifier[0].valueListEntitySetName, "VL_SH_GL_ACCT_CC_FLAGS");
			assert.strictEqual(mAnnotation.additionalAnnotationsWithPVQualifier[0].valueListTitle, "G/L account with delete/lock flag in company code");
			assert.strictEqual(mAnnotation.additionalAnnotationsWithPVQualifier[0].isSearchSupported, false);
			assert.strictEqual(mAnnotation.additionalAnnotationsWithPVQualifier[0].qualifier, "GL_ACCT_CC_FLAGS");

			done();
		});
	});

	QUnit.test("getValueListAnnotationLazy shall return the primary annotation and only additional annotations with Relevant Qualifiers", function(assert) {
		var done = assert.async();
		var oPromise;
		var sProperty = "model/0/1/ReconciliationAccount";
		var sBindingContextPath = "/Main('1')";
		var oVLA = {
			"": {
				"Label": {},
				"CollectionPath": {},
		...
		var oVLPromise = Promise.resolve(oVLA);
		var oBindingContext = {getModel: function(){
			return this.oMetadataAnalyser.oModel;
		}.bind(this)};

		this.oMetadataAnalyser.oModel = sinon.createStubInstance(ODataModel);
		this.oMetadataAnalyser.oModel.getContext.withArgs(sBindingContextPath).returns(oBindingContext);
		this.oMetadataAnalyser.oModel.getObject.withArgs("USER_TYPE", oBindingContext).returns("personnel");
		this.oMetadataAnalyser.oModel.getObject.withArgs("IS_MANAGER", oBindingContext).returns(true);

		this.oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		this.oMetadataAnalyser._oMetaModel.createBindingContext.returns({});
		this.oMetadataAnalyser._oMetaModel.getODataProperty.returns(sProperty);
		this.oMetadataAnalyser._oMetaModel.createBindingContext.withArgs(sProperty).returns({getObject: function(){
			return {"com.sap.vocabularies.Common.v1.ValueListRelevantQualifiers": [{"String": "GL_ACCT_CC_ALWAYS"},
																					{"If": [{"Eq": [{"Path": "USER_TYPE"},
																									{"String": "personnel"}]},
																									{"String": "GL_ACCT_CC_PERSONNEL"}]},
																					{"If": [{"Path": "IS_MANAGER"},
																								{"String": "GL_ACCT_CC_MANAGER"}]},
																					{"If": [{"Eq": [{"Path": "USER_TYPE"},
																									{"String": "customer"}]},
																									{"String": "GL_ACCT_CC_CUSTOMER"}]}]};
		}});

		this.oMetadataAnalyser._oMetaModel.getODataValueLists.returns(oVLPromise);

		oPromise = this.oMetadataAnalyser.getValueListAnnotationLazy("com.sap.GL.ZAF.SomeEntity/ReconciliationAccount", sBindingContextPath);

		assert.ok(oPromise, "Promise returned");

		oPromise.then(function(mAnnotation) {
			assert.ok(mAnnotation && mAnnotation.primaryValueListAnnotation, "Promise Resolved");
			assert.strictEqual(mAnnotation.primaryValueListAnnotation.qualifier, "GL_ACCT_CC_ALWAYS");
			// only 2 additional annotations found
			// the one with "GL_ACCT_CC_CUSTOMER" will be filtered out, because USER_TYPE is NOT equal to "customer"
			assert.strictEqual(mAnnotation.additionalAnnotations.length, 2);
			assert.strictEqual(mAnnotation.additionalAnnotations[0].qualifier, "GL_ACCT_CC_PERSONNEL");
			assert.strictEqual(mAnnotation.additionalAnnotations[1].qualifier, "GL_ACCT_CC_MANAGER");
	...
	QUnit.test("getValueListAnnotationLazy shall return the primary annotation and only additional annotations with Relevant Qualifiers", function(assert) {
		var done = assert.async(),
			oPromise,
			oMetadataAnalyser = this.oMetadataAnalyser,
			sProperty = "model/0/1/ReconciliationAccount",
			sBindingContextPath = "/Main('1')",
			oVLA = {
				"": {
					"Label": {},
					"CollectionPath": {},
			...
			oVLPromise = Promise.resolve(oVLA),
			oBindingContext = {getModel: function(){
				return oMetadataAnalyser.oModel;
			}};

		oMetadataAnalyser.oModel = sinon.createStubInstance(ODataModel);
		oMetadataAnalyser.oModel.getContext.withArgs(sBindingContextPath).returns(oBindingContext);
		oMetadataAnalyser.oModel.getObject.withArgs("USER_TYPE", oBindingContext).returns("personnel");
		oMetadataAnalyser.oModel.getObject.withArgs("IS_MANAGER", oBindingContext).returns(true);

		oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		oMetadataAnalyser._oMetaModel.createBindingContext.returns({});
		oMetadataAnalyser._oMetaModel.getODataProperty.returns(sProperty);

		oMetadataAnalyser._oMetaModel.createBindingContext.withArgs(sProperty).returns({getObject: function(){
			return {"com.sap.vocabularies.Common.v1.ValueListRelevantQualifiers": [	{"If": [{"Eq": [{"Path": "USER_TYPE"},
																									{"String": "personnel"}]},
																									{"String": "GL_ACCT_CC_PERSONNEL"}]},
																					{"If": [{"Path": "IS_MANAGER"},
																								{"String": ""}]}
		...
		oMetadataAnalyser._oMetaModel.getODataValueLists.returns(oVLPromise);

		oPromise = oMetadataAnalyser.getValueListAnnotationLazy("com.sap.GL.ZAF.SomeEntity/ReconciliationAccount", sBindingContextPath);

		assert.ok(oPromise, "Promise returned");

		oPromise.then(function(mAnnotation) {
			assert.ok(mAnnotation && mAnnotation.primaryValueListAnnotation, "Promise Resolved");
			assert.strictEqual(mAnnotation.primaryValueListAnnotation.qualifier, "");
			// only 1 additional annotations found
			// the one with "GL_ACCT_CC_CUSTOMER" will be filtered out, because USER_TYPE is NOT equal to "customer"
			assert.strictEqual(mAnnotation.additionalAnnotations.length, 1);
			assert.strictEqual(mAnnotation.additionalAnnotations[0].qualifier, "GL_ACCT_CC_PERSONNEL");
	...
	QUnit.test("getValueListAnnotationLazy shall return the primary annotation and only additional annotations with Relevant Qualifiers", function(assert) {
		...
		oPromise.then(function(mAnnotation) {
			assert.ok(mAnnotation && mAnnotation.primaryValueListAnnotation, "Promise Resolved");
			assert.strictEqual(mAnnotation.primaryValueListAnnotation.qualifier, "GL_ACCT_CC_PERSONNEL");
			// there should not be additional annotations
			// the annotation with qualifier GL_ACCT_CC_PERSONNEL is now considered primary
			assert.strictEqual(mAnnotation.additionalAnnotations.length, 0);
	...
	QUnit.test("getPDFSupportedAnnotation shall return the pdf featured annotation", function(assert) {
		var oPDFAnnotation, oAnnotation;
		...
		this.oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		this.oMetadataAnalyser._oMetaModel.getObject.returns(oPDFAnnotation);
		var sEntityContainerPath = "/dataServices/schema/0/entityType/1";

		this.oMetadataAnalyser._oMetaModel.getODataEntityContainer.returns(sEntityContainerPath);
		oAnnotation = this.oMetadataAnalyser.getPDFSupportedAnnotation();

		assert.ok(oAnnotation);
		assert.strictEqual(oAnnotation.DocumentDescriptionCollection, "Foo", "DocumentDescriptionCollection. is set to Foo");
		assert.strictEqual(oAnnotation.ArchiveFormat, true, "ArchiveFormat. is set to true");
		assert.strictEqual(oAnnotation.Border, false, "Border. is set to false");
		assert.strictEqual(oAnnotation.CoverPage, true, "CoverPage. is set to true");
		assert.strictEqual(oAnnotation.Margin, true, "Margin. is set to true");
		assert.strictEqual(oAnnotation.FontName, true, "FontName. is set to true");
		assert.strictEqual(oAnnotation.FontSize, true, "FontSize. is set to true");
		assert.strictEqual(oAnnotation.FitToPage, false, "FitToPage. is set to false");
		assert.strictEqual(oAnnotation.Signature, true, "Signature. is set to true");
		assert.strictEqual(oAnnotation.ResultSizeDefault, 2000, "ResultSizeDefault. is set to int 2000");
		assert.strictEqual(oAnnotation.ResultSizeMaximum, 10000, "ResultSizeMaximum. is set to int 10000");
		...
		this.oMetadataAnalyser._oMetaModel.getObject.returns(oPDFAnnotation1);
		oAnnotation = this.oMetadataAnalyser.getPDFSupportedAnnotation();
		assert.strictEqual(oAnnotation.ArchiveFormat, true, "ArchiveFormat. is set to true");
		assert.notOk(oAnnotation.Border, "Border is unavailable");
		assert.notOk(oAnnotation.CoverPage, "CoverPage. is unavailable");
	});
	...
	QUnit.test("_getPresentationVariantQualifierForVHD  shall return the matching PresentationVariant annotation qualifier", function(assert) {
		var	oEntity = {
			"name": "Item",
			"key": {
				"propertyRef": [
		...
		var sPresentationAnnotationQualifier = this.oMetadataAnalyser._getPresentationVariantQualifierForVHD("com.sap.foo.Item");
		assert.equal(sPresentationAnnotationQualifier, "someQualifier", "Returned correct qualifier");
	});
	...
	QUnit.test("getTextArrangmentValue shall return the resolved TextArrangement annotation's displayBehaviour from nested TextArrangement into Text of the EDM Property", function(assert) {
		var oTextArrangmentAnnotaion,
			oEdmProperty,
			sDisplayBehaviour,
			sPropertyFullName = "com.sap.Item/testProperty";
		...
		this.oMetadataAnalyser._oMetaModel = sinon.createStubInstance(ODataMetaModel);
		this.oMetadataAnalyser._oMetaModel.getODataProperty.returns(oEdmProperty);

		sDisplayBehaviour = this.oMetadataAnalyser.getTextArrangementValue(sPropertyFullName);
	...
	QUnit.test("Test semanticObject Annotation with path", function(assert) {
		var oProperty = {
			"type": "Edm.String",
			"com.sap.vocabularies.Common.v1.SemanticObject": {
				"Path": "SemanticObjectName"
			}
		};

		var oSemanticObject = MetadataAnalyser.getSemanticObjectsFromProperty(oProperty);
		assert.ok(oSemanticObject.defaultSemanticObject.startsWith("{"), "DefaultSemanticObject contains a binding since a path was found");
	});

	QUnit.test("_mapSeparateNavigationProperties should create mapped object from array", function(assert) {
		// Arrange
		var oResult,
			aNavProperties = ["to_CompanyCode/CompanyName", "to_CompanyCode/CompanyCodeName", "to_Supplier/SupplierName",
				"to_Customer/CustomerName", "to_Customer/CustomerCodeName", "to_Customer/CustomerDescription"];

		// Act
		oResult = this.oMetadataAnalyser._mapSeparateNavigationProperties(aNavProperties);

		assert.equal(typeof oResult, "object", "object is correтly created");
		assert.equal(Object.keys(oResult).length, 3, "Object has correct number of properties");
		assert.equal(Array.isArray(oResult["to_CompanyCode"]), true, "The first property has an array with two elements");
		assert.equal(oResult["to_Supplier"].length, 1, "The first property has an array with two elements");
		assert.equal(oResult["to_Customer"].length, 3, "The first property has an array with two elements");
	});
	...
	QUnit.test("it should interpret the com.sap.vocabularies.Common.v1.IsCalendarYearMonth OData v4 annotation", function(assert) {
		...
		oMetadataAnalyzer._oMetaModel = new ODataModel("/odataFake/");

		// Assert
		assert.ok(MetadataAnalyser.isCalendarType(oProperty));
		assert.ok(CalendarMetadata.getCalendarAnnotationType(oProperty));

		// Act
		oField = oMetadataAnalyzer._parseV4PropertyAnnotations(oProperty, {}, "");

		// Assert
		assert.ok(oField.isCalendarType, true);
	});

	QUnit.test("it should interpret the com.sap.vocabularies.Common.v1.IsCalendarYearQuarter OData v4 annotation", function(assert) {
	...
	QUnit.test("it should interpret the com.sap.vocabularies.Common.v1.IsCalendarYearWeek OData v4 annotation", function(assert) {
	...
	QUnit.test("_parseV4PropertyAnnotations should load property TextArrangement annotation standalone", function (assert) {
		// Arrange
		var oProperty = {
				"name": "STRING_AUTO",
				"type": "Edm.String",
			...
			// Act
			oResultAnnotation = oMetadataAnalyzer._parseV4PropertyAnnotations(oProperty, oEntityDef);

			// Assert
			assert.equal(oResultAnnotation.displayBehaviour, "idAndDescription", "Display Behaviour should be 'idAndDescription'");
		});

		QUnit.module("com.sap.vocabularies.Common.v1.Timezone");

		QUnit.test("should return 'undefined' when property type is not Edm.DateTimeOffset", function(assert) {
			var oProperty = {
				name: "propertyName",
				type: "Edm.String",
				extensions: [{
			...
			assert.strictEqual(MetadataAnalyser.getTimezonePropertyPath(oProperty), undefined);
		});

		QUnit.test("should return timezone property path, if property type is Edm.DateTimeOffset and Timezone annotation is available on the property", function(assert) {
			var oProperty = {
				name: "propertyName",
				type: "Edm.DateTimeOffset",
				"com.sap.vocabularies.Common.v1.Timezone": {
					Path: "TimezoneProperty"
				}
			};

			assert.strictEqual(MetadataAnalyser.getTimezonePropertyPath(oProperty), "TimezoneProperty");
		});

		QUnit.test("Field metadata should be enriched with the timezone path", function(assert) {
			// Arrange
			var oProperty = {
			...
			// Act
			oResultAnnotation = oMetadataAnalyzer._parseV4PropertyAnnotations(oProperty, oEntityDef);
			assert.strictEqual(oResultAnnotation.timezone, "TimezoneProperty", "Field annotation is enrich with the 'timezone' path");
		});

		QUnit.module("com.sap.vocabularies.Common.v1.IsTimezone");

		QUnit.test("should return 'undefined' when property type is not Edm.String", function(assert) {
			var oProperty = {
				name: "propertyName",
				type: "Edm.Single",
				extensions: [{
			...
			assert.strictEqual(MetadataAnalyser.getIsTimezoneProperty(oProperty), undefined);
		});

		QUnit.test("should return true, if property type is Edm.String and IsTimezone annotation is available on the property", function(assert) {
			var oProperty = {
				name: "propertyName",
				type: "Edm.String",
			...
			assert.strictEqual(MetadataAnalyser.getIsTimezoneProperty(oProperty), true);
		});
