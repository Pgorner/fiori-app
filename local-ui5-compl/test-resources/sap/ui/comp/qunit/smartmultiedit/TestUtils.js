sap.ui.define([
...
], function (JSONModel, MockServer, BindingMode, ODataModel, ResourceModel, Field, SmartForm, Group, GroupElement,
			 Container, DateFormat, NumberFormat, Lib, _ResponsiveGridLayout) {
	"use strict";

	// taken from ControlFactoryBase
	var oResourceBundle = Lib.getResourceBundleFor("sap.ui.comp"),
	...
	TestUtils.FIELDS_POOL = {
		...
		"Distance": {
			...
			singleContextDisplayText: "654.321\xa0mi",
			newItemIndex: 4,
		...
		"Salary": {
			...
			singleContextDisplayText: "6\xa0543,21\xa0USD",
			newItemIndex: 3,
		...
		"SalaryFixed": {
			...
			singleContextDisplayText: "6\xa0543,21\xa0USD",
			newItemIndex: 3,
			newItemKey: "{\"SalaryFixed\":\"1234567\",\"SalaryUnitFixed\":\"JPY\"}"
		},
		"URL": {
			labelText: "URL",
			nullable: true,
		...
		"DepartureTime":{
			labelText: "Departure Time (DateTimeOffset with Timezone annotation)",
			nullable: true,
			valueHelp: false,
			multiContextSelectKeys: ["\/Date(1406851200000+0000)\/", null],
			singleContextSelectKeys:  null,
			singleContextDisplayText: "\/Date(1406851200000+0000)\/",
			newItemIndex: 3,
			newItemKey: new Date("Aug 10 2024 2:19:11 AM GMT")
		},
		"DepartureTimezone": {
			labelText: "Departure Timezone",
			nullable: true,
			valueHelp: true,
			multiContextSelectKeys: ["Europe/Berlin"],
			singleContextSelectKeys: "Europe/Berlin",
			singleContextDisplayText: "Europe/Berlin",
			newItemIndex: 3,
			newItemKey: "Europe/Berlin"
		},
		"ArrivalTime": {
			labelText: "Arrival Time (DateTimeOffset with Timezone annotation)",
			nullable: true,
			valueHelp: false,
			multiContextSelectKeys: ["\/Date(1406851200000+0000)\/", null],
			singleContextSelectKeys:  null,
			singleContextDisplayText: "\/Date(1406851200000+0000)\/",
			newItemIndex: 3,
			newItemKey: new Date("Aug 11 2024 2:19:11 AM GMT")
		},
		"ArrivalTimezone": {
			labelText: "Arrival Timezone",
			valueHelp: true,
			nullable: true,
			multiContextSelectKeys: ["America/Santiago"],
			singleContextSelectKeys: "America/Santiago",
			singleContextDisplayText: "America/Santiago",
			newItemIndex: 3,
			newItemKey: "America/Santiago"
		}
	};
	...
	TestUtils.destroyMockServer = function (sMetadataPath, sServiceURL) {
		sMetadataPath = sMetadataPath || "metadata.xml";
		sServiceURL = sServiceURL || "smartmultiedit.Employees/";
		if (this.mServers[sMetadataPath]) {
			this.mServers[sMetadataPath].destroy();
			this.mServers = null;
		}
	};
	...
	TestUtils.createDataModel = function (sServiceURL, sEntitySet) {
		...
		if (!oModel) {
			...
			// oDataModel.setDefaultBindingMode(BindingMode.OneTime);
