...
sap.ui.define([
	...
	QUnit.test("applyCriticality - Should apply criticality taken from DataFieldDefault, LineItem, DataPoint and FieldGroup", function(assert) {
		//Arrange
		var oDataHelper = this.oMetaData,
		oMetadataWithDataFieldDefault = {
			property: {
				"property": {
					"name": "Name",
					"type": "Edm.String",
					...
					"com.sap.vocabularies.UI.v1.DataFieldDefault": {
						"Label": {
							"String": "Product (non-edit, with Criticality)"
						},
						"Criticality": {
							"Path": "Criticality"
						},
			...
			annotations: {
				lineitem: {
					criticality: {
						"Name": {
							"path": "Criticality",
							"criticalityRepresentationType": "com.sap.vocabularies.UI.v1.CriticalityRepresentationType/WithIcon"
						}
					}
				}
			}
		},
		oMetadataWithDataPoints = {
			property: {
				"property": {
					"name": "Name",
					"type": "Edm.String",
				...
				datapoint: {
					...
					"additionalAnnotations": {
						"NegotiationType": {
							...
							"Criticality": {
								"Path": "CriticalityNegotiationType",
								"EdmType": "Edm.String"
							},
							"CriticalityRepresentation": {
								"EnumMember": "com.sap.vocabularies.UI.v1.CriticalityRepresentationType/WithoutIcon"
						...
						"IsSimulatedIndicator": {
							"Value": {
								"Path": "Name",
								"EdmType": "Edm.String"
							},
							"Criticality": {
								"Path": "CriticalityIsSimulatedIndicator",
								"EdmType": "Edm.String"
							},
							"CriticalityRepresentation": {
				...
				fieldgroup: [
					{
						...
						"groupName": "IsSimulatedIndicator",
						"fields": [
							"Name"
						],
						"criticality": {
		...
		oTestingObjects = [
			{criticalityFromAnnotation: "DataFieldDefault", metadata: oMetadataWithDataFieldDefault, qualifiers: [null]},
			{criticalityFromAnnotation: "LineItem", metadata: oMetadataWithLineItem, qualifiers: [null]},
			{criticalityFromAnnotation: "DataPoint", metadata: oMetadataWithDataPoints, qualifiers: [null, "NegotiationType", "IsSimulatedIndicator"]},
			{criticalityFromAnnotation: "FieldGroup", metadata: oMetadataWithFieldGroup, qualifiers: [null, "IsSimulatedIndicator"]}
		];

		for (let i = 0; i < oTestingObjects.length; i++) {
			const oTestingObject = oTestingObjects[i];

			for (let k = 0; k < oTestingObject.qualifiers.length; k++) {
				const sCriticalityQualifier = oTestingObject.qualifiers[k],
					sPath = "Criticality" + (sCriticalityQualifier === null ? "" : sCriticalityQualifier),
					oMetadata = oTestingObject.metadata;

				//Act
				oDataHelper.applyCriticality(oMetadata, sCriticalityQualifier);

				//Assert
				assert.ok(oMetadata.property.property?.criticalityInfo,
					"Criticality from " + oTestingObject.criticalityFromAnnotation + " has been applied to the property" + (sCriticalityQualifier === null ? "." : " (From qualifier " + sCriticalityQualifier + ")."));
				assert.ok(oMetadata.property.property?.criticalityInfo?.path === sPath, "The expected criticality path has been set.");
			}
		}
	...
	QUnit.test("getAbsolutePropertyPathToValueListEntity it should return the path to a OData entity data model property specified in the " +
		"ValueList text annotation (test case 4 - with special characters)", function(assert) {
		var sEntityId = "@#$!%^&",
			sEncodedEntityId = encodeURIComponent(sEntityId);
		...
		// Assert
		assert.strictEqual(sPath, "/VL_SH_H_CATEGORY('" + sEncodedEntityId + "')/LTXT", "Absolute property path to value list entity is correct.");
	});
	...
	QUnit.test("getDropdownChangeHandler", function(assert) {
		var bFire = false;
		var bExc = false;
		var bFired = false;
		var oControl = {
		...
		var oHandler = this.oMetaData.getDropdownChangeHandler(oControl);
		oHandler({
			"mParameters": true,
