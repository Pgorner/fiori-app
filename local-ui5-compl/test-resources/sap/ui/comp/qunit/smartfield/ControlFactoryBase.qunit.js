...
sap.ui.define([
	...
	QUnit.test("_addAriaLabelledBy", function(assert) {
		...
		assert.equal(Element.getElementById(oControl.control.getAriaLabelledBy()[0]).getText(), "Project Description (ariaLabelledBy)", "ARIA label taken over from SmartField");
	});
	...
	QUnit.test("addValidations - UoM Validation - error", function(assert) {
		// Arrange
		var fFireValidationError,
		...
		oControl.attachValidationError = function(fErr) {
			fFireValidationError = fErr;
		};

		oFactory.addValidations(oControl);

		// Act
		fFireValidationError(oEvent);

		// Assert
		assert.equal(oUoMControl.setValueState.callCount, 1);
		assert.equal(oUoMControl.setValueStateText.callCount, 1);
	...
	QUnit.test("createValueHelp - configdata", function(assert) {
		var oControl = {
			setModel: function () {
			...
			data: function() {
				return {
					configdata: {
							annotations: {
								valueListData: {},
								valuelist: "Test Annotation Path"
							}
					}
				};
			},
		...
		var mAnnotation = {
			...
			primaryValueListAnnotation: {annotation : "Test annotation"}
		};

		var oVHPStub = this.stub(ValueHelpProvider.prototype, "destroy").returns(function() {});
		var oVLPStub = this.stub(ValueListProvider.prototype, "destroy").returns(function() {});
		var oMAStub = this.stub(MetadataAnalyser.prototype, "getValueListAnnotationForFunctionImport").returns(mAnnotation);
		var oFactory = new ControlFactoryBase(this.oVHModel, oControl);
		var oMetaData = new ODataHelper(this.oVHModel);
		var oEntitySet = oMetaData.oMeta.getODataEntitySet("Headers");
		var oType = oMetaData.oMeta.getODataEntityType(oEntitySet.entityType);
		...
		oMetaData.getProperty(oMeta);
		var oAnnotation = {
			annotation: "Test annotation object"
		};
		oMeta.property["sap:value-list"] = "standard";
		...
		oFactory.createValueHelp({
			...
			valueHelp: {
				annotation: oAnnotation,
				analyser : new MetadataAnalyser("foo")
			},
			model: this.oVHModel
		});

		assert.strictEqual(oFactory._aProviders[0]._sFullyQualifiedFieldName, oControl.data().configdata.annotations.valuelist, "Annotation path is correct");
	...
	QUnit.test("getFormatSettings", function(assert) {
		...
		mAttributes = assert.equal(mAttributes.style, "long"); // eslint-disable-line
	...
	QUnit.test("_getDisplayBehaviourConfiguration - the default display behaviour should be 'descriptionAndId'", function(assert) {
		// Arrange
		var sDisplay,
			oFactory,
			oParent = sinon.createStubInstance(SmartField);
		...
		// Act
		sDisplay = oFactory._getDisplayBehaviourConfiguration();

		// Assert
		assert.equal(sDisplay, "descriptionAndId");
