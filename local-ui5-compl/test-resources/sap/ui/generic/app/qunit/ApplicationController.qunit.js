...
sap.ui.define([
...
], function(ODataModel, ApplicationController, DraftController) {
	"use strict";
	...
	var oEntityType =
		{
			...
			"com.sap.vocabularies.Common.v1.SideEffects#GLOBAL_SIDE_EFFECTS": {
				"TargetEntities": {
					"Collection": [
						{
							"NavigationPropertyPath": "Item/ScheduleLine"
	...
	QUnit.module("sap.ui.generic.app.ApplicationController", {
		beforeEach: function () {
			...
			this.oModel.requestSideEffects = function () {
	...
	QUnit.test("field group validation event successful", function (assert) {
		...
		this.oModel.submitChangesWithChangeHeaders = function (p1) {
			bSubmit = true;
			return p1.success({
				"mockdata": true
			}, {
		...
		this.oController.registerGroupChange("V_MANDATORY_ATTRIBUTES");
	...
	// For transient controls ex: empty creation row , the side effect GET does not make sense as the context is local and a relevant record is yet to be created in the backend.
	QUnit.test("Preparation Action call only scenario for transient controls", function (assert) {
		var done = assert.async(), oContext, bSubmitCalled = false, bIsReadCalled = false, bIsActionCalled = false;
		var mAdditionalParameters = {
			draftRootContext : {},
			callPreparationOnDraftRoot : true,
			callPreparationOnly : true
		};
		this.oModel.refresh = function () {
			bRefresh = true;
		};
		this.oModel.callFunction = function (p1, p2) {
			bIsActionCalled = true;
			this.mDeferredRequests = {
				"Changes": {
					"requests": [{}]
				}
		...
		this.oModel.read = function(p1, p2) {
			bIsReadCalled = true;
		}
		this.oModel.submitChanges = function (p1) {
			bSubmitCalled = true;
		...
		mAdditionalParameters.draftRootContext = this.oModel.getContext();
		oContext = this.oModel.getContext();

		this.oController.attachEvent("beforeSideEffectExecution", function(oEvent) {
			var oPromise = oEvent.getParameter("promise");
			oPromise.then(function	(oResult) {
				assert.ok(!bIsReadCalled); //Global Side Effect GET should not be called for transient controls
				assert.ok(bIsActionCalled);
				assert.ok(bSubmitCalled);
				done();
			}, function	(oResult) {
				assert.ok(false);
				done();
		...
		this.oController.executeSideEffects(oContext, null, null, false, mAdditionalParameters);
	});

	QUnit.test("Preparation Action call Etag validation is disabled", function (assert) {
		var done = assert.async(), oContext, bSubmitCalled = false, bIsReadCalled = false, bIsActionCalled = false;
		var mAdditionalParameters = {
			draftRootContext : {},
			callPreparationOnDraftRoot : true,
		...
		const spy = sinon.spy(DraftController.prototype, "prepareDraft");
		this.oModel.refresh = function () {
			bRefresh = true;
		};
		...
		sinon.assert.calledWith(spy, sinon.match.any, sinon.match({ headers: { "If-Match": "*" } }));
		done();
	...
	QUnit.test("propertyChanged shall trigger submitChanges", function (assert) {
		...
		var oContext = {
			isTransient: Function.prototype
		};

		this.oController.propertyChanged("mockProperty", oContext).then(function (oResult) {
	...
	QUnit.test("dataReceived event should be fired on list bindings which are side effect targets", function (assert) {
		//Preparation
		var done = assert.async();
		var mParameters = {
			context: {
		...
		//Creating list item bindings and respective spies
		function ListItemBinding(sPath) {
			this.sPath = sPath;
			this.fireDataReceived = Function.prototype
		}
		var oSalesOrderItemListBinding = new ListItemBinding("to_Item");
		var oPurchaserItemListBinding = new ListItemBinding("to_Purchaser");

		var oSalesOrderItemFireDataReceivedSpy = sinon.spy(oSalesOrderItemListBinding, "fireDataReceived");
		var oPurchaserItemListBindingFireDataReceivedSpy = sinon.spy(oPurchaserItemListBinding, "fireDataReceived");

		//Mocking "requestSideEffects"
		this.oModel.requestSideEffects = function () {
			return Promise.resolve([oSalesOrderItemListBinding, oPurchaserItemListBinding]);
		}

		// Action
		var oReadPromise = this.oController._read("sales_order_id", mParameters, true);

		// Assertion
		oReadPromise.then(function (aResponse){
			assert.equal(aResponse.length, 2, "The promise should return 2 items");
			assert.equal(aResponse[0], oSalesOrderItemListBinding, "First item in the response should be sales order item binding");
			assert.equal(aResponse[1], oPurchaserItemListBinding, "Second item in the response should be purchaser item binding");
			assert.equal(oSalesOrderItemFireDataReceivedSpy.callCount, 1, "fireDataReceived method in sales order item binding should be invoked once");
			assert.ok(oSalesOrderItemFireDataReceivedSpy.calledWithExactly({"reason": "SIDE_EFFECT_TARGET"}), "Check the parameters of fireDataReceived method invocation");
			assert.equal(oPurchaserItemListBindingFireDataReceivedSpy.callCount, 1, "fireDataReceived method in purchaser item list binding should be invoked once");
			done();
		});
	});
