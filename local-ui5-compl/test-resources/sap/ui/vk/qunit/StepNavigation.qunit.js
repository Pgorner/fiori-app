	...
	*/
	...
	QUnit.test("Test loading scene", function(assert) {
		...
		viewer.attachSceneLoadingFailed(function(event) {
			assert.ok(false, "Viewer scene loading failed.");
			done();
		});

		// VDS file load successfuly handler
		viewer.attachSceneLoadingSucceeded(function(event) {
			// This forces stepNavigation control to render HTML.
			core.applyChanges();
			assert.ok(true, "Viewer scene loading succeeded.");
			var listOfThumbnails;
			if (firstPass) {
				// Find all items with class "sapVizKitStepNavigationStepItem"
				listOfThumbnails = jQuery(".sapVizKitStepNavigationStepItem");
