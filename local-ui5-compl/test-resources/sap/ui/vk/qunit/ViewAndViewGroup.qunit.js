...
sap.ui.define([
	...
	QUnit.test("view and view group", function(assert) {
		...
		var view1 = scene.createView({
			...
			navigationMode: NavigationMode.Pan,
			dimension: 3
		});
		var cameraInfo1 = {
			aspect: 1,
		...
		assert.ok(view1.hasHighlight(), "Check highlight");
		...
		assert.equal(view1.getNavigationMode(), NavigationMode.Pan, "view 1 navigation mode");
		assert.equal(view1.getDimension(), 3, "view 1 dimension");
		...
		var view2 = new View();
		assert.equal(view2.getNavigationMode(), NavigationMode.NoChange, "view 2 default navigation mode");
		view2.setViewId("2");
		view2.setName("view2");
		view2.setDescription("view2 description");
		view2.setNodeInfos(meshNodesInfo);
		view2.setNavigationMode(NavigationMode.Turntable);
		view2.setDimension(2);
		...
		assert.equal(view2.getNavigationMode(), NavigationMode.Turntable, "view 1 navigation mode");
		assert.equal(view2.getDimension(), 2, "view 2 dimension");
