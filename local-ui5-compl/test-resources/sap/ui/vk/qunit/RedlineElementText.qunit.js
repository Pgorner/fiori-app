...
sap.ui.define([
	...
	QUnit.test("Testing RedlineElementText", function(assert) {
		...
		DummyRenderManager.prototype.text = function(text) {
			this._text += text;
		}
		...
		DummyRenderManager.prototype.openEnd = function() {
			if (this._style.length > 0) {
				this._text += " style=\"";
				this._style.forEach(style => this._text += style[0] + ":" + style[1] + ";")
				this._text += "\"";
				this._style.length = 0;
			}
			this._text += ">";
		};
		...
		DummyRenderManager.prototype.style = function(styleName, data) {
			this._style.push([styleName, data]);
		};
		...
		var expectedRenderedContent =
			"<foreignObject x='250' y='100' width='1' height='1' style=\"overflow:visible;\"><textArea spellcheck='false' style=\"font-family:Comic Sans MS;font-size:calc(123px * var(--sapUiVizKitRedlineScale, 1));color:rgba(10,20,30,0.4);opacity:0.8;background:transparent;border-style:none;resize:none;width:calc(345px * var(--sapUiVizKitRedlineScale, 1));height:calc(234px * var(--sapUiVizKitRedlineScale, 1));\">ABC 123</textArea></foreignObject>";
