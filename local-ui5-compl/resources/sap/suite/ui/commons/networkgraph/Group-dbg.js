...
sap.ui.define([
	...
	Group.prototype._renderHeader = function (mOptions) {
		...
		if (this._bIsSwimLane && (sColorStyle || this._hasDefaultStatus())) {
			mOptions.renderManager.openStart("div").class("sapSuiteUiCommonsNetworkGroupHeaderStatus " + sColorClass).openEnd();
		...
		if (sIcon) {
			this._renderHtmlIcon(sIcon,
				"sapSuiteUiCommonsNetworkGroupHeaderIcon " + (mOptions.renderManager ? sContentClassVal : sContentClass),
				null,
		...
		this._renderHtmlIcon(
			...
			"sapSuiteUiCommonsNetworkGroupHeaderMenuIcon " + (mOptions.renderManager ? sContentClassVal : sContentClass),
			"collapse",
			mOptions.renderManager ? sContentStyleVal : sContentStyle,
			oResourceBundle.getText("NETWORK_GRAPH_EXPAND_COLLAPSE"),
			mOptions.renderManager
		);
