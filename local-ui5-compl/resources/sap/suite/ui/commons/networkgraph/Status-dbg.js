...
sap.ui.define([
...
], function (Element, library, mobileLibrary, Parameters) {
	"use strict";

	const SemanticColorType = library.SemanticColorType;
	...
	Status.prototype.setBackgroundColor = function (sValue, bSuppressInvalidate) {
		var bIsValueSet = false;
		if (SemanticColorType.hasOwnProperty(sValue) ){
			bIsValueSet = true;
		} else {
			/**
		...
		return this.setProperty("backgroundColor", bIsValueSet ? sValue : null, bSuppressInvalidate);
	}

	Status.prototype.setContentColor = function (sValue, bSuppressInvalidate) {
		var bIsValueSet = false;
		...
		return this.setProperty("contentColor", bIsValueSet ? sValue : null, bSuppressInvalidate);
	}

	Status.prototype.setHeaderContentColor = function (sValue, bSuppressInvalidate) {
		var bIsValueSet = false;
		...
		return this.setProperty("headerContentColor", bIsValueSet ? sValue : null, bSuppressInvalidate);
	}

	// TODO: change this to _getLegendClass .. which will return class instead of color
	Status.prototype._getLegendColor = function () {
		var sLegendColor = this.getLegendColor() || this.getBackgroundColor();

		if (SemanticColorType.hasOwnProperty(sLegendColor) ){
			return sLegendColor;
		} else {
			/**
			* @deprecated As of version 1.120
