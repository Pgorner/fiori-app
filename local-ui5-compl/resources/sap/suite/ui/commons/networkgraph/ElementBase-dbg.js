...
sap.ui.define([
	...
	ElementBase.prototype._afterRenderingBase = function () {
		...
		if (!bIsHtmlNode && !bIsGroup) {
				var sHtml = this._render({
					renderManager: oRm
				});
				if (sHtml) {
					this.$()[0].outerHTML = sHtml;
				} else {
					oRm.flush(this.$()[0], true, false);
					oRm.destroy();
				}
		}
	...
	ElementBase.prototype._getColor = function (sType, sStatus) {
		...
		if (oStatus) {
			...
			if (SemanticColorType.hasOwnProperty(sName)) {
				sColor = sName;
			} else {
				...
				{	
					if (sName) {
						...
						if (!sColor && ValueCSSColor.isValid(sName)) {
							sColor = sName;
						}
					}
				}
	...
	ElementBase.prototype._getStatusStyle = function (oParameters, bCreateStyle) {
		...
		Object.keys(oParameters).forEach(function (sKey) {
			...
			if(SemanticColorType.hasOwnProperty(sValue)){
				var sClassName = Utils.SEMANTIC_CLASS_NAME[sKey.toUpperCase()];
				sClass += sClassName ? (sClassName + sValue + " ") : "";
			} else{
				if (sValue) {
					if (sStyle) {
						sStyle += ";";
