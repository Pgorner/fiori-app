...
sap.ui.define([
	...
	Timeline.prototype.setModelFilter = function (mArguments) {
		switch (mArguments.type) {
			...
			case TimelineFilterType.Time:
				...
				if (!mArguments.filter){
					//In case filter is not defined dates should be reset to null
	...
	Timeline.prototype._fnAddDate = function (iValue, sDateRoundType) {
		var oNewDate, oRoundedDown, oRoundedUp,
			...
			fnReturnCorrectDate = function (oDirect, oRoundedDown, oRoundedUp) {
				if (sDateRoundType === DateRoundType.UP) {
					...
					return UI5Date.getInstance(Math.min.apply(null, [this._maxDate, oRoundedUp]));
				}
				if (sDateRoundType === DateRoundType.DOWN) {
					fnSetTime.call(oRoundedDown, 0, 0, 0);
					return UI5Date.getInstance(Math.max.apply(null, [this._minDate, oRoundedDown]));
				}
				return oDirect;
			};
		...
		switch (this._rangeFilterType) {
			case TimelineGroupType.Year:
				oNewDate = UI5Date.getInstance(UI5Date.getInstance(this._minDate).setFullYear(this._minDate.getFullYear() + iValue));
				oRoundedDown = UI5Date.getInstance(oNewDate.getFullYear(), 0, 1);
				oRoundedUp = UI5Date.getInstance(oNewDate.getFullYear(), 11, 31);
				break;

			case TimelineGroupType.Month:
				oNewDate = UI5Date.getInstance(UI5Date.getInstance(this._minDate).setMonth(this._minDate.getMonth() + iValue));
				oRoundedDown = UI5Date.getInstance(oNewDate.getFullYear(), oNewDate.getMonth(), 1);
				oRoundedUp = UI5Date.getInstance(oNewDate.getFullYear(), oNewDate.getMonth() + 1, 0);
				break;

			case TimelineGroupType.Quarter:
				oNewDate = UI5Date.getInstance(UI5Date.getInstance(this._minDate).setMonth(this._minDate.getMonth() + (iValue * 3)));
				var iQuarterStart = oNewDate.getMonth() % 3;
				oRoundedDown = UI5Date.getInstance(oNewDate.getFullYear(), oNewDate.getMonth() - iQuarterStart, 1);
				oRoundedUp = UI5Date.getInstance(oNewDate.getFullYear(), oNewDate.getMonth() + (2 - iQuarterStart) + 1, 0);
				break;

			case TimelineGroupType.Day:
				// we do this for hour rounding
				oNewDate = oRoundedDown = oRoundedUp = UI5Date.getInstance(UI5Date.getInstance(this._minDate).setDate(this._minDate.getDate() + iValue));
				break;
		...
		if (oBinding) {
			...
			if (this._filter && !bResetAll) {
				var filterIndex = findIndex(aFilters, function (oFilter) {
					return oFilter === that._filter || oFilter === that.aPrevFilters;
				});
	...
	Timeline.prototype._getRangeMessage = function () {
		...
		if (!text) {
			...
			if (Localization.getTimezone() !== Intl.DateTimeFormat().resolvedOptions().timeZone && titleStart instanceof Date && titleEnd instanceof Date) {
	...
	Timeline.prototype.updateContent = function () {
		...
		var oFilterIcon = this._objects.getFilterIcon();
		if (this.getAggregation("content").length <= 1) {
			oSortIcon.setEnabled(false);
			oFilterIcon.setEnabled(false);
		} else {
			oSortIcon.setEnabled(true);
			oFilterIcon.setEnabled(true);
		}
	...
	Timeline.prototype.onAfterRendering = function () {
		...
		if (!this.getShowHeaderBar()) {
			this.oItemNavigation.iFocusedIndex = 0;
		}
