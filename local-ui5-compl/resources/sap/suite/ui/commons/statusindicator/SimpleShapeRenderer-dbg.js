...
sap.ui.define([
...
], function (library, MobileLibrary, HtmlElement, Renderer) {
	"use strict";
	...
	SimpleShapeRenderer._getHtmlModel = function (oControl) {
		...
		var sFillColor =  oControl._resolveFillColor();

		if (oSemanticColorType.hasOwnProperty(sFillColor)) {
			oShapeElement.addClass("fillSemanticColor" + sFillColor);
		} else if (ValueCssColor.isValid(sFillColor)){
			/**
			* @deprecated As of version 1.120
			*/
			oShapeElement.setAttribute("fill", sFillColor);
		}
	...
	SimpleShapeRenderer._updateDomColor = function (oControl, sNewFillColor) {
		if (oSemanticColorType.hasOwnProperty(sNewFillColor)) {
			oControl.$("shape").addClass("fillSemanticColor" + sNewFillColor);
		} else if (ValueCssColor.isValid(sNewFillColor)){
			/**
			* @deprecated As of version 1.120
			*/
