...
sap.ui.define([
	...
	"sap/base/i18n/Localization"
			 ...
			 GroupHeaderListItem, Item, KeyCodes, Log, TargetFilterRenderer,Lib, Localization) {
	"use strict";
	...
	var TargetFilterLinksCloud = Control.extend("sap.suite.ui.commons.TargetFilterLinksCloud", {
		...
		init: function () {
			var that = this;
			this._oRb = Lib.getResourceBundleFor("sap.suite.ui.commons");
		...
		updateVisibleLinks: function () {
			...
			oLinks.each(function () {
				var oLink = Element.getElementById(this.id);
	...
	var TargetFilterQuadrant = Control.extend("sap.suite.ui.commons.TargetFilterQuadrant", {
		...
		removeFilter: function (sValue) {
			...
			if (oFilter.linkId) {
				Element.getElementById(oFilter.linkId).setEmphasized(false).setSubtle(true);
			} else {
		...
		setColumn: function (sColumnId, bDontUpdateFilter) {
			this._oColumn = Element.getElementById(sColumnId);
				...
				var oRowTmpl = new TargetFilterLink({
					...
					press: function (oEvent) {
						if (this.$()[0].scrollWidth > this.$().innerWidth()) { // line truncated - popover needed
							...
							if (Element.getElementById(sId + "-popover")) that._selectPopover.destroy();
							that._selectPopover = new Popover(sId + "-popover", {
								title: Element.getElementById(sId + "-cb").getValue(),
								content: [new Text({
									text: this.getText(),
									wrapping: true
	...
	TargetFilter.prototype.setQuadCBoxItem = function (iQuad, sName) {
		...
		for (var i = 0; i < aItemsCB.length; i++) {
			oNamesCBox[Element.getElementById(aItemsCB[i].getKey()).getPath()] = aItemsCB[i].getKey();
		}
		if (oNamesCBox[sName]) {
