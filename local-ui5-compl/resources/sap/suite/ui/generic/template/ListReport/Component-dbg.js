		...
		return {
			...
			*/
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet, oInternalManifest, oModel, oDraftContext) {
					...
					if (
						...
						if (sMultiEditAnnotation) {
							...
							var aMultiEditFields = aRecords.filter(function (oRecord) {
								var oProperty = oRecord.RecordType === "com.sap.vocabularies.UI.v1.DataField" && oMetaModel.getODataProperty(oEntityType, oRecord.Value.Path);
								return oProperty && oProperty["sap:updatable"] !== "false" && (!oRecord["com.sap.vocabularies.UI.v1.Hidden"] || oRecord["com.sap.vocabularies.UI.v1.Hidden"].Bool === "false")
				...
				function fnAdjustSelectAll(oTableSettings) {
					if (oTableSettings.type === "ResponsiveTable") {
						oTableSettings.selectAll = oTableSettings.selectAll !== false;
					}
					oTableSettings.selectAll = oTableSettings.selectAll !== undefined && oTableSettings.selectAll;
				}
				...
				if (oLrSettings.quickVariantSelectionX) {
					...
					for (var sKey in oVariants) {
						...
						var sEntitySet = oVariant.entitySet || sLeadingEntitySet;
						var oEntitySet = oMetaModel.getODataEntitySet(sEntitySet);
						...
						oVariant.isSmartChart = checkIfSmartChart(sEntitySet, oVariant);
						if (!oVariant.isSmartChart) {
							...
							var oLineItem = preparationHelper.getLineItemFromVariant(oMetaModel, oMetaModel.getODataEntitySet(sEntitySet).entityType, oVariant.annotationPath && oVariant.annotationPath.split("#")[1]);
							oVariant.tableSettings = oVariant.tableSettings || oDefaultTableSettings;
							oVariant.tableSettings = preparationHelper.getNormalizedTableSettings(oMetaModel, oVariant, Device, sEntitySet, oExtensionActions, oLineItem);
							if (oLrSettings.isResponsiveTable === undefined){
								oLrSettings.isResponsiveTable = oVariant.tableSettings.type === "ResponsiveTable";
							...
							if (oVariantEntityType && oVariantEntityType.property && oVariant.tableSettings && oVariant.tableSettings.createWithParameterDialog) {
								staticChecksHelper.checkErrorforCreateWithDialog(oVariantEntityType, oVariant.tableSettings);
							...
							if (oVariant.tableSettings.multiEdit) {
								oMultiEditSettings = getMultiEditSettings(oVariant, oLrSettings, oEntitySet, oEntityType);
								oVariant.tableSettings.multiEdit.enabled = oMultiEditSettings.multiEditEnabled;
								if (oMultiEditSettings.multiEditEnabled) {
									oVariant.tableSettings.multiEdit.fields = oMultiEditSettings.fields;
									oVariant.tableSettings.mode = oMultiEditSettings.selectionMode;
								}
							}
							setTargetEntity(oEntitySet);
							oVariant.tableSettings = deepExtend(oVariant.tableSettings, fnTableLevelActions(oVariant.key, sKey));
						}
					}

					delete oLrSettings.tableSettings;
					//handle where variants contain only charts
					oLrSettings.isResponsiveTable = oLrSettings.isResponsiveTable === undefined || oLrSettings.isResponsiveTable;
				} else {
					//for single  variant
				...
				var oTemplatePrivate = oComponent.getModel("_templPriv");
				oTemplatePrivate.setProperty("/listReport/datePropertiesSettings", oDateProperties);
				if (!oLrSettings.subPages || oLrSettings.subPages.length === 0) { //LR only app
					oTemplatePrivate.setProperty("/listReport/bSupressCardRowNavigation", true);
				} else if (oLrSettings.subPages[0].navigation) { //OP with external navigation via manifest
					oTemplatePrivate.setProperty("/listReport/bSupressCardRowNavigation", true);
				} else if (oLrSettings.quickVariantSelectionX) {
					oTemplatePrivate.setProperty("/listReport/bSupressCardRowNavigation", true);
				}
				...
				oLrSettings.bInsightsEnabled = !!oTemplatePrivateGlobal.getProperty("/generic/insights/oInsightsInstance");
				return oLrSettings;
			},
				...
				properties: {
					...
					quickVariantSelectionX: {
						...
						properties: { // Currently, everything here is just for documentation, but has no functionality. In future, a mechanism to fill default values shall be added
							...
							enableAutoBinding: { // obsolete - use dataLoadSettings instead
								type: "boolean",
								defaultValue: false
							},
