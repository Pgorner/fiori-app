	...
	FilterItemChart.prototype._updateBinding = function() {
		...
		if (oModel) {
			...
			if (!bIsChartDonut) { //for line and bar chart
				...
				this._updateBindingInfo(oDataBindingInfo, [measureField], aRelativeToProperties, 1, oExtensionContext, navProperty, navPropertyForUnitField, bIsChartDonut, true, filterList);
		...
		if (this.getDimensionFieldIsDateTime()) {
			if (typeof oContext === "string" && sDimFieldValue instanceof Date) {
				...
				if (oContext.ranges.length) {
					if (oContext.ranges[0].value1 instanceof Date && FilterUtil.getDateInMedium(oContext.ranges[0].value1) === FilterUtil.getDateInMedium(sDimFieldValue)) {
						return true;
					} else if (typeof oContext.ranges[0].value1 === 'string' && FilterUtil.getDateInMedium(new Date(oContext.ranges[0].value1)) === FilterUtil.getDateInMedium(sDimFieldValue)) {
						return true;
					}
				}
