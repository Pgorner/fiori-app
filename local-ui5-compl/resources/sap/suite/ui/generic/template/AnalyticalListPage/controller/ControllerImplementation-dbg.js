			...
			getMethods: function(oViewProxy, oTemplateUtils, oController) {
				...
				function createDefaultFilter(oSmartFilterbar) {
					var oModel = oSmartFilterbar.getModel(),
						...
						oResultEntityType = oMetaModel && oMetaModel.getODataEntityType(metadataAnalyser.getEntityTypeNameFromEntitySetName(oSmartFilterbar.getEntitySet(), oMetaModel)),
						oDataSuiteFormat,
						sResultEntityType = oMetaModel && oMetaModel.getODataEntityType(metadataAnalyser.getEntityTypeNameFromEntitySetName(oSmartFilterbar.getEntitySet(), oMetaModel), true),
				...
				return {
					...
					handlers: {
						...
						onBeforeRebindTable: function(oEvent) {
							...
							var oContainerView = oTemplatePrivateModel.getProperty('/alp/contentView');
							if (oContainerView === CONTAINER_VIEW_CHART) { // Check for the Chart View and not to create a new binding for the SmartTable
								oBindingParams.preventTableBind = true;
								bTableRebindRequired = true;
							}
							...
							oState.detailController._updateExpandLevelInfo(groupList, oBindingParams);
							...
							oBindingParams.events.dataRequested = function(oRequestEvent){
								var that = this;
								oState.detailController.onSmartTableDataRequested(oSmartTable).then(function () {
						...
						onContentViewSegmentButtonPressed: function(oEvent){
							...
							if (!oState.oSmartFilterableKPI && !oState.oController.getOwnerComponent().getContentTitle()) {
								var oContentView = oState.oController.getView(),
									sToolbarId;
								if (sSelectedKey === "customview1") {
									sToolbarId = "template::contentViewExtensionToolbar";
								} else if (sSelectedKey === "customview2") {
									sToolbarId = "template::contentViewExtension2Toolbar";
								} else if (sSelectedKey === "chart" || sSelectedKey === "charttable") {
