sap.ui.define([
	...
	Element
		 ...
		 */
		FilterUtil.getAllMandatoryFilters = function (oModel, sEntitySet) {
			var oMetadataAnalyser = new MetadataAnalyser(oModel);
			var sEntityTypePath = metadataAnalyser.getEntityTypeNameFromEntitySetName(sEntitySet, oModel.getMetaModel());
			var mAllProps = oMetadataAnalyser.getFieldsByEntityTypeName(sEntityTypePath);
			var aFilters = [];
			for (var key in mAllProps) {
				var oDataProperty = mAllProps[key];
		...
		FilterUtil.isNullable = function (oModel, entitySet, sPropertyName) {
			var oMetadataAnalyser = new MetadataAnalyser(oModel);
			var sEntityTypePath = metadataAnalyser.getEntityTypeNameFromEntitySetName(entitySet, oModel.getMetaModel());
			var oAllProps = oMetadataAnalyser.getFieldsByEntityTypeName(sEntityTypePath);
			for (var key in oAllProps) {
				var oDataProperty = oAllProps[key];
				if (oDataProperty && oDataProperty.name === sPropertyName && oDataProperty.type === "Edm.String") {
					if (oDataProperty.nullable === "false") {
						return false;
					} else {
						return true;
					}
		...
		FilterUtil.getPropertyNameDisplay = function (model, entitySet, propertyName, oAppI18nModel, showQuickinfo) {
			...
			if (showQuickinfo && sQuickInfo) {
				if (sQuickInfo.match(/{@i18n>.+}/gi) && oRB) {
					sQuickInfo = oRB.getText(sQuickInfo.substring(sQuickInfo.indexOf(">") + 1, sQuickInfo.length - 1));
				}
				return sQuickInfo;
			}
			if (sLabel) {
