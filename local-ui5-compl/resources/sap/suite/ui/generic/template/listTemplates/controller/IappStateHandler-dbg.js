sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oState, oController, oTemplateUtils) {
		...
		function resolveAppStatePromise (oAppData, oURLParameters, sNavType) {
			//AddCardtoInsights card navigation
			if (sNavType === sap.fe.navigation.NavType.hybrid) {
			...
			if (sNavType !== sap.fe.navigation.NavType.initial) {
				...
				var oTemplatePrivate = oState.oController.getOwnerComponent().getModel("_templPriv");
				if ( sNavType === sap.fe.navigation.NavType.xAppState && oTemplatePrivate.getProperty('/alp/filterMode') === FILTER_MODE_VISUAL) {
					triggerMerge();
				}
			...
			*/
			...
			fnHidePlaceholderForMaterExtensionAndCustomViews();
		}

		function fnHidePlaceholderForMaterExtensionAndCustomViews() {
			var sContentView = oState.oController.getView().getModel("_templPriv").getProperty("/alp/contentView");
			if (sContentView !== "charttable" && sContentView !== "table") {
				if (!oState.oSmartChart || sContentView === "customview1" || sContentView === "customview2" || sContentView === "crosstable") {
					oTemplateUtils.oComponentUtils.hidePlaceholder();
				}
			}
		}
			...
			try {
				var oRet = new Promise(function(fnResolve){//not added fnResolve and fnReject
					...
					oParseNavigationPromise.fail(function(oNavError, oURLParameters, sNavType){
						...
						resolveAppStatePromise({}, oURLParameters, sap.fe.navigation.NavType.initial); // Use NavType initial, as this will enforce selection in case auto-binding is true.
						fnResolve();
					});
				});
				return oRet;
