sap.ui.define([
	...
	function getMethods(oComponent, oComponentRegistryEntry) {
		...
		// This fn. returns a Promise which resolves to an object holding (level 1) Main object's getText and getRootExpand methods, and its entitySet property, if the main component at level 1 is
		...
		// This method returns the expand string that is used in the bindElement for this component.
		// Note that this will return a faulty value for the root page (even if the OP for the same entity set has a valid expand).
		// Moreover, note that this method may also return a faulty result if called too early.
		// This would be the case before viewRegistered property of the component registry entry is resolved.
		function getRootExpand(){
			var oPreprocessorsData = getPreprocessorsData();
			var aStreamEnabledAssociatedEntities = getParameterModelForTemplating().getObject("/templateSpecific/streamEnabledAssociatedEntites");
			...
			aRootExpandArray = aStreamEnabledAssociatedEntities && aStreamEnabledAssociatedEntities.length > 0 ? aRootExpandArray.concat(aStreamEnabledAssociatedEntities) : aRootExpandArray;
			if (aRootExpandArray) {
		...
		function getTemplateName(oController) {
			// todo: simplify
			// as component instance and controller instance is 1:1, name is set to the controller from component (in TemplateAssembler), and we have 1 instance of componentUtils per component, this is 
		...
		// 0 is returned if both components are identical or there is no predecessor/successor relationship between them
		function getHierarchicalDistance(sOtherComponentId){
			...
			oNavigationFinishedPromise.then(function() {
				if (isComponentActive()){
					...
					if (oContext && isDraftEnabled()) {
						...
						// 	3. All the listeners registered on the PageDataLoaded event is notified
		...
		function fnEndBusy(){
			...
			if (fnBindingRefreshResolve){
				fnBindingRefreshResolve();
				fnBindingRefreshResolve = null;
			}
		...
		function fnUnbind(){
			fnNewDataAvailablePromise(); // old HeaderDataAvailablePromise points to outdated data
		...
		function fnCompareKeysAndStoreNewOnes(){
			...
			var bNoDifference = (aNewKeys.length === aCurrentKeys.length);
			for (var i = 0; i < aNewKeys.length && bNoDifference; i++){
				bNoDifference = aNewKeys[i] === aCurrentKeys[i];
			}
			aCurrentKeys = aNewKeys;
			return !bNoDifference;
		}
		...
		function fnRefreshBindingUnconditional(bWithoutAssociations){
			...
			if (oElementBinding) {
				...
				if (oElementBinding.isSuspended()){ // component is currently invisible
					...
					if (!fnBindingRefreshResolve){
						var oBindingRefreshPromise = new Promise(function(fnResolve){
							fnBindingRefreshResolve = fnResolve;
						});
						// Consider header refresh as a kind of side-effect execution
						oComponentRegistryEntry.oTemplateContract.fnAddSideEffectPromise(oBindingRefreshPromise);
					}
		...
		function fnMessagesRefresh(sBatchGroupId){
			var oModel = oComponent.getModel();
			var oParameters = {
				...
				batchGroupId: sBatchGroupId || "facets",
		...
		// oTransferEnded is a Promise that will be resolved/rejected as soon as this draft transfer has finished successfully/unsuccessfully
		function onBeforeDraftTransfer(oTransferEnded){
			if (isDraftEnabled()){
				var oComponentContainer = oComponent.getComponentContainer();
				var oElementBinding = oComponentContainer.getElementBinding();
				if (oElementBinding && !oElementBinding.isSuspended()){
					oElementBinding.suspend(); // suspend the binding to suppress any refresh operation which is done during the activation process
		...
		// Returns whether state messages should be suppressed when reading tables or charts
		...
		function setPaginatorInfo(oPaginatorInfo){
			...
			// The following code is used to find out whether we need to suppress the buttons (if this is not already known).
			// To this end we first need to find out which entity set the paginatorInfo points to (relevant in multi entity set scenarios).
			// This is derived from one of the contexts in oPaginatorInfo.objectPageNavigationContexts.
			// Note that this list may contain undefined entries in case of tree table.
			var oSampleContext = oPaginatorInfo && !oPaginatorInfo.suppressButtons && oPaginatorInfo.objectPageNavigationContexts.find(function(oContext){
				return oContext;
			});
			if (oSampleContext){
		...
		oComponentRegistryEntry.oStatePreserverPromise = oComponentRegistryEntry.reuseComponentsReady.then(function(mReuseComponentProxies){
			...
			var fnProduceStateFunction = function(bForInitial){
				var sGetState = bForInitial ? "getInitialState" : "getCurrentState";
				var sGetStateForEmbeddedComponent = bForInitial ? "stGetInitialState" : "stGetCurrentState";
				return function(){
					var oRet = oComponentRegistryEntry.methods[sGetState] ? oComponentRegistryEntry.methods[sGetState]() : Object.create(null);
					var fnAddReuseInfo = function(oProxy, sKey){
						if (oProxy.component[sGetStateForEmbeddedComponent]){
							var oTempState = oProxy.component[sGetStateForEmbeddedComponent]();
			...
			var oSettings = extend({
				...
				getCurrentState: fnProduceStateFunction(false),
				getInitialState: fnProduceStateFunction(true),
		...
		function getCRUDActionHandler(){
			oCRUDActionHandler = oCRUDActionHandler || new CRUDActionHandler(oComponentRegistryEntry.oTemplateContract, oComponentRegistryEntry.oController, oComponentRegistryEntry.oControllerUtils.oCommonUtils, oComponentRegistryEntry.methods.prepareForMessageHandling || function(){ 
				return Promise.resolve(); 
			});
		...
		function fnHidePlaceholder(){
			...
			if (oTreeNode.level === 0){ // placeholders on root level should only be shown once
				...
				delete oTarget._oOptions.placeholder;                         
			}	
		}
		
		// This method should only be called when the component is currently active.
		// In this case it calls method prepareForMessageHandling for this component and all active components with a higher level
		// for the given set of messages and returns a Promise that resolves if all the corresponding Promises are resolved.
		function fnPrepareForMessageHandling(aMessages){
			var aActiveComponents = oComponentRegistryEntry.oTemplateContract.oNavigationControllerProxy.getActiveComponents();
			...
			for (var i = iIndex; i < aActiveComponents.length; i++){
				...
				if (oEntry.methods.prepareForMessageHandling){
					aPromises.push(oEntry.methods.prepareForMessageHandling(aMessages));
				}
			}
		...
		return {
			...
			getHierarchicalDistance: getHierarchicalDistance,
			...
			prepareForMessageHandling: fnPrepareForMessageHandling,
			...
			canNavigateToSubEntitySet: fnCanNavigateToSubEntitySet,
