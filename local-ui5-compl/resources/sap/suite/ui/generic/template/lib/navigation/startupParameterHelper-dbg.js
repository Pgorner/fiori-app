sap.ui.define(["sap/base/util/extend",
	...
	ModelUtil, FeLogger, oDataModelHelper, testableHelper, CRUDHelper, deepExtend, CacheHelper, IllustratedMessageType){
		...
		function fnPrepareCreate(oTemplateContract, oModel, sEntitySet, mParameters) {
			...
			var aDateTimeProperties = fnReturnPropertiesOfGivenType(oModel, sEntitySet, "Edm.DateTime");
			fnTransformEdmDateTimeParams(aDateTimeProperties, mParameters);
		...
		*/
		function fnTransformEdmDateTimeParams(aPropertiesOfTypeDate, mParameters) {
			...
			aPropertiesOfTypeDate.forEach(function (oProperty) {
				var aPropertyValues = mParameters[oProperty.name];
				if (aPropertyValues && typeof aPropertyValues[0] === "string" && oProperty["sap:display-format"] === "Date") {
					var oDate = UI5Date.getInstance(aPropertyValues[0]);
		...
		*/
		...
		function fnNavigateForAction(oTemplateContract, mParameters, oModel){
			...
			var oCallFunctionPromise = new Promise(function(fnResolve){
				...
				oInfoForContentIdPromise.then(function(oInfoObject) {
					if (oInfoObject.contentIdRequestPossible && oInfoObject.parametersForContentIdRequest.sRootExpand) {
						mParameters.expand = oInfoObject.parametersForContentIdRequest.sRootExpand;
					}
		...
		var fnNavigateForCreate = testableHelper.testableStatic(fnNavigateForCreate, "startupParameterHelper_fnNavigateForCreate");
		/* eslint-enable */
