...
sap.ui.define([
	...
	ViewCreationHelper
			...
			// - aKeys: An array containing the last used keys for this component
			...
			return {
				...
				//   ~ getCurrentState, getInitialState, applyState: functions that are used by the StatePreserver attached to this component.
				//   ~ getStatePreserverSettings: Provide additional settings for the StatePreserver (curently only callAlways) is used 
				...
				//   ~ prepareForMessageHandling(aMessages, oItemBinding, oHelperModel): This function is called in order to prepare the page for "handling" of the messages
				//     provided in aMessages.
				//     If only aMessages is provided this means that it should be ensured that "sufficient" controls on the page are available the messages might be assigned to.
				//     If additional parameters oItemBinding and oHelperModel are provided this means that the messages are going to be displayed in a popup which should be prepared as well.
				//     oItemBinding is the list binding representing the messages to be displayed. It is the task of this function to apply a sorter to this binding.
				//     Moreover, this function supports two additional functionalities on the popup as well, namely grouping and displaying of a subtitle for the messages.
				//     For this functionality oHelperModel should be a JSONModel. The function will update properties messageToGroupName and mMessageToSubtitle within this model.
				//     These properties map the ids of the messages onto the corresponding group name resp. subtitle.
				//     In all cases this method returns a Promise indicating that the preparation is finished. No assumption is made on the value this Promise resolves to.
