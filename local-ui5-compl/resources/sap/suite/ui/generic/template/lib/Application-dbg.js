sap.ui.define(["sap/ui/base/Object",
	...
	function(BaseObject, Device, JSONModel, MessageToast, ActionSheet, Dialog, Popover, deletionHelper,
		ContextBookkeeping, CRUDHelper, FocusHelper, MessageUtils, testableHelper, FeLogger, oDataModelHelper,
		...
		var GHOSTNAVIGATIONHANDLER = {
			...
			parseNavigation: function(){
				return {
						...
						setTimeout(fnHandler.bind(null, {}, {}, sap.fe.navigation.NavType.initial), 0);	
					},
					fail: Function.prototype
				};
			}
		...
		function getMethods(oTemplateContract) {
			...
			function fnNavigateAfterActivation(oActiveContext, bAsNavigationOptions){
				return oTemplateContract.oNavigationControllerProxy.navigateAfterActivation(oActiveContext, bAsNavigationOptions);
			}
			...
			function getNavigationHandler() {
				if (!oNavigationHandler){
					oNavigationHandler = new NavigationHandler(oTemplateContract.oAppComponent, "ODataV2");
			...
			function fnDeregisterCustomMessageProvider(oCustomMessageProvider){
				aCustomMessageProviders = aCustomMessageProviders.filter(function(oCandidate){
					return oCandidate !== oCustomMessageProvider;
				});
		   }
			...
			function setStates(bToInitial){
				var aActiveComponents = oTemplateContract.oNavigationControllerProxy.getActiveComponents();
				var aStatePreserverPromises = aActiveComponents.map(function(sComponentId){
					var oRegistryEntry = oTemplateContract.componentRegistry[sComponentId];
					return oRegistryEntry.oStatePreserverPromise.then(function(oStatePreserver){
						oStatePreserver.setState(bToInitial);                         
					});
				});
				oTemplateContract.oBusyHelper.setBusy(Promise.all(aStatePreserverPromises));
			}
			
			function getComponentUtilsIfLoaded(sEntitySet){
				var oTreeNode = oTemplateContract.mEntityTree[sEntitySet];
				var oComponentRegistryEntry = oTreeNode && oTreeNode.componentId && oTemplateContract.componentRegistry[oTreeNode.componentId];
				return oComponentRegistryEntry && oComponentRegistryEntry.utils;
			}
			...
			return {
				...
				deregisterCustomMessageProvider: fnDeregisterCustomMessageProvider,
				isTransientMessageNoCustomMessage: isTransientMessageNoCustomMessage,
				removeTransientMessages: fnRemoveTransientMessages,
				getTransientMessages: getTransientMessages,
				setNextFocus: oFocusHelper.setNextFocus,
