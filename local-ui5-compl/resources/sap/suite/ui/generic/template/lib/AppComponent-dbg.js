	...
	function getMethods(oAppComponent, oAppId) {
		...
		var oTemplateContract = { // template contract object which is used for data interchange between framework classes
			...
			mBusyTopics: Object.create(null), // This map is used by an infrastructure which allows to add functionality to busy sessions is a generic way.
			                                  ...
			                                  //             be removed from the topic definition. Hence, the code that uses this infrastructure has to add this property again (if needed).
		...
		//     For the root node the value is static and already set in class routingHelper.
		...
		function setGlobalTemplateModels(){
			...
			var oVersionInfoPromise = VersionInfo.load();
			oVersionInfoPromise.then(function (oVersionInfo){
				oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/ui5VersionInfo", oVersionInfo);
			});
		...
		function init(){
			...
			Promise.all([oExpandHashPromise,oMessageScopeSupportedPromise]).then(function(aResolvedPromiseValue){
				...
				var oComponentData = oAppComponent.getComponentData();
				var bIsReadOnly = oComponentData && oComponentData.feEnvironment && oComponentData.feEnvironment.isReadOnly && oComponentData.feEnvironment.isReadOnly();
				if (!bIsReadOnly && UShellContainer) {
					UShellContainer.registerDirtyStateProvider(fnGetShellDataLossPopup);
				}
			});
			setLeaveAppPromise();
		...
		function exit() {
			(oTemplateContract.oNavigationControllerProxy ? oTemplateContract.oNavigationControllerProxy.leave : Function.prototype)();
			var sAppComponentId =  oAppComponent.getId();
			oLogger.info("Exit called for AppComponent " + sAppComponentId);
			delete mDesigntimeHandlers[sAppComponentId];
			if (UShellContainer) {
				UShellContainer.deregisterDirtyStateProvider(fnGetShellDataLossPopup);
			}
			...
			/* In some scenarios, exit event is triggered from FLP for app which is suspended in keep-alive mode. Since we have set
		...
		function getConfig() {
			if (!oConfig) {
				oConfig = oAppComponent.getManifestEntry("sap.ui.generic.app");
				if (Object.keys(oConfig).length === 0){ // test scenario
					return oConfig;
				}
		...
		var bRefreshDataNeeded = false;

		// Refresh the data. Triggered by function refreshForStartupParameter (see below). Either called directly (if the app is currently visible) or delayed (if app is not yet visible).
		function fnRefreshData(){
			bRefreshDataNeeded = false;
			var oModel = oAppComponent.getModel();
			oModel.invalidate();
			for (var sComponentId in oTemplateContract.componentRegistry){
				var oRegistryEntry = oTemplateContract.componentRegistry[sComponentId];
				oRegistryEntry.utils.refreshBindingUnconditional();
			}
		...
		var oMethods = {
			...
			navigateBasedOnStartupParameter: function(oStartupParameters) {
				if (bRefreshDataNeeded){
					var oRefreshPromise = fnRefreshData();
					oTemplateContract.oBusyHelper.setBusy(oRefreshPromise);
				}
			...
			designtime: {
				tool : {
					...
					stop: function(oAppComponent) {
						var sAppComponentId = oAppComponent.getId();
						oLogger.info("Ui Adaptation stopped for AppComponent " + sAppComponentId);
						var oDesigntimeHandler = mDesigntimeHandlers[sAppComponentId];
