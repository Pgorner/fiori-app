sap.ui.define(["sap/ui/base/Object",
	...
	"sap/suite/ui/generic/template/js/AnnotationHelper"
		...
		function getMethods(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper) {
			...
			function fnGetAllMandatoryFields(oModel) {
				var oEntitySet = oModel.getMetaModel().getODataEntitySet(oController.getOwnerComponent().getEntitySet());
				return AnnotationHelper.getRequiredProps(oEntitySet);
			}

			// In the given array of properties, returns the unfilled properties in the context
			function fnGetUnfilledProperties(aProperties, oBindingContext) {
				var oObject = oBindingContext.getObject();
				return aProperties.filter(function (sPropName){
                    return !(oObject[sPropName]);
                });
			}
			...
			function saveEntityImpl(resolve, reject, oCreateWithDialogFilters) {
				...
				if (!oCreateWithDialogFilters) {
					var aMandatoryFields = fnGetAllMandatoryFields(oModel),
						oBindingContext = oController.getView().getBindingContext(),
						aUnfilledMandatoryFields = fnGetUnfilledProperties(aMandatoryFields, oBindingContext);
			...
			function saveEntity(oCreateWithDialogFilters) {
				...
				var oRet = new Promise(function(fnResolve, fnReject) {
					saveEntityImpl(fnResolve, fnReject, oFilterForCreateWithDialog);
				});
				return oRet;
			}
			...
			function callAction(mParameters, oState) {
				...
				if (sFunctionImportReturnType === sEntityType) {
					...
					var getRootExpand = oOPComponentUtils && oOPComponentUtils.viewRegistered && !oOPComponentUtils.fnViewRegisteredResolve && oOPComponentUtils.getRootExpand;
					...
					return oInfoForContentIdPromise.then(function(oInfoObject) {
						...
						mParameters.sRootExpand = sRootExpand;
						return callActionPromise(mParameters, oState);
					});
				}
				return callActionPromise(mParameters, oState);
			}

			function callActionPromise(mParameters, oState) {
				var oRet = new Promise(function(fnResolve, fnReject){
			...
			function addEntry(oTable, oPredefinedValues) {
				...
				if (iViewLevel > 0) { // Details page 
					...
					// Normally the corresponding entity set can be derived via oComponent.getEntitySet().
					// However, there are two exceptions: a) Property "creationEntitySet" may override this (on LR), b) in multi entity set scenarios the entity set should be taken from the table
					sEntitySet = (oComponent.getCreationEntitySet && oComponent.getCreationEntitySet()) || oTable.getEntitySet();
			...
			return {
				...
				getUnfilledProperties: fnGetUnfilledProperties
			};
		}
