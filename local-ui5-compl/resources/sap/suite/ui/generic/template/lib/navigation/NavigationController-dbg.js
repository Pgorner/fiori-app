...
sap.ui.define(["sap/ui/base/Object",
	...
	var oCrossAppNavigatorPromise = routingHelper.getCrossAppNavServicePromise();
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		var oHistoryState, sHistoryKey; // Initialized below
		...
		var aLastActiveComponentForFCLColumn = ["-", "-", "-"];
		...
		function fnInitSession(bIsFirst){
			...
			var oIntentFromShellPromise = UShellContainer && new Promise(function(fnResolve){
				UShellContainer.getServiceAsync("AppLifeCycle").then(function(oAppLifeCycle){
		...
		function fnAfterActivationImpl(oTitleProvider){
			...
			var aActiveDraftDetailTemplatePrivateModels = [];
			...
			Promise.all(aPageDataLoadedPromises).then(function(){
				...
				// Make sure that no state messages are retrieved in case of an automatic draft merge
				var bIsDraftEditing = aActiveDraftDetailTemplatePrivateModels.some(function(oTemplatePrivateModel){
					return 	oTemplatePrivateModel.getProperty("/objectPage/displayMode") > 1;
				});
				var oModel = oTemplateContract.oAppComponent.getModel();
				oModel.setTransitionMessagesOnlyForGroup("Changes", bIsDraftEditing);
			});
		}
		...
		//  3. oOptions and oRoutingOptions are both truthy (edge case): A navigation to the target specified by oRoutingOptions has already been triggered but now the target is changed as specified by oOptions
		...
		function fnNavigateToSubContext(vContext, bReplace, iDisplayMode, oContextInfo, mAppStates){
			...
			var oTargetPromise = aIdentityKeys ? Promise.resolve({
				...
				var fnProcessTargetIdentity = function(){
					if (iCount == aContext.length){
						...
						var oTargetIdentityInfo = getTargetIdentityInfoForContext(oCurrentTargetIdentity, oContext, true, true);
						oTargetIdentityInfo.promise.then(function(oTargetIdentity){
							iCount++;
			...
			var bWillShowPlaceholder = !oTargetNode || fnWillNavigationToNodeShowPlaceholder(oTargetNode);
		...
		function fnNavigateToDetailContextIfPossible(oContext, bReplace, iDisplayMode, oContextInfo){
			...
			var oTargetPromise = oTargetInfo.promise;
			oTemplateContract.oBusyHelper.setBusy(oTargetPromise, undefined, undefined, oTargetInfo.willShowPlaceholder);
		...
		function getSwitchToSiblingPromise(oSiblingContext, iDisplayMode, oTargetInfo, bAsIdentity){
			...
			var vRet = bAsIdentity ? oTargetIdentity : oNavigationQueue.makeQueuable(fnNavigateToIdentity.bind(null, oTargetIdentity, true, iDisplayMode, false)); // The navigation function the Promise which is returned will resolve to
		...
		// iDisplayMode: as described in fnNavigateToRoute
		function fnNavigateToIdentity(oNewIdentity, bReplace, iDisplayMode, bAsNavigationOptions){
			...
			if (oTreeNodeInfo.navigationProperty){
				return new Promise(function(fnResolve, fnReject){
					...
					oModel.createBindingContext(oTreeNodeInfo.navigationProperty, oNavigationContext, null, function(oTargetContext) { // translate the given navigation context into a navigation context fitting to the target node
						var oNavPromise = oTargetContext && getTargetIdentityPromiseForContext(oSourceNode, aSourceKeys, oTargetContext, {
		...
		function fnWillNavigationToNodeShowPlaceholder(oTreeNode){
			return oTemplateContract.bEnablePlaceholder &&
				// special logic of FCL:Placeholder in FCL is not shown if the view to be shown in a column is the same as the last which was shown there.
				// Note that fCLLevel 3 is handled like 2, since corresponding views will technically also be shown in the end column (but always as fullscreen):
				oTreeNode.componentId !== aLastActiveComponentForFCLColumn[Math.min(oTreeNode.fCLLevel, 2)] &&
				(oTreeNode.level > 0 || !oTreeNode.componentId) && // on root level the placeholder will not be shown when the corresponding view was already shown once
				!!(oTreeNode.behaviour && oTreeNode.behaviour.getPlaceholderInfo()); // only if a placeholder has been specified for the target floorplan a placeholder will be shown
		}

		// This method returns target information for navigation. More precisely, it returns an object with two properties:
		// - targetNode: The target tree node for the navigation (also contained as treeNode in the partial identity mentioned below, but here it can be accessed synchronously)
		// - promise: A Promise that resolves to a partial identity for navigation. More precisely, the object the Promise resolves to contains properties treeNode and keys, but not appStates.
		// - willShowPlaceholder: A boolean indicating whether navigating to the target node will trigger showing a placeholder
		// If it is not possible to determine this result the Promise is rejected (and targetNode will be faulty)
		...
		// only can be reached via a to 1-association from the entity set of oNavigationContext. However, in this case the corresponding navigation property must have been configured as property 'navigationProperty'
		// in the pages definition of the target node/page. Currently, this is only supported for the scenario that the target node has level 1.
		function getTargetIdentityInfoForContext(oSourceIdentity, oNavigationContext, bWithNavProperty, bConsiderParentNodes){
			...
			var oTargetNode = oTreeNodeInfo && oTreeNodeInfo.treeNode;
			return oTargetNode ? {
				promise: getTargetIdentityPromiseForContext(oSourceNode, aSourceKeys, oNavigationContext, oTreeNodeInfo),
				targetNode: oTargetNode,
				willShowPlaceholder: fnWillNavigationToNodeShowPlaceholder(oTargetNode)
		...
		function fnNavigateAfterActivation(oActiveContext, bAsNavigationOptions){
			var oTargetIdentityInfo = getTargetIdentityInfoForContext({ treeNode: oTemplateContract.mRoutingTree.root }, oActiveContext, false, true);
			...
			var oRet = oTargetIdentityPromise.then(function(oTargetIdentity){
				...
				// If we reach this point we know that the target node is different from the current node. There are two possible scenarios for this:
				...
				return oAppStatePromise.then(fnNavigateToIdentity.bind(null, oTargetIdentity, true, 1, bAsNavigationOptions));
			});
		...
		function fnNavigateForNonDraftCreate(sEntitySet, vPredefinedValues, oPredefinedContext, mAppStates){
			...
			var oPredefinedKeyPromise;
			if (vPredefinedValues){
				oPredefinedKeyPromise = oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
					return oCrossAppNavigator.createEmptyAppState(oTemplateContract.oAppComponent).then(function(oAppState){
				...
				oPredefinedKeyPromise = Promise.resolve();
			}
			...
			var oRet = Promise.all([oPredefinedKeyPromise, oAppStatePromise]).then(function(aPromiseResults){
				var sPredefinedKey = aPromiseResults[0];
				if (sPredefinedKey){
		...
		function fnAdaptUrlAfterNonDraftCreateSaved(oSavedContext){
			var oTargetIdentityInfo = getTargetIdentityInfoForContext(null, oSavedContext, false, false);
			var oTargetIdentityPromise = oTargetIdentityInfo.promise;
			var oRet = oTargetIdentityPromise.then(function(oTargetIdentity){
				oTargetIdentity.appStates = Object.create(null);
				extend(oTargetIdentity.appStates, oCurrentIdentity.appStates);
		...
		// Check documentation of getTargetIdentityInfoForContext for details how oSourceNode, the corresponding elements of oCurrentIdentity.keys, and oNavigationContext specify treeNode and keys of the target identity.
		// iDisplayMode: as described in fnNavigateToRoute
		...
		function fnNavigateFromNodeAccordingToContext(oSourceNode, oNavigationContext, iDisplayMode, bReplace){
			var oTargetIdentityInfo = getTargetIdentityInfoForContext({ treeNode: oSourceNode }, oNavigationContext, true, false); // this determines treeNode and keys of the target identity
			var oTargetIdentityPromise = oTargetIdentityInfo.promise;
			var oRet = oTargetIdentityPromise.then(function(oTargetIdentity){
				oTargetIdentity.appStates = Object.create(null);
				...
				return oAppStatePromise.then(fnNavigateToIdentity.bind(null, oTargetIdentity, bReplace, iDisplayMode, false)); // navigate when all parameters have been determined
			});
			oTemplateContract.oBusyHelper.setBusy(oRet, undefined, undefined, oTargetIdentityInfo.willShowPlaceholder);
			return oRet;
		}
		...
		// - A Promise resolving to a truthy value will be returned
		function fnNavigateToMessageTarget(oMessage, sFullTarget){
			...
			var fnFocusFunction = function(oBeforeData, fnFallback){
				...
				var oCurrentMessage;
				oMessageBinding.getAllCurrentContexts().some(function(oContext) {
				...
				if (oCurrentMessage){
					...
					MessageUtils.prepareForMessageHandling([oCurrentMessage], oTemplateUtils);
		...
		function fnTransferMessageParametersToGlobalModelAndDisplayMessage(mParameters) {
			...
			if (oTemplateContract.oFlexibleColumnLayoutHandler){
				...
				if (!oTemplateContract.ghostapp){
					var oDisplayPromise = oTargets.display("messagePage");
					oTemplateContract.oBusyHelper.setBusy(oDisplayPromise);
				}
		...
		function getAppStateFromShell(sAppStateKey, bApplyDeStringify){
			return sAppStateKey ? new Promise(function(fnResolve, fnReject){
				oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
					var oShellPromise = oCrossAppNavigator.getAppState(oTemplateContract.oAppComponent, sAppStateKey);
					oShellPromise.then(function(oWrapper){
					...
					.catch(function() {
						oShellPromise.catch(fnReject.bind(null, undefined));
					});
				});
			}) : Promise.reject();
		}
		...
		function fnActivateOneComponent(sPath, oActivationInfo, oComponent) {
			...
			var oTreeNode = oTemplateContract.mRoutingTree[sRoute];
			aLastActiveComponentForFCLColumn[Math.min(oTreeNode.fCLLevel, 2)] = sComponentId;
		...
		// All future calls of fnHandleRouteMatchedImpl will be delayed accordingly.
		// Moreover, the method returns true in this case to indicate that it needs not to be called again for that target.
		function fnTreeNodeFirstDisplay(oTreeNode){
			var bIsTreeNodeReallyDisplayed = !oTemplateContract.oFlexibleColumnLayoutHandler || oTreeNode === oCurrentIdentity.treeNode || !oTemplateContract.oFlexibleColumnLayoutHandler.hasIdentityFullscreenLayout(oCurrentIdentity);
			if (bIsTreeNodeReallyDisplayed){
				oTemplateComponentsAvailablePromise = Promise.all([oTemplateComponentsAvailablePromise, oTreeNode.componentCreated]);
			}
			return bIsTreeNodeReallyDisplayed;
		}

		// Propagate named models from one managed object to another managed object
		function fnPropogateModels(oTargetObject, oSourceObject) {
		...
		function fnNavigationWillLeavePage(oSourceTreeNode, sTargetEntitySet){
			if (!sTargetEntitySet){
				return oSourceTreeNode.viewLevel > 0;
			}
			if (oSourceTreeNode.viewLevel > 0 && oSourceTreeNode.entitySet === sTargetEntitySet){
				return false;
			}
			var oTargetNode = oTemplateContract.mEntityTree[sTargetEntitySet];
			if (!oTargetNode){
				return false;
			}
			if (oTargetNode.fCLLevel === 0 || oTargetNode.level <= oSourceTreeNode.level){
				return true;
			}
		...
		oNavigationControllerProxy.navigationWillLeavePage = fnNavigationWillLeavePage;
		...
		oTemplateContract.oBusyHelper.setBusy((getParsedShellHashFromFLP() || Promise.resolve()).then(function(oResults){
			...
			var oHistoryStateAvailablePromise = oCrossAppNavigatorPromise.then(function(oCrossAppNavigator){
				return oCrossAppNavigator.createEmptyAppState(oTemplateContract.oAppComponent, true).then(function(oState){
					oHistoryState = oState;
					sHistoryKey = oHistoryState.getKey();
				});
			});
			return oHistoryStateAvailablePromise.then(function(){
				return routingHelper.generateRoutingStructure(oTemplateContract).then(fnInitSession.bind(null, true)); // initialize the first session;
			});
		}), undefined, undefined, true);
