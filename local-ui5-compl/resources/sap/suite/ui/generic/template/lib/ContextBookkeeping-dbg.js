sap.ui.define(["sap/ui/base/Object", "sap/base/util/each", "sap/base/util/extend", "sap/suite/ui/generic/template/genericUtilities/oDataModelHelper"], function (BaseObject, each, extend, oDataModelHelper) {
		...
		function getMethods(oTemplateContract) {
			...
			//                 - bIsDraftModified: Is this a draft which has been modified. The flag would also be truthy for cases where the draft has been modified on the
			//									   UI but the changes has not been yet sent to the backend because of the delayed draft synchronization.
			...
			//   In the case with bReplaceActive set true, when Alternate context is requested the draft is suppressed and the corresponding active version is displayed.
			...
			var mPath2ContextData = Object.create(null); // currently only used for draft scenarios
			...
			// Method that creates the ContextInfo for a given context. Used inside this class, but can also be used externally
			function fnCreateDraftInfo(oContext) {
				...
				var oActiveEntity = bIsDraftSupported && oContext.getObject();
				...
				var bOwnDraftExists = bIsDraftSupported && oActiveEntity.HasDraftEntity && oContext.getObject("DraftAdministrativeData/DraftIsCreatedByMe");
				var bIsDraftModified = false;
				if (bIsDraft && oActiveEntity.DraftEntityCreationDateTime && oActiveEntity.DraftEntityLastChangeDateTime) {
					var iLastChangeTime = oActiveEntity.DraftEntityLastChangeDateTime.getTime();
					var sPath = oContext.getPath();
					var oPreviousEntry = mPath2ContextData[sPath];
					var oPreviousEntity = oPreviousEntry && oPreviousEntry.oContext && oPreviousEntry.oContext.getObject();
					var iPreviousEntryLastChangeTime = oPreviousEntity && oPreviousEntity.DraftEntityLastChangeDateTime && oPreviousEntity.DraftEntityLastChangeDateTime.getTime();
					if (iPreviousEntryLastChangeTime === iLastChangeTime) { // if it is exactly the same draft that we already know, just return the existing metadata 
					//Even if the drafts are same ,if the previous scenario was create then bIsCreate of previous context should be overwritten with the current createMode value 
						return extend(oPreviousEntry.oContextInfo, { "bIsCreate": bIsCreate });
					}
					bIsDraftModified = oActiveEntity.DraftEntityCreationDateTime.getTime() !== iLastChangeTime;
				}
				return {
					bIsDraft: bIsDraft,
			...
			function registerContext(oContext, iViewLevel, sEntitySet, aKeysFromIdentity) {
				...
				sEntitySet = sEntitySet || oDataModelHelper.analyseContext(oContext).entitySet;
			...
			function draftRemovalStarted(oContext, oRemovalPromise, bIsCancellation, sActionType) {
				// When the removal is successful, property oContext of the context info is set to be faulty.
				...
				var aEditableChildContextsPathes = []; // Collect paths to contexts which should be invalidated
				// if deletion of  record no need to add oRemovalPromise to other child contexts
				...
				oRemovalPromise.then(function (oResponse) {
					...
					// not accessible by this class, as they might have been retrieved via a navigation property.
					...
					// Note: The invalidation is postponed to the end of the busy session to ensure that no view is bound to the draft context
						...
						if (aEditableChildContextsPathes.length > 0){
							...
							for (var sContextKey in mContexts) {
								...
								if (oPotentialChildContext !== oContext){
									...
									if (bIsChildContext){
										// If the context is transient (inline creation rows), delete it.
										// Otherwise, invalidate the model entry which corresponds to the context
										if (oPotentialChildContext.isInactive && oPotentialChildContext.isInactive()) {
											oPotentialChildContext.delete();
										} else { 
			...
			// When an edit draft is activated we should invalidate the entries in the OData model cache which correspond to
			// items of the active version, since they will possibly change due to modifications done in the draft which would bypass the model cache.
			// Note that this only needs to be done for items (and subitems), since the entry of the main object will anyway be updated by the activate process itself.
			function fnInvalidateModelCacheForActiveVersion(oDraftRootContext){
				var oDraftRootContextData = getContextData(oDraftRootContext);
				var sActiveRootPath = oDraftRootContextData.sActiveContextPath;
				if (!sActiveRootPath){
					return; // no active entries there for create drafts
				}
				var oModel = oDraftRootContext.getModel();
				var oActiveRootInfo = oDataModelHelper.analyseContextPath(sActiveRootPath, oModel);
				var sActiveRootEntitySet = oActiveRootInfo.entitySet;
				var sActiveRootKey = oActiveRootInfo.key;
				for (var sPath in mPath2ContextData){ // loop over all contexts that are potentially affected
					var oContextData = mPath2ContextData[sPath];
					if (sPath !== sActiveRootPath && oContextData.oRootContextInfo && oContextData.oContextInfo.bIsDraftSupported && !oContextData.oContextInfo.bIsDraft){
						var sRootContextKey = oContextData.oRootContextInfo.key;
						var sRootEntitySet = oContextData.oRootContextInfo.treeNode.entitySet;
						if (sRootContextKey === sActiveRootKey && sRootEntitySet === sActiveRootEntitySet){
			...
			function activationStarted(oContext, oActivationPromise) {
				fnInvalidateModelCacheForActiveVersion(oContext);
			...
			function getAlternativeIdentityPromise(oIdentity){
				...
				return getAlternativeContextPromise(sMainPath).then(function(oAlternativeContext){
					...
					var fnHandleTargetNode = function(oTargetNode){
						...
						var aCandidateKeys = oIdentity.keys.slice(0, oCandidateNode.level + 1);
						var sCandidatePath = oCandidateNode.getPath(3, aCandidateKeys);
			...
			function getAlternativeContextPromise(sPath){
				...
				if (!oContextData.oContext && !oContextData.oRemovalPromise){ // entity identified by sPath has been deleted meanwhile without replacement
					return Promise.resolve({ });
				}
				return new Promise(function (fnResolve) {
					...
					var fnHandleEditingPromise = function (oEditingPromise) { // function to be called when there is an EditingPromise for the object to be displayed
						oEditingPromise.then(function (oEditingInfo) { // oEditingInfo contains the context for the draft that currently replaces the object
							...
							//    Therefore, we check for the corresponding entry of the draft. If this entry exists, but no context is available anymore
					...
					if (oContextData.oRemovalPromise){ // sPath describes a draft for which an activation/cancellation has been started
						oContextData.oRemovalPromise.then(function(oActiveContext){ // activation was successful
					...
					} else if (!oContextData.oContextInfo.bIsDraft && oContextData.oContextInfo.bOwnDraftExists) { // sPath describes an active object for which a draft is being created (and not suppressed)
						if (oContextData.oEditingPromise) {
						...
						if (oHistoricIdentity && oNewIdentity && oHistoricIdentity.treeNode.entitySet === oNewIdentity.treeNode.entitySet) {
							...
							if (oNewIdentityContext && (!oNewIdentityContextInfo.bIsDraft || oNewIdentityContextInfo.bIsCreate)) {
								...
								oSiblingPromise.then(function(oSiblingContext) {
									...
									fnResolve(false); // if sibling promise failed resolve to false, temporary solution.
								});
							}
						} else {
							fnResolve(false);
			...
			return {
				...
				getDraftInfoForContext: fnCreateDraftInfo
			};
		}
