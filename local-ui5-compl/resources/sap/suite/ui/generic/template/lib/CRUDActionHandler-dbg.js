 ...
 */
 ...
 "sap/suite/ui/generic/template/genericUtilities/controlHelper", "sap/ui/core/message/MessageType"
	...
	function getMethods(oTemplateContract, oController, oCommonUtils, prepareForMessageHandling) {
		...
		function getConfiguredPopoverIfNeeded(iSituation, sCRUDAction, mParameters){
			...
			var oRet, oLocalModel;
			return oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.MessageInfluencingCRUDAction", {
				onMessageSelect: function(){
					var oFirstFocusableButton = oRet.getButtons().find(function(oButton){
						return oButton.isFocusable();
					});
					controlHelper.focusUI5Control(oFirstFocusableButton);
				...
				titlePressed: function(oEvent){ // the user wants to navigate from the message to the corresponding control
					...
					if (fnAfterClose){
						oLocalModel.setProperty("/afterClose", fnAfterClose);
					}
				},
				afterClose: function(){
					fnYes = null;
					fnNo = null;
					oLocalModel.setProperty("/messageToGroupName", Object.create(null));
					oMessageView.navigateBack();
					oMessageView.setModel(null, "msg");
					var fnAfterClose = oLocalModel.getProperty("/afterClose");
					fnAfterClose();
				},
				getSeverityIconFromIconPool: MessageUtils.getSeverityIconFromIconPool,
				getIconColor: MessageUtils.getIconColor
				...
				var oItemBinding = oMessageView.getBinding("items");
				if (!aMessages) {
					var aFilters = [];
					var aActiveComponents = oTemplateContract.oNavigationControllerProxy.getActiveComponents();
				...
				return prepareForMessageHandling(aMsg, oItemBinding, oLocalModel).then(function(){
					return oRet;
				});
			});
		}
	...
	return BaseObject.extend("sap.suite.ui.generic.template.lib.CRUDActionHandler", {
		constructor: function(oTemplateContract, oController, oCommonUtils, prepareForMessageHandling) {
			extend(this, getMethods(oTemplateContract, oController, oCommonUtils, prepareForMessageHandling));
		}
	});
});
