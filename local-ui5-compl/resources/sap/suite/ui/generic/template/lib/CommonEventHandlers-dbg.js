sap.ui.define(["sap/ui/base/Object",
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getSmartAggregationControlFromEvent(oEvent){
			return oCommonUtils.getOwnerControl(oEvent.getSource(), true);
		}
		...
		function getPaginatorInfo(oTable, oRow, oState){
			...
			if (oRet){
				// if this function was called from onListNavigate which itself was triggered from an fnPaginate (see below), then we should
				// just reuse the existing paginator info. Note that in this case selectedRelativeIndex was already updated by
				// function fnHandleNavigateToObject in PaginatorButtonsHelper.
				oCurrentPaginatorInfo = null;
				return oRet;
			}
			...
			var fnPaginate = function(oContext, oViewProxy, bIsEditItem){
				var fnPaginateImpl = function(oTargetContext){
					var sPath = oTargetContext.getPath();
					...
					if (!oSourceItem){
						...
						oVirtualItem.setBindingContext(oTargetContext);
				...
				onDelete: function(oEvent) {
					...
					oDeletePromise.then(
						function(aFailedPath) {
							...
							var bWarningOccurred = aFailedPath.some(function(oFailedPathInfo){
				...
				Promise.all(aPromise).then(function(aResponses) {
					...
					if (mJSONData.itemsCount > 1) {
						...
						if (mJSONData.lockedItemsCount === mJSONData.itemsCount) {
							...
							// 2nd part of message
							// if item is locked and is undeletable active object then exclude from further calculation as their counts are already calculated for locked items
					...
					// Unsavedchanges checkbox is required if from the total number of items to be deleted there is at least one active object having unsavedchanges from others and one deletable object without others unsaved changes
		...
		*/
		function fnGetSelectedItemContextForDeleteMessage(oSmartTable, oSelectedItem, isObjectPage) {
			var oPresentationControlHandler = oServices.oPresentationControlHandlerFactory.getPresentationControlHandler(oSmartTable);
			var oTitleInfo = oPresentationControlHandler.getTitleInfoForItem(oSelectedItem);
			return getDeleteText(oTitleInfo.title, oTitleInfo.subTitle, isObjectPage, false, oSmartTable);
		}

		function onTeamsCollabOptionPress(oEvent) {
			var aOptions = oComponentUtils.getParameterModelForTemplating().oData.templateSpecific.teamsCollabOptions;
			var sType = oEvent.getSource().data("type");
			var oSelectedOption = aOptions.find(function(option) {
		...
		function getObjectForNavigationMergedWithOutboundAndPageContext(oPageContext, oOutbound, sFilterBarSelectionVariant, oLineContextObject) {
			...
			// Filter bar selection variant should have higher priority than empty outbound parameters.
			// As mixAttributesAndSelectionVariant gives higher priority to the first calling parameter, we remove the properties that already exists in the selection variant from the empty parameters object
		...
		function fnNavigateIntent(oOutbound, aContexts, oSmartFilterBar, bDisplayOrCreateNavigation) {
			...
			oServices.oApplication.navigateExternal(oOutbound.semanticObject, oOutbound.action, oSelectionVariant.toJSONString(), null, oCommonUtils.handleError);
		}
		...
		function fnHideTitleArea(oSmLiContent,aContactTitleArea) {
			...
			if (sIcon === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
				sIcon = undefined;
			}
			var oTitle = oSmLiContent.byId("title");
			var sTitle = oTitle && oTitle.getText();//oTitle must always be there
			if (sTitle === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
				sTitle = undefined;
			}
			...
			if (sDescription === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
			...
			for (var j = 0; j < aContactTitleArea.length; j++) {
				...
				if (sContactTitleAreaIcon === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
					sContactTitleAreaIcon = undefined;
				}
				var sContactTitleAreaTitle = oContactTitleAreaIdTitle && oContactTitleAreaIdTitle.getText();
				if (sContactTitleAreaTitle === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
				...
				if (sContactTitleAreaDescription === ""){ // can be undefined - to make a later comparison easier set it to undefined if the value is ""
		...
		function fnOnSemanticObjectLinkNavigationTargetObtained(sEntitySet, mFieldSemanticObjects, oEventParameters, oState, sTitle) {
			var sSourceClickedField, oControl, sControlBindingContextPath;
			var oMetaModel = oController.getOwnerComponent().getModel().getMetaModel();
			...
			var getTargetAnnotation = function() {
				var getClickedControl = function(){
					...
					//table fields get an absolute id "__link0-__clone34" - then jQuery is used to retrieve this absolute id (jQuery doesn't work with "::" as in ::Field-sl)
					if (!oControl ){
			...
			var renderQuickViewFragment = function(aTeamsCollabOptions) {
		...
		function fnRenderTeamsCollabOptionsOnContactCard(oEntityType, oEventParameters, aTeamsCollabOptions, oControl, sControlBindingContextPath) {
			...
			if (aTeamsCollabOptions && aTeamsCollabOptions.length && oControl.mProperties.contactAnnotationPath && oContactCardForm && oContactCardForm.length) {
				...
				var oContactAnnotation = oNavigationEntityType["com.sap.vocabularies.Communication.v1.Contact"];
				var sEmailPath = oContactAnnotation && oContactAnnotation.email[0].address && oContactAnnotation.email[0].address.Path;
				// the complete path is obtained by combining the parent entity with the target navigation property instead of constructing a simple path
				// starting from the navigation context. This is done because the navigation popover which brings up the contact card has the same binding path
				var sEmailContextPath = sControlBindingContextPath + '/' + oControl.mProperties.contactAnnotationPath + '/' + sEmailPath;
				var oButtonPressHandler = function(callBackHandler, oEvent) {
				var sEmailValue = oModel.getProperty(sEmailContextPath);
					oEvent.getSource().addCustomData(new CustomData({
						key: "email",
						value: sEmailValue
					}));
				...
				oContactCardForm[0].insertContent(oVbox, 1);
			}
		}
				...
				if (oCrossApp) {
					...
					if (!oTablePathProp) {
						...
						oXApplNavigation.then(function(oCrossAppNavigator){
							var oSupportedPromise = oCrossAppNavigator.isNavigationSupported([oNavArguments], appComponent);
							oSupportedPromise.then(function(oTargets){
		...
		*/
		...
		function onListNavigateImpl(oEventSource, oEventParameters, oState, oBindingContext, bReplace, oViewProxy){
			// Unfortunately, the interface of onListNavigationExtension expects an instance of Event.
			// As this method may be called asynchronously (e.g. because of the dataloss popup) the original event cannot be used.
			// Moreover, there are scenarios in which the original event could not be used (e.g. when using the paginator buttons).
			// Hence the original event if faked for the extension in a way, that the event in the extension is like the extension developer expects
			...
			var oTable = getSmartAggregationControlFromEvent(oEvent);
		 ...
		 */
		function onListNavigate(oEventOrSource, oState, oBindingContext, bIsProgrammatic, oViewProxy) {
			// method is called in different ways:
			// - UI table: source of event is context to navigate to
			// - responsive Table: itemPress event of table must be used, context to navigate to is provided in parameter listItem
			// - programmatic navigation: no event, but an item (ColumnListItem?) is provided
			// - chart: navigation target provided in oBindingContext, Extension not provided???
			// event source contains also information in custom data, if navigation is overridden by external navigation
			var oEventSource, oEventParameters = {};
			/* When this method is called from LR/OP's onListNavigate event handler, a copy of the original event is passed here, which
			   would not be of Event object type and thus, we need to have a check on its metadata's name to check whether the passed object
			   is of Event type instead of checking it through 'X instanceOf Event'. */
			if (oEventOrSource.getMetadata().getName() === "sap.ui.base.Event"){
			...
			if (bIsProgrammatic){
				onListNavigateImpl(oEventSource, oEventParameters, oState, oBindingContext, true, oViewProxy);
			} else {
		...
		function onDataFieldWithNavigationPath(oEvent) {
			...
			var aKeys = oCommonUtils.getNavigationKeyProperties(sTargetEntitySet),
				oNavigationController = oServices.oNavigationController, oUrlParameters = {};

			var oDraftContext = oServices.oDraftController.getDraftContext();
			if (oDraftContext.isDraftEnabled(sTargetEntitySet)) {
				oUrlParameters = {
		 ...
		 * @param {object} oSmartFilterbar additional smart filter bar object
		 */
		...
		function handleUploadComplete(oEvent){
			...
			if (iStatus >= 400) {
				...
				var oFileUploaderSettings;
				var sURL = oFileUploader.getUploadUrl();

				oFileUploaderSettings = {};
				if (sNavigationProperty) {
					oPrivModel.setProperty("/generic/controlProperties/" + "fileUploader/" + sNavigationProperty , oFileUploaderSettings);
				} else {
				...
				oFileUploaderSettings.icon = bIcon;
				oFileUploaderSettings.url = sURL;
				if (sNavigationProperty) {
					oPrivModel.setProperty("/generic/controlProperties/" + "fileUploader/" + sNavigationProperty , oFileUploaderSettings);
				} else {
					oPrivModel.setProperty("/generic/controlProperties/" + "fileUploader/" + sEntitySet, oFileUploaderSettings);
		...
		function onCallActionFromToolBar(oEvent, oState) {
			...
			var oControl = getSmartAggregationControlFromEvent(oEvent);
		...
		function onCallActionFromToolBarForContexts(oSmartControl, oSourceControl, oCustomData, aContexts, oState, sEntitySet, bIsSmartTable) {
			...
			if (oCustomData.ActionFor && mJSONData.inApplicableCount > 0 && (bIsSmartTable && ((oSourceControl.getMode && oSourceControl.getMode() === "MultiSelect") ||
				...
				getActionDialog(oSmartControl).then(function (oActionDialog) {
		...
		function getActionDialog(oControl){
			return oCommonUtils.getDialogFragmentAsync("sap.suite.ui.generic.template.fragments.ActionConfirmation", {
				...
				onContinue: function(oEvent){
					var oDialog = oEvent.getSource().getParent(),
						jsonActionData = oDialog.getModel("Action").getProperty("/");
					...
					if (aResponses) {
						aResponses.some(function(oResponse){
							...
							if (oResponse.response && oResponse.context && (!oResponse.actionContext || sResponsePath !== oResponse.actionContext.getPath())) {
								oServices.oViewDependencyHelper.setMeToDirty(oController.getOwnerComponent(), sEntitySet, false, sResponsePath === "/undefined" || (mParams.contexts && mParams.contexts.length > 1),fnRefreshCallback);
								return true;
							}
						});
					}
		...
		function addEntry(oEventSource, bSuppressNavigation, oSmartFilterBar, oPredefinedValues, bIgnoreTableRefresh, bCreateWithDialogEnabled) {
			var oTable = oCommonUtils.getOwnerControl(oEventSource, true);
			var sEntitySet = oTable.getEntitySet();
			var oComponent = oController.getOwnerComponent();
			var oBusyHelper = oServices.oApplication.getBusyHelper();
			// intent based navigation
		...
		/**
		 *  Event handler for smart multiInput fields. This method can also be called for non-draft cases programmatically.
		...
		function submitChangesForSmartMultiInput(oEvent) {
			var oParameter = {
				...
				pendingChanges: true,
				forceSubmit: true
			}, oSmartMultiInputField, bIsProgrammatic = true;

			if (oEvent instanceof Event) {
				bIsProgrammatic = false;
				oSmartMultiInputField = oEvent.getSource();
			}
			if (oComponentUtils.isDraftEnabled() && !bIsProgrammatic) {
				// Side effects for multi input fields can be annotated under SourceEntities as it maintains 1..n association
				var oView = oController.getView();
				var oContext = oEvent.getSource().getBindingContext();
				var sMultiInputPropertyPath = oSmartMultiInputField.getDataProperty && oSmartMultiInputField.getDataProperty().typePath;
				// Smart Field does not have a public API to return the fieldgroup Ids
				var aInnerControls = oSmartMultiInputField.getInnerControls();
				var aFieldGroupIds = aInnerControls && aInnerControls[0] && aInnerControls[0].getFieldGroupIds && aInnerControls[0].getFieldGroupIds() || [];
				// For each fieldGroup ID register the side effect if the property is annotated in source properties
				for (var i = 0 ; i < aFieldGroupIds.length ; i++) {
					var oSmartFieldInfo = oView.data(aFieldGroupIds[i]);
					var bSourcePropertyExists = false;
					var oSideEffects = oServices.oApplicationController._getSideEffect(oSmartFieldInfo);
					var aSourceProperties = oSideEffects && oSideEffects.SourceProperties || [];

					for (var j = 0 ; j < aSourceProperties.length ; j++) {
						if (aSourceProperties[j].PropertyPath === sMultiInputPropertyPath) {
							bSourcePropertyExists = true;
						}
					}

					if (bSourcePropertyExists) {
						var sSideEffectQualifier = oServices.oApplicationController._getSideEffectsQualifier(oSmartFieldInfo.name);
						oServices.oApplicationController.registerGroupChange(sSideEffectQualifier);
					}
				}
		...
		function deleteEntry(oEvent) {
			var oSmartTable = getSmartAggregationControlFromEvent(oEvent);
			var oListItem = oEvent.getParameter("listItem");
			var oListItemContext = oListItem.getBindingContext();
			deleteContextsFromTable(oSmartTable, [oListItemContext]);
		}
		...
		function deleteEntries(oEvent) {
			...
			deleteContextsFromTable(oSmartTable, aSelectedContexts);
		}
		 ...
		 * @public
		 */
		function deleteContextsFromTable(oSmartTable, aSelectedItems) {
		...
		function fnFetchTeamsOptions() {
			var aTeamsCollabOptions = oComponentUtils.getParameterModelForTemplating().oData.templateSpecific.teamsCollabOptions;
			if (!aTeamsCollabOptions) {
				return ServiceContainer.getServiceAsync().then(function (oTeamsHelper) {
					if (typeof oTeamsHelper.isContactsCollaborationSupported === 'function' && oTeamsHelper.isContactsCollaborationSupported()) {
						return oTeamsHelper.getTeamsContactCollabOptions();
					} else {
						return Promise.resolve(aTeamsCollabOptions);
					}
				}).then(function (aTeamsCollabOptions) {
					oComponentUtils.getParameterModelForTemplating().setProperty("/templateSpecific/teamsCollabOptions", aTeamsCollabOptions);
					return Promise.resolve(aTeamsCollabOptions);
				});
			} else {
				return Promise.resolve(aTeamsCollabOptions);
		...
		function onDataFieldWithEmail(oEvent, sEmail) {
			var sEmail = oEvent.getSource().data("email");
			fnRenderTeamsContactCollabOptions(oEvent.getSource(), sEmail);
		}

		function fnRenderTeamsContactCollabOptions(oEvent, sEmail){
			...
			.catch(function () {
				mLibrary.URLHelper.triggerEmail(sEmail);
			});
		}
		...
		function fnHandleSideEffectForField(oEvent, bIsSideEffectTypeComputed) {
			SideEffectUtil.handleSideEffectForField(oEvent, oComponentUtils, bIsSideEffectTypeComputed);
		}
		function onSmartFieldModeToggled(oSmartField, sEntitySet) {
			var oOwnerComponent = oController.getOwnerComponent();
			SideEffectUtil.assignFieldGroupIds(oSmartField, oOwnerComponent.getModel().getMetaModel(), sEntitySet || oOwnerComponent.getEntitySet());
		}
		...
		/**
			...
			* @param sObjectSubtitle - Value of the description property
		...
		*/
		function getDeleteText(sObjectTitle, sObjectSubtitle, isObjectPage, isSubObjectPage, oSmartTable){
			var oTemplateComponent = oController.getOwnerComponent();
			var oAppComponent = oTemplateComponent.getAppComponent();
			var bIsFullScreen;
			if (oAppComponent.getFlexibleColumnLayout()){
				var oTemplatePrivateGlobalModel = oTemplateComponent.getModel("_templPrivGlobal");
				bIsFullScreen = oTemplatePrivateGlobalModel.getProperty("/generic/FCL/isVisuallyFullScreen");
			} else {
				bIsFullScreen = true;
			}
			var bIsItem = isObjectPage ? oSmartTable : isSubObjectPage;
			var sI18NKey, aParams;
			if (sObjectTitle){
				if (sObjectSubtitle && !bIsFullScreen){
					aParams = [" ", sObjectTitle, sObjectSubtitle]; // for compatibility reasons the first parameter must be an empty string in this case
					sI18NKey = bIsItem ? "DELETE_SELECTED_ITEM_WITH_OBJECTINFO" : "DELETE_WITH_OBJECTINFO";
				} else {
					aParams = [sObjectTitle];
					if (sObjectSubtitle){
						aParams.push(sObjectSubtitle);
					}
					sI18NKey = bIsItem ? "DELETE_SELECTED_ITEM_WITH_OBJECTTITLE" : "DELETE_WITH_OBJECTTITLE";
				}
			} else {
				sI18NKey = bIsItem ? "DELETE_SELECTED_ITEM" : "ST_GENERIC_DELETE_SELECTED";
			}
		...
		function fnOnBeforeExport (oExportEvent) {
			...
			if (bIncludeFilterSettings) {
				...
				// Include the entity set parameters
				oCommonUtils.includeEntitySetParametersToExportedFile(oSmartTable, aFilterSettings);
			}
			// Pass the original filters (aFilterSettings) to extension method when "Include filter settings" is selected.
		...
		return {
			...
			onDataFieldWithEmail: onDataFieldWithEmail,
			onInlineDataFieldForAction: onInlineDataFieldForAction,
			onMultiSelectionChange: onMultiSelectionChange,
			onCallActionFromToolBar: onCallActionFromToolBar,
			onCallActionFromToolBarForContexts: onCallActionFromToolBarForContexts,
			addEntry: addEntry,
			deleteEntry: deleteEntry,
			deleteEntries: deleteEntries,
			deleteContextsFromTable: deleteContextsFromTable,
