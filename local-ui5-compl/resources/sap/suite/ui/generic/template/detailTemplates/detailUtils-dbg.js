	...
	function(FeLogger, UIComponent, HashChanger, Filter, FilterOperator, controlHelper, NavigationController, MessageButtonHelper, testableHelper, DiscardEditHandler, PaginatorButtonsHelper, DraftTransactionController, NonDraftTransactionController, MessageType) {
		...
		function getControllerBase(oViewProxy, oTemplateUtils, oController){
			...
			function fnApplyAndUpImpl(oControl) {
				...
				var oUIModel = oController.getView().getModel("ui");
				var oApplyPromise = oTemplateUtils.oServices.oApplicationController.synchronizeDraftAsync().then(function() {
					fnNavigateUp();
					...
					oTemplateUtils.oCommonUtils.processDataLossTechnicalErrorConfirmation(function() {
						...
						}, Function.prototype, oControllerBase.state, "LeavePage");
					});
				});
			...
			oControllerBase = {
				onInit: function(oRequiredControls, oMessageButtonHost){
					...
					oViewProxy.getScrollFunction = function(aControlIds){
						var sControlId = oTemplateUtils.oCommonUtils.getPositionableControlId(aControlIds, true);
						if (sControlId){
							return function(){
								(oViewProxy.prepareForControlNavigation || Function.prototype)(sControlId);
								controlHelper.focusControl(sControlId);
							};
						}
					};
				},
