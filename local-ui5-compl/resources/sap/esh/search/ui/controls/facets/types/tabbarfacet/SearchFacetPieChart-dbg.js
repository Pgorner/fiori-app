...
(function(){
sap.ui.define(["../../../../i18n", "sap/ui/thirdparty/d3", "sap/ui/core/Control", "../../FacetTypeUI", "sap/ui/core/Element"], function (__i18n, thirdpartyD3, Control, ____FacetTypeUI, Element) {
  "use strict";
   ...
   */
  const SearchFacetPieChart = Control.extend("sap.esh.search.ui.controls.SearchFacetPieChart", {
    renderer: {
      apiVersion: 2,
      render(oRm, oControl) {
        // outer div
    ...
    getFacetIndex: function _getFacetIndex() {
    ...
    getFacetIndexByIdForLargePieChart: function _getFacetIndexByIdForLargePieChart(chartId) {
      let index = -1;
      const tabStrip = $("#" + chartId).parent().parent()[0];
      const tabStripId = tabStrip.id; // e.g. "__container6"
      const ar = $(".searchFacetLargeChartContainer");
    ...
    getDataForPieChart: function _getDataForPieChart(data, model, facetIndex) {
      ...
      let overallTotal = searchResultTotal; //temporary equation with search result total
      let pieChartTotal = 0;
      this.iMissingCnt = 0;
      for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].facetType === FacetTypeUI.Attribute) {
          ...
          for (let j = 0; j < data[i].items.length; j++) {
            ...
            if (itemValue) {
              ...
              // ToDo consider if only show top 5 how will user see there is a filter in place? on list view of facet the filter condition is listed sso leave it here too for now
          ...
          if (percMissing > 0) {
            ...
            const sizeOfWedge = 18 * pieChartTotal / 350; // 18 is 5% of 360Â°
    ...
    isFacetPanel: function _isFacetPanel() {
      let element = this.getDomRef();
      while (element) {
        if (element.classList.contains("sapUshellSearchFacetTabBar")) {
          return true;
        }
    ...
    onAfterRendering: function _onAfterRendering() {
      ...
      const that = this;
      let data, chartElements, chart, model, piechartParent, facetIndex;
      const application = null;
      const settings = {};
      ...
      if (model) {
        ...
        if (this.isFacetPanel()) {
          ...
          this.getDomRef().setAttribute("data-test-id-facet-dimension-value", `${selectedFacetItem.title}-${selectedFacetItem.dimension}`);
          ...
          const oInfoLine = Element.getElementById(infoZeile.id); // ToDo 'any cast'
  ...
  class PieChart {
    ...
    tweenGenText;
    arcSequence;
    constructor(parentSelector, settings, application, model) {
      this.init(parentSelector, settings, application, model);
    }
    init(parentSelector, settings, application, model) {
      this.parent = parentSelector;
      this.application = application;
      this.model = model;
      this.chartElements = [];
      const parent = SearchFacetPieChart.d3.select(parentSelector);
      let svgHeight = $(parentSelector).innerHeight();
      const svgWidth = $(parentSelector).innerWidth();
      ...
      this.svg = parent.append("svg:svg").attr("width", svgWidth).attr("height", svgHeight).attr("id", parentSelector.id + "_svg").append("svg:g").attr("transform", "translate(" + xOrigin + "," + yOrigin + ")");
    ...
    static Tweens = function () {
      ...
      Tweens1.tweenGenSimpleText = function (d, i, a, innerRadius, outerRadius, chartInstance, filtered) {
        ...
        const labelpositionBefore = new Labelposition(chartInstance.options.width, chartInstance.options.height, d.oldArc.startAngle, d.oldArc.endAngle, chartInstance.options.outerRadius, d.labelElement.childNodes[1].getBBox().width, d.labelElement.childNodes[1].getBBox().height, chartInstance.svgArcGen(d).centroid(d), labelText, chartInstance.svg);
        labelpositionBefore.update();
        const coordinationBefore = [labelpositionBefore.x, labelpositionBefore.y];
        const labelpositionAfter = new Labelposition(chartInstance.options.width, chartInstance.options.height, d.startAngle, d.endAngle, chartInstance.options.outerRadius, d.labelElement.childNodes[1].getBBox().width, d.labelElement.childNodes[1].getBBox().height, chartInstance.svgArcGen(d).centroid(d), labelText, chartInstance.svg);
    ...
    // ======================================================================
    createAttributeService(cls, attribute, initFunction) {
    ...
    notAnimatedUpdate(data) {
      ...
      // create new arcs
      sel.enter().append("svg:path").attr("data-label", d => d.data.label).attr("transform", function (d) {
        let translateStr;
        // translate all segments that.options.padding4arcs in outer direction
        // cons: no longer a mathematic circle and padding not a constant
        // translateStr = PieChart.translateStr4Padding(d.startAngle,d.endAngle,that.options.padding4arcs,xOldOrig,yOldOrig);
        ...
        SearchFacetPieChart.d3.select(this).transition().duration(1000).ease(SearchFacetPieChart.d3.ease("elastic")).attr("transform", translateStr).style("stroke", d => {
          let strokeColor;
          if (d.data.stroke === "none") {
            strokeColor = d.data.stroke;
      ...
      if (!sel.empty()) {
        sel.style("opacity", function (d) {
          ...
          if (this.childNodes[1] && this.childNodes[1].childNodes[0]) {
            labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, this.getBoundingClientRect().height, that.svgArcGen(d).centroid(d), d.data.label, that.svg);
            // log.debug("non animation change/remove existing labels, text exists: ", this.childNodes[1].childNodes[0].data);
          } else {
            labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, this.getBoundingClientRect().height, that.svgArcGen(d).centroid(d), " ", that.svg);
      ...
      if (!sel.enter().empty()) {
        ...
        textsG.style("opacity", function (d) {
          ...
          const labelposition = new Labelposition(that.options.width, that.options.height, d.startAngle, d.endAngle, that.options.outerRadius, this.getBoundingClientRect().width, this.getBoundingClientRect().height, that.svgArcGen(d).centroid(d), d.data.label, that.svg);
          labelposition.update();
          // in case that label shall be cut
          if (labelposition.labelWidth < this.childNodes[1].getBoundingClientRect().width) {
            that.adjustLabelWidth(this, labelposition.labelWidth);
    ...
    svgArcGen(...args) {
      // eslint-disable-next-line @typescript-eslint/no-this-alias
      const that = this;
      const a = SearchFacetPieChart.d3.svg.arc().innerRadius(function () {
    ...
    // ======================================================================
    static generateHistoricalArcCalculator(pieChart, ...args) {
  ...
  class Labelposition {
    ...
    translateStr;
    debug;
    constructor(backgroundWidth, backgroundHeight, startAngle, endAngle, r, labelWidth, labelHeight, d3centroid, text, svgBackground) {
    ...
    update() {
      ...
      let outset = Math.max(this.outsetMin, Math.sin(this.apexAngle / 2) / (this.apexAngle / 2) * 2 / 3); // Outset for centroid (Schwerpunkt) of segment
