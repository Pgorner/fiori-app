...
sap.ui.define(["sap/base/Log", "sap/fe/base/BindingToolkit", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/converters/controls/Common/Action", "sap/fe/core/converters/helpers/ConfigurableObject", "sap/fe/core/converters/helpers/IssueManager", "sap/fe/core/converters/helpers/Key", "sap/fe/core/helpers/BindingHelper", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/StableIdHelper", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/templating/DataModelPathHelper", "sap/fe/core/templating/DisplayModeFormatter", "sap/fe/core/templating/EntitySetHelper", "sap/fe/core/templating/FieldControlHelper", "sap/fe/core/templating/PropertyHelper", "sap/fe/core/templating/UIFormatters", "sap/fe/core/templating/CriticalityFormatters", "sap/fe/macros/formatters/TableFormatter", "sap/fe/macros/internal/helpers/ActionHelper", "sap/ui/core/Lib", "sap/ui/core/message/MessageType", "../../ManifestSettings", "../../helpers/Aggregation", "../../helpers/DataFieldHelper", "../../helpers/ID", "./Criticality", "./table/StandardActions"], function (Log, BindingToolkit, DataField, Action, ConfigurableObject, IssueManager, Key, BindingHelper, ModelHelper, StableIdHelper, TypeGuards, DataModelPathHelper, DisplayModeFormatter, EntitySetHelper, FieldControlHelper, PropertyHelper, UIFormatters, CriticalityFormatters, tableFormatters, ActionHelper, Library, MessageType, ManifestSettings, Aggregation, DataFieldHelper, ID, Criticality, StandardActions) {
  ...
  var ColumnType = /*#__PURE__*/function (ColumnType) {
    ...
    return ColumnType;
  }(ColumnType || {}); // Custom Column from Manifest
  ...
  function getTableActions(lineItemAnnotation, visualizationPath, converterContext, standardActions, navigationSettings) {
    ...
    standardActions.cut, standardActions.paste, standardActions.moveUp, standardActions.moveDown].filter(action => action !== undefined);
  ...
  function createRequestedProperties(lineItem, converterContext, operationAvailable, presentationVariantAnnotation) {
    ...
    if (capabilitiesAnnotation) {
      pushField(capabilitiesAnnotation.DeleteRestrictions?.Deletable?.path);
      pushField(capabilitiesAnnotation.UpdateRestrictions?.Updatable?.path);
    }
    ...
    entityType.entityProperties.forEach(property => {
      ...
      if (getDisplayMode(property) === "Description") {
        ...
        if (isPathAnnotationExpression(property?.annotations?.Common?.Text)) {
          tableColumns.push(getColumnDefinitionFromProperty(property, converterContext.getEntitySetBasedAnnotationPath(property.fullyQualifiedName), property.name, false, false, aggregationHelper, converterContext, displayModeOfDescriptionPropertiesMap, restrictionsOnProperties, tableCreationMode));
        }
      }
    });
    // Create a propertyInfo for each related property.
    const relatedColumns = _createRelatedColumns(columnsToBeCreated, tableColumns, converterContext, entityType, displayModeOfDescriptionPropertiesMap, tableCreationMode, restrictionsOnProperties);
  ...
  const getColumnDefinitionFromProperty = function (property, fullPropertyPath, relativePath, useDataFieldPrefix, availableForAdaptation, aggregationHelper, converterContext, displayModeOfDescriptionPropertiesMap, restrictionsOnProperties, tableCreationMode, relatedPropertiesInfo, relativePathForMultiValue) {
    ...
    const availability = !availableForAdaptation || isHidden || displayModeOfDescriptionPropertiesMap[relativePath] ? "Hidden" : "Adaptation";
    ...
    if (aggregationHelper.isAnalyticsSupported() && ModelHelper.isObjectPathDraftSupported(converterContext.getDataModelObjectPath()) && (relativePath === "HasActiveEntity" || relativePath === "HasDraftEntity")) {
  ...
  const getColumnsFromAnnotations = function (lineItemAnnotation, visualizationPath, converterContext) {
    ...
    if (lineItemAnnotation) {
      ...
      lineItemAnnotation.forEach(dataField => {
        ...
        const sLabel = getLabel(dataField, isGroup);
  ...
  function _updatePropertyInfosWithAggregatesDefinitions(tableVisualization) {
    ...
    tableVisualization.columns.forEach(column => {
      column = column;
      const aggregatablePropertyName = Object.keys(tableVisualization.analyticsExtensions).find(aggregate => aggregate === column.name);
  ...
  function getFieldList(fields, converterContext) {
    ...
    if (fields.startsWith("com.sap.vocabularies.UI.v1.FieldGroup")) {
  ...
  function computeHiddenOnRelatedColumns(existingColumns, name, column) {
    const relatedAnnotationColumns = existingColumns.filter(existingColumn => existingColumn.propertyInfos?.includes(name) && existingColumn.isPartOfLineItem);
