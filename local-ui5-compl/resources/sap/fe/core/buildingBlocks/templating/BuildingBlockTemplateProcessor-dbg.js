    ...
    try {
      ...
      if (oParent) {
        ...
        if (oMetadata.fragment) {
          ...
          let templateString;
          if (oInstance.getRuntimeBuildingBlockTemplate) {
            templateString = await jsx.renderAsXML(async () => oInstance.getRuntimeBuildingBlockTemplate(oNode));
          } else {
          ...
          if (templateString) {
            ...
            if (hasParseError) {
              ...
              parsedTemplate = await processXmlInTrace(async () => {
                let initialTemplate;
                if (oInstance.getRuntimeBuildingBlockTemplate) {
                  initialTemplate = await jsx.renderAsXML(async () => oInstance.getRuntimeBuildingBlockTemplate(oNode));
                } else {
