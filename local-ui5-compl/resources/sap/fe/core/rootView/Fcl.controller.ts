...
class FclController extends BaseController {
		...
		return new Promise<boolean>(async (resolve, reject) => {
			try {
				...
				const illustratedMessage = new IllustratedMessage({
					...
					illustrationType: parameters.errorType ? `sapIllus-${parameters.errorType}` : "sapIllus-ErrorScreen"
				});
	...
	async _setShellMenuTitle(oAppComponent: AppComponent, sTitle: string, sAppTitle: string): Promise<void> {
		if (this.getHelper().getCurrentUIState().isFullScreen !== true) {
			await oAppComponent.getShellServices().setTitle(sAppTitle);
		} else {
	 ...
	 */
	private async getStoredLayout(proposedLayout?: LayoutTypeType): Promise<LayoutTypeType | undefined> {
