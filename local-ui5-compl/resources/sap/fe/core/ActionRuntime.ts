...
import { compileConstant, compileExpression, constant, equal, isConstant, transformRecursively } from "sap/fe/base/BindingToolkit";
...
const ActionRuntime = {
	 ...
	 */
	getIsActionCritical: async function (metaModel: ODataMetaModel, path: string, contexts: Context[] = []): Promise<boolean> {
		// default is true.
		...
		if (isConstant(isActionCriticalBindingExp)) {
			...
			isActionCriticalValue = new Promise((resolve) => {
				try {
					// Creating Binding to resolve
					const anyObject = new Any({ anyBoolean: compileExpression(isActionCriticalBindingExp) });
					anyObject.setModel(contexts[0].getModel());

					// Attaching change events
					const booleanBinding = anyObject.getBinding("anyBoolean") as PropertyBinding;
					const fnChange = function (): void {
						const valueExt = booleanBinding.getExternalValue();
						anyObject.destroy();
						resolve(valueExt);
					};
					booleanBinding.attachEventOnce("change", fnChange);

					// setting binding context to trigger the binding resolution
					anyObject.setBindingContext(contexts[0]);
				} catch (err: unknown) {
					// error while creating or resolving the binding
					const message = err instanceof Error ? err.message : String(err);
					Log.error(`FE : Actions : Error while resolving IsActionCritical for ${path} : ${message}`);
					// On the safe side, we shall return action is critical if value couldn't be determined.
					resolve(true);
				}
			});
		}
