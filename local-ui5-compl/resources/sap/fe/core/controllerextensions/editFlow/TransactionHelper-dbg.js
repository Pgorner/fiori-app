...
sap.ui.define(["sap/base/Log", "sap/base/util/deepClone", "sap/fe/core/CommonUtils", "sap/fe/core/UIProvider", "sap/fe/core/controllerextensions/BusyLocker", "sap/fe/core/controllerextensions/editFlow/draft", "sap/fe/core/controllerextensions/editFlow/editFlowConstants", "sap/fe/core/controllerextensions/editFlow/operations", "sap/fe/core/controllerextensions/editFlow/sticky", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/converters/annotations/DataField", "sap/fe/core/helpers/DeleteHelper", "sap/fe/core/helpers/FPMHelper", "sap/fe/core/helpers/MetaModelFunction", "sap/fe/core/helpers/ModelHelper", "sap/fe/core/helpers/PromiseKeeper", "sap/fe/core/helpers/ToES6Promise", "sap/fe/core/helpers/TypeGuards", "sap/fe/core/library", "sap/m/Button", "sap/m/Dialog", "sap/m/MessageBox", "sap/m/MessageToast", "sap/m/Popover", "sap/m/Text", "sap/m/VBox", "sap/ui/core/Lib", "sap/ui/core/Messaging", "sap/ui/core/message/MessageType"], function (Log, deepClone, CommonUtils, UIProvider, BusyLocker, draft, UiModelConstants, operations, sticky, messageHandling, MetaModelConverter, DataField, deleteHelper, FPMHelper, MetaModelFunction, ModelHelper, PromiseKeeper, toES6Promise, TypeGuards, FELibrary, Button, Dialog, MessageBox, MessageToast, Popover, Text, VBox, Library, Messaging, MessageType) {
  ...
  let TransactionHelper = /*#__PURE__*/function () {
    ...
    _proto.createDocumentContext = async function createDocumentContext(mainListBinding, parameters) {
      ...
      return this.createContext(mainListBinding, parameters.data, {
    ...
    _proto.performCreationWithAction = async function performCreationWithAction(actionName, mainListBinding, parameters, appComponent, resourceModel, messageHandler) {
      ...
      const result = await this.callAction(actionName, {
        contexts: mainListBinding.getModel().bindList(headerContextPath).getHeaderContext(),
    ...
    _proto.saveDocument = async function saveDocument(context, appComponent, resourceModel, executeSideEffectsOnError, bindingsForSideEffects, messageHandler, isNewObject) {
      ...
      try {
        ...
        const entitySet = MetaModelConverter.getInvolvedDataModelObjects(metaContext).targetObject;
        ...
        if (!skipConfirmationMsg && !(messagesReceived.length === 1 && messagesReceived[0].getType() === MessageType.Success)) {
          ...
          MessageToast.show(isNewObject ? resourceModel.getText("C_TRANSACTION_HELPER_OBJECT_CREATED", undefined, entitySet.name) : resourceModel.getText("C_TRANSACTION_HELPER_OBJECT_SAVED", undefined, entitySet.name));
        }
     ...
     * @param mParameters.bObjectPage
     * @param mParameters.bIsCreateAction
    ...
    _proto.callAction = async function callAction(sActionName, mParameters, oView, appComponent, messageHandler, disableStrictHandling) {
      ...
      // ensure deepClone so the original SideEffects mapping is not modified (BCP: 2380117956)
      const mSideEffectsParameters = deepClone(appComponent.getSideEffectsService().getODataActionSideEffects(sName || sActionName, contextToProcess)) || {};
      const messageHandlingKey = messageHandler ? messageHandler.registerToHoldMessages() : undefined;
      try {
        let oResult;
        if (contextToProcess && mParameters.contexts) {
          oResult = await operations.callBoundAction(sActionName, mParameters.contexts, oModel, appComponent, {
            ...
            selectedItems: Array.isArray(mParameters.contexts) ? mParameters.contexts : [mParameters.contexts],
        ...
        if (messageHandler) {
          await this._handleActionResponse(messageHandler, mParameters, sActionName, messageHandlingKey);
        }
        return oResult;
      } catch (err) {
        if (messageHandler) {
     ...
     * @final
     */;
    _proto._handleActionResponse = async function _handleActionResponse(messageHandler, mParameters, sActionName, messageHandlingKey) {
      ...
      let control;
      if (mParameters.controlId && mParameters.parentControl && mParameters.parentControl.isA("sap.ui.core.mvc.View")) {
      ...
      return messageHandler.showMessages({
        ...
        control: control,
        unHoldKey: messageHandlingKey,
        aSelectedContexts: mParameters?.contexts
      });
    }
      ...
      const onExitDialog = async event => {
        ...
        try {
          ...
          messageHandler.showMessages({
            forceShowUIElement: createDialog.isOpen()
          });
        }
      };
