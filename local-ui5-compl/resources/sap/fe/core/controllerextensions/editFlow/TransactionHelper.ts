...
class TransactionHelper {
			...
			if (!skipConfirmationMsg && !(messagesReceived.length === 1 && messagesReceived[0].getType() === MessageType.Success)) {
				...
				MessageToast.show(
					isNewObject
						...
						: resourceModel.getText("C_TRANSACTION_HELPER_OBJECT_SAVED", undefined, entitySet.name)
				);
			}
	...
	async callAction(
		...
		const mSideEffectsParameters =
			deepClone(
				appComponent.getSideEffectsService().getODataActionSideEffects(sName || sActionName, contextToProcess as ODataV4Context)
			) || {};
		...
		try {
			...
			if (contextToProcess && mParameters.contexts) {
				oResult = await operations.callBoundAction(sActionName, mParameters.contexts as ODataV4Context[], oModel, appComponent, {
	...
	async _handleActionResponse(
		...
		sActionName: string,
		messageHandlingKey?: string
	): Promise<void | boolean | Dialog> {
		...
		let control;
		if (mParameters.controlId && mParameters.parentControl && mParameters.parentControl.isA<View>("sap.ui.core.mvc.View")) {
			control = mParameters.parentControl.byId(mParameters.controlId) as Control | undefined;
		} else {
	...
	async createWithDialog(
		...
		const onExitDialog = async (event: ExitDialogEvent): Promise<void> => {
			...
			try {
				...
				messageHandler.showMessages({ forceShowUIElement: createDialog.isOpen() });
			}
		};
