...
sap.ui.define(["sap/base/Log", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/editFlow/draftDataLossPopup", "sap/fe/core/controllerextensions/messageHandler/messageHandling", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/ResourceModelHelper", "sap/fe/core/helpers/TypeGuards", "sap/m/Button", "sap/m/Dialog", "sap/m/MessageBox", "sap/m/Text", "sap/ui/core/Lib", "../../helpers/ModelHelper", "../../operationsHelper"], function (Log, CommonUtils, draftDataLossPopup, messageHandling, MetaModelConverter, ResourceModelHelper, TypeGuards, Button, Dialog, MessageBox, Text, Library, ModelHelper, operationsHelper) {
  ...
  async function executeDraftEditAction(oContext, bPreserveChanges, oView, groupId, messageHandler, overwriteChange) {
    ...
    if (oContext.getObject() === undefined || oContext.getProperty("IsActiveEntity")) {
      ...
      const oEditPromise = oOperation.execute(groupId, ignoreETag, operationsHelper.fnOnStrictHandlingFailed.bind(draft, groupId, {
        appComponent: CommonUtils.getAppComponent(oView),
        label: sActionName,
        model: oContext.getModel()
      }, null, null, null, messageHandler, undefined, undefined), oContext.getBinding().isA("sap.ui.model.odata.v4.ODataListBinding"));
      if (overwriteChange) {
   ...
   * @returns Resolve function returns the context of the operation
   */
  async function executeDraftActivationAction(oContext, oAppComponent, messageHandler, sGroupId) {
    ...
    if (!oContext.getProperty("IsActiveEntity")) {
      ...
      try {
        return await oOperation.execute(sGroupId, bIgnoreEtag, sGroupId ? operationsHelper.fnOnStrictHandlingFailed.bind(draft, sGroupId, {
          ...
          label: sActionName,
          model: oContext.getModel()
        }, null, null, null, messageHandler, undefined, undefined) : undefined, oContext.getBinding().isA("sap.ui.model.odata.v4.ODataListBinding"));
      } catch (e) {
  ...
  async function executeDraftDiscardAction(oContext, messageHandler, oAppComponent, bEnableStrictHandling) {
    if (!oContext.getProperty("IsActiveEntity")) {
      ...
      const oDiscardPromise = !bEnableStrictHandling ? oDiscardOperation.execute(sGroupId) : oDiscardOperation.execute(sGroupId, undefined, operationsHelper.fnOnStrictHandlingFailed.bind(draft, sGroupId, {
        ...
        label: sActionName,
        model: oContext.getModel()
      }, null, null, null, messageHandler, undefined, undefined), false);
  ...
  async function deleteDraft(oContext, messageHandler, oAppComponent, bEnableStrictHandling) {
    ...
    if (bIsActiveEntity || !bIsActiveEntity && !sDiscardAction) {
      ...
      if (oContext.hasPendingChanges() && !oContext.isInactive()) {
        return oContext.getBinding().resetChanges().then(async function () {
