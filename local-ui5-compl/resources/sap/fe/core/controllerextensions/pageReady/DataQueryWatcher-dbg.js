  ...
  let DataQueryWatcher = /*#__PURE__*/function () {
    ...
    _proto.onSearch = function onSearch(oEvent) {
      const aMDCTableLinkedToFilterBar = this._aMDCTables.filter(oTable => {
        return oEvent.getSource().getId() === oTable.getFilter() && oTable.getVisible() && !oTable.getParent()?.getProperty("bindingSuspended");
      });
    ...
    _proto.register = function register(_event, data) {
      const binding = data.binding || data.table?.getRowBinding() || (data.chart?.getControlDelegate()).getInnerChart(data.chart).getBinding("data");
...
//# sourceMappingURL=DataQueryWatcher-dbg.js.map
