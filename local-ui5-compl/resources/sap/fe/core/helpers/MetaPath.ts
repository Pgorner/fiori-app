...
export default class MetaPath<T> {
	...
	public getMetaPathForPath<SubType>(targetPath: string): MetaPath<SubType> {
		return new MetaPath<SubType>(this.convertedMetadata, enhancePath(this.serviceObjectPath, targetPath), this.contextPath);
	}

	public getMetaPathForObject<SubType extends { fullyQualifiedName: string }>(targetObject: SubType): MetaPath<ResolvedTarget<SubType>> {
