...
import type { PropertyInfo } from "sap/ui/mdc/Table";
...
const PasteHelper = {
	 ...
	 */
	getColumnInfo: async function (table: Table, columns?: Column[], ignoreNavigationProperty = true): Promise<PasteColumnInfo[]> {
		...
		return (table.getControlDelegate() as TableDelegate).fetchProperties(table).then((propertyInfo: PropertyInfo[]) => {
			const propertyInfoDict: Record<string, PropertyInfo> = Object.assign(
				{},
				...propertyInfo.map((property: PropertyInfo) => ({ [property.name]: property }))
			);
			...
			(columns ?? table.getColumns()).forEach((column) => {
				...
				if (infoProperty.propertyInfos) {
					...
					if ((infoProperty as { exportDataPointTargetValue?: boolean }).exportDataPointTargetValue) {
						columnInfos.push({
