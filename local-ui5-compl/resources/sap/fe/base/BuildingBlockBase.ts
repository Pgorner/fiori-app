...
class BuildingBlockBase<T extends UI5Element = Control, K extends object = {}> extends Control {
	...
	@defineState()
	protected state: StateOf<K> = {} as StateOf<K>;
	...
	override getDomRef(): Element | null {
		const oContent = this.content;
		let domRef: Element | null = oContent?.getDomRef() ?? super.getDomRef();
		if (!domRef) {
	 ...
	 */
	protected bindState(path: keyof K): PathInModelExpression<PrimitiveType> | ConstantExpression<PrimitiveType> {
