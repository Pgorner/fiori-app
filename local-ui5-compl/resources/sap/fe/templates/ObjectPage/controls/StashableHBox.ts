...
class StashableHBox extends HBox {
	...
	getTitleControl(): Link | null {
		...
		if (items && items[0] && (items[0] as HBox).getItems) {
			aItems = (items[0] as HBox).getItems();
		} else if (items && items[0] && (items[0] as EnhanceWithUI5<MicroChartContainer>).getMicroChartTitle) {
			aItems = (items[0] as EnhanceWithUI5<MicroChartContainer>).getMicroChartTitle();
		}
