...
sap.ui.define(["sap/base/Log", "sap/base/assert", "sap/base/util/extend", "sap/base/util/isEmptyObject", "sap/base/util/merge", "sap/ui/base/Object", "sap/ui/core/UIComponent", "sap/ui/core/routing/HashChanger", "sap/ui/thirdparty/URI", "sap/ui/thirdparty/jquery", "sap/ui/util/openWindow", "./NavError", "./SelectionVariant", "./library"], function (Log, assert, extend, isEmptyObject, merge, BaseObject, UIComponent, HashChanger, URI, jQuery, openWindow, NavError, SelectionVariant, NavLibrary) {
  ...
  let NavigationHandler = /*#__PURE__*/function (_BaseObject) {
     ...
     *         </table>
     */
    function NavigationHandler(oController) {
      var _this;
      let sMode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Mode.ODataV4;
      let sParamHandlingMode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ParamHandlingMode.SelVarWins;
    ...
    _proto._getAppNavigationService = function _getAppNavigationService() {
      ...
      // eslint-disable-next-line deprecation/deprecation
      return ushellContainer.getService("CrossApplicationNavigation");
    }
     ...
     * @private
     */;
    _proto._getAppNavigationServiceAsync = async function _getAppNavigationServiceAsync() {
      const ushellContainer = sap.ui.require("sap/ushell/Container");
      return ushellContainer?.getServiceAsync("Navigation").then(function (oCrossAppNavService) {
    ...
    _proto.navigate = function navigate(sSemanticObject, sActionName, vNavigationParameters, oInnerAppData, fnOnError, oExternalAppData, sNavMode) {
      ...
      const fnNavigate = async function (oCrossAppNavService) {
      ...
      const fnStoreAndNavigate = function (oCrossAppNavService) {
        oNavHandler.storeInnerAppStateAsync(oInnerAppData, true).then(async function (sAppStateKey) {
          if (sAppStateKey) {
      ...
      oNavHandler._getAppNavigationServiceAsync().then(async function (oCrossAppNavService) {
        try {
          const oTargets = await oCrossAppNavService.isNavigationSupported([oNavArguments], oNavHandler.oComponent);
    ...
    _proto.parseNavigation = function parseNavigation() {
      ...
      const hybridIAppStateKeyParams = oStartupParameters?.[HYBRID_IAPP_STATE_KEY]?.[0];
        ...
        if (bIsXappStateNavigation) {
          ...
          .then(function (oAppState) {
            ...
            if (oAppStateData) {
              ...
              const failureHandlerHybridState = function (error, _ref, navType) {
     ...
     */;
    _proto._getAppSpecificHash = async function _getAppSpecificHash(appHash) {
      try {
        const crossAppService = await this._getAppNavigationServiceAsync();
        return await crossAppService.getHref({
          appSpecificRoute: appHash
        });
      } catch (error) {
     ...
     * </code>
     */;
    _proto.mixAttributesAndSelectionVariant = function mixAttributesAndSelectionVariant(vSemanticAttributes, sSelectionVariant) {
      let iSuppressionBehavior = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : SuppressionBehavior.standard;
    ...
    _proto._saveAppStateAsync = async function _saveAppStateAsync(oAppData, fnOnAfterSave, fnOnError) {
      const oNavHandler = this;
    ...
    _proto._fnSaveAppStateAsync = async function _fnSaveAppStateAsync(oAppData, fnOnError) {
      const oNavHandler = this;
      return this._getAppNavigationServiceAsync().then(async function (oCrossAppNavService) {
...
//# sourceMappingURL=NavigationHandler-dbg.js.map
