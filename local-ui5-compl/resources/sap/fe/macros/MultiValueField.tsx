...
type MultiInputSettings = {
	...
	collectionBindingDisplay: PropertyBindingInfo;
	collectionBindingEdit: PropertyBindingInfo;
...
export default class MultiValueFieldBlock extends BuildingBlock<Control> {
	...
	private _overrideValue(
		collectionBindingDisplay: PropertyBindingInfo,
		collectionBindingEdit: PropertyBindingInfo & { parameters?: object },
		...
		if (this.items) {
			...
			collectionBindingDisplay = { path: `${model}>${this.items.path}` };
			collectionBindingEdit = { path: `${model}>${this.items.path}` };
	 ...
	 */
	getPossibleValueHelp(): ValueHelp | undefined {
	...
	createContent(): Control | undefined {
		...
		if (metaContextPath) {
			...
			if (vhControl && vhControl.getContent()) {
				...
				//calculate valueHelp
				valueHelp = ValueHelpTemplating.generateID(
					undefined,
					this.vhIdPrefix,
			...
			const multiValueField = (
				<MultiValueField
					...
					label={label}
					change={(FieldRuntime.handleChangeMultiValueField as unknown as Function).bind(FieldRuntime, controller)}
				...
				multiValueField?.addEventDelegate(
					{
						onfocusin: (evt: jQuery.Event & { srcControl: Control }) => {
							...
							} as unknown as UI5Event<{ relatedTarget: Node }>);
