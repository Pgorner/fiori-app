...
export function getCommandExecutionForAction(
	...
	const actionVisible = action.visible ? resolveBindingString<boolean>(action.visible) : undefined;
	const actionEnabled = action.enabled ? resolveBindingString<boolean>(action.enabled) : undefined;
	switch (action.type) {
		case "ForAction":
			executeFunction = parameters.onExecuteAction!;
			isEnabled =
				parameters.isActionEnabled !== undefined ? resolveBindingString<boolean>(parameters.isActionEnabled) : actionEnabled;
			break;
		case "ForNavigation":
			executeFunction = parameters.onExecuteIBN!;
			isEnabled = parameters.isIBNEnabled !== undefined ? resolveBindingString<boolean>(parameters.isIBNEnabled) : actionEnabled;
			break;
		default:
			executeFunction = parameters.onExecuteManifest!;
			isEnabled = parameters.isEnabled !== undefined ? resolveBindingString<boolean>(parameters.isEnabled) : actionEnabled;
	}
