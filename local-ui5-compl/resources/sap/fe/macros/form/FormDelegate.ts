...
const Delegate = {
	...
	createLayout: async function (mPropertyBag: {
		...
		valueHelp: Element | UI5Element | Element[] | UI5Element[] | undefined;
	}> {
		...
		const sFormId = (mPropertyBag.element as UI5Element)?.getId() || (mPropertyBag.element as Element)?.id;
		...
		async function fnTemplateValueHelp(sFragmentName: string): Promise<Element | UI5Element | Element[] | UI5Element[]> {
