...
class FormContainerAPI extends MacroAPI {
	 ...
	 */
	static setInvisibleTextWhereTextAreaLabelIsHidden(formElement: FormElement, field: Field): void {
	...
	static setTextAreaLabelVisibility(formContainer: FormContainer): void {
		...
		if (formElements.length === 1) {
			...
			if (fields.length === 1) {
				const control = FieldAPI.getControlInFieldWrapper((fields[0] as Field).getContent());
				const isTextArea = control?.isA("sap.fe.macros.field.TextAreaEx");
				const isRequired = (fields[0] as Field).getRequired() === true;
				...
				if (!textAreaLabelVisibility && !isInvisibleTextAdded) {
					this.setInvisibleTextWhereTextAreaLabelIsHidden(formElements[0], fields[0] as Field);
				}
			}
		}
	}
