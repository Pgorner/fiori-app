...
import ConditionalWrapper from "sap/fe/macros/controls/ConditionalWrapper";
...
export default class DataPoint extends BuildingBlock<Control> {
	...
	createObjectNumber(dataModelPath: DataModelObjectPath<DataPointType>): ObjectNumber {
		...
		const unit =
			this.formatOptions?.measureDisplayMode === "Hidden"
				? undefined
				: compileExpression(UIFormatters.getBindingForUnitOrCurrency(this.valueDataModelPath));
	 ...
	 */
	createObjectNumberWithEventualWrapper(dataModelPath: DataModelObjectPath<DataPointType>): typeof ConditionalWrapper | ObjectNumber {
		if (this?.formatOptions?.isAnalytics ?? false) {
			return (
				<ConditionalWrapper
					condition={UIFormatters.hasValidAnalyticalCurrencyOrUnit(this.valueDataModelPath) as unknown as boolean}
				>
					{{
	...
	createContent(): ObjectStatus | ObjectNumber | RatingIndicator | ProgressIndicator | typeof ConditionalWrapper | undefined {
