...
sap.ui.define(["sap/fe/macros/CommonHelper", "sap/fe/macros/table/TableHelper", "sap/m/library"], function (CommonHelper, Table, library) {
  ...
  const DefaultActionHandler = {
     ...
     */
    getDefaultActionHandler: function (oCtx, oAction, oDataFieldForDefaultAction, defaultActionContextOrEntitySet, mode, collectionContext) {
        ...
        try {
          switch (oAction.defaultAction.type) {
            case "ForAction":
              {
                if (mode === "Table" && oCtx) {
                  const actionObject = typeof defaultActionContextOrEntitySet === "string" ? oCtx.metaPath.getModel().createBindingContext(defaultActionContextOrEntitySet)?.getObject() : defaultActionContextOrEntitySet;
                  return Table.pressEventDataFieldForActionButton(oCtx, oDataFieldForDefaultAction, collectionContext.getObject("@sapui.name"), oCtx.tableDefinition.operationAvailableMap, actionObject, oAction.isNavigable, oAction.enableAutoScroll, oAction.defaultValuesExtensionFunction);
                }
                return undefined;
              }
            case "ForNavigation":
