 ...
 */
const TableRuntime = {
	displayTableSettings: function (oEvent: UI5Event<{}, Control>): void {
		/*
				 Temporary solution
	 ...
	 */
	updateCutPasteEnablement: function (modelObject: { [key: string]: unknown }, table: Table, forContextMenu = false): void {
		...
		const updateableContextsModel = !forContextMenu
			? modelObject["updatableContexts"]
			: (modelObject.contextmenu as { [key: string]: unknown })["updatableContexts"];
		const cutableContexts = (updateableContextsModel as Context[] | undefined)?.filter((context) => {
			return (
		...
		if (!forContextMenu) {
			...
			(modelObject.contextmenu as { [key: string]: unknown })["cutableContexts"] = cutableContexts?.map((context) =>
				context.getPath()
			);
		}
		...
		if (isMoveToPositionAllowedFunction && (modelObject.numberOfSelectedContexts as number) <= 1) {
			const pastableContext = modelObject.pastableContexts ? (modelObject.pastableContexts as Context[])[0] : undefined;
			if (
				pastableContext &&
				!(
				...
				)(pastableContext, (modelObject.selectedContexts as Context[])[0])
			) {
	...
	onFieldChangeInCreationRow: function (
		...
		oCustomValidationFunction?: Function
	): void {
		// CREATION ROW CASE
