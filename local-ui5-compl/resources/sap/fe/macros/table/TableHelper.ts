...
import type { NavigationTargetConfiguration, TableColumnSettings } from "sap/fe/core/converters/ManifestSettings";
...
import type { AnnotationTableColumn, TableVisualization } from "sap/fe/core/converters/controls/Common/Table";
...
const TableHelper = {
	...
	getIBNData: function (outboundDetail: NavigationTargetConfiguration["outboundDetail"]): string | undefined {
		if (outboundDetail) {
			const oIBNData = {
	...
	getDelegate: function (table: TableVisualization, isALP: boolean, entityName: string, filterOnActiveEntities: boolean): string {
		let delegate: {
			name: string;
			payload?:
				| { collectionName?: string; filterOnActiveEntities?: boolean }
				| { hierarchyQualifier: string; initialExpansionLevel?: number; filterOnActiveEntities: boolean; createInPlace?: boolean };
		};
		if (isALP) {
