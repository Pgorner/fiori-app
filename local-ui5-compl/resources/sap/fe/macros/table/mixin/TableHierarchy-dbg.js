...
sap.ui.define(["sap/base/Log", "sap/fe/core/CommonUtils", "sap/fe/core/controllerextensions/BusyLocker", "sap/fe/core/converters/MetaModelConverter", "sap/fe/core/helpers/FPMHelper", "sap/fe/core/helpers/ResourceModelHelper", "sap/m/MessageToast", "../TableRuntime"], function (Log, CommonUtils, BusyLocker, MetaModelConverter, FPMHelper, ResourceModelHelper, MessageToast, TableRuntime) {
  ...
  let TableHierarchy = /*#__PURE__*/function () {
     ...
     */;
    _proto._onCut = function _onCut(evt) {
    ...
    _proto._onDropDocument = async function _onDropDocument(ui5Event) {
      ...
      try {
        ...
        await Promise.all([this.dropContext(ui5Event.getParameter("dragSource"), dropInfo), this.requestSideEffectsForChangeNextSiblingAction()]);
      } catch (error) {
     ...
     * @returns A Promise
     */;
    _proto.requestSideEffectsForChangeNextSiblingAction = async function requestSideEffectsForChangeNextSiblingAction() {
      const entityPath = this.getContent().data("metaPath");
      const tableContext = this.getContent().getBindingContext();
      ...
      const actionName = entityType.annotations.Hierarchy?.[`RecursiveHierarchyActions#${this.getTableDefinition().control.hierarchyQualifier ?? ""}`]?.ChangeNextSiblingAction;
      if (actionName) {
