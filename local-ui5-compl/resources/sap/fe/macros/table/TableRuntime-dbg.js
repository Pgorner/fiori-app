  ...
  const TableRuntime = {
    displayTableSettings: function (oEvent) {
      ...
      const oParent = oEvent.getSource().getParent(),
        oSettingsButton = UI5Element.getElementById(`${oParent.getId()}-settings`);
    ...
    updateCutPasteEnablement: function (modelObject, table) {
      ...
      // Cutable contexts are updatable contexts, removing the ones that are excluded by the custom logic (if any) and the ones with a draft instance if we're in a ListReport
      const updateableContextsModel = !forContextMenu ? modelObject["updatableContexts"] : modelObject.contextmenu["updatableContexts"];
      const cutableContexts = updateableContextsModel?.filter(context => {
        return TableRuntime.checkIfNodeIsMovable(context, table) && (!checkDraftInstance || context.getProperty("HasDraftEntity") !== true);
      });
      if (!forContextMenu) {
        modelObject["cutableContexts"] = cutableContexts?.map(context => context.getPath());
      } else {
        modelObject.contextmenu["cutableContexts"] = cutableContexts?.map(context => context.getPath());
      }
      ...
      if (isMoveToPositionAllowedFunction && modelObject.numberOfSelectedContexts <= 1) {
        const pastableContext = modelObject.pastableContexts ? modelObject.pastableContexts[0] : undefined;
        if (pastableContext && !FPMHelper.getCustomFunction(isMoveToPositionAllowedFunction.moduleName, isMoveToPositionAllowedFunction.methodName, table)(pastableContext, modelObject.selectedContexts[0])) {
