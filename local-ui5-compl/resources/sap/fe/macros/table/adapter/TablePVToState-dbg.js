...
sap.ui.define([], function () {
   ...
   */
  const overridePVGroupLevels = function (newGroupLevels, oldGroupLevels, propertyInfoNames) {
    const groupsInNewVariant = newGroupLevels.map(group => {
      return group;
    });
    const groupLevels = newGroupLevels.map(group => {
      return {
        name: propertyInfoNames.includes(`Property::${group}`) ? `Property::${group}` : group
      };
    }) || [];
    groupLevels.push(...oldGroupLevels.filter(group => {
   ...
   */
  const overridePVSorters = function (newSortOrder, oldSortOrder, propertyInfoNames) {
    ...
    sorters.push(...oldSortOrder.filter(sorter => {
   ...
   */
  const overridePVAggregations = function (newAggregations, oldAggregations, propertyInfoNames) {
    const newAggregationsKeys = Object.keys(newAggregations);

    // Return object
    const mixedAggregates = {};
   ...
   */
  const overridePVContent = function (newPVContent, oldPVContent) {
    const itemsInNewVariant = newPVContent.map(item => {
      return item["Value"];
    }) || [];
    const items = newPVContent?.map((item, itemIndex) => {
      return {
        name: item.Value,
        visible: true,
        position: itemIndex
    ...
    items.push(...oldPVContent.filter(item => {
   ...
   */
  const convertPVToState = function (pvToSet, pvToReplace, propertyInfoNames) {
    const newPVProps = pvToSet.getProperties() ?? {};
    const oldPVProps = pvToReplace.getProperties() ?? {};
    const newPVContent = pvToSet.getTableVisualization()?.Content || [];
    const oldPVContent = pvToReplace.getTableVisualization()?.Content || [];
  ...
  _exports.convertPVToState = convertPVToState;
  return _exports;
}, false);
//# sourceMappingURL=TablePVToState-dbg.js.map
