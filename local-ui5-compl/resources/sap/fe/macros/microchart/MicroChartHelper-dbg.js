 ...
 */
sap.ui.define(["sap/base/Log", "sap/fe/macros/CommonHelper", "sap/m/library", "sap/ui/core/format/DateFormat", "sap/ui/core/format/NumberFormat"], function (Log, CommonHelper, library, DateFormat, NumberFormat) {
  "use strict";
  ...
  const MicroChartHelper = {
     ...
     * @private
     */
    getSelectParameters: function (groupId, sortOrder, criticalityCalculation, criticality, otherPaths) {
      const propertyPath = [],
        sorters = [],
        parameters = [];
      if (groupId) {
     ...
     * @private
     */
    getAggregationForMicrochart: function (aggregationType, collection, dataPoint, uiName, dimension, measure, sortOrder, measureOrDimensionBar) {
      ...
      if (dimension?.targetObject?.$target?.annotations?.Common?.Text) {
        dimensionPropertyPath = dimension?.targetObject?.$target?.annotations?.Common?.Text?.path;
      } else if (dimension) {
        dimensionPropertyPath = dimension.targetObject?.value;
      }
      ...
      const functionValue = MicroChartHelper.getSelectParameters(groupId, sortOrder, dataPointCriticallityCalc, dataPointCriticallity, [currencyOrUnit, targetValuePath, dimensionPropertyPath, measureOrDimensionBar]);
      return `{path:'${path}'` + `, parameters : {${functionValue}} }`;
    },
