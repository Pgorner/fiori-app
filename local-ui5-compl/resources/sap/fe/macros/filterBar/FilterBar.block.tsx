...
import MDCValueHelp from "sap/ui/mdc/ValueHelp";
...
import FixedList from "sap/ui/mdc/valuehelp/content/FixedList";
...
export default class FilterBarBlock extends BuildingBlockTemplatingBase {
	...
	_processPropertyInfos(propertyInfo: string): void {
		...
		if (this.showDraftEditState) {
			...
			draftEditState = (
				<MDCFilterField
					...
					valueHelp={generate([this.id, "FilterFieldValueHelp", "DraftEditingStatus"])}
					display="Description"
				>
					{{
						dependents: (
							<MDCValueHelp id={generate([this.id, "FilterFieldValueHelp", "DraftEditingStatus"])}>
								{{
									typeahead: (
										<Popover>
											<FixedList useFirstMatch="true">
												{{
													items: draftStates.map((state) => {
														return <FixedListItem key={state.id} text={state.display} />;
													})
												}}
											</FixedList>
										</Popover>
