  ...
  const InteractiveChartHelper = {
    ...
    _getUOM: function (UOMObjectPath, UOM, collection, customData, aggregatablePropertyPath) {
      ...
      if (UOMObjectPath && UOM && isPathAnnotationExpression(UOMObjectPath)) {
        ...
        return UOMObjectPath && customData && isPathAnnotationExpression(UOMObjectPath) && UOMObjectPath.path ? JSON.stringify(UOMObject) : UOMObjectPath;
      }
    },
    ...
    getInteractiveChartProperties: function (visualFilter) {
      ...
      if (visualFilter.chartMeasure && chartAnnotation?.Dimensions && chartAnnotation.Dimensions[0]) {
        ...
        const dimensionContext = visualFilter.metaPath?.getModel()?.createBindingContext(visualFilter.contextPath.getPath() + "/@" + dimension.fullyQualifiedName.split("@")[1]);
        if (dimensionContext) {
        ...
        interactiveChartProperties.displayedValue = InteractiveChartHelper.getChartDisplayedValue(visualFilter.chartMeasure, dataPointAnnotation?.ValueFormat, visualFilter.metaPath?.getPath());
