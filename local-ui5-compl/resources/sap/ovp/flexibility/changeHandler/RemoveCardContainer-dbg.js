...
sap.ui.define([
    ...
    RemoveCardContainer.changeHandler.applyChange = function (oChange, oControl, mPropertyBag) {
        var oModifier = mPropertyBag.modifier,
            ...
            oMainView = oMainController.getView(),
         ...
         */
        var sMessage = OvpResources.getText("OVP_KEYUSER_TOAST_MESSAGE_FOR_REMOVE", [
            oCardControl.getComponentInstance().getComponentData().settings.title
        ]);
    ...
    RemoveCardContainer.changeHandler.revertChange = function (oChange, oControl, mPropertyBag) {
        ...
        if (oUIModel.getProperty("/containerLayout") === "resizable") {
            var oLayoutUtil = oLayout.getDashboardLayoutUtil();
            oLayoutUtil.updateCardVisibility([
                { id: oLayoutUtil.getCardIdFromComponent(oRevertedData.id), visibility: true }
            ]);
        }
        oLayout.invalidate();
        return true;
     ...
     * @public
     */
    RemoveCardContainer.changeHandler.completeChangeContent = function (
        oChange,
        oSpecificChangeInfo,
        mPropertyBag
    ) {
