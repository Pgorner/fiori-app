...
sap.ui.define([
    ...
    ReplaceItems.prototype._dragMoveHandler = function (actionObject, qElement) {
        ...
        var moveY =
            actionObject.moveY - this.jqLayout.get(0).getBoundingClientRect().top + this.jqLayout.scrollTop();
        if (!qElement) {
        ...
        if (
            this.lastCollidedEl &&
            this.lastCollidedEl !== collidedItem &&
            this.lastCollidedEl !== sourceElement &&
            this.lastCollidedEl.posDnD.left === collidedItem.posDnD.left &&
            this.lastCollidedEl.posDnD.left === sourceElement.posDnD.left
        ) {
        ...
        if (!!this.positionChanges && targetElement && targetElement.children) {
            ...
            this.positionChanges.push({
                ...
                content: {
                    ...
                    cardId: sTargetElementId.substring(
                        sTargetElementId.indexOf("mainView--") + 10,
                        sTargetElementId.length
                    ),
            ...
            if (sourceElement && sourceElement.children) {
                var sSourceElementId = sourceElement.children[0].getAttribute("id");
                this.positionChanges.push({
                    changeType: "position",
                    content: {
                        // sSourceElementId gives full core ID, so remove the prefix "mainView--"
                        cardId: sSourceElementId.substring(
                            sSourceElementId.indexOf("mainView--") + 10,
                            sSourceElementId.length
                        ),
                        position: targetElementIndex
                    },
                    isUserDependent: true
                });
            }
    ...
    ReplaceItems.prototype.initCardsSettingsForDragAndDrop = function () {
        ...
        this.isRTLEnabled = Localization.getRTL() ? 1 : -1;
        this.aCardsOrder = [];
        ...
        this.horizontalMargin = parseInt(getComputedStyle(firstItemEl)["margin-bottom"], 10);
        ...
        jUtils.addStyleToAllElements(this.aCardsOrder, "position", "absolute");
        this.drawLayout(this.aCardsOrder);
    ...
    ReplaceItems.prototype.drawLayout = function (aCardsLayout) {
        ...
        for (var naturalIndex = 0; naturalIndex < aCardsLayout.length; naturalIndex++) {
            ...
            var currentLeft =
                this.left - this.isRTLEnabled * (currentColumn * this.verticalMargin + currentColumn * this.width);
    ...
    ReplaceItems.prototype._dragStartHandler = function (evt, cardElement) {
        ...
        if (Device.system.desktop) {
            jUtils.addClassToAllElements(document.querySelectorAll("body"), "sapOVPDisableUserSelect");
            jUtils.addClassToAllElements(document.querySelectorAll("body"), "sapOVPDisableImageDrag");
        }
    ...
    KeyboardNavigation.prototype.init = function (easyScanLayout, swapItemFunction) {
        ...
        this.jqElement
            .find(".sapUshellEasyScanLayoutInner")
            .on("focus.keyboardNavigation", this.layoutFocusHandler.bind(this));
        this.jqElement.on(
            "focus.keyboardNavigation",
            ".easyScanLayoutItemWrapper",
            this.layoutItemFocusHandler.bind(this)
        );
    ...
    KeyboardNavigation.prototype.layoutItemFocusHandler = function () {
        ...
        if (jqFocused) {
            ...
            var sCardHeader = "cardHeaderType_";
            if (jqFocused.hasClass("sapOvpCardHeader") && !jqFocused.hasClass("sapOvpStackCardContent")) {
                ...
                if (cardHeaderDiv.length === 0) {
                    ...
                    sCardHeaderTypeDivId = sCardHeader + new Date().getTime();
            ...
            var labelledElement, layoutWrapper;
            if (jqFocused.hasClass("easyScanLayoutItemWrapper")) {
                layoutWrapper = jqFocused.find(".sapOvpCardHeader");
                labelledElement = layoutWrapper.find("[aria-label]");
            } else {
                labelledElement = jqFocused.find("[aria-label]");
            }
            //  Add every element id with aria label
            // Skip for stackcard header as they are nested children aria references
            var isNotStackCardHeader = jqFocused.find(".sapOvpStackCardHeader").length !== 1;
            if (isNotStackCardHeader ) {
                for (i = 0; i < labelledElement.length; i++) {
                    if (!labelledElement[i].id.includes(sCardHeader) && !labelledElement[i].id.includes('sapOvpCardAdditionalActions') && !labelledElement[i].id.includes('listItem')) {
            ...
            if (jqFocused.prop("nodeName") === "LI" && jqFocused.find(".linkListHasPopover").length !== 0) {
                if (jqFocused.find("#hasDetails").length === 0) {
                    jqFocused.append(
                        "<div id='hasDetails' hidden>" + OvpResources.getText("HAS_DETAILS") + "</div>"
                    );
    ...
    KeyboardNavigation.prototype._changeItemsFocus = function (jqItem) {
        ...
        var jqTabbableElements = document.querySelectorAll(".easyScanLayoutItemWrapper");
        jUtils.addAttributeToAllElements(jqTabbableElements, "tabindex", "-1");
    ...
    KeyboardNavigation.prototype.altPageDownAndEndHandler = function (e) {
        ...
        var item;
        var layoutItems = this.getVisibleLayoutItems();
        if (indexDelta == colCount - 1 || focusedElementIndex + (colCount - indexDelta) > layoutItems.length) {
    ...
    */
    ...
    KeyboardNavigation.prototype.keyupHandler = function (e) {
        ...
        } else if (e.keyCode === this.keyCodes.F6) {
            if (e.shiftKey) {
                var $EasyScanLayoutInner = document.getElementsByClassName('sapUshellEasyScanLayoutInner');
                var $FirstElement = $EasyScanLayoutInner[0].firstElementChild;
                if ($FirstElement.firstElementChild) {
                   document.getElementsByClassName($FirstElement.firstElementChild.className)[0].focus();
                }
                this.handleScrollBar(true);
            }
        }
    ...
    KeyboardNavigation.prototype.spacebarHandler = function (e) {
        ...
        var item = CoreElement.getElementById(e.target.id);
        if (item && item.mEventRegistry.hasOwnProperty("press")) {
            var targetId = "#" + e.target.id;
            jUtils.addClassToAllElements(document.querySelectorAll(targetId), "sapMLIBActive");
            item.firePress();
        }
    };
    ...
    KeyboardNavigation.prototype.keydownHandler = function (e) {
        ...
        switch (e.keyCode) {
            ...
            case this.keyCodes.ARROW_UP:
                ...
                // scroll page up to display card in visible area.
    ...
    /**
     * Method to handle page scroll when "up" arrow key or "shift+F6" or "F6" is pressed.
     *
     * @method 
     * @param {boolean} bKeyCodeF6OrShiftF6 boolean value passed to check if the KeyCode is either F6 or Shift+F6
     */
         KeyboardNavigation.prototype.handleScrollBar = function (bKeyCodeF6OrShiftF6) {
            var sViewId = this.easyScanLayout._sOwnerId + "---mainView--ovpMain";
            if (sViewId) {
                var oView = CoreElement.getElementById(sViewId);
                if (oView) {
                    setTimeout( function () {
                        var oScrollDelegate = oView.getScrollDelegate();
                        if (oScrollDelegate) {
                            var fActiveCardTop = document.activeElement.getBoundingClientRect().height;
                            var fScrollPositon = oScrollDelegate.getScrollTop();
                            var iYAxis = bKeyCodeF6OrShiftF6 ? 0 : fScrollPositon - fActiveCardTop - 32;
                            oScrollDelegate.scrollTo(0, iYAxis /* card top-bottom spacing*/);
                        }
                    }, 0);
                }
            }
    ...
    var EasyScanLayout = Control.extend("sap.ovp.ui.EasyScanLayout", {
        ...
        renderer: {
            render: function (oRm, oControl) {
                var aCards = CoreComponent.getComponentById(oControl._sOwnerId) && CoreComponent.getComponentById(oControl._sOwnerId).oOvpConfig.cards;
                oRm.openStart("div", oControl);
                oRm.class("sapUshellEasyScanLayout");
                oRm.openEnd();
                oRm.openStart("div");
                oRm.class("sapUshellEasyScanLayoutInner");
                oRm.attr("role", "list");
                oRm.attr("aria-label", "Cards");
                ...
                columnList.forEach(function (column) {
                    oRm.openStart("div");
                    oRm.class("easyScanLayoutColumn");
                    ...
                    column.forEach(function (item, index) {
                        ...
                        oRm.class("easyScanLayoutItemWrapper");
                        oRm.accessibilityState(undefined, { role: "listitem" });
                        oRm.attr("aria-setsize", filteredItems.length);
                        oRm.attr("aria-posinset", itemCounter);
                        itemCounter++;
                        oRm.openEnd();
                        ...
                        oRm.openStart("div")
                           .class("lastElement")
                           .accessibilityState(undefined, { role:"none"})
                           .attr("tabindex", 0)
                           .openEnd();
    ...
    EasyScanLayout.prototype.getColumnResolutionList = function () {
        return [
    ...
    EasyScanLayout.prototype.init = function () {
        this.columnResolutionList = this.getColumnResolutionList();
        this.columnCount = this.columnResolutionList[0].columnCount;
        this.columnStyle = "";
        this.updateColumnClass(this.columnResolutionList[0].styleClass);
        this.initResizeHandlerOrMediaListeners(this.columnResolutionList);
        if (CoreComponent.getComponentById(this._sOwnerId) && CoreComponent.getComponentById(this._sOwnerId).oOvpConfig.cards.length === 0) {
    ...
    var mediaListenersDebounce = function (columnCount, columnStyle, mq) {
        ...
        if (mq.matches) {
            ...
            mediaListenerHandlerTimerId = window.setTimeout(
                mediaListenerHandler.bind(this, columnCount, columnStyle),
    ...
    EasyScanLayout.prototype.initMediaListeners = function (colResList) {
        ...
        for (var i = 0; i < colResList.length; i++) {
            ...
            var boundedListener = mediaListenersDebounce.bind(
                this,
                colResList[i].columnCount,
                colResList[i].styleClass
            );
    ...
    EasyScanLayout.prototype.initResizeHandler = function (colResList) {
        ...
        var resizeHandlerDebounce = function () {
            ...
            resizeHandlerTimerId = window.setTimeout(
                this.oControl.resizeHandler.bind(this, colResList),
                debounceTime
            );
        };
    ...
    EasyScanLayout.prototype.initResizeHandlerOrMediaListeners = function (colResList) {
        var matchMediaSupported = Device.browser.msie && Device.browser.version > 9;
        if (matchMediaSupported && this.getUseMediaQueries()) {
            //if matchMedia supported and full page --> use media queries
            this.mediaQueryList = this.initMediaListeners(colResList);
        } else {
            //if not full page --> use resize handler
            this.resizeHandlerId = this.initResizeHandler(colResList);
        }
    };
    ...
    EasyScanLayout.prototype.resizeHandler = function (colResList) {
        ...
        var resObject;
        if (document.querySelectorAll(".ovpWrapper .sapFDynamicPageContent").length > 0) {
    ...
    EasyScanLayout.prototype.headerMargin = function (that) {
        ...
        var resObject,
            ...
            columnWidth,
            val = 0;
        var colResList = this.getColumnResolutionList();
        var iRemToPx = CommonUtils.getPixelPerRem();
        var screenWidth =
            (parseInt(
                window
                    .getComputedStyle(document.querySelector(".ovpWrapper .sapFDynamicPageTitleWrapper"), null)
        ...
        margin = (screenWidth - columnsContainerWidth) / 2;

        //Add additional spacing on the sides for desktop only
        if (Device.system.desktop) {
            jQuery(".ovpWrapper .sapFDynamicPageTitle").css({
                "margin-left": margin + "rem",
                "margin-right": margin + "rem"
            });
            jQuery(".ovpWrapper .sapFDynamicPageHeader").css({
                "margin-left": margin + "rem",
                "margin-right": margin + "rem"
            });
        }
        ...
        if (Device.system.desktop) {
            ...
            if (document.querySelectorAll(".ovpWrapper .sapFDynamicPageContentFitContainer").length > 0) {
                contentWidth = document.querySelectorAll(".ovpWrapper .sapFDynamicPageContentFitContainer")[0]
                    .clientWidth;
            }

            if (screenWidth === contentWidth) {
                var dynamicPageTitle = document.querySelector(".ovpWrapper .sapFDynamicPageTitle") || undefined;
                var marginLeft = dynamicPageTitle && getComputedStyle(dynamicPageTitle)["margin-left"];
                var marginRight = dynamicPageTitle && getComputedStyle(dynamicPageTitle)["margin-right"];
                ...
                if (typeof marginRight === "string" || marginRight instanceof String) {
                    //take string with a rem unit
                    marginRight = marginRight.length > 0 ? parseInt(marginRight.split("px")[0], 10) : 0;
                }
                //CHECK-TO scrollWidth = getComputedStyle(document.querySelector('.sapFDynamicPageScrollBar'))['width'];
                scrollWidth = jQuery(".ovpWrapper .sapFDynamicPageScrollBar").css("width");
                if (typeof scrollWidth === "string" || scrollWidth instanceof String) {
                    //take string with a rem unit
    ...
    EasyScanLayout.prototype.afterDragAndDropHandler = function (aElements) {
        ...
        aElements.forEach(function (el, index) {
            ...
            var oControl = CoreElement.getElementById(elementId);
