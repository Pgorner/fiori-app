...
sap.ui.define([
    ...
    var DashboardLayout = Control.extend("sap.ovp.ui.DashboardLayout", {
        ...
        renderer: {
            ...
            render: function (oRm, oControl) {
                var filterVisibleCards = function(element) {
                    return element.getVisible();
                };
                var filterById = function (element) {
                    var sComponentId = this.getId();
                    var iMainComponentIdEndIndex = sComponentId.indexOf("---") + 3;
                    var sMainViewId = sComponentId.substring(iMainComponentIdEndIndex);
                    return element.id === sMainViewId.split("--")[1];
                };

                var ctrlWidth = oControl.$().width();   // get viewport width depending layout data
                var bRTL = Localization.getRTL();
                var oLayoutData = oControl.dashboardLayoutUtil.updateLayoutData(
                    ctrlWidth ? ctrlWidth : window.innerWidth
                );
                ...
                oRm.class("sapOvpDashboard");
                
                if (!Device.system.phone) {
                    oRm.class("sapOvpDashboardDragAndDrop");
                }

                if (bRTL) {
                    oRm.style("margin-right", oLayoutData.marginPx + "px");
                } else {
                    oRm.style("margin-left", oLayoutData.marginPx + "px");
                }
                ...
                if (aCards.length > 0) {
                    ...
                    for (counter = 0, iLength = filteredItems.length; counter < iLength; counter++) {
                        ...
                        oControl.dashboardLayoutUtil.setCardCssValues(card);    //re-set css values for current card
                        bSideCard = card.dashboardLayout.column + card.dashboardLayout.colSpan === colCount + 1;
                        if (bSideCard) {
                            card.dashboardLayout.colSpan === 1
                                ? aStyleClasses.push("sapOvpNotResizableLeftRight")
                                : aStyleClasses.push("sapOvpNotResizableRight");
                        }
                        ...
                        oRm.openStart("div", oControl.dashboardLayoutUtil.getCardDomId(card.id));

                        for (var i = 0; i < aStyleClasses.length; i++) {
                            oRm.class(aStyleClasses[i]);    
                        }

                        var sTransformValue = "translate3d(" + card.dashboardLayout.left + " ," + card.dashboardLayout.top + " ,0px)";
                        oRm.style("transform", sTransformValue);
                        oRm.style("-ms-transform", sTransformValue);
                        oRm.style("-moz-transform", sTransformValue);
                        oRm.style("-webkit-transform", sTransformValue);
                        oRm.style("height", card.dashboardLayout.height);
                        oRm.style("width", card.dashboardLayout.width);
                        oRm.attr("aria-setsize", iLength);
                        oRm.attr("aria-posinset", counter + 1);
                        oRm.attr("tabindex", 0);
        ...
        init: function () {
            ...
            var oComponent = CoreComponent.getComponentById(this._sOwnerId);
            this.dashboardLayoutUtil = oComponent.getDashboardLayoutUtil();
            this.dashboardLayoutUtil.setLayout(this);
            if (this.dashboardLayoutUtil.dashboardLayoutModel.getCards().length === 0) {
        ...
        onAfterRendering: function () {
            ...
            if (this.getDragAndDropEnabled()) {
                ...
                this.dashboardLayoutUtil.setAriaPos();
    ...
    KeyboardNavigation.prototype.init = function (ovpLayout) {
        ...
        var layoutClckHandlr = this.layoutClickHandler.bind(this);
        var dynamicPageContent = document.querySelectorAll(".ovpWrapper .sapFDynamicPageContent");
        for (var i = 0; i < dynamicPageContent.length; i++) {
            dynamicPageContent[i].addEventListener("click", layoutClckHandlr);
        }
    ...
    KeyboardNavigation.prototype.spacebarHandler = function (e) {
        ...
        if (item && item.mEventRegistry.hasOwnProperty("press")) {
            ...
            var allMatchedElem = document.querySelectorAll(elemId);
            for (var i = 0; i < allMatchedElem.length; i++) {
                allMatchedElem[i].classList.add("sapMLIBActive");
            }
            item.firePress();
        }
    };
    ...
    KeyboardNavigation.prototype.layoutFocusHandler = function (event) {
        ...
        if (this.sLastFocusableCard) {
            this.sLastFocusableCard = jQuery(this.sLastFocusableCard);
            // If the last focused element is the focusable card then do not set focus again
            if (!this.sLastFocusableCard.is(this.lastFocussedElement) && event.relatedTarget) {
                this.sLastFocusableCard.focus();
                this.layoutUtil.sLastFocusableCard = this.sLastFocusableCard;
            } else {
                this.lastFocussedElement = undefined;
            }
            return;
        }
    ...
    KeyboardNavigation.prototype.layoutClickHandler = function (e) {
        if (
            ...
            e.target.getAttribute("id").indexOf("mainView") !== -1
        ) {
            if (this.sLastFocusableCard) {
    ...
    KeyboardNavigation.prototype.shiftArrowHandler = function (oCard, oSpan, cardElement) {
        ...
        var oResizeObj = this.layoutUtil.dashboardLayoutModel.resizeCard(
            oCard.id,
            oSpan,
            true,
        ...
        switch (e.keyCode) {
            ...
            case this.keyCodes.F6:
                if (e.shiftKey) {
                    ...
                    if ($EasyScanLayoutInner[0].firstElementChild) {
                       document.getElementById($EasyScanLayoutInner[0].firstElementChild.id).focus();
                    }
    ...
    KeyboardNavigation.prototype.keyDownHandler = function (e) {
        var activeItem = document.activeElement,
            ...
            aCards = Object.assign([], this.layoutModel.aCards),
        ...
        if (this.layoutUtil.getCards().length === 0) {
            this.sLastFocusableCard = null;
        }
        switch (e.keyCode) {
            ...
            case this.keyCodes.F6:
                ...
                if (e.shiftKey) {
                    ...
                    this.sCurrentFocusId = $CurrentFocus.id;
                    if (this.sCurrentFocusId) {
                       document.getElementById(this.sCurrentFocusId).focus();
                    }
                } else {
                    this.bIgnoreSelfFocus = true;
                    ...
                    var $ShellHeader = document.getElementById('shell-header') && document.getElementById('shell-header').firstChild;
                    var $ShellHeaderFirstChild = $ShellHeader && $ShellHeader.firstChild;
                    if ($ShellHeaderFirstChild && $ShellHeaderFirstChild.tabIndex === 0) {
                        document.getElementById($ShellHeaderFirstChild.id).focus();
                    }   
                    this.handleScrollBar(true);
                }
                break;
            ...
            case this.keyCodes.ARROW_DOWN:
                ...
                if (cardId && e.ctrlKey) {
                    ...
                    if (
                        ...
                        oCard.dashboardLayout.rowSpan >= 45
                    ) {
                        return;
                    }
                    oSpan = {
            ...
            case this.keyCodes.ARROW_LEFT:
                ...
                if (cardId && e.ctrlKey) {
                    ...
                    if (bIsRTL) {
                        if (
                            (oCard.template === "sap.ovp.cards.list" && oCard.dashboardLayout.colSpan === 2) ||
                            ((oCard.template === "sap.ovp.cards.linklist" ||
                                oCard.template === "sap.ovp.cards.v4.linklist") &&
                                oCard.settings.listFlavor === "carousel" &&
                                oCard.dashboardLayout.colSpan === 3)
                        ) {
                            return;
                        }
                        oSpan = {
                    ...
                    this.setFocusInCaseOfSpace(oCard, "Left");
                    e.stopImmediatePropagation();
                }
                break;
            case this.keyCodes.ARROW_RIGHT:
                ...
                if (cardId && e.ctrlKey) {
                    ...
                    this.setFocusInCaseOfSpace(oCard, "Right");
                    e.stopImmediatePropagation();
                }
                break;
            case this.keyCodes.PAGE_UP:
                ...
                if (e.altKey == true) {
                    ...
                    for (var i = cardsLength; i > 0; i--) {
                        currentCard = document.getElementById(
                            this.layoutUtil.getCardDomId(tempCards[columnIndex][i].id)
                        );
            ...
            case this.keyCodes.PAGE_DOWN:
                ...
                if (e.altKey == true) {
                    cardId =
                        columnIndex + oCard.dashboardLayout.colSpan > columnCount
                            ? aCards[aCards.length - 1].id
                            : tempCards[columnCount][rowIndex] && tempCards[columnCount][rowIndex].id;
                } else {
            ...
            case this.keyCodes.END:
                ...
                if (e.ctrlKey == true) {
                    ...
                    cardId =
                        lastCardInColumn === rowIndex
                            ? aCards[aCards.length - 1].id
                            : tempCards[columnIndex][lastCardInColumn] &&
                            tempCards[columnIndex][lastCardInColumn].id;
                } else {
                    cardId =
                        columnIndex + oCard.dashboardLayout.colSpan > columnCount
                            ? aCards[aCards.length - 1].id
    ...
    /**
     * Method to handle page scroll when "up" arrow key or "F6" or shift+F6" is pressed.
     *
     * @method handleScrollBar
    ...
    */
    KeyboardNavigation.prototype.handleScrollBar = function (bKeyCodeF6OrShiftF6) {
        var sViewId = this.layoutUtil.componentDomId + "--ovpMain";
        if (sViewId) {
            ...
            if (oView) {
                setTimeout( function () {
                    ...
                    if (oScrollDelegate) {
                        ...
                        var fActiveCardTop = document.activeElement.getBoundingClientRect().top;
                        var iYAxis = bKeyCodeF6OrShiftF6 ? 0 : fScrollPositon - fActiveCardTop - 32;
                        oScrollDelegate.scrollTo(0,iYAxis /* card top-bottom padding and spacing*/);
                    }
                }, 0);
            }
        }
    ...
    /**
     * Method to set focus on card in case if space is enabled in between card
     *
     * @method setFocusInCaseOfSpace
     * @param {Object} oCard - Instance of the card
     * @param {String} sType - Left or Right movement
     */
    KeyboardNavigation.prototype.setFocusInCaseOfSpace = function (oCard, sType) {
        var aCards = Object.assign([], this.layoutModel.aCards),
            oAriaPosMap = this.layoutUtil.ariaPos,
            currentPos = oAriaPosMap[oCard.id],
            aNextCard = aCards.filter(function (oCard) {
                return sType === "Right"
                    ? oAriaPosMap[oCard.id] === currentPos + 1
                    : oAriaPosMap[oCard.id] === currentPos - 1;
            }, oAriaPosMap);
        this.setFocusOnCard(aNextCard[0] && aNextCard[0].id);
    };
    ...
    KeyboardNavigation.prototype.afterFocusHandler = function (e) {
