...
sap.ui.define([
    ...
    KeyboardNavigation.prototype.init = function (objectStream) {
        ...
        this.jqElement.on(
            ...
            this.ObjectStreamFocusAccessabilityHandler.bind(this)
        );
    };
    ...
    KeyboardNavigation.prototype._scrollItemToView = function(e, jqItemTo) {
        var sControlID = e && e.currentTarget && e.currentTarget.id;

        if (sControlID) {
            var oControl = CoreElement.getElementById(sControlID),
                oWrapper = oControl && oControl.wrapper,
                oTargetElement = jqItemTo && jqItemTo.get() && jqItemTo.get()[0];

            if (oTargetElement && oWrapper) {
                var oTargetElementRect = oTargetElement.getBoundingClientRect(),
                    oWrapperRect = oWrapper.getBoundingClientRect(),
                    iElementLeft = oTargetElementRect && oTargetElementRect.left,
                    iElementRight = oTargetElementRect && oTargetElementRect.right,
                    iWrapperLeft = oWrapperRect && oWrapperRect.left,
                    iWrapperRight = oWrapperRect && oWrapperRect.right;
                
                if (iWrapperRight < iElementRight || iWrapperLeft > iElementLeft) {
                    oTargetElement.scrollIntoView();
                }
            }
        }
    };
    ...
    KeyboardNavigation.prototype.leftRightHandler = function (e, isRight) {
        ...
        this._scrollItemToView(e, nextFocus);
    };
    ...
    var ObjectStream = Control.extend("sap.ovp.ui.ObjectStream", {
        metadata: {
            library: "sap.ovp",
            aggregations: {
    ...
    ObjectStream.prototype.init = function () {
        ...
        }).addStyleClass("sapOvpObjectStreamClose");
    };
    ...
    ObjectStream.prototype._initScrollVariables = function () {
        ...
        if (this.shouldShowScrollButton) {
            this.jqRightEdge
                .add(this.jqLeftEdge)
                .on("mouseenter.objectStream", this, this._mouseEnter)
    ...
    ObjectStream.prototype._afterOpen = function () {
        if (Device.os.ios && this.$().length) {
            ...
            this.$().on("touchmove.scrollFix", function (e) {
                e.stopPropagation();
            });
        }
    ...
    ObjectStream.prototype._mouseEnter = function (evt) {
        ...
        if (evt.target == evt.data.jqRightEdge.get(0) || evt.currentTarget == evt.data.jqRightEdge.get(0)) {
            scrollDirection = Localization.getRTL() ? "left" : "right";
        }
        if (evt.target == evt.data.jqLeftEdge.get(0) || evt.currentTarget == evt.data.jqLeftEdge.get(0)) {
            scrollDirection = Localization.getRTL() ? "right" : "left";
        }
    ...
    ObjectStream.prototype._checkEdgesVisibility = function () {
        ...
        if (Localization.getRTL() && this.scrollReverse) {
    ...
    ObjectStream.prototype._createPopup = function () {
        this._oPopup = new Dialog({
            ...
            .addStyleClass("sapOvpStackedCardPopup");
    ...
    ObjectStream.prototype.getNewContentAvailable = function () {
        return this.onRefreshNewContent;
    };
    ObjectStream.prototype.setNewContentAvailable = function (value) {
        this.onRefreshNewContent = value;
    };
    ...
    ObjectStream.prototype.updateContent = function (reason) {
        ...
        if (reason === "change") {
            oBindingInfo.clearQuickViewCard();
            var fnFactory = oBindingInfo.factory,
                i = 0,
                aItems = this.getContent(),
                addNewItemFn = function (oContext) {
                    var sId = this.getId() + "-" + uid(),
                        oClone = fnFactory(sId, oContext);
                    oClone.setBindingContext(oContext, oBindingInfo.model);
                    this.addContent(oClone);
                };
            var addNewItem = addNewItemFn.bind(this);

            //incase of new content on refresh interval; destroy the previous containers and create new ones for the whole content
            if (aItems.length < aBindingContexts.length) {
                for (i = 0; i < aItems.length; ++i) {
                    aItems[i].destroy();
                }
                aItems.length = 0;
                this.setNewContentAvailable(true);
                for (i = 0; i < aBindingContexts.length; i++) {
                    addNewItem(aBindingContexts[i]);
                }
            }

            // Bind as many context as possible to existing elements.
