    ...
    return BaseObject.extend("sap.ovp.ui.UIActions", {
        constructor: function (cfg) {
            ...
            this.init = function (cfg) {
                ...
                this.wrapper = cfg.wrapperSelector
                    ? document.querySelector(cfg.wrapperSelector)
                    : this.container.parentNode;
                this.clickCallback = typeof cfg.clickCallback === "function" ? cfg.clickCallback : this.noop;
                this.beforeDragCallback =
                    typeof cfg.beforeDragCallback === "function" ? cfg.beforeDragCallback : this.noop;
                this.doubleTapCallback =
                    typeof cfg.doubleTapCallback === "function" ? cfg.doubleTapCallback : this.noop;
                ...
                this.dragStartCallback =
                    typeof cfg.dragStartCallback === "function" ? cfg.dragStartCallback : this.noop;
                this.dragMoveCallback =
                    typeof cfg.dragMoveCallback === "function" ? cfg.dragMoveCallback : this.noop;
                ...
                this.resizeStartCallback =
                    typeof cfg.resizeStartCallback === "function" ? cfg.resizeStartCallback : this.noop;
                this.resizeMoveCallback =
                    typeof cfg.resizeMoveCallback === "function" ? cfg.resizeMoveCallback : this.noop;
                this.resizeEndCallback =
                    typeof cfg.resizeEndCallback === "function" ? cfg.resizeEndCallback : this.noop;
            ...
            this.insertBefore = function (scope, item, referenceItem) {
                var itemIndex, referenceItemIndex, splice;
            ...
            this.captureStart = function (evt) {
                ...
                if (evt.type === "touchstart" && evt.touches.length === 1) {
                    ...
                    eventObj = evt;
                    if (evt.which !== 1) {
                        //Only LEFT click operation is enabled. Otherwise do nothing.
                        return;
                    }
                }
                ...
                if (eventObj) {
                    ...
                    if (this.element) {
                        ...
                        if (this.layoutDetails) {
                            var rightX = this.isRTLEnabled
                                ? this.startX - elemLeft < this.resizeHandleDistance
                                : elemLeft + elemWidth - this.startX < this.resizeHandleDistance;
            ...
            this.moveHandler = function (evt) {
                ...
                switch (this.mode) {
                    case "normal":
                        if (
                            Math.abs(this.startX - this.moveX) > this.moveTolerance ||
                            Math.abs(this.startY - this.moveY) > this.moveTolerance
                        ) {
                            if (evt.type === "touchmove") {
                                ...
                                //In desktop start dragging immediately
            ...
            this.clickHandler = function (event) {
                ...
                var vizTooltipContainerID =
                    vizTooltipContainer && vizTooltipContainer[0] && vizTooltipContainer[0].id;
                var vizTooltip = CoreElement.getElementById(vizTooltipContainerID);
                if (vizTooltip) {
            ...
            this.endHandler = function (evt) {
                this.log("endHandler");
                if (
                    (evt.type == this.keyUpEvent && evt.which == KeyCodes.ESCAPE) ||
                    evt.type == this.mouseUpEvent ||
                    evt.type == this.touchEndEvent
                ) {
                    this.captureEnd(evt);
                    switch (this.mode) {
                        case "normal":
