...
sap.ui.define([
    ...
    "use strict";
    var oLogger = new OVPLogger("OVP.ui.DashboardLayoutRearrange");
    ...
    Rearrange.prototype._resizeStartHandler = function (evt, cardElement) {
        ...
        // Store all the cards configuration when resize starts
        this.layoutUtil.startResizeCardConfig = merge([], this.layoutUtil.getCards());
    ...
    Rearrange.prototype._resizeEndHandler = function (evt, cardElement) {
        if (cardElement) {
            ...
            if (
                this.uiActions.isResizeX &&
                !this.uiActions.isResizeY &&
                cardElement.classList.contains("sapOvpNotResizableLeftRight")
            ) {
                return;
            }
            this.updatedScrollTop = 0;
            ...
            if (oCard) {
                ...
                this.layoutUtil.dragOrResizeChanges = [];
                // push all cards config to dragOrResizeChanges changes, this will later be filtered by layoutChanged method in main controller
                var cardsConfig = this.layoutUtil.getCards();
                var startResizeCardConfig = this.layoutUtil.startResizeCardConfig;
                for (var i = 0; i < cardsConfig.length; i++) {
                    var cardConfigAfterResize = cardsConfig[i];
                    var aCardConfigBeforeResize = filterCardsById(startResizeCardConfig, cardConfigAfterResize.id);
                    var oCardConfigBeforeResize = aCardConfigBeforeResize[0];
                    this.layoutUtil.dragOrResizeChanges.push({
                        changeType: "dragOrResize",
                        content: {
                            cardId: cardConfigAfterResize.id,
                            dashboardLayout: {
                                row: cardConfigAfterResize.dashboardLayout.row,
                                oldRow: oCardConfigBeforeResize.dashboardLayout.row,
                                column: cardConfigAfterResize.dashboardLayout.column,
                                oldColumn: oCardConfigBeforeResize.dashboardLayout.column,
                                rowSpan: cardConfigAfterResize.dashboardLayout.rowSpan,
                                oldRowSpan: oCardConfigBeforeResize.dashboardLayout.rowSpan,
                                colSpan: cardConfigAfterResize.dashboardLayout.colSpan,
                                oldColSpan: oCardConfigBeforeResize.dashboardLayout.colSpan,
                                maxColSpan: cardConfigAfterResize.dashboardLayout.maxColSpan,
                                oldMaxColSpan: oCardConfigBeforeResize.dashboardLayout.maxColSpan,
                                noOfItems: cardConfigAfterResize.dashboardLayout.noOfItems,
                                oldNoOfItems: oCardConfigBeforeResize.dashboardLayout.noOfItems,
                                autoSpan: cardConfigAfterResize.dashboardLayout.autoSpan,
                                oldAutoSpan: oCardConfigBeforeResize.dashboardLayout.autoSpan,
                                showOnlyHeader: cardConfigAfterResize.dashboardLayout.showOnlyHeader,
                                oldShowOnlyHeader: oCardConfigBeforeResize.dashboardLayout.showOnlyHeader
                            }
                        },
            ...
            if (Device.system.desktop) {
                document
                    .querySelector("body")
                    .classList.remove("sapOVPDisableUserSelect", "sapOVPDisableImageDrag");
            }
            jQuery(this.settings.wrapper).removeClass("dragAndDropMode");
            jUtils.removeElementById("sapOvpOverlayDivForCursor"); // id identifier (#) not required
            jUtils.removeElementById("ovpResizeRubberBand"); // id identifier (#) not required
            ...
            this.layoutUtil.setAriaPos();
            this.layoutUtil.startResizeCardConfig = [];
            if (window.getSelection) {
                var selection = window.getSelection();
                selection.removeAllRanges();
            }
    ...
    Rearrange.prototype._resizeMoveHandler = function (actionObject) {
        ...
        if (actionObject.element) {
            var cardDetails,
                ...
                ghostWidth,
                ghostHeight,
                oCard = this.layoutUtil.dashboardLayoutModel.getCardById(
                    this.layoutUtil.getCardId(actionObject.element.id)
                );
            ...
            if (actionObject.evt.clientY - viewElement.offsetTop + this.SCROLL_OFFSET > viewHeight) {
                ...
                actionObject.evt.clientY - viewElement.offsetTop < viewRect.top + this.SCROLL_OFFSET &&
            ...
            if (
                cardDetails.ghostWidthCursor <= this.layoutUtil.getColWidthPx() &&
                this.uiActions.isResizeX &&
                !this.uiActions.isResizeY
            ) {
                return;
            ...
            ghostWidth =
                cardDetails.ghostWidthCursor <= this.layoutUtil.getColWidthPx()
                    ? this.layoutUtil.getColWidthPx()
                    : cardDetails.ghostWidthCursor;
            if (!this.uiActions.isResizeY) {
                //If the resize is done only in X-direction
                ...
                if (
                    actionObject.element.classList.contains("sapOvpNotResizableLeftRight") ||
                    (actionObject.element.classList.contains("sapOvpNotResizableRight") &&
                        cardDetails.ghostWidthCursor >
                        oCard.dashboardLayout.colSpan * this.layoutUtil.getColWidthPx())
                ) {
                    return;
                } else {
                    oLogger.info("Not a valid scenario");
                }
                //For list card user can not resize more than two columns
                if (
                    ((oCard.template === "sap.ovp.cards.list" || oCard.template === "sap.ovp.cards.v4.list") &&
                        ghostWidth > this.layoutUtil.getColWidthPx() * 2) ||
                    ((oCard.template === "sap.ovp.cards.linklist" ||
                        oCard.template === "sap.ovp.cards.v4.linklist") &&
                        oCard.settings.listFlavor === "carousel" &&
            ...
            if (
                (oCard.template === "sap.ovp.cards.linklist" || oCard.template === "sap.ovp.cards.v4.linklist") &&
                oCard.settings.listFlavor === "carousel" &&
            ...
            if (ghostHeight <= leastHeight) {
                ...
                if (
                    !this.uiActions.isResizeX &&
                    (oCard.template === "sap.ovp.cards.list" ||
                        oCard.template === "sap.ovp.cards.v4.list" ||
                        oCard.template === "sap.ovp.cards.table" ||
                        oCard.template === "sap.ovp.cards.v4.table")
                ) {
                    var iContentWithoutHeader =
                        cardSizeProperties.headerHeight +
                        cardSizeProperties.dropDownHeight +
                        2 * this.layoutUtil.CARD_BORDER_PX;
                    var iPredictedNoOfItems = Math.round(
                        (ghostHeight - iContentWithoutHeader) / cardSizeProperties.itemHeight
                    );
            ...
            if (this.resizeData.colSpan && this.resizeData.rowSpan) {
                ...
                this.layoutUtil.resizeCard(
                    actionObject.element.getAttribute("id"),
                    this.resizeData,
                    this.layoutUtil.dragOrResizeChanges
                );
            }
    ...
    Rearrange.prototype._dragStartHandler = function (evt, cardElement) {
        ...
        if (Device.system.desktop) {
            document.querySelector("body").classList.add("sapOVPDisableUserSelect", "sapOVPDisableImageDrag");
        }
        ...
        // Store all the cards configuration when dragging starts
        this.layoutUtil.dragStartCardsConfig = merge([], this.layoutUtil.getCards(this.columnCount));
        ...
        var oCardRect = cardElement && cardElement.children[0].getBoundingClientRect();
        this.floaterData = {
    ...
    Rearrange.prototype._dragMoveHandler = function (actionObject) {
        if (actionObject.element) {
            ...
            if (document.getElementsByClassName("sapUshellShellHeadSearchContainer")[0]) {
                uShellHeadHeight = document.getElementsByClassName("sapUshellShellHeadSearchContainer")[0]
                    .offsetHeight;
            }
            var viewHeight = viewElement.offsetHeight;
            var viewRect = viewElement.getBoundingClientRect();
            ...
            this.floaterData.top =
                actionObject.clone.getBoundingClientRect().top +
                updatedScrollTop -
                (viewTitleHeight + mainHeaderWrapperHeight + uShellHeadHeight);
            ...
            var oCard = this.layoutUtil.dashboardLayoutModel.getCardById(
                this.layoutUtil.getCardId(this.floaterData.id)
            );
            ...
            jQuery
                .when(
                    this.layoutUtil.dashboardLayoutModel._arrangeCards(
                        oCard,
                        this.floaterData,
                        "drag",
                        this.layoutUtil.dragOrResizeChanges
    ...
    Rearrange.prototype._dragEndHandler = function (evt, cardElement, cloneObj) {
        if (cardElement) {
            ...
            cloneObj.style[this.layoutUtil.cssVendorTransform] =
            ...
            jQuery(cloneObj).one(
                ...
                function (event) {
                    //Save all the layout changes to LREP
                    jUtils.removeElementById("ovpDashboardLayoutMarker"); //remove insert marker
                    this.layoutUtil.getDashboardLayoutModel().extractCurrentLayoutVariant();
                    this.layoutUtil.dragOrResizeChanges = [];
                    // push all cards config to dragOrResizeChanges changes, this will later be filtered by layoutChanged method in main controller
                    var cardsConfig = this.layoutUtil.getCards();
                    var dragStartCardsConfig = this.layoutUtil.dragStartCardsConfig;
                    for (var i = 0; i < cardsConfig.length; i++) {
                        var cardConfigAfterDrag = cardsConfig[i];
                        var aCardConfigBeforeDrag = filterCardsById(dragStartCardsConfig, cardConfigAfterDrag.id);
                        var oCardConfigBeforeDrag = aCardConfigBeforeDrag[0];
                        this.layoutUtil.dragOrResizeChanges.push({
                            changeType: "dragOrResize",
                            content: {
                                cardId: cardConfigAfterDrag.id,
                                dashboardLayout: {
                                    row: cardConfigAfterDrag.dashboardLayout.row,
                                    oldRow: oCardConfigBeforeDrag.dashboardLayout.row,
                                    column: cardConfigAfterDrag.dashboardLayout.column,
                                    oldColumn: oCardConfigBeforeDrag.dashboardLayout.column,
                                    rowSpan: cardConfigAfterDrag.dashboardLayout.rowSpan,
                                    oldRowSpan: oCardConfigBeforeDrag.dashboardLayout.rowSpan,
                                    colSpan: cardConfigAfterDrag.dashboardLayout.colSpan,
                                    oldColSpan: oCardConfigBeforeDrag.dashboardLayout.colSpan,
                                    maxColSpan: cardConfigAfterDrag.dashboardLayout.maxColSpan,
                                    oldMaxColSpan: oCardConfigBeforeDrag.dashboardLayout.maxColSpan,
                                    noOfItems: cardConfigAfterDrag.dashboardLayout.noOfItems,
                                    oldNoOfItems: oCardConfigBeforeDrag.dashboardLayout.noOfItems,
                                    autoSpan: cardConfigAfterDrag.dashboardLayout.autoSpan,
                                    oldAutoSpan: oCardConfigBeforeDrag.dashboardLayout.autoSpan,
                                    showOnlyHeader: cardConfigAfterDrag.dashboardLayout.showOnlyHeader,
                                    oldShowOnlyHeader: oCardConfigBeforeDrag.dashboardLayout.showOnlyHeader
                                }
                            },
                            isUserDependent: true
                        });
                    ...
                    this.layoutUtil.oLayoutCtrl.fireAfterDragEnds({
                        positionChanges: this.layoutUtil.dragOrResizeChanges
                    });
                    // Cleanup added classes and styles before drag
                    if (Device.system.desktop) {
                        document.body.classList.remove("sapOVPDisableUserSelect");
                        document.body.classList.remove("sapOVPDisableImageDrag");
                    }
                    jQuery(this.settings.wrapper).removeClass("dragAndDropMode");
                    if (window.getSelection) {
                        var selection = window.getSelection();
                    ...
                    var iContainerHeight = this.layoutUtil.dashboardLayoutModel._findHighestOccupiedRow() + 32;
                    jUtils.addStyleToAllElements(
                        document.querySelectorAll(".sapUshellEasyScanLayoutInner"),
                        "height",
                        iContainerHeight + "px"
                    );
                    jUtils.addStyleToAllElements(
                    ...
                    jUtils.addStyleToAllElements(
                        document.querySelectorAll(".sapUshellEasyScanLayout"),
                        "height",
                        iContainerHeight + "px"
                    );
                    this.layoutUtil.setAriaPos();
                    this.layoutUtil.dragStartCardsConfig = [];
                }.bind(this)
            );
        }
    };
    ...
    Rearrange.prototype.initCardsSettings = function () {
        ...
        this.jqLayoutInner = this.jqLayout && this.jqLayout.children().first();
        var layoutScroll = this.jqLayout.scrollTop();
        var layoutHeight = this.jqLayoutInner.height();
        this.isRTLEnabled = Localization.getRTL();
        this.aCardsOrder = [];
        ...
        this.columnCount = this.layoutUtil.dashboardLayoutModel.getColCount();
        ...
        var jqFirstColumn = this.jqLayoutInner && this.jqLayoutInner.children().first();
    ...
    Rearrange.prototype.drawLayout = function (aCardsLayout) {
        function updateCSS(domElement) {
            ...
            domElement.style[this.layoutUtil.cssVendorTransform] =
                "translate3d(" + $element.left + "," + $element.top + ", 0px) ";
        }
    ...
    Rearrange.prototype.showGhostWhileDragMove = function (newCardPosition, actionObject) {
        var element = document.getElementById("ovpDashboardLayoutMarker"),
            ...
            pos = {
                ...
                left: this.isRTLEnabled
                    ? -iColumnValue - this.layoutUtil.CARD_BORDER_PX
                    : iColumnValue + this.layoutUtil.CARD_BORDER_PX
            };
        if (!element) {
            ...
            oDiv.style[this.layoutUtil.cssVendorTransform] =
                "translate3d(" + pos.left + "px," + pos.top + "px, 0px) ";
            ...
            element.style[this.layoutUtil.cssVendorTransform] =
                "translate3d(" + pos.left + "px," + pos.top + "px, 0px) ";
        }
        actionObject.element.style[this.layoutUtil.cssVendorTransition] = "all 300ms ease";
        actionObject.element.style[this.layoutUtil.cssVendorTransform] =
            "translate3d(" + pos.left + "px," + pos.top + "px, 0px) ";
    };
    ...
    Rearrange.prototype.showGhostWhileResize = function (actionObject, oCard) {
        var element = document.getElementById("ovpResizeRubberBand"),
            ...
            pos = {
                top:
                    (oCard.dashboardLayout.row - 1) * this.layoutUtil.getRowHeightPx() +
                    this.layoutUtil.CARD_BORDER_PX,
                left: this.isRTLEnabled
                    ? -iColumnValue - this.layoutUtil.CARD_BORDER_PX
                    : iColumnValue + this.layoutUtil.CARD_BORDER_PX
            },
            height =
                this.resizeData.rowSpan * this.layoutUtil.getRowHeightPx() - 2 * this.layoutUtil.CARD_BORDER_PX + 2,
            width =
                this.resizeData.colSpan * this.layoutUtil.getColWidthPx() - 2 * this.layoutUtil.CARD_BORDER_PX + 2;
        if (!element) {
    ...
    Rearrange.prototype._calculateMinimumCardHeight = function (actionObject) {
        ...
        if (this.uiActions.isResizeX && !this.uiActions.isResizeY) {
            ghostWidthCursor = this.isRTLEnabled
                ? iCardRight - actionObject.moveX
                : actionObject.moveX - $elemPosition.left;
    ...
    function filterCardsById(cardsConfig, cardId) {
        return cardsConfig.filter(function (item) {
