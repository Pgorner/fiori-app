...
sap.ui.define([
    ...
    var oLogger = new OVPLogger("OVP.stack.Stack");
    var LinkAccessibleRole = SapMLibrary.LinkAccessibleRole;
    ...
    return CardController.extend("sap.ovp.cards.stack.Stack", {
        onInit: function () {
            ...
            this.initialLoadCardLength = 10;
            this.quickViewCardCount = 0;
        ...
        onAfterRendering: function () {
            ...
            this.ovpLayout = CoreElement.getElementById(this.oCardComponent._sOwnerId + "---mainView--ovpLayout");
            /**
            ...
            if (Device.system.phone) {
                ...
                if (!this.bAfterColumnUpdateAttached) {
                    ...
                    if (
                        oCompData &&
                        oCompData.mainComponent &&
                        oCompData.appComponent.oOvpConfig &&
                        oCompData.appComponent.oOvpConfig.containerLayout !== "resizable"
                    ) {
                        // For Fixed Layout
                        var oMainComponent = oCompData.mainComponent,
                            oLayout = oMainComponent.byId("ovpLayout");
                        oLayout.attachAfterColumnUpdate(
                            function (oEvent) {
            ...
            if (this.oObjectStream) {
                ...
                oListBinding.attachDataRequested(function () {
                    ...
                    if (this.getView().byId("stackSize") !== undefined && this.getView().byId("stackTotalSize") !== undefined) {
                        ...
                        // this.getView().byId('stackSize').getDomRef().style.visibility = 'hidden';
                        jQuery(this.getView().byId("stackTotalSize").getDomRef()).css("visibility", "hidden");
                        // this.getView().byId('stackTotalSize').getDomRef().style.visibility = 'hidden';
                    }
                }, this);
                ...
                oListBinding.attachDataReceived(function () {
                    ...
                    jQuery(stackContentDomRef).attr(
                        "aria-label",
                        OvpResources.getText("stackCardContent", [nCardCount, nTotalCardCount, category])
                    );
                    // stackContentDomRef.setAttribute("aria-label", OvpResources.getText("stackCardContent", [nCardCount, nTotalCardCount, category]));
                    ...
                    // stackSizeDomRef.setAttribute("aria-label", OvpResources.getText("stackCard", [nCardCount]));
                    ...
                    if (this.getView().byId("stackSize") !== undefined && this.getView().byId("stackTotalSize") !== undefined) {
                        ...
                        // this.getView().byId('stackSize').getDomRef().style.visibility = 'visible';
                        jQuery(this.getView().byId("stackTotalSize").getDomRef()).css("visibility", "visible");
                        // this.getView().byId('stackTotalSize').getDomRef().style.visibility = 'visible';
                    }
                ...
                if (NavigationHelper.checkNavigation(this.getModel(), this.getEntityType(), this.getCardPropertiesModel())) {
        ...
        getCardItemsBinding: function () {
            return this && this.oObjectStream && this.oObjectStream.getBinding("content");
        },
        ...
        _createObjectStream: function () {
            ...
            if (this.bStackFlavorAssociation) {
                if (sObjectStreamCardsTemplate === "sap.ovp.cards.quickview") {
                    oLogger.error(
                        "objectStreamCardsTemplate cannot be 'sap.ovp.cards.quickview' when objectStreamCardsNavigationProperty is provided"
                    );
                    this.bSetErrorState = true;
                    //this.setErrorState();
                    return;
                }
                this.oStackFilterMapping = this._determineFilterPropertyId(
                    this.oModel,
                    oEntitySet,
                    oEntityType,
                    sObjectStreamCardsNavigationProperty
                );
                oObjectStreamCardsSettings.entitySet = this.oModel
                    .getMetaModel()
                    .getODataAssociationSetEnd(oEntityType, sObjectStreamCardsNavigationProperty).entitySet;
            } else {
                if (sObjectStreamCardsTemplate !== "sap.ovp.cards.quickview") {
                    oLogger.error(
                        "objectStreamCardsTemplate must be 'sap.ovp.cards.quickview' when objectStreamCardsNavigationProperty is not provided"
                    );
                    this.bSetErrorState = true;
                    //this.setErrorState();
                    return;
            ...
            oBindingInfo.clearQuickViewCard = function () {
                this.quickViewCardContextArray = [];
            }.bind(this);
            ...
            oObjectStreamTitle.addCustomData(
                new CustomData({
                    key: "tabindex",
                    value: "0",
                    writeToDom: true
        ...
        _renderQuickViewCards: function (sId, oContext, oComponentContainer) {
            return new Promise(
                function (resolve, reject) {
                    var oSettings = this.oCardPropsModel.getProperty("/objectStreamCardsSettings"),
                        oFilters;
                    if (this.bStackFlavorAssociation) {
                        oFilters = {
                            filters: [
                        ...
                        oSettings = merge({}, oFilters, this.oObjectStreamCardsSettings);
                    }
                    //                    var oComponentContainer = new ComponentContainer();
                    ...
                    UIComponent.create(oComponentConfig).then(function (oComponent) {
        ...
        _determineFilterPropertyId: function (oModel, oEntitySet, oEntityType, sNavigationProperty) {
            ...
            var oRefs = oAssociation.referentialConstraint,
                filterMapping = {};
            if (oRefs) {
        ...
        _openStack: function () {
            ...
            if (this.oObjectStream.getNewContentAvailable()) {
                if (this.quickViewCardContextArray.length > 10) {
                    var quickViewCardPromiseArray = [];
                    for (var i = this.initialLoadCardLength; i < this.quickViewCardContextArray.length; i++) {
                        quickViewCardPromiseArray.push(
                            this._renderQuickViewCards(
                                this.quickViewCardContextArray[i].sId,
                                this.quickViewCardContextArray[i].oContext,
                                this.quickViewCardContextArray[i].oComponentContainer
                            )
                        );
                    }
                }
                this.oObjectStream.setNewContentAvailable(false);
            }
        },
        ...
        openStack: function () {
            ...
            if (this.oObjectStream.getNewContentAvailable()) {
                var quickViewCardPromiseArray = [];
                var cardsInitiallyLoaded =
                    this.quickViewCardContextArray.length <= this.initialLoadCardLength
                        ? this.quickViewCardContextArray.length
                        : this.initialLoadCardLength;
                for (var i = this.quickViewCardCount; i < cardsInitiallyLoaded; i++) {
                    quickViewCardPromiseArray.push(
                        this._renderQuickViewCards(
                            this.quickViewCardContextArray[i].sId,
                            this.quickViewCardContextArray[i].oContext,
