...
sap.ui.define([
    ...
    var oLogger = new OVPLogger("OVP.cards.OVPCardsAsAPIUtils");
    var supportedCards = [
        "sap.ovp.cards.list",
        "sap.ovp.cards.table",
        ...
        "sap.ovp.cards.v4.charts.smart.chart",
        "sap.ovp.cards.v4.charts.bubble",
        "sap.ovp.cards.v4.charts.donut",
        "sap.ovp.cards.v4.charts.line",
        "sap.ovp.cards.error"
    ];
    ...
    function errorHandlingForAPI(sMessage, reject, oError) {
        ...
        oLogger.error(oErrorObject.message);
        reject(oErrorObject);
    }
    ...
    function recreateCard(oCardProperties, oComponentData) {
        ...
        if (oCard) {
            ...
            oOVPCardAsAPIUtils.createCardComponent(
                oComponentData.parentView,
                oManifest,
                oComponentData.containerId,
                oComponentData.selectionVariant
            );
        }
    }
    ...
    function _checkIfModelExists(oCardManifest) {
        ...
        if (
            oCardManifest.template === "sap.ovp.cards.error" ||
            ((oCardManifest.template === "sap.ovp.cards.linklist" ||
                oCardManifest.template === "sap.ovp.cards.v4.linklist") &&
                oCardManifest.settings.staticContent)
        ) {
            bCheckIfModel = false;
        }
    ...
    function _filterModel(oCardManifest, oSelectionVariant) {
        ...
        for (i = 0; i < aSelectOptionPropertyNames.length; i++) {
            var aSelectOption = oSelectionVariant.getSelectOption(
                aSelectOptionPropertyNames[i]
            );
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (!!oManifest) {
                ...
                if (!!oCardManifest && !!cardId) {
                    if (
                        !!oCardManifest.template &&
                        !!oCardManifest.settings &&
                        (!!oCardManifest.model || !_checkIfModelExists(oCardManifest))
                    ) {
                        /*
                        ...
                        if (supportedCards.indexOf(oCardManifest.template) !== -1) {
                            if (!!sContainerId && typeof sContainerId === "string") {
                                if (!!oView) {
                                    ...
                                    if (!_checkIfModelExists(oCardManifest)) {
                                        ...
                                        if (
                                            oModel &&
                                            oModel.getODataVersion &&
                                            oModel.getODataVersion() == "4.0"
                                        ) {
                                            getMetaModelPromise = oModel
                                                .getMetaModel()
                                                .requestObject(
                                                    "/" + oCardManifest.settings.entitySet
                                                );
                                        } else {
                                            getMetaModelPromise = oModel
                                                .getMetaModel()
                                    ...
                                    getMetaModelPromise.then(
                                        function () {
                                            var bIgnoreSelectionVariant = false;
                                            if (oSelectionVariant) {
                                                oCardManifest = _filterModel(
                                                    oCardManifest,
                                                    oSelectionVariant
                                                );
                                            ...
                                            CoreComponent.create(oComponentConfig).then(
                                                function (oComponent) {
                                                    ...
                                                    if (!!oComponentContainer) {
                                                        ...
                                                        sMessage =
                                                            "Component Container '" +
                                                            sContainerId +
                                                            "' is not present in the current View";
                            ...
                            sMessage =
                                oCardManifest.template +
                                " card type is not supported in the API";
                            errorHandlingForAPI(sMessage, reject);
                        }
                    } else {
