...
sap.ui.define([
    ...
    /*
     * Module for drawing background shapes and performing animations on user events
     ...
     * Then, the cubic bezier curve is drawn by canvas, while each pair of anchors are passed as start and end points and their corresponding control points are passed as control points.
     */
    ...
    manager.prototype = {
        ...
        _getCanvas: function () {
            var canvas = document.getElementById("shell-shapes");
            if (canvas) {
                return canvas;
            }
            return this._createCanvas("shell-shapes");
        },

        _createCanvas: function (sId) {
            var oCanvas = document.createElement("canvas");
            oCanvas.setAttribute("id", sId);
            oCanvas.setAttribute("role", "presentation");

            this._updateCanvasDimensions(oCanvas);

            Container.getRendererInternal().setShellShapes(oCanvas);
            return oCanvas;
        },

        _updateCanvasDimensions: function (oCanvas) {
            oCanvas.height = (window.innerHeight > 0) ? window.innerHeight : screen.height + "px";
            oCanvas.width = (window.innerWidth > 0) ? window.innerWidth : screen.width + "px";
        },
        ...
        onThemeChanged: async function () {
            const [sShapesColor] = await ushellUtils.getThemingParameters(["sapUiShellBackgroundPatternColor"]);
            if (sShapesColor && sShapesColor !== "transparent") { // new themes have transparent shapes
        ...
        resizeHandler: function () {
            ...
            if (canvas) {
                this._updateCanvasDimensions(canvas);
                this.bIsDirty = true;
                this.onThemeChanged();
            }
        },
        ...
        drawShapes: async function () {
            ...
            if (this.bIsDirty && canvas && canvas.getContext) {
                ...
                if (this.sShapesColor && this.sShapesColor !== "transparent") { // new themes have transparent shapes
