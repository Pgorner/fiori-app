...
sap.ui.define([
    ...
    utils.getLocalSystemInSidFormat = function () {
        ...
        var oSystem = oContainer.getLogonSystem();
        var sSystemName = oSystem.getSystemName();
    ...
    utils.isSAPLegacyApplicationType = function (sType, sFramework, bGuiVariantsOnly) {
        var arrTypes = bGuiVariantsOnly === true ? ["TR", "GUI", "NWBC"] : ["TR", "GUI", "NWBC", "WDA", "WCF"];
    ...
    utils.getParamKeys = function (sUrl) {
        ...
        if (sUrl.indexOf("sap-xapp-state=") > 0) {
            ...
            if (aParams && aParams.length === 2) {
                try {
                    var sDecodedParam = decodeURIComponent(aParams[1]);
                    aAppStateNamesArray.push("sap-xapp-state-data");
    ...
    utils.appendSapShellParamSync = function (sUrl, sApplicationType, sVersion) {
        var sUrlSuffix = sApplicationType === "TR" ? "" : "-NWBC";
        if (sVersion) {
    ...
    function _extractUi5Version (sVersion) {
        ...
        var oMatch = /\d+(\.\d+){0,2}/.exec(sVersion);
        if (oMatch && oMatch[0]) {
            return oMatch[0];
        }
        return undefined;
    ...
    utils.getParameterValueBoolean = function (sParameterName, sParams) {
        var oUriParameters = new URLSearchParams(sParams || window.location.search),
    ...
    utils.getVisibleTiles = function () {
        ...
        var oControl = Element.getElementById("dashboardGroups");
        var oNavContainer = Element.getElementById("viewPortContainer");
        var shellHdrHeight = 0;
        var aTiles = [];
        var aVisibleTiles = [];
        ...
        return new Promise(function (resolve) {
            sap.ui.require(["sap/ushell/Container"], function (Container) {
                return Promise.all([
                    ...
                    var oNavigationService = aResults[1];
    ...
    utils.isColdStart = function () {
        ...
        var oRenderer = oContainer.getRendererInternal("fiori2");
