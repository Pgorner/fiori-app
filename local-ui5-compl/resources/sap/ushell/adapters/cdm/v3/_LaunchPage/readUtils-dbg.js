...
sap.ui.define([
    ...
    var oContentProviderLabelPromises = {};
    ...
    readUtils.getVizData = async function (oSite, oVizReference, oSystemContext) {
        ...
        const aCdmParts = this.getCdmParts(oSite, oVizReference);
        const oVizConfig = aCdmParts && aCdmParts[1] || {};
        const oApp = aCdmParts && aCdmParts[3] || {};
        const oViz = readVisualizations.get(oSite, readHome.getTileVizId(oVizReference)) || {};
        const oVizData = {
        ...
        oVizData.contentProviderLabel = await this.getContentProviderLabel(oVizData.contentProviderId);
    ...
    /**
     * Determines the contentProviderLabel based on the contentProviderId.
     *
     * To get the label, the systemContext is required, which is retrieved by the ClientSideTargetResolution.
     * The label is cached by contentProviderId.
     *
     * @param {string} sContentProviderId The contentProviderId
     *
     * @returns {Promise<string>} Returns promise which resolves with the label.
     *
     * @since 1.121
     * @private
     */
    readUtils.getContentProviderLabel = function (sContentProviderId) {
        if (Object.hasOwn(oContentProviderLabelPromises, sContentProviderId)) {
            return oContentProviderLabelPromises[sContentProviderId];
        }

        oContentProviderLabelPromises[sContentProviderId] = new Promise(async (resolve) => {
            try {
                const oCSTR = await Container.getServiceAsync("ClientSideTargetResolution");
                const oSystemContext = await oCSTR.getSystemContext(sContentProviderId);
                resolve(oSystemContext?.label);
            } catch (oError) {
                resolve(sContentProviderId);
            }
        });
     ...
     * Additionally, the flat object format is supported.
     *
     ...
     * </pre>
     *
     * Example for the flat object format:
     * <pre>
     *  {
     *      a: {
     *          format: "plain",
    ...
    readUtils.harmonizeTarget = function (oTarget) {
        var vParameters = oTarget.parameters;
        var oParametersOut = {};
        ...
        if (Array.isArray(vParameters)) {
            // Array structure
            vParameters.forEach(function (oParam) {
            ...
            // Object structure
            oParametersOut = Object.keys(vParameters).reduce((oParameters, sKey) => {
                if (!vParameters[sKey].value || !vParameters[sKey].value.hasOwnProperty("value")) {
                    // Flat object structure
                    vParameters[sKey] = {
                        value: vParameters[sKey]
                    };
                }

                return {
                    ...oParameters,
                    [sKey]: vParameters[sKey]
                };
            }, {});
        }
