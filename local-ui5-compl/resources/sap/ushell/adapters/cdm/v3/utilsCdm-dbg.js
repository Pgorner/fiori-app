...
sap.ui.define([
     ...
     * @returns {object[]} Inbounds suitable for ClientSideTargetResolution service consumption.
     */
     ...
     * @param {string} [sAppId] The application id of the app the inbound belongs to
     * @returns {string} The constructed hash without leading '#' or undefined if not successful
     */
    utilsCdm.toHashFromInbound = function (oInbound, sAppId) {
        var oShellHash = {
            target: {
        ...
        if (sAppId) {
            oShellHash.params["sap-ui-app-id-hint"] = sAppId;
        }
    ...
    /**
     * Constructs the hash for a targetintent object consisting of semanticObject and action.
     * Additionally, parameters get formatted.
     *
     * @param {object} oTargetIntentData The targetIntent object of a vizData instance for which the hash should be constructed
     * @param {object} oVizTarget The parameter object of a vizData instance for which the hash should be constructed
     * @returns {string} Constructed hash or <code>undefined</code> in case something went wrong
     * @private
     * @since 1.126.0
     */
    utilsCdm.toHashFromTargetIntent = function (oTargetIntentData, oVizTarget) {
        try {
            if (oVizTarget.type === "URL") {
                // Navigation with an external URL
                return oVizTarget.url;
            ...
            var oTarget = {};
            oTarget.semanticObject = oTargetIntentData.semanticObject;
            oTarget.action = oTargetIntentData.action;
            oTarget.parameters = oVizTarget.parameters || {};
            oTarget.parameters["sap-ui-app-id-hint"] = {
                value: {
                    value: oTargetIntentData.businessAppId,
                    format: "plain"
                }
            };
            oTarget.appSpecificRoute = oVizTarget.appSpecificRoute || "";
