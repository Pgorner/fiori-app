...
sap.ui.define([
    ...
    // shortcut for InvisibleMessageMode
     ...
     * @class
     * @classdesc Constructor for a new Page.
    ...
    Page.prototype._saveFocus = function (oEvent) {
        ...
        // Section Control
        if (oSrcControl.isA("sap.ushell.ui.launchpad.Section")) {
            if (oSrcControl) {
                // Check whether the focused section changed
                if (oSrcControl.indexOfVisualization(this._oLastFocusedViz) === -1) {
                    this._oLastFocusedViz = oSrcControl.getVisualizations()[0];
                    // this is only set if the focused section is empty
                    this._oLastFocusedSection = this._oLastFocusedViz ? undefined : oSrcControl;
                }
            } else {
