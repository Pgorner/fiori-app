...
sap.ui.define([
    ...
    ShellHeaderRenderer.render = function (rm, shellHeader) {
        ...
        this.renderBackButton(rm, shellHeader);
    ...
    ShellHeaderRenderer.renderNewExperienceSwitch = function (rm, shellHeader) {
        var oNewExperienceSwitch = shellHeader.getNewExperienceSwitchControl();
        if (oNewExperienceSwitch) {
            ...
            rm.class("sapUshellShellNewExperienceSwitch");
            rm.openEnd(); // div - tag
    ...
    ShellHeaderRenderer.renderHeaderItems = function (rm, oHeader) {
        ...
        aItems.forEach(function (oItem) {
            if (oItem.getId() !== "backBtn") {
                rm.renderControl(oItem);
            }
        });
    };

    ShellHeaderRenderer.renderBackButton = function (rm, oHeader) {
        var aItems = oHeader.getHeadItems();
        aItems.forEach(function (oItem) {
            // Ensure to only render the backButton
            if (oItem.getId() === "backBtn") {
                rm.renderControl(oItem);
            }
        });
