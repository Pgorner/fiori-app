...
sap.ui.define([
    ...
    var User = function (oSettings) {
        ...
        this.getInitials = function () {
            var sFullName = oSettings.fullName || "";
            var sInitials = "";
            var bNext = true;
        ...
        this.applyTheme = function (sNewTheme) {
            ...
            const _applyTheme = Core["applyTheme"];
            if (!_applyTheme) { // ui5 v.2
                sap.ui.require("sap/ui/core/Theming").setTheme(oTheme.theme);
            } else if (oTheme.suppliedRoot) { // Keep the legacy code until Workzone/cFLP adds all theme roots to sap-ui-config correctly
                _applyTheme(oTheme.theme, oTheme.suppliedRoot + "/UI5/"); // the supplied path is the first choice
            } else if (oTheme.path) {
                _applyTheme(oTheme.theme, oTheme.path + "/UI5/"); // second choice is the amended path
            } else {
                _applyTheme(oTheme.theme); // sap_ theme from the front-end server
            }
            ...
            sap.ui.require(["sap/ushell/components/applicationIntegration/application/PostMessageUtils"], function (PostMessageUtils) {
                PostMessageUtils.postMessageToMultipleIframes("sap.ushell.appRuntime", "themeChange", {
        ...
        oBootTheme = oSettings.bootTheme || { theme: "", root: "" };
