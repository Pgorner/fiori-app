...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.renderer.allMyApps.AllMyApps", {
        ...
        onInit: function () {
            ...
            oView.setModel(ushellResources.getTranslationModel(), "i18n");
        ...
        handleSwitchToMasterAreaOnPhone: function () {
            ...
            var sPathArray = sSelectedPath.split("/");
        ...
        handleMasterListItemPress: function (event) {
            ...
            var oAllMyAppsLevel = this._getConfigModel().getProperty("/allMyAppsMasterLevel");
        ...
        handleMasterListItemPressToSecondLevel: function (sClickedDataSourcePath) {
            ...
            var oBindingContext = this._setBindingContext(sClickedDataSourcePath + "/groups/0", oView.byId("oItemsContainerlist"));
            oDataSourceList.invalidate();

            // Selecting the first item in the list and setting the focus
            oDataSourceList.setSelectedItem(oDataSourceList.getItems()[0]);
            var oSelectedItem = oDataSourceList.getSelectedItem();
            if (oSelectedItem) {
                oSelectedItem.focus();
            }
        ...
        handleMasterListItemPressToDetails: function (sClickedDataSourcePath) {
            var oView = this.getView();
            var oAllMyAppsModel = oView.getModel("allMyAppsModel");
            var oBindingContext = this._setBindingContext(sClickedDataSourcePath, oView.byId("oItemsContainerlist"));
        ...
        onMasterLoad: function () {
            var oView = this.getView();
            var oBackButton = this._getPopoverHeaderBackButton();
        ...
        updateMasterFocusAndDetailsContext: function (sChannelId, sEventId, oData) {
            var oView = this.getView();
            var oAllMyAppsModel = oView.getModel("allMyAppsModel");
            var iFirstSelectedDataSourceIndex = this._getInitialFirstLevelSelectionIndex();
            var oDataSourceList = this._getDataSourceList();
            var oCustomPanelLabel = this._getCustomPanelLabel();
            var oCustomPanelLink = this._getCustomPanelLink();
            var oBindingContext;
            ...
            var oSelectedDataSource = oDataSourceList.getSelectedItem();
        ...
        handleCustomPanelLinkPress: function (event) {
            var oView = this.getView();
            var oLink = oView.byId("sapUshellAllMyAppsCustomPanelLink");
            var oData = oLink.getVisible() ? oLink.getBindingContext("allMyAppsModel").getObject() : {};
        ...
        getToggleListButtonVisible: function () {
            ...
            var bIsRangePhone = Device.media.getCurrentRange(Device.media.RANGESETS.SAP_STANDARD).name === "Phone";
            var bVisible = (bIsRangePhone || Device.system.phone) && (oAllMyAppsState === AllMyAppsState.Details);
            return bVisible;
        },

        getBackButtonVisible: function () {
            var sShellAppTitleState = this._getConfigModel().getProperty("/shellAppTitleState");
        ...
        onAppItemClick: function (oPressEvent) {
            ...
            var sClickedItemPath = oPressEvent.getSource().getBindingContext("allMyAppsModel").getPath();
            var sUrl = oAllMyAppsModel.getProperty(sClickedItemPath + "/url");
        ...
        _getInitialFirstLevelSelectionIndex: function () {
            var oView = this.getView();
            var oAllMyAppsModel = oView.getModel("allMyAppsModel");
            var aDataSources = oAllMyAppsModel.getProperty("/AppsData");

            for (var index = 0; index < aDataSources.length; index++) {
                var oTempDataSource = aDataSources[index];
        ...
        _getPopoverHeaderContent: function (iContentIndex) {
            var oCustomHeader = this._getPopoverObject().getCustomHeader();
            var aHeaderLeftContent = oCustomHeader.getContentLeft();
            var oContent = aHeaderLeftContent[iContentIndex];
        ...
        _getPopoverHeaderLabel: function () {
            var oCustomHeader = this._getPopoverObject().getCustomHeader();
            var oContentMiddle = oCustomHeader.getContentMiddle();
        ...
        _setBindingContext: function (sBindingPath, oBoundContainer) {
            var oAllMyAppsModel = this.getView().getModel("allMyAppsModel");
            var oBindingContext = oAllMyAppsModel.createBindingContext(sBindingPath);
