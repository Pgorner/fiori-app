...
sap.ui.define([
    ...
    var SessionHandler = function (AppLifeCycle) {
        ...
        this.init = function (oConfig) {
            ...
            // Sanitize config and set defaults
            if (oConfig.enableAutomaticSignout === undefined) {
                oConfig.enableAutomaticSignout = false; // Default is to not doing automatic logout (kiosk mode)
            }
            if (oConfig.sessionTimeoutReminderInMinutes === undefined) {
                oConfig.sessionTimeoutReminderInMinutes = 0; // Default is to show the session timeout message box
            }
            if (oConfig.sessionTimeoutIntervalInMinutes <= oConfig.sessionTimeoutReminderInMinutes) {
                oConfig.sessionTimeoutReminderInMinutes = (oConfig.sessionTimeoutReminderInMinutes - 1) < 0 ? 0 : oConfig.sessionTimeoutReminderInMinutes - 1;
                Log.error("sessionTimeoutReminderInMinutes needs to be higher than sessionTimeoutIntervalInMinutes. sessionTimeoutReminderInMinutes adapted to: "
                  + oConfig.sessionTimeoutReminderInMinutes, "SessionHandler");
            }
        ...
        this.initLogout = function () {
            if (bLogoutInitialized === false) {
                Container.registerLogout(this.logout);
                this._registerCommHandlers();
        ...
        this._registerCommHandlers = function () {
            var fnExtendSession = () => {
            ...
            AppLifeCycle.registerShellCommunicationHandler({
                "sap.ushell.sessionHandler": {
                    ...
                    oServiceCalls: {
                        notifyUserActive: {
                            executeServiceCallFn: fnExtendSession
                        },
                        extendSessionEvent: {
                            executeServiceCallFn: fnExtendSession
                        }
        ...
        this.handleSessionOver = function () {
            ...
            EventBus.getInstance().publish("launchpad", "sessionTimeout");
        ...
        this.notifyServer = function () {
            ...
            if (timeSinceLastActionInMinutes <= this.oConfig.sessionTimeoutIntervalInMinutes) {
                ...
                PostMessageUtils.postMessageToMultipleIframes("sap.ushell.sessionHandler", "extendSessionEvent", {});
            } else {
        ...
        this._setConnectionActive = function (active) {
            ...
            EventBus.getInstance().publish("launchpad", "setConnectionToServer", { active: active });
        ...
        this._setTilesInvisibleOnHomepage = function () {
            ...
            return new Promise(function (resolve, reject) {
                return Container.getServiceAsync("FlpLaunchPage").then(function (LaunchPageService) {
        ...
        this.createContinueWorkingDialog = function () {
            ...
            if (this.oConfig.enableAutomaticSignout === true) {
                this.oSignOutButton = new Button({
                    ...
                    press: Container.defaultLogout.bind(Container)
                });
         ...
         * @returns {int} Timestamp of current time in seconds
         * @since 1.84.0
         * @private
         */
         ...
         * @returns {int} Timestamp of last user interaction (in seconds)
         * @since 1.84.0
         * @private
         */
         ...
         * @returns {int} Timestamp a session timeout is going to happen (in seconds)
         * @since 1.84.0
         * @private
         */
         ...
         * if no further user interactions should happen from now on
         * @returns {int} timestamp a session timeout warning dialog is going show up (in seconds)
         * @since 1.84.0
         * @private
         */
