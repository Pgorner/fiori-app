...
sap.ui.define([
    ...
    const oFesrEnhancer = {
         ...
         * @since 1.121.0
         */
        shortenWithPostfix: function (vValue, sFormatKey) {
                ...
                if (oFormatter.postfix) {
                    ...
                    if (vValue.endsWith(oFormatter.postfix)) {
                        ...
                        const oPostfixFormatter = {
                            ...oFormatter,
                            maxLength: oFormatter.maxLength - oFormatter.postfix.length
                        };
        ...
        sanitizeEnhancement: function (oEnhancement) {
            return Object.keys(oEnhancement).reduce((oResult, sKey) => {
                ...
                if (aSupportedFields.includes(sKey) && !!vValue) {
                    // shorten if max Length is provided else keep it as it is
                    oResult[sKey] = this.shortenWithPostfix(vValue, sKey);
                }
                return oResult;
            }, {});
        },
