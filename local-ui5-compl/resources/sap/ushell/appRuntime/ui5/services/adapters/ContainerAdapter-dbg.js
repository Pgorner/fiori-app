    ...
    var ContainerAdapter = function (oSystem, sParameter, oConfig) {
        ...
        function setSAPUI5Settings (oSettings) {
            if (oSettings.sapDateFormat) {
                Formatting.setABAPDateFormat(oSettings.sapDateFormat);
            }
            if (oSettings.sapDateCalendarCustomizing) {
                Formatting.setCustomIslamicCalendarData(oSettings.sapDateCalendarCustomizing);
            }
            if (oSettings.sapNumberFormat) {
                Formatting.setABAPNumberFormat(oSettings.sapNumberFormat);
            }
            if (oSettings.sapTimeFormat) {
                Formatting.setABAPTimeFormat(oSettings.sapTimeFormat);
            }
            if (typeof oSettings.currencyFormats === "object") {
                Formatting.addCustomCurrencies(oSettings.currencyFormats);
            }
        }
    };
