...
sap.ui.define([
...
], function (deepExtend, Deferred, ObjectPath, Log, PostMessageAPIInterface) {
    "use strict";

    var CURR_OBJ_ID = "sap.ushell.appRuntime.ui5.AppCommunicationMgr";

    var oPostMessageAPIs = {},
    ...
    function AppCommunicationMgr () {
        this.init = function (bInitIframeValidMsg) {
            ...
            PostMessageAPIInterface.init(false, this.registerCommHandlers.bind(this));
        };
        ...
        this.registerCommHandlers = function (oCommunication) {
            deepExtend(oPostMessageAPIs, oCommunication);
        };
        ...
        this._handleMessageRequest = function (oContainer, oMessage, oMessageData) {
            var oPostMessageServiceConfig = ObjectPath.create("sap-ushell-config.services.PostMessage.config"),
                    ...
                    try {
                        oServiceCall.executeServiceCallFn(oServiceParams)
                            .then((oResult) => {
                                ...
                                this.sendResponseMessage(oMessage, oMessageData, "error", { message: sMessage, stack: sStack });
                            });
                    } catch (oError) {
                        this.sendResponseMessage(oMessage, oMessageData, "error", { message: oError.message, stack: oError.stack });
                    }
                } else {
