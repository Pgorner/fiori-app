...
sap.ui.define([
        ...
        var oPromise = new Promise(function (fnResolve, fnReject) {
            ...
            _resolveSystemAlias(sSystemAlias, sSystemDataSrc, fnExternalSystemAliasResolver)
                ...
                .then(function (oResolvedSystemAlias) {
                    ...
                    _interpolatePathPrefixIntoURI(oURIWithProtocol, sURIType, oSystemAliasData.pathPrefix, fnExternalSystemAliasResolver)
                        .catch(function (sErrorMessage) {
                            fnReject(sErrorMessage);
                        })
                        .then(function (oURIWithPathPrefix) {
                            ...
                            if (oResolvedSystemAlias.hasOwnProperty("rfc") && sURIType === "NATIVEWEBGUI") {
                                var sWebguiParameters = _constructNativeWebguiParameters(oResolvedSystemAlias.rfc, oSystemAliasData.host, oURIWithPathPrefix);
                                var sPathWithParameters = oURIWithPathPrefix.path() + ";" + sWebguiParameters;
    ...
    function addSystemAlias (sUrl, oContainer) {
        ...
        if (oContainer.getFrameworkId() === "WDA" || oRes?.applicationType === "WDA") {
            const sParamValue = this.getSystemAliasInProvider(
                oContainer.getSystemAlias(),
                oRes.contentProviderId
            );
            return new URI(sUrl)
    ...
    */
    function getSystemAliasInProvider (fullyQualifiedSystemAlias = "", contentProviderId = "") {
        ...
        if (fullyQualifiedSystemAlias.startsWith(contentProviderId)) {
            throw new Error("Unexpected format for fully qualified systemAlias: missing underscore delimiter.");
        }
