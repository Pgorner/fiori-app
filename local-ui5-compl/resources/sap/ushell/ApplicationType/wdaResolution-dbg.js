...
sap.ui.define([
    ...
    async function constructWDAURLParameters (oParams, aMappedDefaultedParameterNames) {
        return new Promise(async (fnResolve, fnReject) => {
            ...
            oShellNavigationInternal.compactParams(
                ...
                .then(function (oEffectiveCompactedIntentParams) {
    ...
    function constructFullWDAResolutionResult (oMatchingTarget, sBaseUrl, fnExternalSystemAliasResolver) {
        ...
        return Promise.all([
            ...
            var oWdaURIWithSystemAlias = aResults[0];
            var sUrlParameters = aResults[1];

            var sFinalWDAURL = appendParametersToURI(oWdaURIWithSystemAlias, sUrlParameters);
            var sCalcSapSystem = oMappedIntentParamsPlusSimpleDefaults["sap-system"] && oMappedIntentParamsPlusSimpleDefaults["sap-system"][0];
            var sCalcSapSystemDataSrc = oMappedIntentParamsPlusSimpleDefaults["sap-system-src"] && oMappedIntentParamsPlusSimpleDefaults["sap-system-src"][0];

            var oResolutionResult = createWDAResolutionResult(oMatchingTarget.inbound, sFinalWDAURL, sCalcSapSystem, sCalcSapSystemDataSrc);
            return oResolutionResult;
        });
    }
