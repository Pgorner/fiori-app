...
sap.ui.define([
    ...
    Pages.prototype._getModelForPage = async function (page, visualizations, applications, vizTypes) {
        ...
        await Promise.all(page.payload.layout.sectionOrder.map(async (sSectionId) => {
            ...
            await Promise.all(oCDMPageSection.layout.vizOrder.map(async (id) => {
                const oVisualizationReference = oCDMPageSection.viz[id];
                ...
                if (!bIntentIsSupported) {
                    const iIndex = oSection.visualizations.findIndex(function (oViz) {
