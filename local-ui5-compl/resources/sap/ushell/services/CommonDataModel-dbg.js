...
sap.ui.define([
    ...
    CommonDataModel.prototype._ensureStandardVizTypesPresent = async function (oSite) {
        ...
        Object.keys(oStandardVizTypes).map((sKey) => {
    ...
    CommonDataModel.prototype.getMenuEntries = function (sMenuKey) {
        return new Promise(function (resolve, reject) {
            this.getSite().then(function (oSite) {
                ...
                /*
                    Since 1.132.0 the icon property is added to the menu factory of the menu bar.
                    In CDM scenarios menu entries could have an icon, which would be shown in the menu.
                    As we currently only want to support this feature for CEP, we set the icon property to undefined in CDM scenarios.
                */
                aMenuEntries?.forEach(function (entry) {
                    entry.icon = undefined;
                    entry?.menuEntries?.forEach(function (subEntry) {
                        subEntry.icon = undefined;
                    });
                });
