...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.components.shell.Settings.userDefaults.controller.UserDefaultsSetting", {
        ...
        getSystemContextsModel: function () {
            ...
            var oClientSideTargetResolutionPromise = Container.getServiceAsync("ClientSideTargetResolution");
            var oUserDefaultParametersServicePromise = Container.getServiceAsync("UserDefaultParameters");
        ...
        handleSystemContextChanged: function () {
            if (this.oDirtyStateModel.getProperty("/isDirty")) {
                var sUserDefaultSaveText = ushellResources.i18n.getText("userDefaultsSave");
                var sUserDefaultDiscardText = ushellResources.i18n.getText("userDefaultsDiscard");
                ...
                if (oParameter.editorMetadata.editorInfo && oParameter.editorMetadata.editorInfo.propertyName) {
                    return this._createOdataModelBinding(oParameter, oJsonModel)
                        .then(function (oModelBind) {
                            ...
                            var oODataMetadataModel = oModelBind.model ? oModelBind.model.getMetaModel() : undefined;

                            if (oODataMetadataModel) {
                                return oODataMetadataModel.loaded().then(function () {
        ...
        _saveIsSupportedPlatform: function () {
            var sPlatform = Container.getLogonSystem().getPlatform();
