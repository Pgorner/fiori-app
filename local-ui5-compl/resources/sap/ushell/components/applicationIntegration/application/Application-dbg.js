...
sap.ui.define([
    ...
    var oActiveApplicationContainer,
        BlueBoxHandler,
        PostMessageUtils,
    ...
    function Application () {
        ...
        this.handleServiceMessageEvent = function (oContainer, oMessage, oMessageData, oMessageStatus, bIsGlobalMessageListener) {
            ...
            if (sService.indexOf("sap.ushell.services.appLifeCycle") === 0) {
                sService = sService.replace(/appLifeCycle/gi, "AppLifeCycle");
            }
                ...
                try {
                    fnServiceCall(oServiceParams)
                        .then((oResult) => {
                            ...
                            let sStack;
                            let sMessage = vMessage;
                            if (vMessage instanceof Error) {
                                sMessage = vMessage.message;
                                sStack = vMessage.stack;
                            }
                            sendResponseMessage("error", { message: sMessage, stack: sStack });
                        });
                } catch (oError) {
                    sendResponseMessage("error", { message: oError.message, stack: oError.stack });
                }
            }
            ...
            function storeSapSystemData (oSapSystemData, sSapSystemSrc) {
                ...
                if (sStringifiedSapSystemData) {
                    ...
                    sKey = ushellUtils.generateLocalStorageKey("sap-system-data", aSystemIds);
            ...
            function getLocalSystemInSidFormat () {
                var oSystem = Container.getLogonSystem();
        ...
        this.init = function (inBlueBoxHandler, inPostMessageUtils) {
            BlueBoxHandler = inBlueBoxHandler;
            PostMessageUtils = inPostMessageUtils;
        };
        ...
        this._createWaitForRendererCreatedPromise = function () {
            ...
            oRenderer = Container.getRendererInternal();
            if (oRenderer) {
            ...
            oPromise = new Promise(function (resolve, reject) {
                ...
                fnOnRendererCreated = function () {
                    ...
                    resolve();
                    Container.detachRendererCreatedEvent(fnOnRendererCreated);
                };
                oRenderer = Container.getRendererInternal();
                if (oRenderer) {
                    ...
                    Container.attachRendererCreatedEvent(fnOnRendererCreated);
                }
            });
            return [oPromise];
        };
        ...
        this.createComponent = async function (oResolvedHashFragment, oParsedShellHash) {
            ...
            await ushellUtils.promisify(Ui5ComponentLoader.createComponent(
        ...
        this.createApplicationContainer = function (sStorageAppId, oResolvedHashFragment) {
            var oTempTarget = {};

            this._cleanTargetResolution(oResolvedHashFragment, oTempTarget);
            oActiveApplicationContainer = new ApplicationContainer(sStorageAppId, oResolvedHashFragment);
            this._restoreTargetResolution(oResolvedHashFragment, oTempTarget);

            if (oActiveApplicationContainer.setBlueBoxCapabilities) {
                oActiveApplicationContainer.setProperty("blueBoxCapabilities", {}, true);
            }
            if (oActiveApplicationContainer.setCurrentAppId) {
                oActiveApplicationContainer.setProperty("currentAppId", sStorageAppId, true);
            }
            if (oActiveApplicationContainer.setCurrentAppUrl) {
                oActiveApplicationContainer.setProperty("currentAppUrl", oResolvedHashFragment.url, true);
            }
            if (oResolvedHashFragment.appCapabilities) {
                oActiveApplicationContainer.setProperty("frameworkId", oResolvedHashFragment.appCapabilities.appFrameworkId, true);
            }
            if (oActiveApplicationContainer.setCurrentAppTargetResolution) {
                oActiveApplicationContainer.setProperty("currentAppTargetResolution", oResolvedHashFragment, true);
            }

            oActiveApplicationContainer.setHandleMessageEvent(this.handleMessageEvent);
            BlueBoxHandler.addNewBlueBox(oActiveApplicationContainer, oResolvedHashFragment);
            return oActiveApplicationContainer;
        };
        ...
        this.restore = function (oApp) {
            ...
            if (oApp.container && oApp.container._getIFrame && oApp.container._getIFrame() != undefined) {
                PostMessageUtils.postMessageToIframeApp(oApp.container, "sap.ushell.appRuntime", "keepAliveAppShow", {}, false);
            }

            if (oApp.app) {
                EventBus.getInstance().publish("sap.ushell", "appKeepAliveActivate", oApp.appTarget);

                //is Isolated application
                if (oApp.app.getUrl) {
                    //send post message restore
                    sDomainEnd = oApp.app.getUrl();
                    oActiveApplicationContainer = BlueBoxHandler.getBlueBoxByUrl(this.stripURL(sDomainEnd));

                    if (oActiveApplicationContainer) {
                        BlueBoxHandler.restoreApp(oActiveApplicationContainer.sApplication);
                    }
                }

                if (oApp.app.isKeepAliveSupported && oApp.app.isKeepAliveSupported() === true) {
                    oApp.app.activate(); // todo: [FLPCOREANDUX-10024] clarify this case
                } else {
                    if (oApp.app.restore) {
                        oApp.app.restore();
                    }
                ...
                oActiveApplicationContainer = oApp.app;
            }
        };
        ...
        this.store = function (oApp) {
            ...
            if (oApp.container && oApp.container._getIFrame && oApp.container._getIFrame() != undefined) {
                PostMessageUtils.postMessageToIframeApp(oApp.container, "sap.ushell.appRuntime", "keepAliveAppHide", {}, false);
            }

            //destroy the application and its resources
            // invoke the life cycle interface "suspend" for the suspend application
            if (oApp.app) {
                EventBus.getInstance().publish("sap.ushell", "appKeepAliveDeactivate", oApp.appTarget);
                ...
                if (oApp.app.getUrl) {
                    ...
                    if (oActiveApplicationContainer) {
                        BlueBoxHandler.storeApp(oActiveApplicationContainer.sApplication);
                        // triggers "sap.ushell.services.appLifeCycle.store" to multiple iframes
                    }
                }
        ...
        this.destroy = function (oApp) {
            ...
            if (oApp.getUrl) {
                const sDomainEnd = oApp.getUrl();
                const oActiveApplication = BlueBoxHandler.getBlueBoxByUrl(this.stripURL(sDomainEnd));

                if (BlueBoxHandler.isStatefulContainer(oActiveApplication)) {
        ...
        // todo: [FLPCOREANDUX-10024] check: this does not seem to be always the actual current/active AppContainer
        // seems to be set directly when created and for stateful open case
        // reuse of nonKeepAlive gui is probably not set correctly
        this.setActiveAppContainer = function (oContainer) {
            oActiveApplicationContainer = oContainer;
        };
                    ...
                    try {
                        ...
                        if (typeof oMessageData === "object" && oMessageData.type === "request" && oMessageData.service === "sap.ushell.services.MessageBroker") {
                            Container.getServiceAsync("MessageBroker").then(function (oMessageBrokerService) {
                                var sVal = oMessageBrokerService.getAcceptedOrigins().find(function (sOrigin) {
        ...
        if (Config.last("/core/shell/enableMessageBroker") && Container.isInitialized()) {
            Container.getServiceAsync("MessageBroker").then(function (oMessageBrokerService) {
                addEventListener("message", handleMessageEventForDetaultListener.bind(that));
            });
        }
    }
    ...
    Application.prototype._cleanTargetResolution = function (oResolvedNavigationTarget, oTempTarget) {
        ...
        if (oResolvedNavigationTarget) {
            for (sKey in oResolvedNavigationTarget) {
                if (!oApplicationContainerProps.hasOwnProperty(sKey)) {
