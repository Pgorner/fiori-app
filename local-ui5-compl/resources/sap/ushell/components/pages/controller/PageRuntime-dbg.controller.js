...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.components.pages.controller.Pages", /** @lends sap.ushell.components.pages.controller.Pages.prototype */ {
        ...
        onInit: function () {
            ...
            this._oInitPromise = this._openFLPPage().then(function () {
                ...
                if (!document.activeElement || document.activeElement === document.body) {
                    ...
                    var oCurrentLaunchpadPage = oCurrentPage && oCurrentPage.getContent()[0];
                    var oFirstSection = oCurrentLaunchpadPage && oCurrentLaunchpadPage.getSections()[0];
                    if (oFirstSection) {
                        oFirstSection.focus();
                    } else {
                        // jQuery Plugin "firstFocusableDomRef"
                        var oFocusableDomRef = this.oPagesNavContainer.$().firstFocusableDomRef();
                        if (oFocusableDomRef) {
        ...
        _updateHomeUri: function (oCurrentSpaceAndPage) {
            ...
            if (Config.last("/core/spaces/homeNavigationTarget") === "origin_page") {
                var sNewHash = oCurrentSpaceAndPage ? encodeURI(oCurrentSpaceAndPage.hash) : sRootIntent;
        ...
        onRouteMatched: function (bIsHomeRoute) {
            ...
            var oActionModeButton = Element.getElementById("ActionModeBtn");
        ...
        navigateToErrorPage: function (vError, sPageId, sSpaceId) {
            ...
            if (vError instanceof Error) {
                ...
                this._oErrorPageModel.setProperty("/technicalError", vError.toString());
                this._oErrorPageModel.setProperty("/pageAndSpaceId", "");

            } else if (sPageId && sSpaceId) {
                this._oErrorPageModel.setProperty("/title", resources.i18n.getText("PageRuntime.CannotLoadPage.Title"));
                this._oErrorPageModel.setProperty("/details", resources.i18n.getText("PageRuntime.CannotLoadPage.Details", [JSON.stringify(vError)]));
                ...
                this._oErrorPageModel.setProperty("/technicalError", JSON.stringify(vError));
            } else {
                this._oErrorPageModel.setProperty("/title", resources.i18n.getText("PageRuntime.GeneralError.Text"));
                this._oErrorPageModel.setProperty("/details", resources.i18n.getText("PageRuntime.CannotLoadPage.SystemErrorMessagePrefix", [JSON.stringify(vError)]));
                this._oErrorPageModel.setProperty("/technicalError", JSON.stringify(vError));
                this._oErrorPageModel.setProperty("/pageAndSpaceId", "");
            }
        ...
        _navigate: function (targetPageId, spaceId, keepActionMode) {
            ...
            return Container.getServiceAsync("Menu")
                .then(function (oService) {
                    ...
                    return Promise.all([
                        oMenuService.hasMultiplePages(spaceId),
                        oMenuService.isPinned(spaceId)
                    ]);
                })
                .then(function (aResult) {
                    var bHasMultiplePages = aResult[0];
                    var isPinned = aResult[1];
                    ...
                    oMenuService.getSpaceAndPageTitles(spaceId, targetPageId)
                        .then(function (oTitles) {
                            ...
                            var oFLPPageControl = oPageControl.getContent()[0];
                            var bShowTitle = bHasMultiplePages || !isPinned;
                            if (bShowTitle) {
                                oFLPPageControl.setTitle(this._formatPageTitle(oTitles.pageTitle, oTitles.spaceTitle, bHasMultiplePages, !isPinned));
                            }
        ...
        _copyToClipboard: function () {
            ...
            var bResult = ushellUtils.copyToClipboard(sError);
            if (bResult) {
