		...
		var loadDataNode = function(dataNode, nodeElement, dataTypePath) {
			...
			toArray(nodeElement.E).forEach(function(instanceElement, index) {
				...
				var instance = keyAttribute
					? library.findInArray(dataNode, function(instance) { return instance[keyAttribute.name] === instanceElement[keyAttribute.alias]; })
					: dataNode[index];
				if (!instance) {
			...
			toArray(automation.Call).forEach(function(call) {
				switch (call.handler) {
					...
					case "FLYTOHANDLER":
						...
						var scene = library.findInArray(this._context.scenes, function(s) {
							return s.id === (library.findInArray(params, function(p) {
								return p.name === "scene";
							}) || {})["#"] || "";
						});

						if (scene) {
							var abcissae = (library.findInArray(params, function(p) { return p.name === "x"; }) || {})["#"] || "0";
							var ordinate = (library.findInArray(params, function(p) { return p.name === "y"; }) || {})["#"] || "0";
							var applicate = (library.findInArray(params, function(p) { return p.name === "z"; }) || {})["#"] || "0";
							scene.position = abcissae + ";" + ordinate + ";" + applicate;

							scene.zoom = (library.findInArray(params, function(p) { return p.name === "lod"; }) || {})["#"] || "0";
							scene.pitch = (library.findInArray(params, function(p) { return p.name === "pitch"; }) || {})["#"] || "0";
							scene.yaw = (library.findInArray(params, function(p) { return p.name === "yaw"; }) || {})["#"] || "0";
