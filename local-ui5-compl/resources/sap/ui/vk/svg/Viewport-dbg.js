...
sap.ui.define([
	...
	Viewport.prototype.init = function() {
		...
		const miniMap = this._miniMap = new MiniMap({ viewport: this });
		const miniMapDialog = new Dialog({
			showHeader: false,
			horizontalScrolling: false,
			verticalScrolling: false,
			content: miniMap,
			escapeHandler: function() { } // ignore ESC key
		}).addStyleClass("sapUiVkMiniMapDialog");
		...
		miniMap.attachSizeChanged((event) => {
			miniMapDialog.setContentWidth(event.getParameter("width") + 4 + "px"); // add 4px for left and right margins
			miniMapDialog.setContentHeight(event.getParameter("height") + 4 + "px"); // add 4px for top and bottom margins
		});
		this.setMiniMap(miniMapDialog);
	};
	...
	Viewport.prototype.onAfterRendering = function() {
		...
		this._miniMap._onContentUpdate();
	};
	...
	Viewport.prototype._handleResize = function(event) {
		...
		const miniMapDialog = this.getMiniMap();
		miniMapDialog.$().css(miniMapDialog._calcPosition());
	 ...
	 * @deprecated Since version 1.80.0. Viewport now gets reference to the scene via {@link sap.ui.vk.ContentConnector}.
	 * @public
	 */
	...
	Viewport.prototype.onSetViewStateManager = function(viewStateManager) {
		...
		this._miniMap.setViewStateManager(viewStateManager);
	...
	Viewport.prototype.onUnsetViewStateManager = function(viewStateManager) {
		...
		this._miniMap?.setViewStateManager(null);
	};
	...
	Viewport.prototype._setContent = function(content) {
		...
		this._setScene(content instanceof Scene ? content : null);
	...
	Viewport.prototype._onContentLoadingFinished = function(event) {
		if (this._scene) {
			...
			if (nodeHierarchy) {
				...
				var hotspotNodes = nodeHierarchy.getHotspotNodeRefs();
				hotspotNodes.forEach(function(hotspotNode) {
					this._hotspotHelper.updateHotspot(hotspotNode);
					if (color != null) {
	...
	Viewport.prototype.activateView = function(view) {
		...
		if (camera?.zoom <= OrthographicCamera._MIN_ZOOM) {
			this.zoomTo(ZoomTo.Visible, null, 0, 0);
		}
		...
		if (viewInfo.visibility) {
			switch (viewInfo.visibility.mode) {
				case VisibilityMode.Complete:
					const visibleNodes = viewInfo.visibility.visible.map(veId => veIdToNodeRefMap.get(veId)).filter(n => n != null),
						hiddenNodes = viewInfo.visibility.hidden.map(veId => veIdToNodeRefMap.get(veId)).filter(n => n != null);
					this._viewStateManager.setVisibilityState(visibleNodes, true, false);
					this._viewStateManager.setVisibilityState(hiddenNodes, false, false);
					break;
	...
	Viewport.prototype.setShowAllHotspots = function(value) {
		...
		if (vsm) {
			...
			var nodes = vsm.getNodeHierarchy().getHotspotNodeRefs();
