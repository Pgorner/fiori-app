...
sap.ui.define([
	 ...
	 * @param {any} [reject] From content manager, called in setScene function, only used for vds file reading (matai.js).
	 *
	 * @private
	 * @author SAP SE
	 * @version 1.132.0
	...
	SceneBuilder.prototype.insertView = function(viewInfo, sceneId) {
		...
		let view = this._views.get(viewInfo.viewId);

		if (view == null) {
			view = this._vkScene.createView({
				viewId: viewInfo.viewId,
				name: viewInfo.name,
				description: viewInfo.description ? "<pre class=\"sapThemeVizKitViewGalleryStepDescriptionPre\">" + viewInfo.description + "</pre>" : viewInfo.description, // Currently this is plain text so we preserve it's formatting (line breaks)
			...
			view.setDescription(viewInfo.description ? "<pre class=\"sapThemeVizKitViewGalleryStepDescriptionPre\">" + viewInfo.description + "</pre>" : viewInfo.description);
	...
	SceneBuilder.prototype.createNode = function(nodeInfo, sceneId) {
		...
		const node = nodeInfo.contentType == "SVG"
			? new SvgImage({
				sid: nodeInfo.sid,
				name: nodeInfo.name,
				matrix: [1, 0, 0, -1, 0, 0],
				sceneId: sceneId,
				imageId: nodeInfo.imageId,
		...
		if (nodeInfo.pageIndex != null) {
			node.setPageIndex(nodeInfo.pageIndex);
		}
		...
		if (nodeInfo.contentType === "HOTSPOT") {
			...
			node._vkSetNodeContentType(NodeContentType.SvgImage);
		}
