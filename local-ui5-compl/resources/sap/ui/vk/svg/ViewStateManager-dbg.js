...
sap.ui.define([
	...
	ViewStateManager.prototype.getCurrentView = function() {
		var viewManager = coreElement.getElementById(this.getViewManager());
		if (!viewManager) {
			return null;
		}
	 ...
	 * @returns {this} <code>this</code> to allow method chaining.
	 * @deprecated Since version 1.80.0. Use {@link sap.ui.vk.svg.ViewStateManager#setSelectionStates} instead.
	 * @public
	 */
	...
	function toABGR(color) {
		switch (typeof color) {
			...
			case "string":
				...
				return CSSColor.isValid(color) ? colorToABGR(cssColorToColor(color)) : undefined;
			default:
	...
	/**
	 * Convert a matrix into a 2x3 column-major matrix for use in SVG transformations.
	 *
	 * @param {float[]} array An input 3x1, 3x4 or 4x4 column-major matrix.
	 * @returns {Float32Array} A 2x3 column-major matrix for use in SVG transformations.
	 */
