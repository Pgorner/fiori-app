...
sap.ui.define([
	...
	var ContentConnector = Element.extend("sap.ui.vk.ContentConnector", /** @lends sap.ui.vk.ContentConnector.prototype */ {
		...
		constructor: function(sId, mSettings) {
			...
			this._consumptionScenario = null;
			this._retryCount = 1;
	...
	ContentConnector.prototype._getContentManagerByClassName = function(className) {
		...
		return new Promise(function(resolve, reject) {
			sap.ui.require([
				...
				that._assignConsumptionScenario([contentManager]);
	...
	ContentConnector.prototype._assignConsumptionScenario = function(contentManagers) {
		for (var i = 0; i < contentManagers.length; i++) {
			contentManagers[i].setConsumptionScenario(this._consumptionScenario);
		}
	};
	 ...
	 *
	 * @param {sap.ui.vk.ConsumptionScenario} consumptionScenario The consumption scenario.
	 * @returns {this} <code>this</code> to allow method chaining.
	 * @public
	 * @since 1.114.0
	 */
	ContentConnector.prototype.setConsumptionScenario = function(consumptionScenario) {
		this._consumptionScenario = consumptionScenario;
		this._assignConsumptionScenario(this.getAggregation("contentManagers", []));
		return this;
	};
	...
	ContentConnector.prototype._setContent = function(newContent, newContentManager) {
		...
		if (oldContent !== newContent) {
			...
			if (newContentManager) {
				...
				switch (newContentManagerClassName) {
					case "sap.ui.vk.threejs.ContentManager":
						newDefaultViewStateManagerClassName = "sap.ui.vk.threejs.ViewStateManager";
						break;
					case "sap.ui.vk.dvl.ContentManager":
						newDefaultViewStateManagerClassName = "sap.ui.vk.dvl.ViewStateManager";
						break;
					case "sap.ui.vk.svg.ContentManager":
						newDefaultViewStateManagerClassName = "sap.ui.vk.svg.ViewStateManager";
						break;
					case "sap.ui.vk.pdf.ContentManager":
						newDefaultViewStateManagerClassName = "sap.ui.vk.pdf.ViewStateManager";
						break;
					default:
						break;
				}
				...
				if (newDefaultViewStateManagerClassName) {
					...
					var Class = sap.ui.require(newDefaultViewStateManagerClassName.replaceAll(".", "/"));
	...
	ContentConnector.prototype.updateContent = async function(parameters) {
		const contentResources = this.getContentResources();
		const contentManager = this.getContentManager();
		const content = this.getContent();

		if (contentResources.length === 0 || contentManager == null || content == null) {
			throw new Error("No content is loaded.");
		}

		const result = await contentManager.updateContent(contentResources, content, parameters);

		// NOTE: This event is not documented and is not intended for public use.
		this.fireEvent("_contentUpdated", { result });
	...
	/**
	 * In UI5 v1 we still use DVL for content resource type "vds"
	 * @deprecated As of version 1.120
	 */
	(function() {
		resolvers[1].contentManagerClassName = "sap.ui.vk.dvl.ContentManager";
	}());
