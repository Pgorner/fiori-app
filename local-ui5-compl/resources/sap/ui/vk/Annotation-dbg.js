...
sap.ui.define([
	...
	Annotation.prototype.setAttachmentNode = function(attNode) {
		...
		if (attNode) {
			this.invalidate(); // Replay animation
		}
	};
	...
	Annotation.prototype.setSelected = function(selected) {
		...
		if (vsm.getSelectionState(nodeRef) !== selected) {
			if (selected) {
				vsm.setSelectionStates(nodeRef, []);
			} else {
				vsm.setSelectionStates([], nodeRef);
			}
		}
	...
	Annotation.prototype._setSpanText = function(textValue) {
		...
		for (var i = 0; i < textValue.length; i++) {
			...
			if (textValue[i] === "&") {
				newText += "<span class=\"sapThemeVITAnnotation\">&";
				while (textValue[i] !== ";") {
					i++;
					newText += textValue[i];
				}
			...
			if (this._textDiv) {
				...
				if (domRef) {
					...
					for (var i = 0; i < children.length; i++) {
						if (children[i].className === "sapThemeVITAnnotation") {
	...
	Annotation.prototype.getTransform = function() {
		...
		return TransformationMatrix.convertTo4x3(annotationNode.matrix.elements);
	};
