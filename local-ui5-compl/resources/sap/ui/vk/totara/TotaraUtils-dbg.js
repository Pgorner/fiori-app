...
sap.ui.define([
	...
	TotaraUtils.arrayToColumnMajorMatrixArray16 = function(arr) {
		...
		if (arr.length === 12) {
			return TransformationMatrix.convertTo4x4(arr);
		}
	...
	// Adds 'elem' to the array in the 'map' with 'key', if it is not already the
	// last element in that array and returns the new length of the array.
	TotaraUtils.pushElementIntoMapArray = function(map, key, elem) {
		...
		if (!array) {
			...
			return array.length; // the last array element matches the new element
		}
		array.push(elem);
		return array.length;
	};
