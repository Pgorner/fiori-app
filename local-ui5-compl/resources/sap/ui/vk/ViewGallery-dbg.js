...
sap.ui.define([
	...
	const FlexAlignContent = mLibrary.FlexAlignContent;
	...
	ViewGallery.prototype.createToolbar = function() {
		...
		this._invisStepCount = new InvisibleText();

		this._currentStepCount = new Input({
			ariaLabelledBy: this._invisStepCount,
			width: "auto",
			type: InputType.Number,
			submit: (event) => {
				const value = event.getParameter("value");
				if (value < 1 || value > this._viewItems.length) {
					return;
				}
				this.setSelectedItem(this._viewItems[value - 1]);
			},
			liveChange: (event) => {
				const value = event.getParameter("value");
				const newValue = value.replaceAll(".", "");
				this._currentStepCount.setValue(newValue);
				if (newValue < 1 || newValue > this._viewItems.length) {
					this._currentStepCount.setValueState(ValueState.Error);
					this._currentStepCount.setValueStateText(getResourceBundle().getText(newValue === "" ? "VIEW_COUNT_EMPTY" : "VIEW_COUNT_ERROR", [newValue]));
				} else {
					this._currentStepCount.setValueState(ValueState.None);
				}
			}
		}).addStyleClass("sapVizKitViewGalleryCurrentStepInput").addEventDelegate({
			onfocusout: () => {
				const value = this._currentStepCount.getValue();
				if (value < 1 || value > this._viewItems.length || parseInt(value, 10) === (this.getSelectedViewIndex() + 1)) {
					return;
				}
				this.setSelectedItem(this._viewItems[value - 1]);
			}
		...
		this._totalStepCount = new Title({
			level: "H5",
			titleStyle: "H5"
		}).addStyleClass("sapVizKitViewGalleryTotalStepTitle");
		...
		this.toolbar.addContent(this._currentGroupTitle)
			...
			.addContent(this._invisStepCount)
			.addContent(this._currentStepCount)
			.addContent(this._totalStepCount)
			.addContent(this._nextItemButton)
			.addContent(new ToolbarSeparator())
		...
		this._stepDescription = new VBox({
			...
			items: [
				this._stepDescriptionToolbar = new Toolbar({
					...
					content: [
						this._stepDescriptionIcon = new Icon({
						...
						new ToolbarSpacer(),
						this._stepDescriptionOrientationIcon = new Icon({
		...
		this._stepDescriptionVertical = new HBox({
			...
			items: [
				...
				this._stepDescriptionVerticalToolbar = new Toolbar({
					design: ToolbarDesign.Solid,
					content: [
						this._stepDescriptionVerticalIcon = new Icon({
						...
						this._stepDescriptionVerticalOrientationIcon = new Icon({
	...
	ViewGallery.prototype._refreshProcedures = function(currentViewGroupId) {
		...
		if (viewGroups && viewGroups.length > 0) {
			...
			viewGroups.forEach(function(viewportGroup) {
				...
				if (this.getAggregation("toolbar")) {
					this._procedureList.addItem(new Item({
	...
	ViewGallery.prototype._clearUI = function() {
		if (this.getAggregation("toolbar")) {
			...
			this._totalStepCount.setText("");
			this._currentStepCount.setValue(null);
	...
	ViewGallery.prototype._refreshItems = function() {
		...
		if (this._modelViews) {
			...
			const width = (Math.max(this._viewItems.length.toString().length, 2) - 2) * 0.5;
			this._currentStepCount.setWidth(2 + width + "rem");
			this.rerender();
		}
	};
	...
	ViewGallery.prototype.viewActivated = function(index) {
		...
		if (this.getAggregation("toolbar")) {
			this._totalStepCount.setText("/ " + this._viewItems.length);
			this._invisStepCount.setText(getResourceBundle().getText("VIEW_COUNT_INPUT", [index + 1, this._viewItems.length]));
			this._currentStepCount.setValue(index + 1);
			this._currentStepCount.setValueState(ValueState.None);
