...
sap.ui.define([
	...
	ViewStateManager.prototype._setContent = function(content) {
		var scene = null;
		if (content?.isA?.(["sap.ui.vk.Scene", "sap.ui.vk.pdf.Document"])) {
			scene = content;
		}
			...
			if (!reuseImplementation) {
				...
				switch (sceneType) {
					...
					case "sap.ui.vk.svg.Scene":
						...
						break;
					case "sap.ui.vk.pdf.Document":
						newImplementationType = "sap.ui.vk.pdf.ViewStateManager";
						break;
					default:
						break;
				}
				...
				if (newImplementationType) {
					var that = this;
					// The ViewStateManager implementation classes from the `dvl`, `threejs`, `svg`
					// and `pdf` namespaces are loaded by the corresponding content managers, so
					// there is no need to load them here. We can safely assume that they are
					// available at this point.
					var Class = sap.ui.require(newImplementationType.replaceAll(".", "/"));
					...
					if (viewManager) {
						var animationPlayer = Element.getElementById(viewManager.getAnimationPlayer());
						if (animationPlayer) {
	...
	ViewStateManager.prototype.getAnimationPlayer = function() {
		...
		var viewManager = Element.getElementById(this.getViewManager());
		if (viewManager) {
			animationPlayer = Element.getElementById(viewManager.getAnimationPlayer());
		}
	 ...
	 * @returns {this} <code>this</code> to allow method chaining.
	 * @deprecated Since version 1.56.3. Use {@link sap.ui.vk.ViewStateManager#setSelectionStates} instead.
	 * @public
	 */
