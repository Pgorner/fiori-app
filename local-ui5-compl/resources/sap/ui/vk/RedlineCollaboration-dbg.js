...
sap.ui.define([
	...
	RedlineCollaboration.prototype.exit = function() {
		...
		if (viewport) {
			if (viewport.dettachCameraChanged) {
				viewport.dettachCameraChanged(this._onCameraChanged, this);
			}
	...
	RedlineCollaboration.prototype.setViewport = function(viewport) {
		...
		if (viewport.attachCameraChanged) {
			viewport.attachCameraChanged(this._onCameraChanged, this);
		}
	...
	RedlineCollaboration.prototype._onCameraChanged = function() {
		const activeConversation = Element.getElementById(this.getActiveConversation());
		if (activeConversation?.getViewInfo()) {
			activeConversation.setViewInfo(this._getViewInfo());
		}
	};
	...
	RedlineCollaboration.prototype._onMeasurementsAdded = function(event) {
		...
		if (this.getActiveConversation()) {
			var conversation = Element.getElementById(this.getActiveConversation());
			var comment;
			if (this.getActiveComment()) {
				comment = Element.getElementById(this.getActiveComment());
			}
	...
	RedlineCollaboration.prototype._onElementCreated = function(event) {
		...
		if (element) {
			...
			if (this.getActiveComment()) {
				var comment = Element.getElementById(this.getActiveComment());
	 ...
	 * @param {boolean} activateView The property that indicates whether the conversation view should be activated
	 * @returns {this} <code>this</code> to allow method chaining..
	 */
	RedlineCollaboration.prototype.setActiveConversation = function(conversation, activateView = true) {
		var activeConversation = Element.getElementById(this.getActiveConversation());
		if (activeConversation) {
			if (activateView && activeConversation.getViewInfo()) {
				activeConversation.setViewInfo(this._getViewInfo());
			}
		...
		if (viewport.isA("sap.ui.vk.pdf.Viewport")) {
			const pageId = conversation.getPageId();
			if (pageId >= 0) {
				viewport.setCurrentPageIndex(pageId);
			}
			if (!this._tool.getActive()) {
				this._tool.setActive(true, viewport);
			}
		} else if (conversation.getViewInfo()) {
			var scene = Element.getElementById(viewport.getContentConnector()).getContent();
			const view = scene.getViews().find(view => view.getViewId() === conversation.getViewId());
			if (view && activateView) {
				...
				var vm = Element.getElementById(vsm.getViewManager());
				...
				viewport.setViewInfo(conversation.getViewInfo());
			}
	...
	RedlineCollaboration.prototype._readyForAnimation = function() {
		...
		var conversation = Element.getElementById(this.getActiveConversation());
		if (conversation) {
			var animationPlayer = Element.getElementById(viewport.getViewStateManager()).getAnimationPlayer();
	...
	RedlineCollaboration.prototype.createElement = function(redlineElement) {
		...
		if (conversation) {
			if (viewport.isA("sap.ui.vk.pdf.Viewport")) {
				conversation.setPageId(viewport.getCurrentPageIndex());
			} else if (!conversation.getViewInfo()) {
				conversation.setViewId(viewport.getCurrentView().getViewId());
				conversation.setViewInfo(this._getViewInfo());
				conversation.setAnimationOffset(Element.getElementById(viewport.getViewStateManager()).getAnimationPlayer().getTime());
			}
		}
		if (!this._tool.getActive()) {
			this._tool.setActive(true, viewport);
	...
	RedlineCollaboration.prototype._getViewInfo = function() {
		var viewport = Element.getElementById(this.getViewport());
		if (!viewport || viewport.isA("sap.ui.vk.pdf.Viewport")) {
			return null;
		}
		var query = {
			camera: {
	...
	RedlineCollaboration.prototype.exportJSON = function() {
		const activeId = Element.getElementById(this.getActiveConversation())?.getElementId() ?? "";
		...
		var json = {
			schemaVersion: "1.0",
			header: Object.fromEntries(this._header),
			conversations: this.getConversations().map(conversation => conversation.exportJSON())
		};
