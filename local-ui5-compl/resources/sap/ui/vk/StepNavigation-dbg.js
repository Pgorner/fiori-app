...
sap.ui.define([
	...
	StepNavigation.prototype.init = function() {
		...
		if (this.getShowThumbnails()) {
			this._thumbnailsScroller = new ScrollContainer(this.getId() + "-scroller", {
				width: "100%",
				horizontal: true,
				vertical: false
			});
		...
		this.playPreviousButton = new Button(this.getId() + "-playPreviousButton", {
		...
		this.playNextButton = new Button(this.getId() + "-playNextButton", {
			type: sap.m.ButtonType.Transparent,
		...
		// Create the play next button
		this.playOptionButton = new Button(this.getId() + "-playOptionButton", {
			type: sap.m.ButtonType.Transparent,
		...
		this.procedureList = new Select(this.getId() + "-procedureList", {
		...
		this.playMenuButton = (new Select(this.getId() + "-playMenuButtonIcon", {
			selectedKey: "0",
			type: sap.m.SelectType.Default,
		...
		this.pauseButton = new Button(this.getId() + "-pauseButton", {
			type: sap.m.ButtonType.Transparent,
		...
		this.showStepInfoButton = new ToggleButton(this.getId() + "-showStepInfoButton", {
			icon: "sap-icon://hide",
			type: sap.m.ButtonType.Transparent,
			pressed: false,
		...
		return {
			...
			stepInfo: {
				...
				openPopup: function(popupTitle, textContent, target, placement) {
					...
					this._customHeaderText = this._customHeaderText || new MText({
						width: "100%",
					...
					this._customHeader = this._customHeader || new Bar({
						contentLeft: [
							this._customHeaderText
						]
					});

					this._textContent = this._textContent || new MText({});
					...
					this.stepMessagePopup = this.stepMessagePopup || new ResponsivePopover({
						placement: placement,
		...
		if (oSettings.currentProcedureId != "" || that.oModel.oData.procedures.length > 0) {
			if (that.getShowThumbnails()) {
				...
				for (var i = 0; i < steps.length; i++) {
					...
					img.addCustomData(new CustomData({
						key: "stepId",
						value: steps[i].id
					}));
