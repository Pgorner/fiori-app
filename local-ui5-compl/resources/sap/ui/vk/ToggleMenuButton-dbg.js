...
sap.ui.define([
	...
	var ToggleMenuButton = Control.extend("sap.ui.vk.ToggleMenuButton", /** @lends sap.ui.vk.ToggleMenuButton.prototype */ {
		metadata: {
			...
			aggregations: {
				...
				_toggleButton: { type: "sap.m.OverflowToolbarToggleButton", multiple: false, visibility: "hidden" },
	...
	ToggleMenuButton.prototype.init = function() {
		...
		var toggleButton = new ToggleButton({
			...
			press: function() {
				var item = Element.getElementById(this.getDefaultItem());
				if (item) {
	...
	ToggleMenuButton.prototype._onBeforeEnterOverflow = function() { this._getToggleButton()._bInOverflow = true; };
	ToggleMenuButton.prototype._onAfterExitOverflow = function() { this._getToggleButton()._bInOverflow = false; };

	ToggleMenuButton.prototype.getOverflowToolbarConfig = function() {
		return {
			canOverflow: true,
			propsUnrelatedToSize: ["enabled", "type", "icon", "accesskey", "pressed"],
	...
	ToggleMenuButton.prototype._menuItemSelected = function(oEvent) {
		...
		if (menuItem.getToggleable()) {
			...
			var oldItem = toggleButton.getPressed() ? Element.getElementById(this.getDefaultItem()) : null;
			if (menuItem !== oldItem) {
	...
	ToggleMenuButton.prototype.getText = function() {
		return Button.prototype._getText.call(this._getToggleButton());
	};
	...
	ToggleMenuButton.prototype.setDefaultItem = function(item) {
		...
		if (typeof item === "string") {
			item = Element.getElementById(item);
		}
		var icon = item.getIcon();
		var toggleButton = this._getToggleButton();
		toggleButton.setIcon(icon);
		toggleButton.setText(item.getText());
