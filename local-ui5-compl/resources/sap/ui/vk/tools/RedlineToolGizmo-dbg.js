	...
	RedlineToolGizmo.prototype.onAfterRendering = function() {
		...
		function attachTextEventHandlers(element) {
			...
			textArea.onblur = function(event) {
				...
				let redlineScale = computedStyle.getPropertyValue("--sapUiVizKitRedlineScale");
				redlineScale = redlineScale === "" ? 1 : Number(redlineScale);

				// set element width
				element.setWidth(parseFloat(computedStyle.width) / redlineScale);

				// set element height
				var height = parseFloat(computedStyle.height) / redlineScale;
				const originalFontSize = this.style.fontSize;
				this.style.fontSize = `${element.getFontSize()}px`;
				...
				this.style.height = `calc(${height}px * var(--sapUiVizKitRedlineScale, 1))`;
