...
sap.ui.define([
	...
	RotateToolGizmo.prototype.init = function() {
		...
		this._noHitRotation = new THREE.Euler();
	...
	RotateToolGizmo.prototype._onVisibilityChanged = function(event) {
		if (this._tool.getEnableSnapping()) {
			this._computeBoundsTree();
		}
	};
	...
	RotateToolGizmo.prototype.selectHandle = function(index, gizmoIndex) {
		...
		this._viewport.setShouldRenderFrame(true);
	};

	RotateToolGizmo.prototype.beginGesture = function() {
		this._beginValue = this._value.clone();
		this._rotationDelta.setScalar(0);
		this._noHitRotation.set(0, 0, 0);

		this._nodes.forEach((nodeInfo, i) => {
			this._updateGizmoObjectTransformation(this._gizmo, i);
			nodeInfo.matGizmo = this._gizmo.matrixWorld.clone();
	...
	RotateToolGizmo.prototype._rotate = function(euler, noSnapping = false) {
		...
		for (let i = 0; i < 3; i++) {
			const angle = eulerArray[i];
			if (angle) {
				const axisIndex = eulerArray[3].charCodeAt(i) - 88; // 88 = char code of 'X'
				if (axisIndex >= 0 && axisIndex < 3) {
					this._nodes.forEach((nodeInfo) => {
						if (nodeInfo.ignore && this._coordinateSystem !== CoordinateSystem.Local) {
							return; // ignore selected nodes that have an ancestor selected to prevent multiple rotations
						}

						const axis = new THREE.Vector3().setFromMatrixColumn(nodeInfo.matGizmo, axisIndex).normalize();
						const matRotate = new THREE.Matrix4().makeRotationAxis(axis, angle);
						const pos = new THREE.Vector3().setFromMatrixPosition(nodeInfo.matGizmo);
						...
						const scale = new THREE.Vector3().setFromMatrixScale(nodeInfo.matOrigin);
						const localAxis = axis.clone().transformDirection(new THREE.Matrix4().copy(nodeInfo.matOrigin).invert()).multiply(scale).normalize();
		...
		this._updateEulerForCreatingAnimationKey(eulerArray);

		if (!noSnapping && this._tool.getEnableSnapping()) {
			if (this._checkSceneSelectionIntersection()) {
				const testRotation = new THREE.Euler();
				for (let i = 0; i < 6; i++) {
					testRotation.set((euler.x + this._noHitRotation.x) * 0.5, (euler.y + this._noHitRotation.y) * 0.5, (euler.z + this._noHitRotation.z) * 0.5);
					this._rotate(testRotation, true);
					(this._checkSceneSelectionIntersection() ? euler : this._noHitRotation).copy(testRotation);
				}
				if (!this._noHitRotation.equals(testRotation)) {
					this._rotate(this._noHitRotation, true);
				}
			} else {
				this._noHitRotation.copy(euler);
			}
		}
	...
	RotateToolGizmo.prototype._updateGizmoTransformation = function(i, camera) {
		...
		if (this._beginValue != null && this._placementMode === GizmoPlacementMode.ObjectCenter) {
			// gizmo position should be fixed when gesture is started, but objects center changes when objects are rotated
			this._gizmo.position.setFromMatrixPosition(this._nodes[i].matGizmo);
			this._gizmo.matrix.setPosition(this._gizmo.position);
			this._gizmo.matrixWorld.setPosition(this._gizmo.position);
		}
