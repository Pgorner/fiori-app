...
sap.ui.define([
	...
	AngleMeasurementToolGizmo.prototype._updateCurrentFeature = function(nodeRef, point, viewport, allowFace, allowEdge, allowVertex) {
		...
		if (meshAnalyzer != null && (allowFace || allowEdge || allowVertex)) {
			const r = this._computeSphereRadius(point, viewport);
			const pointRadius = this._getPointRadius(viewport);
			const intersection = meshAnalyzer.intersectSphere(point, r, allowFace, allowEdge, allowVertex, false, false, pointRadius);
			if (this._is2D) {
