...
sap.ui.define([
	...
	MoveToolGizmo.prototype.init = function() {
		...
		this._noHitOffset = new THREE.Vector3();
		...
		function createGizmoArrow(dir, color, touchAreas) {
			...
			var mergedGeometry = BufferGeometryUtils.mergeGeometries([touchGeometry, touchGeometry2]);
	...
	MoveToolGizmo.prototype._onVisibilityChanged = function(event) {
	...
	MoveToolGizmo.prototype._move = function(offset, noSnapping = false) {
		...
		if (!noSnapping && this._tool.getEnableSnapping()) {
			if (this._checkSceneSelectionIntersection()) {
				...
				for (let i = 0; i < 6; i++) {
					testOffset.copy(offset).add(this._noHitOffset).multiplyScalar(0.5);
					this._move(testOffset, true);
					(this._checkSceneSelectionIntersection() ? offset : this._noHitOffset).copy(testOffset);
				}
				if (!this._noHitOffset.equals(testOffset)) {
					this._move(this._noHitOffset, true);
				}
			} else {
				this._noHitOffset.copy(offset);
			}
		}
