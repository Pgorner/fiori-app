...
sap.ui.define([
	...
	ScaleToolGizmo.prototype.init = function() {
		...
		this._noHitScale = new THREE.Vector3().setScalar(1);
	...
	ScaleToolGizmo.prototype._onVisibilityChanged = function(event) {
	...
	ScaleToolGizmo.prototype._getNodesProperties = function() {
		...
		this._nodes.forEach(function(nodeInfo) {
			...
			const userData = this._nodeUserDataMap?.get(node);

			nodeInfo.matParentInv ??= node.parent.matrixWorld.clone().invert();

			const matGizmo = (nodeInfo.matGizmo ?? this._gizmo.matrixWorld).clone();
			var sc = new THREE.Vector3();

			var wmatScale, matScale, scale;
			if (userData?.initialScale) {
				...
				if (this._coordinateSystem === CoordinateSystem.World) {
					...
					matScale = new THREE.Matrix4().copy(matGizmo).invert().multiply(wmatScale).multiply(matGizmo);
	...
	ScaleToolGizmo.prototype._alignAxesBetweenGizmoAndNodeCoordinates = function(node, withNode) {
		...
		const gizmoMat = this._gizmo.matrixWorld.clone();
	...
	ScaleToolGizmo.prototype._scale = function(scale, noSnapping) {
		...
		this._nodes.forEach((nodeInfo) => {
			if (!nodeInfo.ignore) {
				var node = nodeInfo.node;
				const matScale = nodeInfo.matGizmo.clone().scale(scale).multiply(nodeInfo.matGizmo.clone().invert());
		...
		if (!noSnapping && this._tool.getEnableSnapping()) {
			if (this._checkSceneSelectionIntersection()) {
				...
				for (let i = 0; i < 6; i++) {
					testScale.copy(scale).add(this._noHitScale).multiplyScalar(0.5);
					this._scale(testScale, true);
					(this._checkSceneSelectionIntersection() ? scale : this._noHitScale).copy(testScale);
				}
				if (!this._noHitScale.equals(testScale)) {
					this._scale(this._noHitScale, true);
				}
			} else {
				this._noHitScale.copy(scale);
			}
		}
