...
sap.ui.define([
	...
	MeasurementToolGizmo.prototype._createMeshAnalyzer = function(nodeRef) {
		let meshAnalyzer = null;
		let indexCount = 0;
		let lineCount = 0;
		let pointCount = 0;
		let v0, v1, v2;

		let processNode = function(nodeRef) {
			if (nodeRef.geometry && nodeRef.geometry.isBufferGeometry) {
				const pos = nodeRef.geometry.getAttribute("position");
				const idx = nodeRef.geometry.getIndex();
				const rg = nodeRef.userData.renderGroup;
				if (pos && idx) {
					if (meshAnalyzer) {
						for (let i = rg ? rg.start : 0, count = rg ? (rg.start + rg.count) : idx.count; i < count; i += 3) {
							v0 = new THREE.Vector3().fromArray(pos.array, idx.array[i] * 3);
							v1 = new THREE.Vector3().fromArray(pos.array, idx.array[i + 1] * 3);
							v2 = new THREE.Vector3().fromArray(pos.array, idx.array[i + 2] * 3);
					...
					// a point cloud
					if (meshAnalyzer) {
						for (let i = rg ? rg.start : 0, count = rg ? (rg.start + rg.count) : pos.count; i < count; ++i) {
							v0 = new THREE.Vector3().fromArray(pos.array, i * 3);
							v0.applyMatrix4(nodeRef.matrixWorld);
							meshAnalyzer._addVertex([v0.x, v0.y, v0.z]);
						}
					} else {
						pointCount += rg ? rg.count : pos.count;
					}
				}
			}
		...
		for (let pass = 0; pass < 2; ++pass) {
			processNode(nodeRef);
			if (pass === 0) {
				meshAnalyzer = new MeshAnalyzer(indexCount / 3, lineCount, pointCount);
			}
		}
	...
	MeasurementToolGizmo.prototype.show = function(viewport, tool) {
		...
		this._tool = tool;
		this._vsm = Element.getElementById(viewport.getViewStateManager());
	...
	MeasurementToolGizmo.prototype._getPointRadius = function(viewport) {
		const pointScale = viewport.getScene().getSceneRef?.().userData?.pointScale;
		// point radius in px = pointScale * ( viewportWidth / 1024 ) * projectionMatrix[ 0 ][ 0 ] / (distanceToCamera or 1 for orthographic projection)
		// point radius in world units = pointScale * ( viewportWidth / 1024 ) / viewportWidth
		return pointScale ? pointScale / 1024 : undefined;
	};
