...
sap.ui.define([
	...
	Viewport.prototype._setContent = function(content) {
		...
		if (content?.getMetadata().getName() === "sap.ui.vk.pdf.Document") {
			this._setPDFContent(content);
			return this;
		}
	...
	Viewport.prototype._setImage = function(image) {
		...
		if (!this._implementation || this._implementation.getMetadata().getName() !== nativeViewportImplementationType) {
			...
			var Class = sap.ui.require(nativeViewportImplementationType.replaceAll(".", "/"));
	...
	Viewport.prototype._setPDFContent = function(pdfDocument) {
		const pdfViewportImplementationType = "sap.ui.vk.pdf.Viewport";

		if (this._implementation?.getMetadata().getName() !== pdfViewportImplementationType) {
			this._destroyImplementation();

			const Class = sap.ui.require(pdfViewportImplementationType.replaceAll(".", "/"));
			this._implementation = new Class({
			...
			this._implementation.setContentConnector(this.getAssociation("contentConnector"));
			this.invalidate();
		}
		return this;
	};
