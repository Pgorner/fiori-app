...
sap.ui.define([
	...
	Viewport.prototype.init = function() {
		...
		// total number of bytes allocated in GPU (debug info)
		this._totalGpuAllocated = 0;

		// total number of bytes updated in GPU (debug info)
		this._totalGpuUpdated = 0;
		...
		// this._renderer = ThreeUtils.createWebGLRenderer(window.devicePixelRatio, true);
	...
	Viewport.prototype.exit = function() {
		...
		for (var i = 0; i < tools.length; i++) {
			var tool = Element.getElementById(tools[i]);
			if (tool) {
	...
	Viewport.prototype._startRenderLoop = function() {
		if (this._renderLoopRequestId === 0) {
	 ...
	 * @returns {this} <code>this</code> to allow method chaining.
	 * @deprecated Since version 1.50.0. Viewport now gets reference to the scene via {@link sap.ui.vk.ContentConnector}.
	 * @public
	 */
	...
	Viewport.prototype._setScene = function(scene) {
		...
		if (nativeScene) {
			...
			this._backgroundNode = this._findBackgroundNode();
			...
			for (var i = 0; i < nativeScene.children.length; i++) {
				...
				if (group.private && group.userData.defaultLights && group.children.length && group.children[0].isDirectionalLight) {
					this._cameraLight = group.children[0];
				}
			}
		}
	...
	Viewport.prototype.setCamera = function(camera) {
		...
		if (cam && this._renderer) {
			...
			if (!this._homeCamera && cam.getCameraRef() && !cam._dummyCamera) {
	...
	Viewport.prototype.shouldRenderFrame = function() {
		return this._renderLoopRequestId !== 0;
	};
	...
	Viewport.prototype._findBackgroundNode = function() {
		var nativeScene = this._getNativeScene();
		if (nativeScene == null) {
			return null;
		}
		var findBackgroundChildNodeRecursively = function(nodeRefs) {
			for (var i = 0; i < nodeRefs.length; i++) {
				var node = nodeRefs[i];
				if (node._vkGetNodeContentType() === NodeContentType.Background && node.visible === true && (node.name === "plane" || node.name === "sphere")) {
					if (node.parent.name === "Background" && node.parent.visible === false) {
						return null;
					}
					return node;
				} else {
					var child = findBackgroundChildNodeRecursively(node.children);
					if (child != null) {
						return child;
					}
				}
		...
		return findBackgroundChildNodeRecursively(nativeScene.children);
	};

	Viewport.prototype._onVisibilityChanged = function() {
		this._backgroundNode = this._findBackgroundNode();
	...
	Viewport.prototype._onOutliningOrSelectionChanged = function(event) {
		...
		for (var i = 0; i < tools.length; i++) { // loop over all oTools
			var tool = Element.getElementById(tools[i]); // get control for associated control
	...
	Viewport.prototype.isViewport3D = function() {
		return this.getMetadata().getName() === "sap.ui.vk.threejs.Viewport";
	};

	Viewport.prototype.isView3D = function() {
		var view = this.getCurrentView && this.getCurrentView();
		var dimension = view && view.getDimension();
		return this.isViewport3D() && !this._redlineHandler && !this._isPlanarActivated() && dimension !== 2;
	};
	...
	Viewport.prototype._renderLoop = function() {
		...
		if (this._viewportGestureHandler?.animateCameraUpdate()) {
		...
		const vsm = this._getViewStateManagerThreeJS();
		if (vsm != null) {
		...
		// NOTE: The name of the property `autoStartRendering` is misleading.
		if (this.getAutoStartRendering()) {
			if (vsm != null) {
				vsm._setJointNodeMatrix();
				const is3D = this.isView3D();
				const isPanoramic = this.isViewport3D() && this._isPanoramicActivated();
				// No need to do anything for navigation type
				if (is3D && !isPanoramic) {
	...
	function _renderDetailViews(renderer, camera, scene, boundingBox, cameraLight) {
		scene.children.forEach(function(root) {
			if (root.userData._vkDetailViews) {
				...
				root.userData._vkDetailViews.forEach(function(detailView) {
					if (detailView.node.visible) {
						detailView.detailView._render(renderer, camera, scene, boundingBox, cameraLight);
					}
				});
			}
		});
	...
	function checkPointCloudNonIdentityTransformation(node, geometryClusters) {
		const clusterIndex = node.userData.renderGroup?.clusterIndex;
		if (clusterIndex !== undefined) {
			const cluster = geometryClusters[clusterIndex];
			if (cluster.isPoints && node.matrixWorld.elements.some((value, index) => value !== identityMatrixElements[index])) {
				return true;
			}
		}
		return node.children.some(child => checkPointCloudNonIdentityTransformation(child, geometryClusters));
	}

	Viewport.prototype._setPointCloudTransformationDisabled = function(value) {
		this._disablePointCloudTransformation = value;
		// this._measurementSurface.getDomRef().classList[value ? "add" : "remove"]("sapUiVizkitRedlineSurface");
		this.setShouldRenderFrame();

		var nativeScene = this._getNativeScene();
		return value && nativeScene?.userData.geometryClusters && checkPointCloudNonIdentityTransformation(nativeScene, nativeScene.userData.geometryClusters);
	};

	Viewport.prototype._prepareForRenderingRecursive = function(node, geometryClusters, scratchBuf, textureDataArray, textureUpdateResults, force, noClusters, hidePMI) {
		if (node.visible === false || (hidePMI && node._vkGetNodeContentType() === NodeContentType.PMI)) {
			if (textureDataArray) {
		...
		if (!noClusters) {
			...
			ThreeUtils.updateNodeClusterRenderingInfo(node, geometryClusters, scratchBuf, textureDataArray, textureUpdateResults, this._disablePointCloudTransformation);
		}

		// update children
		var children = node.children;
		for (var i = 0, l = children.length; i < l; i++) {
			this._prepareForRenderingRecursive(children[i], geometryClusters, scratchBuf, textureDataArray, textureUpdateResults, force, noClusters, hidePMI);
		}
	};

	function setChildrenVisibility(node, visible) {
		node.children.forEach(function(child) {
			if (!child.userData.defaultLights) {
				child.visible = visible;
			}
		});
	}

	function updateCameraLight(cameraLight, nativeCamera) {
		if (cameraLight?.isDirectionalLight && nativeCamera) { // copy light direction from camera direction
			nativeCamera.updateMatrixWorld();
			cameraLight.position.setFromMatrixColumn(nativeCamera.matrixWorld, 2);
			cameraLight.updateMatrix();
			cameraLight.updateMatrixWorld(true);
		}
	}
	...
	Viewport.prototype.render = function() {
		...
		try {
			...
			updateCameraLight(this._cameraLight, nativeCamera);

			if (sceneUD.pointScale) {
				this._renderer.pointScale = sceneUD.pointScale;
			}
			...
			const hidePMI = this._showPMI === false;
			if (this._shouldProcessHierarchy) {
				...
				if (geometryClusters) {
					ThreeUtils.initClustersBeforeProcessingHierarchy(geometryClusters);
				}
				this._prepareForRenderingRecursive(nativeScene, geometryClusters, instanceScratchBuffer, dataTextureArray, textureUpdateResults, false, false, hidePMI);
				this._shouldDoTraversalRendering = textureUpdateResults[2] === 1;

				if (this._underlayGroup.children.length > 0 || this._overlayGroup.children.length > 0) {
					// we must add the lights to underlay/overlay or else they will render black
					for (let iRootChild = 0, lenRootChild = nativeScene.children.length; iRootChild < lenRootChild; ++iRootChild) {
						const rootChild = nativeScene.children[iRootChild];
						if (rootChild.userData.defaultLights) {
							if (this._underlayGroup.children.length > 0) {
								this._underlayGroup.children.push(rootChild);
							}
							if (this._overlayGroup.children.length > 0) {
								this._overlayGroup.children.push(rootChild);
							}
						}
					}
				}
			...
			sceneUD.doTraversalRendering = this._shouldDoTraversalRendering;
			...
			var partialClusters = geometryClusters ? ThreeUtils.prepareClustersForGpuUpload(geometryClusters) : 0;
			var timerDataTextureEnd = performance.now();

			var tools = this.getTools();
			var i, tool, gizmo, boundingBox;

			const someObjectsWereUpdated = _updateDynamicObjects(renderer, nativeCamera, nativeScene, this);

			// todo: we need to _updateBoundingBoxes() only if some dynamic objects are currently selected
			if (vsm && (shouldUpdateSelectionBoxes || someObjectsWereUpdated)) {
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				...
				if (vsm) {
					// expand the bounding box with selected objects because they can be invisible
					vsm._expandBoundingBoxWithSelected(boundingBox);
				}
				...
				if (!boundingBox.isEmpty()) {
					...
					if (this._camera.getUsingDefaultClipPlanes()) {
						for (i = 0; i < tools.length; i++) { // loop over all oTools
							tool = Element.getElementById(tools[i]); // get control for associated control
					...
					// check scene dimension and distance from camera
					var bboxSize = new THREE.Vector3();
					boundingBox.getSize(bboxSize);
					var cameraOffset = this._camera._nativeCamera.position;
					var maxSceneDimension, maxCameraDistance, val;
					for (var xyz = 0; xyz < 3; ++xyz) {
						val = Math.abs(bboxSize.getComponent(xyz));
						if (!xyz || (val > maxSceneDimension)) {
							maxSceneDimension = val;
						}

						val = Math.abs(cameraOffset.getComponent(xyz));
						if (!xyz || (val > maxCameraDistance)) {
							maxCameraDistance = val;
						}
					}

					if ((maxCameraDistance / maxSceneDimension) > 100 && !sceneUD.totalPointCount) { // this block should not be executed for scenes with point cloud
						// The distance from camera to the scene is extremely large, usually it is in the
						// range of 0.5 .. 2.0, sometimes up to 10 on maximum zoom out. We may be getting
						// precision issues in the cluster shader, as it multiplies a "float" vertex by instance
						// matrix and then once again by viewprojection and float precision is not good enough here...
						// So instead we revert to conventional three.js rendering which uses double matrices.
						sceneUD.useGeometryClusters = false;
					}
				}
			}
			...
			var rr = {
				...
				vramAllocated: renderInfo.vramAllocated,
				vramUpdated: renderInfo.vramUpdated
			};
			...
			_renderDetailViews(renderer, nativeCamera, nativeScene, boundingBox, this._cameraLight);
			...
			var tail = "";
			if (rr.vramAllocated > 0) {
				this._totalGpuAllocated += rr.vramAllocated;
				tail += ", allocated=" + (rr.vramAllocated / 1048576).toFixed(1) + "MB (" + (this._totalGpuAllocated / 1048576).toFixed(1) + "MB total)";
			}
			if (rr.vramUpdated > 0) {
				this._totalGpuUpdated += rr.vramUpdated;
				tail += ", updated=" + (rr.vramUpdated / 1048576).toFixed(1) + "MB (" + (this._totalGpuUpdated / 1048576).toFixed(1) + "MB total)";
			}
			if (partialClusters > 0) {
				tail += ", partialClusters=" + partialClusters;
			}
	...
	Viewport.prototype.getImage = function(width, height, topColor, bottomColor, includeSelection) {
		...
		var renderer = ThreeUtils.createWebGLImageRenderer(this._imageCanvas, width, height);
		...
		updateCameraLight(this._cameraLight, camera.getCameraRef());
		...
		if (!includeSelection) {
			if (vsm !== null) {
				...
				vsm.setSelectionStates([], selectedNodes, false, true);
			}
		}

		// workaround: problems may occur if underlay/overlay meshes are hidden in Viewport.render()
		// and because of this, they would not get rendered as part of a cluster here, because
		// overlay/underlay does not get special treatment in this method.
		// todo: use this.render() for rendering, not renderer.render() directly and this way
		// the underlay/overlay code will not be duplicated in multiple places.
		// Sync change to getObjectImageEx(), getMaterialImage(), getImage()
		var prevValueUseGeometryClusters = nativeScene.userData.useGeometryClusters;
		nativeScene.userData.useGeometryClusters = false;
		renderer.render(nativeScene, camera.getCameraRef());
		nativeScene.userData.useGeometryClusters = prevValueUseGeometryClusters;
	...
	Viewport.prototype.getMaterialImage = function(material, width, height, topColor, bottomColor, margin) {
		...
		const cameraLight = new THREE.DirectionalLight(0xFFFFFF, Math.PI);
		updateCameraLight(cameraLight, cameraRef);
		nodeRef.add(cameraLight);
	...
	Viewport.prototype.getObjectImageEx = function(nodeRefs, width, height, topColor, bottomColor, options) {
		...
		try {
			if (nodeRefs == null) {
				nodeRefs = nativeScene.children.slice().filter(function(node) { return !node.userData.defaultLights; });
			...
			updateCameraLight(lightNode.children[0], cameraRef);
			scene.add(lightNode);
			...
			// the underlay/overlay code will not be duplicated in multiple places.
			// Sync change to getObjectImageEx(), getMaterialImage(), getImage()
			var prevValueUseGeometryClusters = scene.userData.useGeometryClusters;
			scene.userData.useGeometryClusters = false;
			renderer.render(scene, cameraRef);
			scene.userData.useGeometryClusters = prevValueUseGeometryClusters;
	...
	Viewport.prototype._initDummyCamera = function(camera) {
		const boundingBox = this.getScene()._computeBoundingBox(true, true, false, true);
		if (!boundingBox.isEmpty()) {
			camera._dummyCamera = false;
			camera.setFov(14.27); // DVL camera initial FOV = (60 / π) radians = 1094.27° ≡ 14.27°
			const euler = this._upAxis === 4 ?
				new THREE.Euler(THREE.MathUtils.degToRad(90 - 30), 0, THREE.MathUtils.degToRad(55 - 90), "ZXY") : // Z-up
				new THREE.Euler(THREE.MathUtils.degToRad(-30), THREE.MathUtils.degToRad(55 - 90), 0, "YXZ"); // Y-up
	...
	Viewport.prototype.onSetContentConnector = function(contentConnector) {
		...
		contentConnector.attachContentChangesFinished(this.setShouldRenderFrame, this);
	...
	Viewport.prototype.onUnsetContentConnector = function(contentConnector) {
		contentConnector.detachContentChangesFinished(this.setShouldRenderFrame, this);
	...
	Viewport.prototype._onContentLoadingFinished = function(event) {
		...
		if (this.getCamera()?._dummyCamera) {
			this._initDummyCamera(this.getCamera());
		}
	...
	Viewport.prototype.resetCurrentView = function() {
		...
		this._getViewStateManagerThreeJS()._resetNodesStatusByCurrentView(this._currentView, true);
	...
	function registerRendering(viewport, scene) {
		...
		if (registryEntryIndex < 0) {
			...
			var webGLRenderer = ThreeUtils.createWebGLRenderer(window.devicePixelRatio, true);
	...
	Viewport.prototype.setShowPMI = function(showPMI) {
		this._showPMI = showPMI;
		return this;
	};
