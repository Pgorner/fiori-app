...
sap.ui.define([
	...
	var SceneBuilder = function(rootNode, contentResource, resolve, reject) {
		...
		this._geometryProxies = new Map(); // geometryId -> [ {meshId, submeshId, materialId} ] - list of temporary meshes that must be replaced with cluster geometry
		this._geometrySubmeshIndices = new Map(); // "nodeId:geometryId" -> submeshIndex
		this._materialMeshes = new Map(); // materialId -> [ meshId ]
		this._materialClones = new Map(); // materialId -> [ THREE.Material ]
		this._joints = [];
		this._pointCloudGroups = [];
		this._imageAddedToMaterialHandlers = [];
		this._isSmallScene = null; // a flag to enable some fancy effects for small scenes
		this._preprocessedClusterDefinitions = null; // Hint for VRAM allocation, can be null. [{cluster, verts, elems}] per preprocessed cluster
		this._preprocessedSubmeshToClusterMap = null; // Hint for VRAM allocation, can be null. map submeshId to index in _preprocessedClusterDefinitions
		this._temporaryGeometryBuffer = { // used for storing geometry vertices, normals, etc temporarily instead of multiple alloc()/free() [speed optimization]
			points: new Float32Array(65536 * 3),
			normals: new Float32Array(65536 * 3),
			colors: new Float32Array(65536 * 3),
			uvs: new Float32Array(65536 * 2),
			indices: new Uint16Array(65536), // indices can be enlarged as the maximum size is not guaranteed
			previousIndexCount: 0
		};

		/** @type {Map.<string, Map.<string, {string, int}>>} */
		this._nodeWithAssocicatedMarkers = new Map();
		/** @type {Map.<string, string>} */
		this._markNodeIdsToRefNodeIds = new Map();
		...
		this._maximumRenderedPointsBudget = 20 * 1000000.0; // how many point cloud points to display on user's PC.
	};
	...
	SceneBuilder.prototype.setNodePersistentId = function(nodeRef, nodeId, sceneId) {
		...
		var treeNode = userData.treeNode;
		if (treeNode == null) {
			treeNode = userData.treeNode = {};
		}
		var oldNodeId = treeNode.sid;

		this._resetCurrentScene(sceneId);

		userData.nodeId = treeNode.sid = nodeId;
		if (this._nodes) {
			if (oldNodeId) {
				this._nodes.delete(oldNodeId);
			}
	...
	var arrayToMatrix = function(arr) {
		...
		if (arr.length === 3) {
			...
			// 4x3 matrix
			matrix.fromArray(TransformationMatrix.convertTo4x4(arr));
	...
	SceneBuilder.prototype._createTemporaryMaterial = function(materialId, materialType) {
		...
		if (!nativeSceneUD.instanceDataTexture) {
			...
			nativeSceneUD.maxInstanceIndex = Math.floor(2048 * 2048 * 4 / 20) - 1;
			...
			nativeSceneUD.instanceDataTexture.onUpdate = function(texture) {
				texture.userData.textureUpdateParams = null;
			};
	...
	const NODE_CONTENT_TYPES = {
		REFERENCE: NodeContentType.Reference,
		ANNOTATION: NodeContentType.Annotation,
		BACKGROUND: NodeContentType.Background,
		SYMBOL: NodeContentType.Symbol,
		PMI: NodeContentType.PMI,
		POINT_CLOUD_GROUP: NodeContentType.PointCloudGroup
	};
	 ...
	 *       					<code>objectTransform</code>: object matrix as either 12(no shear), 16(full) or 3(position only) values. Optional</br>
	 ...
	 * 							<code>contentType</code>: Enum. Example: "SYMBOL", "PMI". Optional<br/>
	 * 							<code>geometryType</code>: Enum. Example: "PLANE", "SURFACE". Optional<br/>
	 *       					<code>materialId</code>: String. The id of the material the node is associated with. Optional<br/>
	 *       					<code>parametricId</code>: String. The id of the parametric object the node is associated with. Optional<br/>
	 ...
	 * 							<code>usageIds</code>: usageIds. Optional<br/>
	 *
	 *
	...
	SceneBuilder.prototype.createNode = function(nodeInfo, sceneId) {
		...
		if (nodeInfo.objectTransform) {
			userData.objectTransform = new THREE.Matrix4().fromArray(nodeInfo.objectTransform);
		}
		...
		const contentType = NODE_CONTENT_TYPES[nodeInfo.contentType];
		if (contentType) {
			node._vkSetNodeContentType(contentType);
		}
	...
	SceneBuilder.prototype.removeNode = function(nodeRef) {
		...
		this._vkScene.getPointCloudGroups().delete(nodeRef);
		this.setPointCloudGroups([], this._currentSceneId); // update point cloud groups
	...
	SceneBuilder.prototype._addGeometryToCluster = function(bbox, points, normals, colors, uvs, indices, isPolyline, materialId, submeshId, instanceCount, isFinalGeometry) {
		var nativeSceneUD = this._vkScene.getSceneRef().userData;
		...
		var indexCount = indices ? indices.length : 0;
		const isPointCloud = indexCount === 0;
		const elementType = isPolyline ? "l" : "t";
		// triangular mesh: normals are optional (for example, PMI does not have normals)
		// polylines: never use normals (some CAD .vds files may have empty (0,0,0) normals and we ignore them)
		// point clouds: may have normals, but usually do not
		const hasNormals = !isPolyline && (normals && normals.length === vertexCount * 3);
		const hasUVs = !isPolyline && !isPointCloud && (uvs && uvs.length === vertexCount * 2);
		const hasColors = (colors && colors.length === vertexCount * 3) || isPointCloud;
		let clusterType = materialId + ":" + (isPointCloud ? "p" : elementType) + (hasNormals ? "N" : "n") + (hasColors ? "C" : "c") + (hasUVs ? "U" : "u") + instanceCount;

		if (!nativeSceneUD.currentClusters) {
			nativeSceneUD.currentClusters = new Map(); // clusterType -> THREE.BufferGeometry
		}

		var allocatedIndex = this._preprocessedSubmeshToClusterMap ? this._preprocessedSubmeshToClusterMap.get(submeshId) : -1;
		var allocatedCluster = (allocatedIndex >= 0) ? this._preprocessedClusterDefinitions[allocatedIndex] : null;
		if (allocatedCluster) {
			clusterType = "pre" + allocatedIndex;
			maxVertices = allocatedCluster.verts;
			maxIndices = allocatedCluster.elems * (isPolyline ? 2 : 3);
		}

		var cluster = allocatedCluster ? allocatedCluster.cluster : nativeSceneUD.currentClusters.get(clusterType);
		if (!cluster || cluster.freeVertices < vertexCount || cluster.freeIndices < indexCount) {
			// create a new cluster
			cluster = new THREE.BufferGeometry();
			if (allocatedCluster) {
				allocatedCluster.cluster = cluster;
			} else {
			...
			if (isPointCloud) {
				cluster.userData.isPointCloud = true;
			} else if (isPolyline) {
			...
			if (hasColors) {
				cluster.setAttribute("color", new THREE.BufferAttribute(new Float32Array(maxVertices * 3), 3));
			}
			...
			let meshMaterial = this._getMaterialRef(materialId);
			var mesh;
			if (isPointCloud) {
				meshMaterial = meshMaterial.clone(); // clone point cloud material as some point cloud clusters may be transparent and some not
				meshMaterial.userData.isPointCloud = true;
				mesh = new THREE.Points(cluster, meshMaterial);
			} else if (isPolyline) {
				mesh = new THREE.LineSegments(cluster, meshMaterial);
			} else {
				mesh = new THREE.Mesh(cluster, meshMaterial);
				if (!hasNormals) {
					// Make three.js material double-sided if geometry does not have normals defined
					meshMaterial.side = THREE.DoubleSide;
					meshMaterial.userData.originalMaterialSide = THREE.DoubleSide;
				}
			}
			meshMaterial.needsUpdate = true; // the shader may need to be recompiled as some shader parameters are geometry dependent

			mesh.additionalRenderables = [];
		...
		if (cluster.userData.isPointCloud && this._pointCloudGroups.length > 0) {
			ThreeUtils._updateClusterInstanceAttribute(cluster, vertexStart, vertexCount, nativeSceneUD.currentInstanceIndex, this._pointCloudGroups);
		} else {
		...
		if (hasColors) {
			if (!colors) {
				...
				colors.fill(0.7); // use light grey if no colors are specified
			}
		...
		cluster.drawRange.count = isPointCloud ? (vertexStart + vertexCount) : (indexStart + indexCount);
		cluster.contents.push({
			instanceIndex: nativeSceneUD.currentInstanceIndex,
			vertexLast: (vertexStart + vertexCount) * 3,
			isFinal: isFinalGeometry
		});
		cluster.shouldUpload = 2; // 0 - no, 1 - prefer, 2 - must

		// compute bbox/sphere only for this mesh part of the cluster, not for the whole cluster
		if (bbox) {
			cluster.boundingBox = THREE.Box3.newFromPackedArray(bbox);
		} else {
		...
		const geometryDescriptor = {
			...
			indexStart: isPointCloud ? undefined : indexStart,
			indexCount: isPointCloud ? undefined : indexCount,
	...
	function makeRenderGroupFromDescriptor(descriptor, instanceIndex) {
		...
		return {
			...
			start: descriptor.indexStart === undefined ? descriptor.vertexStart : descriptor.indexStart,
			count: descriptor.indexCount === undefined ? descriptor.vertexCount : descriptor.indexCount,
	...
	SceneBuilder.prototype._cloneSubmesh = function(submesh, node) {
		...
		if (node.userData.materialId) {// the parent node overrides the original submesh material
			...
			TotaraUtils.pushElementIntoMapArray(this._materialMeshes, node.userData.materialId, submesh.userData.meshId);
		}
	...
	/**
	 * Called before doing multiple insertSubmesh() calls
	 * @param {any[]} meshes Array of meshes
	 * @private
	 */
	SceneBuilder.prototype.preprocessMeshes = function(meshes) {
		if (this._preprocessedSubmeshToClusterMap) {
			// only run once [this happens on scenes with multiple views]
			return;
		}

		this._preprocessedClusterDefinitions = [];
		this._preprocessedSubmeshToClusterMap = new Map();

		var timerPreprocessBegin = performance.now();
		var preprocessedMeshes = new Set();
		var preprocessedClusters = new Map();
		var totalPreprocessedMeshes = 0, totalPreprocessedUndefinedMeshes = 0, totalPreprocessedMeshInstances = 0;
		var totalPreprocessedLods = 0, totalPreprocessedUndefinedLods = 0;
		var totalPreprocessedVertices = 0, totalPreprocessedTriangles = 0, totalPreprocessedLines = 0, totalVRAM = 0;
		var totalPreprocessedPoints = 0, totalSkippedPoints = 0;

		var mi, mesh, meshId, submeshes, si, slen, submesh, materialId, lod, geometryId;
		var pointCount, elementCount, flags;
		var isPointCloud, isPolyline, hasNormals, hasColors, hasUVs, elementType;
		var meshNodes, instanceCount;
		var meshCount = meshes.length;
		var skipPointCloud, tmp;

		//  sort meshes by priority
		var bboxes = [];
		for (mi = 0; mi < meshCount; ++mi) {
			...
			isPointCloud = false;
			bboxes.length = 0;
			for (si = 0, slen = submeshes.length; si < slen; ++si) {
				// We take the first LOD as all LODs of the same submesh should have identical bounding boxes.
				lod = submeshes[si].lods[0];
				bboxes.push(lod.boundingBox);
			...
			meshNodes = this._meshNodes.get(meshId);
			if (meshNodes) {
				mesh.priority = this._computeMeshPriority(meshNodes, bboxes);
				if (isPointCloud) {
					tmp = meshNodes[0].name;
					tmp = tmp ? tmp.split(":") : null;
					if (tmp && tmp[1] && tmp[0] === "_machete.pointsSubMesh") {
						mesh.priority0 = Number(tmp[1]);
					}
				}
			} else {
				mesh.priority = 0;
		...
		meshes.sort(function(a, b) {
			if (a.priority0 !== undefined && b.priority0 !== undefined) {
				// load all submeshes with index #0 first, then #1, then #2...
				if (a.priority0 < b.priority0) {
					return -1;
				} else if (a.priority0 > b.priority0) {
					return 1;
				}
			}
		...
		for (mi = 0; mi < meshCount; ++mi) {
			mesh = meshes[mi];
			meshId = mesh.id;
			if (preprocessedMeshes.has(meshId)) {
				continue;
			}

			preprocessedMeshes.add(meshId);
			meshNodes = this._meshNodes.get(meshId);
			instanceCount = meshNodes ? meshNodes.length : 0;
			totalPreprocessedMeshInstances += instanceCount;
			if (instanceCount === 0) {
				totalPreprocessedUndefinedMeshes++;
				continue;
			}

			totalPreprocessedMeshes++;
			submeshes = mesh.submeshes;

			// loop submeshes
			for (si = 0, slen = submeshes.length; si < slen; ++si) {
				submesh = submeshes[si];
				materialId = submesh.materialId;
				skipPointCloud = false;

				lod = findBestLOD(submesh.lods);
				geometryId = !lod ? null : lod.id;
				if (geometryId == null) {
					continue;
				}

				pointCount = lod.pointCount;
				elementCount = lod.elementCount || 0;
				flags = lod.flags;
				if (pointCount > 0 && flags != null) {
					// we know enough info about this lod to determine in which cluster to put it
					isPointCloud = elementCount === 0;
					isPolyline = (flags & 1) > 0;
					hasNormals = !isPolyline && ((flags & 2) > 0);
					hasUVs = !isPolyline && !isPointCloud && (flags & 4) > 0;
					hasColors = (flags & 32) > 0;
					elementType = isPolyline ? "l" : "t";

					totalPreprocessedLods++;
					if (isPointCloud) {
						if (mesh.priority0 === 0 || totalPreprocessedPoints < this._maximumRenderedPointsBudget) {
							totalPreprocessedPoints += pointCount;
						} else {
							// note: in theory it is better to do this per mesh, not per submesh. But machete produces meshes with exactly 1 submesh for points.
							for (tmp = 0; tmp < instanceCount; ++tmp) {
								meshNodes[tmp].visible = false;
							}
							skipPointCloud = true;
							mesh.skipPointCloud = true;
							totalSkippedPoints += pointCount;
						}
					} else if (isPolyline) {
						totalVRAM += 2 * 2 * elementCount;
						totalPreprocessedVertices += pointCount;
						totalPreprocessedLines += elementCount;
					} else {
						totalVRAM += 2 * 3 * elementCount;
						totalPreprocessedVertices += pointCount;
						totalPreprocessedTriangles += elementCount;
					}

					if (!skipPointCloud) {
						totalVRAM += (3 + 1 + (hasNormals ? 3 : 0) + (hasColors ? 3 : 0) + (hasUVs ? 2 : 0)) * 4 * pointCount;

						TotaraUtils.pushElementIntoMapArray(preprocessedClusters,
							materialId + ":" + (isPointCloud ? "p" : elementType) + (hasNormals ? "N" : "n") + (hasColors ? "C" : "c") + (hasUVs ? "U" : "u") + instanceCount,
							{
					...
					totalPreprocessedUndefinedLods++;
				}
			}
		}

		preprocessedClusters.forEach(function(value) {
			// todo: we'd like to use spatial clustering here, but it requires building a kdtree
			// and also multiplying LOD bbox by node matrices (combined bbox in case of instancing)
			// and it looks like we don't even have node world matrices computed at this point.
			ThreeUtils.partitionCluster(value, this._preprocessedClusterDefinitions, this._preprocessedSubmeshToClusterMap, meshes);
		}, this);

		this._isSmallScene = totalPreprocessedLods < 2000 && totalPreprocessedVertices < 1000000 && totalPreprocessedPoints < 1000000;
		Log.info(preprocessedClusters.size + " unique cluster types detected in " + (performance.now() - timerPreprocessBegin).toFixed(1) + "ms.  "
			+ "Scene frame classification: " + (this._isSmallScene ? "small.  " : "large.  ")
			+ totalPreprocessedMeshes + " meshes (" + totalPreprocessedUndefinedMeshes + " undefined).  "
			+ totalPreprocessedMeshInstances + " mesh instances.  "
			+ (totalPreprocessedMeshInstances / totalPreprocessedMeshes).toFixed(1) + "X instancing.  "
			+ totalPreprocessedLods + " geometries (" + totalPreprocessedUndefinedLods + " undefined).  "
			+ totalPreprocessedVertices.toLocaleString() + " unique vertices.  "
			+ totalPreprocessedTriangles.toLocaleString() + " unique triangles.  "
			+ totalPreprocessedLines.toLocaleString() + " unique lines.  "
			+ totalPreprocessedPoints.toLocaleString() + " unique points.  "
			+ totalSkippedPoints.toLocaleString() + " skipped points.  "
			+ (totalVRAM / 1048576).toFixed(1) + "MB VRAM in "
			+ this._preprocessedClusterDefinitions.length + " clusters expected.");
	};

	SceneBuilder.prototype.getTempGeometryBuffer = function() {
		return this._temporaryGeometryBuffer;
	};

	SceneBuilder.prototype.setSubmeshes = function(meshes) {
		this.preprocessMeshes(meshes);
	 ...
	 *								<code>id</code>: string id of the submesh<br/>
	 ...
	 *										<code>pointCount</code>: number of vertices<br/>
	 *										<code>elementCount</code>: number of triangles/lines<br/>
	 *										<code>flags</code>: flags: isPolyline, hasUVs, etc<br/>
	...
	SceneBuilder.prototype.insertSubmesh = function(submeshInfo) {
		...
		var elementCount = (lod.elementCount === 0) ? null : lod.elementCount;
		var hasCounts = pointCount > 0 && elementCount > 0;
		...
		if (!boundingBox) {
		...
		} else if (flags != null && pointCount > 0 && elementCount == null) {
			// this is a point set object -> create an empty point set, but allocate it in a cluster
			const ep = ThreeUtils.buildEmptyMesh(boundingBox, flags, pointCount, elementCount, this._temporaryGeometryBuffer);
			geometryDescriptor = this._addGeometryToCluster(boundingBox, ep.points, ep.normals, ep.colors, undefined, undefined, false, submeshInfo.materialId, submeshId, instancesCount, false);
			submesh = new THREE.Points(geometryDescriptor.geometry, material);
		} else if (flags != null && (flags & 1) > 0 && hasCounts) {
			// this is a polyline object -> create an empty polyline, but allocate it in a cluster
			const el = ThreeUtils.buildEmptyMesh(boundingBox, flags, pointCount, elementCount, this._temporaryGeometryBuffer);
			geometryDescriptor = this._addGeometryToCluster(boundingBox, el.points, undefined, undefined, undefined, el.indices, true, submeshInfo.materialId, submeshId, instancesCount, false);
			submesh = new THREE.LineSegments(geometryDescriptor.geometry, material);
		} else {
			// general case - create a triangular temporary mesh
			var innerBoxesLod = findInnerBoxLOD(submeshInfo.lods);
			var innerBoxesData = innerBoxesLod ? innerBoxesLod.data : null;
			...
			var forceSingleCube = !innerBoxesData;
			var forceNonCluster = !hasCounts || instancesCount < 1;
			var tooSmall = hasCounts && (pointCount < 24 || elementCount < 12);
			if (tooSmall && this._isSmallScene) {
				// small object in a small scene -> show it anyways
				forceNonCluster = true;
				tooSmall = false;
			}
			...
			var r = tooSmall ? ThreeUtils.buildEmptyMesh(boundingBox, flags, pointCount, elementCount, this._temporaryGeometryBuffer)
				: ThreeUtils.buildTemporaryMesh(boundingBox, flags, pointCount, elementCount, innerBoxesData, forceSingleCube, limitPointElementCount, this._temporaryGeometryBuffer);
			...
			if (forceNonCluster || r.points.length > pointCount * 3 || r.indices.length > elementCount * 3) {
				...
				var tempIndexAttribute = new THREE.BufferAttribute(r.indices.slice(), 1);
				var tempPositionAttribute = new THREE.BufferAttribute(r.points.slice(), 3);
				tempGeometry.setIndex(tempIndexAttribute);
				tempGeometry.setAttribute("position", tempPositionAttribute);
				if (r.normals) {
					var tempNormalAttribute = new THREE.BufferAttribute(r.normals.slice(), 3);
					tempGeometry.setAttribute("normal", tempNormalAttribute);
				}
				tempGeometry.boundingBox = THREE.Box3.newFromPackedArray(boundingBox);
				submesh = new THREE.Mesh(tempGeometry, material);
			} else {
				// use existing preallocated space in a cluster
				geometryDescriptor = this._addGeometryToCluster(boundingBox, r.points, r.normals, r.colors, r.uvs, r.indices, false, submeshInfo.materialId, submeshId, instancesCount, false);
				submesh = new THREE.Mesh(geometryDescriptor.geometry, material);
			}
		}

		submesh.userData.geometryId = geometryId;
		submesh.userData.lodInfo = lod;

		// if tree has transform, it should overwrite one from the element.
		if (submeshInfo.transform) {
			// todo: will this work for cluster meshes? remove "transform" from submesh?
			submesh.applyMatrix4(arrayToMatrix(submeshInfo.transform));
		}
		submesh.updateMatrixWorld(true);

		// submesh.name = "submesh-" + meshId + "-" + submeshId + "-" + geometryId;
		submesh.userData.submeshId = submeshId;
		...
		var submeshIndex = TotaraUtils.pushElementIntoMapArray(this._meshSubmeshes, submeshInfo.meshId, submesh) - 1;
		if (geometryDescriptor) {
			TotaraUtils.pushElementIntoMapArray(this._geometries, lod.id, geometryDescriptor);
		} else {
			TotaraUtils.pushElementIntoMapArray(this._geometryProxies, lod.id, {
				meshId: submeshInfo.meshId,
				submeshId: submeshId,
				submeshIndex: submeshIndex,
				materialId: submeshInfo.materialId
			});
		}
		...
		if (nodes) {
			...
			nodes.forEach(function(node) {
				...
				if (geometryDescriptor) {
					...
					nativeSceneUD._instanceIndexToNodeMap ??= new Map(); // needs for hit testing nodes associated with some points
					nativeSceneUD._instanceIndexToNodeMap.set(nativeSceneUD.currentInstanceIndex, node);
					ThreeUtils.increaseCurrentInstance(nativeSceneUD);
				}

				// note: per-node index may be different from "submeshIndex" if a node has other children (like group nodes), for example pocketknife4.vds
				this._geometrySubmeshIndices.set(node.userData.nodeId + ":" + geometryId, node.children.length);
				node.add(clonedSubmesh);
				clonedSubmesh.updateMatrixWorld(true);
				applyNodeOpacityToSubmeshMaterial(clonedSubmesh, this._materialClones);
			}, this);
		}
	...
	function replaceSubmeshGeometry(children, submeshIndex, geometryId, geometryDescriptor, sceneUD, increaseInstanceCount) {
		var geometry = geometryDescriptor.geometry;
		var child = children[submeshIndex];

		if (!child || child.userData.geometryId !== geometryId) {
			Log.error("Invalid submesh index in replaceSubmeshGeometry");
			return;
		}

		if (child.geometry !== geometry) {
			var renderGroup = makeRenderGroupFromDescriptor(geometryDescriptor, sceneUD.currentInstanceIndex);
			if (increaseInstanceCount) {
				ThreeUtils.increaseCurrentInstance(sceneUD);
			}

			if (child.isMesh && !geometry.userData.isPolyline) {
				if (child.geometry) {
				...
				children[submeshIndex] = newSubmesh;
				child.parent = null;
			}
		}
	}
	 ...
	 *								<code>isPolyline</code>: boolean, true if the submesh is a polyline<br/>
	 *								<code>isPointCloud</code>: boolean, true if the submesh is a point cloud<br/>
	...
	SceneBuilder.prototype.setGeometry = function(geoInfo) {
		...
		if (geoInfo.isPointCloud) { // update scene point scale
			...
			nativeSceneUD.pointCloudBoundingBox.union(box);
			nativeSceneUD.totalPointCount ??= 0;
			nativeSceneUD.totalPointCount += points.length / 3;

			const size = nativeSceneUD.pointCloudBoundingBox.getSize(new THREE.Vector3());
			nativeSceneUD.pointScale = Math.sqrt((size.x * size.z * 2 + size.x * size.y + size.y * size.z) / nativeSceneUD.totalPointCount) * 700;

			// console.log(nativeSceneUD.pointScale, size, nativeSceneUD.totalPointCount);
		}
		...
		if (existingDescriptors) {
			...
			existingDescriptors.forEach((desc) => {
				if (!ThreeUtils.updateClusterGeometry(desc, data.points, data.normals, data.colors, data.uvs, data.indices, this._pointCloudGroups)) {
		...
		var skipHierarchyProcessing = true;
		var proxies = this._geometryProxies.get(geometryId);
		if (proxies) {
			// note: if we are here, then we will be removing an existing proxy mesh and creating a new cluster mesh instead
			skipHierarchyProcessing = false;

			var instanceCount = 0;
			proxies.forEach(function(submeshInfo) {
				var mn = this._meshNodes.get(submeshInfo.meshId);
				if (mn) {
			...
			var nativeSceneUD = this._vkScene.getSceneRef().userData;
			proxies.forEach(function(submeshInfo) {
				var geometryDescriptor = this._addGeometryToCluster(null, data.points, data.normals, data.colors, data.uvs, data.indices, geoInfo.isPolyline, submeshInfo.materialId, submeshInfo.submeshId, instanceCount, true);
				...
				if (submeshes) {
					replaceSubmeshGeometry(submeshes, submeshInfo.submeshIndex,
						geometryId, geometryDescriptor, nativeSceneUD, false);
				}
			}, this);
		...
		if (node) {
			...
			if (submeshes) {
				submeshes.forEach(function(submesh) {
					...
					this._geometrySubmeshIndices.set(node.userData.nodeId + ":" + submesh.userData.geometryId, node.children.length);
					node.add(clonedSubmesh);
					applyNodeOpacityToSubmeshMaterial(clonedSubmesh, this._materialClones);
				}, this);
			}
	...
	SceneBuilder.prototype.createMaterial = function(materialInfo) {
		...
		if (materialInfo.lineWidth > 0) {
			...
			// note: the same material can be used for rendering lines and triangles in some scenes

			// note: "PolylineMaterial" must be used for rendering lines with non-zero thickness and halo, not "LineBasicMaterial"

			if (materialInfo.lineColour) {
				material.color.fromArray(materialInfo.lineColour);
			}
	...
	SceneBuilder.prototype.updateTextureMap = function(materialId, type) {
		...
		textureLoader.load(imageDataUri, function(texture) {
			...
			if (this._fireSceneUpdated) {
				// note: cannot do _fireSceneUpdated(true) here, as it may be an opacity texture
	...
	SceneBuilder.prototype.insertView = function(viewInfo, sceneId) {
		...
		if (description) {
			...
			description = "<pre class=\"sapThemeVizKitViewGalleryStepDescriptionPre\">" + description + "</pre>";
		}
	...
	SceneBuilder.prototype.cleanup = function() {
		...
		this._vkScene?.clearUnusedMaterials();
		...
		this._geometryProxies.clear();
		this._geometrySubmeshIndices.clear();
		...
		this._preprocessedClusterDefinitions = null;
		this._preprocessedSubmeshToClusterMap = null;
	...
	SceneBuilder.prototype.setParametricContent = function(nodeId, parametricContent, sceneId) {
		...
		const parametricType = parametricContent.type;
		if (parametricType in generateFunc === false) {
			Log.warning("Attempt to load unknown parametric type: " + parametricType);
			return;
		}
		...
		const parametricMesh = ParametricGenerators[generateFunc[parametricType]](parametricContent, material);
		if (node.userData.nodeContentType) {
			parametricMesh.userData.nodeContentType = node.userData.nodeContentType;
		}
		parametricMesh.userData.skipIt = true;
		parametricMesh.renderOrder = node.renderOrder;
		node.add(parametricMesh);
	};
	...
	/**
	 * Set point cloud groups content
	 * @param {any} pointCloudGroups The object with point cloud groups content
	 * @param {any} sceneId The scene identifier
	 */
	SceneBuilder.prototype.setPointCloudGroups = function(pointCloudGroups, sceneId) {
		this._resetCurrentScene(sceneId);

		const sceneUserData = this._vkScene.getSceneRef().userData;

		const sceneGroups = this._vkScene.getPointCloudGroups();
		pointCloudGroups.forEach(groupInfo => {
			const node = this.getNode(groupInfo.nodeId, sceneId);
			if (node) {
				...
				if (groupInfo.items?.length > 0) {
					...
					groupInfo.node = node;
					if (!nodeUserData.renderGroup) {
						sceneUserData.currentInstanceIndex ??= 0;
						nodeUserData.renderGroup = {
							instanceIndex: sceneUserData.currentInstanceIndex,
							clusterSet: new Set()
						};
						sceneUserData._instanceIndexToNodeMap ??= new Map(); // needs for hit testing nodes associated with some points
						sceneUserData._instanceIndexToNodeMap.set(sceneUserData.currentInstanceIndex, node);
						ThreeUtils.increaseCurrentInstance(sceneUserData);
					}

					const groups = groupInfo.items.map(item => {
						const group = new PointCloudGroup(item);
						group.sequence = parseInt(item.sequence, 10);
						group.type = item.type || 0;
						group.node = node;
						group.instanceIndex = nodeUserData.renderGroup.instanceIndex;
						return group;
					});
					sceneGroups.set(node, groups);
				} else {
					sceneGroups.delete(node);
				}
			}
		});

		const groupsFlatList = this._pointCloudGroups = [];
		sceneGroups.forEach(groups => {
			groups.forEach(group => {
				group.node.userData.renderGroup.clusterSet.clear();
				groupsFlatList.push(group);
			});
		});
		groupsFlatList.sort((a, b) => b.sequence - a.sequence); // sorted by sequence
		// console.log(groupsFlatList);

		this._geometries.forEach(descs => {
			descs.forEach(desc => {
				ThreeUtils._updateGeometryClusterInstance(desc, groupsFlatList);
			});
		});
	};
	...
	SceneBuilder.prototype._computeMeshPriority = function(nodes, submeshBBoxes) {
		// For each mesh we calculate its priority in the request queue - the more space all
		// the instances of one mesh occupy the higher its priority.

		meshBBox[0] = meshBBox[1] = meshBBox[2] = Infinity;
		meshBBox[3] = meshBBox[4] = meshBBox[5] = -Infinity;

		for (var i = 0, count = submeshBBoxes.length; i < count; ++i) {
			var submeshBBox = submeshBBoxes[i];

			meshBBox[0] = Math.min(meshBBox[0], submeshBBox[0]);
			meshBBox[1] = Math.min(meshBBox[1], submeshBBox[1]);
			meshBBox[2] = Math.min(meshBBox[2], submeshBBox[2]);
			meshBBox[3] = Math.max(meshBBox[3], submeshBBox[3]);
			meshBBox[4] = Math.max(meshBBox[4], submeshBBox[4]);
			meshBBox[5] = Math.max(meshBBox[5], submeshBBox[5]);
		}

		// We can safely assume that in CAD models all instances of the mesh have the same
		// scale. So, instead of calling `getMaxScaleOnAxis()` for each node we call it for
		// the first node and multiply its scale by the number of mesh instances.
		var node = nodes[0];
		var scale = node.matrixWorld.getMaxScaleOnAxis();

		var width = 0;
		var height = 0;
		var depth = 0;

		if (meshBBox[3] >= meshBBox[0] && meshBBox[4] >= meshBBox[1] && meshBBox[5] >= meshBBox[2]) {
			width = meshBBox[3] - meshBBox[0];
			height = meshBBox[4] - meshBBox[1];
			depth = meshBBox[5] - meshBBox[2];
		}

		var bboxSurfaceHalfArea = width * (height + depth) + height * depth;

		return bboxSurfaceHalfArea * scale * nodes.length;
	};

	/**
	 * Bind a marker's transformation to a reference node
	 * @param {string} referenceNodeId 	Marker's reference node id
	 * @param {string} markerNodeId 	Marker's node id
	 * @param {int} coordinateSpace 	Marker's coordinate space when created
	 * @public
	 */
	SceneBuilder.prototype.bindMarkerNodeTransformationToReferenceNode = function(referenceNodeId, markerNodeId, coordinateSpace) {
		if (!referenceNodeId || !markerNodeId) {
			return;
		}

		var markerNodes = this._nodeWithAssocicatedMarkers.get(referenceNodeId);
		if (markerNodes == null) {
			markerNodes = new Map();
			this._nodeWithAssocicatedMarkers.set(referenceNodeId, markerNodes);
		}
		markerNodes.set(markerNodeId, { coordinateSpace: coordinateSpace });
		this._markNodeIdsToRefNodeIds.set(markerNodeId, referenceNodeId);
	};

	/**
	 * Unbind a marker's transformation from a reference node
	 * @param {string} referenceNodeId 	Marker's reference node id
	 * @param {string} markerNodeId 	Marker's node id
	 * @public
	 */

	SceneBuilder.prototype.unbindMarkerNodeTransformationFromReferenceNode = function(referenceNodeId, markerNodeId) {
		if (!referenceNodeId || !markerNodeId) {
			return;
		}
		var markerNodes = this._nodeWithAssocicatedMarkers.get(referenceNodeId);
		...
		markerNodes.delete(markerNodeId);
		this._markNodeIdsToRefNodeIds.delete(markerNodeId);
	};

	/**
	 *
	 * Get all the bound markers for a given node id
	 * @param {string} referenceNodeId 	Marker's reference node id
	 * @public
	 * @returns {Map.<string, Map.<string, {string, int}>>} Map of node's associated marker information
	 */
	SceneBuilder.prototype.getBoundMarkerNodes = function(referenceNodeId) {
		if (!referenceNodeId) {
			return null;
		}
		return this._nodeWithAssocicatedMarkers.get(referenceNodeId);
	};

	/**
	 *
	 * Get all the nodes with bound markers
	 * @public
	 * @returns {Map.<string, Map.<string, {string, int}>>} Map of node sids and their associated marker information
	 */
	SceneBuilder.prototype.getAllNodesWithBoundMarkers = function() {
		return this._nodeWithAssocicatedMarkers;
	};
	 ...
	 */
	SceneBuilder.prototype.getReferenceNodeFromMarkerNodeId = function(markNodeId) {
		var nodeSid = this._markNodeIdsToRefNodeIds.get(markNodeId);
		if (nodeSid) {
			return this.getNode(nodeSid, this.sceneId);
		}
		return null;
	};

	SceneBuilder.prototype.updateMarkerCoordinateSpace = function(previousMarkerNodeId, referenceNodeId, newMarkerNodeId) {
