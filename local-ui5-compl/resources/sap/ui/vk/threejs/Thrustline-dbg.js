...
sap.ui.define([
	...
	Thrustline.prototype._updateMeshes = function(viewportSize) {
		...
		if (lineStyle.widthCoordinateSpace === 3) {
			lineWidth = lineWidth ? lineWidth * viewportSize.y / 2 : 1;
			dashPatternScale = dashPatternScale ? dashPatternScale * viewportSize.y / 2 : 1;
		}
	...
	Thrustline.prototype._update = function(renderer, camera, viewportSize) {
		...
		this.getSegments().forEach(function(segment) {
			...
			if (polylineMesh) {
				...
				const startPoint = new THREE.Vector3().copy(startItem.boundPoints[segment.startBoundIndex]).applyMatrix4(startItem.target.matrixWorld);

				var endItem = items[segment.endItemIndex];
				const endPoint = new THREE.Vector3().copy(endItem.boundPoints[segment.endBoundIndex]).applyMatrix4(endItem.target.matrixWorld);
