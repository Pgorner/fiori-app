...
sap.ui.define(["sap/ui/thirdparty/jquery", "./BaseController", "./DraftContext", "sap/base/Log", "sap/ui/model/Context", "sap/ui/generic/app/util/ActionUtil"
	 ...
	 */
	var DraftController = BaseController.extend("sap.ui.generic.app.transaction.DraftController", /** @lends sap.ui.generic.app.transaction.DraftController.prototype */ {
		metadata: {
			publicMethods: [
	...
	DraftController.prototype.createDraft = function(sEntitySet, sPath, mParameters) {
		...
		return new Promise(function(resolve, reject) {
			...
			var fnError = function(oError){
				var aPath = createdContext.getPath() ? [createdContext.getPath()] : [];
				that._oModel.resetChanges(aPath, undefined, true);
				reject(oError);
			};
	 ...
	 * @param {boolean} [oParameters.bUseNewActionForCreate] manifest entry indicating whether newAction has to be used for draft creation
	...
	DraftController.prototype.createNewDraftEntity = function(sEntitySet, sPath, vPredefinedValues, bCanonicalRequest, oParameters) {
		...
		var oContextForNewAction = oParameters.bUseNewActionForCreate && new Context(that._oModel, sPath);
		var sFunctionImport = oContextForNewAction && that.getDraftContext().getODataDraftFunctionImportName(oContextForNewAction, "NewAction");
		...
		if (oFunctionImport) {
			...
			var oActionProcessor = new ActionUtil({
				...
				contexts: [oContextForNewAction],
				functionImport: oFunctionImport,
				functionImportPath: sFunctionImport,
				isDraftEnabled: bIsDraft,
				isCreateAction: true,
				contextIndependentParameterData: vPredefinedValues,
				expand: oParameters.sRootExpand,
				operationGrouping: undefined
			});

			// NOTE: The following properties are set using the setter methods to ensure even if the values
			// are i18N paths, it is taken over and not been resolved at the time of setting
			oActionProcessor.setActionLabel(sFunctionImportDialogTitleText);
			oActionProcessor.setActionButtonText(sFunctionImportDialogActionButtonText);
			
			oPromise0 = oActionProcessor.call(true).then(function(oResponse) {
