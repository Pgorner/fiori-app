...
sap.ui.define(["sap/ui/base/EventProvider", "sap/ui/generic/app/util/ModelUtil", "sap/ui/generic/app/util/DraftUtil", "sap/ui/generic/app/util/Queue", "sap/base/Log"],
	function (EventProvider, ModelUtil, DraftUtil, Queue, Log) {
		 ...
		 * @public
		 */
		var BaseController = EventProvider.extend("sap.ui.generic.app.transaction.BaseController", /** @lends sap.ui.generic.app.transaction.BaseController.prototype */ {
			metadata: {
				publicMethods: [
		...
		BaseController.prototype._callAction = function (sFunctionImportName, oContext, mParameters) {
			...
			return new Promise(function (resolve, reject) {
				...
				that._oModel.callFunction(sFunctionImport, {
					...
					expand: mParameters.functionImport.httpMethod === "POST" ? mParameters.expand : undefined
				});
			});
		};
		...
		BaseController.prototype._read = function (sPath, mParameters, bIsSideEffect) {
			...
			if (bIsSideEffect) {
				var oSideEffectsPromise = that._oModel.requestSideEffects(mParameters.context, {	
					groupId: mParameters.batchGroupId,
					urlParameters: urlParams
				});
				Promise.resolve(oSideEffectsPromise).then(/** @param {Array<sap.ui.model.ListBinding>} aAffectedListBindings */ function (aAffectedListBindings){
					var mEventParams = {"reason": "SIDE_EFFECT_TARGET"};
					// Fire "dataReceived" event on all the list bindings which are the side effect targets
					aAffectedListBindings.forEach(/** @param {sap.ui.model.ListBinding} oListBinding */ function (oListBinding){
						oListBinding.fireDataReceived(mEventParams);
					});
				});
				return oSideEffectsPromise;
			} 
		...
		BaseController.prototype._submitChanges = function (mParameters) {
			...
			if (this._oModel.bUseBatch == true) {
				return new Promise(function (resolve, reject) {
					var mCallbacks = that._getRequestCallbacks(resolve, reject);
						that._oModel.submitChangesWithChangeHeaders({
							batchGroupId: mParameters.batchGroupId,
							success: mCallbacks.success,
							error: mCallbacks.error,
							eTag: mParameters.eTag,
							changeHeaders: mParameters.changeHeaders
						});
				});
		 ...
		 * @param {boolean} [mParameters.keepDraftMergeTimer] If set to <code>true</code>, the delayed draft merge timer will not be cleared
		...
		BaseController.prototype.triggerSubmitChanges = function (mParameters) {
			...
			// NOTE: this property (bStrict) is used for a specific use case. Dont use it elsewhere.
			if (mParameters.hasOwnProperty('bStrict')) {
				mParameters.changeHeaders = {
					prefer: mParameters.bStrict ? "handling=strict" : "handling=lenient"
				};

				delete mParameters.bStrict;
			}
			...
			if (this._oDraftMergeTimer.nTimeoutID && !mParameters.keepDraftMergeTimer) {
					...
					if (oResponse && oResponse.responseData && oResponse.responseData.__batchResponses) {
						...
						for (var i = 0; i < oResponse.responseData.__batchResponses.length; i++) {
							...
							if (oBatchResponse.__changeResponses) {
								...
								aNormalizedResponses.push(_check(oBatchResponse, mParameters));
			...
			return this._oQueue.enqueue(fFunc, {
				draftSave: mParameters.draftSave	// draftSave is used by Fiori Elements to change DraftIndicatorState's state
			});
		};
