...
sap.ui.define([
	...
	function(coreLibrary, Message, MessageType, JSONModel, MessageToast, syncStyleClass, Fragment, Messaging) {
		"use strict";
		 ...
		 *
		 * @public
		 * @deprecated As of version 1.132. As transient message handling is done by FE, the applications don't need an API to explicitly handle transient messages.
		 */
		function handleTransientMessages(vMessageDialogData, sActionLabel, oActionButtonConfig) {
			...
			/**
			 * @type {sap.m.Dialog|undefined}
			 */
			var oDialog;
			var oMessageView, oSettingModel;
				...
				.then(function (odialog) {
					...
					if (oMetaModel) {
						aTransientMessages.forEach(function (oMessage) { // loop over all messages, derive group header from their target and put them to mMessageToGroupName
							var sMessageTarget = oMessage.getTargets()[0];
							if (!sMessageTarget) {
								return;
							}
					...
					oSettingModel.setSizeLimit(9999); //By Default the size of the message popover list binding is 100, which is very less. Therefore setting it to 9999.
		...
		function removeTransientMessages() {
			...
			if (aTransientMessages.length > 0) {
				Messaging.removeMessages(aTransientMessages);
			}
		}
		...
		function addTransientErrorMessage(sMessage, sDescription, oModel) {
			...
			Messaging.addMessages(oTransientMessage);
		}
