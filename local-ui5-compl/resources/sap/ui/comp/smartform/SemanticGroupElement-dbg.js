...
sap.ui.define([
	...
	SemanticGroupElement.prototype.getFieldsForRendering = function(){
		var iIndex,
			...
			bFieldHasSemanticObject = false,
			bAllFieldsSmartLink = true,
		...
		for (iIndex = 0; iIndex < aFields.length; iIndex++) {
			...
			if (!bFieldHasSemanticObject) {
				bFieldHasSemanticObject = oField._hasSemanticObject();

				if (!bFieldHasSemanticObject) {
					bAllFieldsSmartLink = false;
				}
			}

			if (!bAllFieldsSmartLink && !bAllFieldsNotEditable) {
				break;
			}
		}
