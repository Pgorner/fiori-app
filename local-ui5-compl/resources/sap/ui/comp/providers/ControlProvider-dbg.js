...
sap.ui.define([
	 ...
	 * @private
	 */
	ControlProvider.prototype._fnNavigationTargetsObtainedListener = function(oEvent) {
		var oObjectIdentifier = oEvent.getSource().getFieldInfo()._getSourceControl();
	...
	ControlProvider.prototype._createObjectIdentifierTemplate = function(oViewField, oType, bFirstKeyField) {
		...
		const fnCheckNavigationTargetsObtained = function(oSemanticObjectController) {
			return !(oSemanticObjectController.getReplaceSmartLinkNavigationTargetsObtained() || oSemanticObjectController.hasListeners("navigationTargetsObtained"));
		};
		...
		oObjectIdentifier.attachEvent("ObjectIdentifier.designtime", function(oEvent) {
			if (oSmartLink) {
				const oFieldInfo = oSmartLink.getFieldInfo();
				oSmartLink.setParent(oEvent.getSource());
				oFieldInfo.setSourceControl(oEvent.getSource());
