...
sap.ui.define([
	 ...
	 * the <code>DataFieldDefault</code>, <code>LineItem</code>, <code>DataPoint</code> or <code>FieldGroup</code> annotations
	 * which is then used during the initialization process to create the
	 * <code>ObjectStatus</code>.
	 *
	 * @param {object} oMetadata the metadata used to initialize the factory
	 * @param {string} sCriticalityQualifier specifies from which qualifier of the <code>DataPoint</code> or <code>FieldGroup</code> annotation the criticality should be taken
	 * @public
	 */
	ODataHelper.prototype.applyCriticality = function(oMetadata, sCriticalityQualifier) {
		var oEdmProperty = this.getEdmProperty(oMetadata),
			...
			oDataFieldDefaultAnnotation, oDataPointRecord;
		...
		//Applying FieldGroup Criticality
		if (oFieldGroupAnnotations) {
			for (let i = 0; i < oFieldGroupAnnotations.length; i++) {
				const oFieldGroup = oFieldGroupAnnotations[i];

				if ((!oFieldGroup.groupName && !sCriticalityQualifier) || oFieldGroup.groupName === sCriticalityQualifier) {
					if (oFieldGroup.criticality && oFieldGroup.criticality[oEdmProperty.name]) {
						oEdmProperty.criticalityInfo = oFieldGroup.criticality[oEdmProperty.name];
						return;
					}
				}
			}
