...
sap.ui.define([
	...
	SmartTable.prototype.setShowDetails = function(bValue) {
		if (!this._isMobileTable || (bValue === this.getShowDetails())) {
			return;
		}

		if (this.getUseTablePersonalisation() && document.location.search?.includes("sap-ui-xx-persistShowDetails")) {
	...
	/**
	 * Returns the value of the <code>showDetails</code> property.
	 *
	 * @returns {boolean} The value of the <code>showDetails</code> property
	 * @private
	 * @ui5-restricted sap.suite.ui.generic
	 */
	SmartTable.prototype.getShowDetails = function() {
	...
	SmartTable.prototype._triggerUI5ClientExport = function(bExportAs) {
		...
		const oLibrary = Library.load({ name: "sap.ui.export" });
		oLibrary.catch(function() {
			MessageBox.error(Library.getResourceBundleFor("sap.ui.comp").getText("SMARTTABLE_ERROR_MISSING_EXPORT_LIBRARY"));
		});
		...
		if (!aSheetColumns || !aSheetColumns.length) {
			MessageBox.error(Library.getResourceBundleFor("sap.ui.comp").getText("SMARTTABLE_NO_COLS_EXPORT"), {
	...
	SmartTable.prototype._createExportColumnConfiguration = function() {
		...
		const aColumns = this._oTable.getColumns(!!this._isMobileTable);
	...
	SmartTable.prototype._getExportColumnConfiguration = function(oColumn, bUtc) {
		...
		if (oP13nData.unit) {
			...
			if (sType === "DateTimeOffset") {
				...
				} else if (typeof oP13nData.additionalProperty === "string") {
	...
	SmartTable.prototype._createTable = function() {
		...
		if (this._isAnalyticalTable) {
			this.getTable().setProperty("extendedGroupHeaderMenu", false);
		}
	...
	SmartTable.prototype._setTableOverflow = function() {
		const oDomRef = this.getDomRef();
		oDomRef.style.overflow = oDomRef.scrollHeight > oDomRef.offsetHeight ? "" : "hidden";
	};

	SmartTable.prototype.onAfterRendering = function(oEvent) {
		// SNOW: CS20240006981749. A SmartTable of type GridTable can start with overflow: hidden. As soon as the inner GridTable overflows
		// its container, the SmartTable removes overflow: hidden. With this approach, the SmartTable should no longer overflow the DynamicPage
		// container unless it's necessary.

		if (!this._isMobileTable && !this._vTableResizeHandlerId) {
			const oPage = this.$().closest(".sapFDynamicPage")[0];
			if (oPage) {
				this._setTableOverflow();
				this._vTableResizeHandlerId = ResizeHandler.register(this._oTable, this._setTableOverflow.bind(this));
			} else {
				this._vTableResizeHandlerId = true;
			}
		}
	};
	...
	SmartTable.prototype._createColumnMenuContent = function(oEvent) {
		...
		if (this.getUseTablePersonalisation()) {
			...
			if (this._bIsSortPanelEnabled && (this._isMobileTable || !this._aExistingColumns.includes(oColumnInfo.columnKey) || (oShowEntry && oShowEntry.sort))) {
				if (!this._oQuickSort) {
			...
			if (this._bIsGroupPanelEnabled && this._isMobileTable) {
			...
			var oColumnMap = this._oPersController.getColumnMap();
			...
			if (this._bIsSortPanelEnabled && PersonalizationUtil.hasSortableColumns(oColumnMap) &&
				(this._isMobileTable || !this._aExistingColumns.includes(oColumnInfo.columnKey) || (oShowEntry && oShowEntry.sort))) {
				if (!this._oSortItem) {
			...
			if (!this._oQuickActionContainer.getEffectiveQuickActions().length && !this._oItemContainer.getEffectiveItems().length) {
				oEvent.preventDefault();
				TableUtil.announceEmptyColumnMenu();
			}
		}
	};
	...
	SmartTable.prototype._getColumnSortProperty = function(oColumn) {
		...
		if (!sSortProperty) {
			sSortProperty = oColumn.data("p13nData")?.sortProperty;
		}
	...
	SmartTable.prototype._onCustomSort = function(oEvent, bAscend, oColumn) {
		...
		if (oColumnData) {
			...
			var sEventName = this._isMobileTable ? "_sort" : "sort";
	...
	SmartTable.prototype._adaptAdditionalProperties = function(oColumn) {
		...
		if (oColumnData.additionalProperty && oColumnData.additionalSortProperty) {
			...
			aSortProperties.every(function(sSortProperty) {
				...
				return !this._updateSortIndicator(oColumn, sSortProperty, this._isMobileTable);
			}.bind(this));
		}
	};
	...
	SmartTable.prototype._adaptCustomGroup = function() {
		if (this._oCurrentVariant) {
			...
			this._oTable.getColumns().forEach(function(oColumn) {
				...
				if (sSortProperty) {
					if (aGroupItems) {
						oSortItem = aGroupItems.find(function (oSortItem) {
							return sSortProperty === oSortItem.columnKey;
						});
					}
	...
	SmartTable.prototype._createInfoToolbar = function() {
		...
		if (sUseInfoToolbar === InfoToolbarBehavior.Off
			|| (!this._isMobileTable && sUseInfoToolbar === InfoToolbarBehavior.Auto)) {
			...
			this._aFilteredColumnsBeforePersonalisationDataChange?.forEach((oFilter) => {
				...
				if (oColumn && oColumn.setFiltered) {
		...
		if (iFiltersLength) {
			...
			aFilters.forEach(function(oFilter) {
				...
				if (oColumn) {
					...
					if (!this._isMobileTable && oColumn.setFiltered) {
						oColumn.setFiltered(false);
					}
				}
			}, this);
	...
	SmartTable.prototype.exit = function() {
		...
		this._oView = null;
		this._oTable = null;
		if (this._vTableResizeHandlerId) {
			if (this._vTableResizeHandlerId !== true) {
				ResizeHandler.deregister(this._vTableResizeHandlerId);
			}
			this._vTableResizeHandlerId = null;
		}
	};
