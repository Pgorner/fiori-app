...
sap.ui.define([
...
], (LinkDelegate, LinkType, SapBaseLog, Util, whitespaceReplacer, Component, isPlainObject, ContactDetailsController, Factory, VBox, mdcLibrary, merge, LinkItem, Title, Link, mLibrary, CustomData, Text, PersonalizationUtil, coreLibrary, Panel, SemanticObjectController, LinkData, isEmptyObject) => {
	"use strict";
	...
	SmartLinkDelegate.fetchAdditionalContent = async(oLink) => {
		...
		const oControl = sap.ui.getCore().byId(oSmartLink.sId);
		if (!oControl) {
	...
	SmartLinkDelegate.fetchNavigationTargets = async(oLink, oBindingContext) => {
		...
		await sap.ui.getCore().loadLibrary('sap.ui.fl', { async: true });
	...
	*/
	...
	SmartLinkDelegate._getParent = (oLink) => {
		return oLink.getParent() ? oLink.getParent() : sap.ui.getCore().byId(oLink.getAssociation("sourceControl"));
	};
