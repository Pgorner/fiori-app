sap.ui.define([
	...
	function generateRuntimeHash(configurationId) {
		return sap.ushell.Container.getServiceAsync("CrossApplicationNavigation").then( function (oService) {
		 var oHref =  oService.hrefForExternalAsync({
			target: {
	...
	function generateRuntimeLink(configurationId) {
		var baseUrl = jQuery(window.location).attr('href').split('#')[0].split('?')[0];
		var hash = generateRuntimeHash(configurationId);
		return hash.then(function(shref){
		return baseUrl + shref;
		});
	}
