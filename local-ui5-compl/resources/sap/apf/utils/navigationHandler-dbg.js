...
sap.ui.define([
...
], function(filterSimplify, utils, executeFilterMapping, exportToGlobal, deepExtend, Uid, HashChanger, jQuery){
	'use strict';
	/**
	 ...
	 * @alias sap.apf.utils.NavigationHandler
	 */
	function NavigationHandler(oInject) {
		...
		this.getNavigationTargets = function() {
			...
			oInject.functions.getCumulativeFilterUpToActiveStep().done(function(oCumulativeFilter){
				...
				enrichedNavigationTargets = deepExtend([], configuredNavigationTargets);
		...
		this.generateSelectionVariant = function (filter) {
			...
			if(!filterReduction || !filterReduction.isContradicted()){
				...
				selectOptionsPromise.done(function(selectionVariant){
					selectionVariant.SelectionVariantID = Uid();
					deferred.resolve(selectionVariant);
				});
			} else {
				var selectionVariant = {};
				selectionVariant = {
						SelectionVariantID: Uid(),
		...
		function convertToResultObject(targets) {
			var copyOfTargets = deepExtend([], targets);
			var resultObject = {
					global : [],
					stepSpecific : []
			};
			...
			function isAssignedToActiveStep(id) {
				...
				if (activeStep && activeStep.getAssignedNavigationTargets) {
					...
					if (assignedNavigationTargets && Array.isArray(assignedNavigationTargets)) {
	...
	exportToGlobal("sap.apf.utils.NavigationHandler", NavigationHandler);
