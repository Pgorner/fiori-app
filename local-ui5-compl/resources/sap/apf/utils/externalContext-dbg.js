...
sap.ui.define([
...
], function(Filter, FilterTerm, constants, exportToGlobal){
	/**
	 * @private
	 ...
	 * @alias sap.apf.utils.ExternalContext
	 */
	function ExternalContext(inject) {
		'use strict';
		...
		this.convertSelectOption = function(selectOption){
			...
			var i;
			var msgObject;
			if (!selectOption.PropertyName || !selectOption.Ranges || !(Array.isArray(selectOption.Ranges))){
		...
		this.convertSelectionVariantToFilter = function (selectionVariant){
			...
			if (selectionVariant.Parameters && !(Array.isArray(selectionVariant.Parameters))){
				return filter;
			}
			if (selectionVariant.SelectOptions && !(Array.isArray(selectionVariant.SelectOptions))){
				return filter;
			}
	...
	exportToGlobal("sap.apf.utils.ExternalContext", ExternalContext);
