...
sap.ui.define([
...
], function (Helper, CoreConstants, Constants, deepExtend, ChartContainerContent,
			 ChartContainerToolbarPlaceholder, Icon, CustomData, Fragment, BaseController,
			 XMLView, mobileLibrary, Link, ToggleButton, Label, Button, ToolbarSpacer, List,
			 StandardListItem, Popover, trace, jQuery) {
	"use strict";
	...
	function _setViewSettingsIcon(oController) {
		...
		var sortButton = new Icon({
			...
			tooltip: toolTipText,
			press: function () {
				oController.getView().addDependent(oController.getCurrentRepresentation().getViewSettingDialog());
				...
				press: function () {
					if (oController.byId("idStepLayout").getBusy() === false) {
						oCoreApi.getPathFilterInformation().then(function (pathFilterInformation) {
							var viewCreated = oController.byId("pathFilterDisplay");
							if(!viewCreated){
							var pathFilterInformationDialog = new XMLView({
							...
							pathFilterInformationDialog.setParent(oController.getView(), "content", true);
			...
			if (oRepresentation !== undefined) {
                if(oRepresentation.aDataResponse !== 0){
				    ...
				    if (!oSwitchDisplayButton) {
					    oSwitchDisplayButton = new ToggleButton({
	...
	function _setChartToolbarContent(oController) {
		...
		//disble button incase of first representation
		if (oController.getCurrentRepresentation().apfId !== 1){
	...
	/**
	 * @class controller of stepContainer view.
	 *
	 ...
	 *
	 * @name sap.apf.ui.reuse.controller.stepContainer
	 */
	return BaseController.extend("sap.apf.ui.reuse.controller.stepContainer", /** @lends sap.apf.ui.reuse.controller.stepContainer.prototype */ {
		/**
		...
		handlePressSelectedPropertyCountLink: function () {
			this.openSelectionDisplayDialog();
		},
		openSelectionDisplayDialog: async function() {
			var oController = this;
			oController.oCoreApi = oCoreApi;
			const selectionDisplayDialog = await Fragment.load({
				type: "JS",
				id: "idSelectionDisplayFragment",
				name: "sap.apf.ui.reuse.fragment.selectionDisplay",
		...
		createToggleRepresentationInstance: function (oRepresentation, orderby) {
			...
			var parameter = deepExtend({}, oRepresentation.getParameter());
		...
		/**
		 * @param {boolean} [bStepUpdated] true during an update or when a new step is added, also during "openPath".
		...
		drawStepContent: function (bStepUpdated, isActiveStep) {
			...
			bRepresentationChanged = false;
			oController.initialText.destroy();
