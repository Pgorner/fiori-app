...
sap.ui.define([
...
], function(Utils, WrappedChartWithCornerTexts, trace, Log, Device, Controller) {
	'use strict';
	...
	/**
	 * @class Controller for view.carousel.
	 * @alias sap.apf.ui.reuse.controller.carousel
	 */
	var CarouselController = Controller.extend("sap.apf.ui.reuse.controller.carousel", /** @lends sap.apf.ui.reuse.controller.carousel.prototype */ {
		cloneCnt : 0,
		paddingLeft : 10,
		paddingRight : 10,
		paddingBottom : 15,
		...
		onBeforeRendering : function() {
			if (!Device.system.desktop){
				this.oUiApi.getLayoutView().getController().showMasterFooter(this.getView().addStepButton, this.getView().moveUpButton, this.getView().moveDownButton);
			} else {
		...
		setSelectionsOnThumbnail : function(oStep, nIndex){
			...
			if (!oClonedChart){
				Log.error("***** setSelectionsOnThumbnail: chart reference is undefined");
				return;
			}
		 ...
		 * @param {Object} oStep
		 * @param {int} nIndex
		 * @returns {{customListItem: (*|sap.m.CustomListItem), wrappedChartWithCornerTexts: (controller.WrappedChartWithCornerTexts|WrappedChartWithCornerTexts|*), oChart: (*|*|*|null), typeOfChart: *, bIsAlternateView: *}}
		 * @private
		 */
		 ...
		 *
		 * @param {object} oStep the newly added step for which this method creates the custom list item containing its thumbnail.
		 * @private
		 */
		...
		/**
		 * This is the central update method. The methods adds/deletes a step (thumbnail) or reflects changes on a step.
		 ...
		 * @param {int} nIndex index of step
		...
		/**
		/**
		 * This function is called when during a update the first step is updated.
		...
		/**
		 * @param {int} dragIndex the index of the step which is moved.
		 * @param {int} dropIndex the index of the position where it is moved to.
		 */
		...
		removeStep : function(indexOfRemoval) {
			...
			this.oUiApi.getLayoutView().getController().setPathTitle();
			if ( this.oCoreApi.getSteps().length >= 1){
				this.getView().moveDownButton.setEnabled(false);
				this.getView().moveUpButton.setEnabled(false);
			}else{
				this.oUiApi.getStepContainer("ChartContainer").stepLayout.getAggregation("content")[0].getAggregation("toolbar").removeAllContent();
				this.oUiApi.getStepContainer("ChartContainer").stepLayout.getAggregation("content")[0].removeAllConten();
			}
		...
		_destroyItemAndChart : function(relation) {
			...
			if (relation.oTable){
				relation.oTable.tableControl.destroy();
				relation.oTable.destroy();
			}
		...
		/**
		 * Used to clean up resources specific to APF during shutdown
		 */
		apfDestroy : function() {
