...
sap.ui.define([
	...
	"sap/apf/ui/utils/determineColumnSettingsForSpreadSheetExport",
	...
	"sap/apf/ui/representations/utils/paginationDisplayOptionHandler",
	"sap/ui/Device",
	...
	constants,
	uiConstants,
	determineColumnSettingsForSpreadSheetExport,
	formatter,
	PaginationHandler,
	BaseUI5ChartRepresentation,
	PaginationDisplayOptionHandler,
	Device,
	BindingMode,
	Sorter,
	Table,
	...
	*/
	function createTitlebarForTable(oController, stepTitle, actualNumberOfRecords, bNotLoadAll, width) {
		...
		oController.titleControl = new Title({
			level : TitleLevel.H1
		...
		if (width){ //if width is set, the table is created for preview content in the modeler, no buttons in the title then
			...
			if (oController.loadAllButton === undefined) {
				oController.loadAllButton = new Button({
					text : text,
	...
	function _createTableAndBindColumns(tableColumns, oStepTitle, oTableInstance, width) {
		...
		var oTable = new Table({
			extension: [ oStepTitle ],
			showNoData : false,
			enableSelectAll : false,
			rowMode : RowMode.Auto
		});
		if(width){
			oTable.setWidth(width + "px");
		}
		oTable.setLayoutData(new FlexItemData({
			growFactor : 1
		}));
		...
		for(var indexTableColumn = 0; indexTableColumn < tableColumns.name.length; indexTableColumn++) {
			...
			oControl.bindText(tableColumns.value[indexTableColumn], formatCellValue(indexTableColumn), BindingMode.OneWay);
			...
			customDataForColumnText = new CustomData({
				value : {
					text : tableColumns.name[indexTableColumn],
		...
		var oModelForTable = new JSONModel();
		...
		if (oTableInstance.metadata !== undefined) {
			for(var fieldIndex = 0; fieldIndex < tableColumns.name.length; fieldIndex++) {
				...
				if (oMetadata["aggregation-role"] === "measure") {
					...
					measureCol.setHAlign(HorizontalAlign.End);
				}
			}
		}
		return oTable;
	...
	*/
	var table = function(oApi, oParameters) {
		...
		this.type = uiConstants.representationTypes.TABLE_REPRESENTATION; //the type is read from step toolbar and step container
		this.oPaginationHandler = new PaginationHandler(this);//initialize the pagination handler
		this.oPaginationDisplayOptionHandler = new PaginationDisplayOptionHandler();
	...
	table.prototype = Object.create(BaseUI5ChartRepresentation.prototype);
	table.prototype.constructor = table;// Set the "constructor" property to refer to table
	/**
	* @method setData
	...
	*/
	table.prototype.setData = function(aDataResponse, metadata, nDataResponseCount, aValidatedFilters) {
		var self = this;
		...
		this.oFormatter = new formatter({ // formatter for the value formatting
	...
	table.prototype.getFilter = function() {
	...
	*/
	table.prototype.markSelectionInTable = function(bIsCalledFromTable) {
	...
	table.prototype.getRequestOptions = function(bFilterChanged, isAlternateRep) {
	...
	*/
	table.prototype.resetPaginationForTable = function() {
		this.omitTopAndSkipOptionsForNextPathUpdate = false;
		this.oPaginationHandler.resetPaginationOption();
	};
	/**
	...
	*/
	table.prototype.getMainContent = function(oStepTitle, width, height) {
		var self = this;
		...
		this.oLoadMoreLink = new Link({// load more link should be shown for the mobile devices
	...
	*/
	table.prototype.getThumbnailContent = function() {
		...
		if (aTableData !== undefined && aTableData.length !== 0) {
			...
			oThumbnailContent = new VerticalLayout({
				content : noDataText
			});
		}
	...
	*/
	table.prototype.removeAllSelection = function() {
		_clearFilters(this);
		this.tableControl.clearSelection();
	...
	*/
	table.prototype.getPrintContent = function(oStepTitle) {
		...
		oPrintTable.setRowMode(new FixedRowMode({
			rowCount: oPrintTable.getModel().getData().tableData.length
		}));
	...
	table.prototype.getViewSettingDialog = function() {
		if (!this.oViewSettingDialog) {
			var oViewData = {
				oTableInstance : this
			};
			var oViewSetting = new sap.ui.view({
				viewName : "module:sap/apf/ui/reuse/view/viewSetting.view",
				viewData : oViewData
			});
	...
	*/
	table.prototype.loadAll = function() {
	...
	*/
	table.prototype.exportExcel = function(stepTitle) {
		var that = this;
		function createColumnInfoForSpreadSheet() {
			...
			for(i = 0; i < columnProperties.value.length; i++) {
				columnInfo = determineColumnSettingsForSpreadSheetExport(columnProperties.value[i], that.metadata);
		...
		// TODO constructor is inspected via global name by ui/representations/tTable test
		var spreadsheet = new sap.ui.export.Spreadsheet(mSettings);
		spreadsheet.build();
	};
	table.prototype.onChartSwitch = function() {
