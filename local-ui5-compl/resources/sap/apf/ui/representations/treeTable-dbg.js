...
sap.ui.define([
	...
	function getSelectionMode(sRequiredFilter, metadata) {
		if (sRequiredFilter) {
			...
			return SelectionMode.MultiToggle;
		}
	...
	function _createTreeTableAndBindColumns(aTreeTableColumns, oStepTitle, oTreeTableInstance) {
		...
		oTreeTable = new TreeTable({ // Creating tree table
			showNoData : false,
			extension: [
				new Title({
					text: oStepTitle
		...
		oTreeTable.setLayoutData(new FlexItemData({
			growFactor : 1
		}));
		...
		aTreeTableColumns.name.forEach(function(columnName, nColumnIndex) {
			oColumnText = new Text().bindText(aTreeTableColumns.value[nColumnIndex], formatCellValue(aTreeTableColumns.value[nColumnIndex]), BindingMode.OneWay);
			oTreeColumn = new Column({
				label : new Label({
					text : columnName
				}),
		...
		oTreeTableInstance.oTreeTableModel.attachBatchRequestCompleted(function() { //Once the batch request is completed, busy indicator is set to false
			...
			_alignMeasureColumn(oTreeTableInstance, oTreeTable, aTreeTableColumns);
                   if (oTreeTableInstance.oRepresentationFilterHandler !== null){
		     if (oTreeTableInstance.oRepresentationFilterHandler !== undefined) {
	...
	function _alignMeasureColumn(oTreeTableInstance, oTreeTable, aTreeTableColumns) {
          if (oTreeTableInstance.metadata !== null){
		if (oTreeTableInstance.metadata !== undefined) {
	...
	*/
	function _getColumnFromProperties(oTreeTableInstance) {
		...
		aPropertiesForTreeTableColumns.forEach(function(property, index) {
			...
			if (property.kind === modelerConstants.propertyTypes.HIERARCHIALCOLUMN) {
	...
	*/
	var treeTable = function(oApi, oParameters) {
		var that = this;
		that.oParameters = oParameters;
		that.requiredFilter = that.oParameters.requiredFilters[0];
		that.oKeyTextLookup = {};
		that.type = uiConstants.representationTypes.TREE_TABLE_REPRESENTATION;
		BaseUI5ChartRepresentation.apply(that, [oApi, oParameters]);
		that.oPaginationDisplayOptionHandler = new PaginationDisplayOptionHandler();
	...
	treeTable.prototype = Object.create(BaseUI5ChartRepresentation.prototype);
	treeTable.prototype.constructor = treeTable; // Set the constructor property to refer to tree table
	/**
	* @method getMainContent
	* @param oStepTitle - title of the main chart 
	* @description draws Main chart into the Chart area
	*/
	treeTable.prototype.getMainContent = function(oStepTitle) {
		if (!this.oTreeTable) {
	...
	treeTable.prototype.getSelections = function() {
	...
	*/
	treeTable.prototype.removeAllSelection = function() {
	...
	*/
	treeTable.prototype.getFilter = function() {
	...
	*/
	treeTable.prototype.setFilterValues = function(aValues) {
	...
	*/
	treeTable.prototype.updateTreetable = function(controlObject, oModel, oMetadata, bFilterChanged) {
	...
	*/
	treeTable.prototype.getPrintContent = function(oStepTitle) {
	...
	treeTable.prototype.getSelectedFilterPropertyLabel = function(sRequiredFilter) {
	...
	*/
	treeTable.prototype.destroy = function() {
		if (this.oParameters) {
