...
sap.ui.define([
	...
	'sap/ui/thirdparty/jquery'
			...
			FilterSimplify, Utils, Filter, exportToGlobal, ExternalContext, StartFilterHandler, StartFilter,
			FilterIdHandler, SerializationMediator, NavigationHandler, StartParameter,
			MessageCallbackForStartup, RepresentationInterfaceProxy, Log, App, Text, OData, jQuery) {
	'use strict';
	 ...
	 * @param {Object|sap.ui.core.Manifest} [manifests] - manifests of the component and the base component itself
	 */
	function API(oComponent, inject, manifests) {
		 ...
		 * @param {object|string} oLabel object or text key
		 ...
		 * @param {object|string} oLabel Label object or text key
		 * @param {string[]} aParameters Array with parameters to replace place holders in text.
		 * @returns {string}
		 */
		 ...
		 * Restriction: Only a single filter term or a disjunction/conjunction of single terms over a single property is supported.
		 * Restriction: Only the operators EQ, LE (less equal than) and GE (greater equal than) are supported.
		...
		this.createApplicationLayout = function() {
			...
			if (!application){
				application = new App().addStyleClass("sapApf");
			}
		...
		try {
			oCoreApi = new (inject && inject.constructors && inject.constructors.CoreInstance || CoreInstance.constructor)({
				...
				constructors : {
					...
					AnalysisPathProxy: (inject && inject.constructors && inject.constructors.AnalysisPathProxy)
				},
				manifests: manifests,
			...
			Log.error("Caught exception during creation of APF API");
		}
		/**
		 * @private
		 * @experimental NOT FOR PRODUCTION USE
		...
		if (oCoreApi) {
			oCoreApi.createRepresentation = function(sRepresentationConstructorPath, oConfig) {
				var sRepresentationPath = sRepresentationConstructorPath.replaceAll(".","/");
				// TODO properly migrate to async
				var Representation = sap.ui.requireSync(sRepresentationPath)
					|| Utils.extractFunctionFromModulePathString(sRepresentationConstructorPath);
				// TODO: replacing sap.apf.ui.representations.RepresentationInterfaceProxy fails the runtime app.
				var interfaceProxy = new RepresentationInterfaceProxy(oCoreApi, oUiApi);
	...
	exportToGlobal("sap.apf.Api", API);
	return API;
}, true /*GLOBAL_EXPORT*/);
