sap.ui.define([
	...
	var oTranslationFormatForStepTitle = textPoolHelper.TranslationFormatMap.STEP_TITLE;
	var oTranslationFormatForStepLongTitle = textPoolHelper.TranslationFormatMap.STEP_LONG_TITLE;
	...
	function _setVisibilityOfFilterMappingFields(oEvent) {
		...
		if (!bIsVisible) {
			...
			oController.getView().fireEvent(modelerConstants.events.step.RESETFILTERMAPPINGFIELDS);
		} else {
			oController.byId("idFilterMapping").setText(oTextReader("filterMap"));
		}
	}
	...
	function _instantiateSubViews(oController) {
		...
		//attach events
		stepRequestView.attachEvent(modelerConstants.events.step.SETDATAREDUCTIONSECTION, oController.setDataReductionSection.bind(oController));
		stepRequestView.attachEvent(modelerConstants.events.step.SETVISIBILITYOFFILTERMAPPINGFIELDS, _setVisibilityOfFilterMappingFields.bind(oController));
		oController.getView().attachEvent(modelerConstants.events.step.SETVISIBILITYOFFILTERMAPPINGFIELDS, _setVisibilityOfFilterMappingFields.bind(oController));
		stepRequestView.attachEvent(modelerConstants.events.step.UPDATEFILTERMAPPINGFIELDS, stepFilterMappingView.getController().updateFilterMappingFields.bind(stepFilterMappingView.getController()));
		oController.getView().attachEvent(modelerConstants.events.step.RESETFILTERMAPPINGFIELDS, stepFilterMappingView.getController().resetFilterMappingFields.bind(stepFilterMappingView.getController()));
		//To be fired to update subView facet filter and config editor instances after reset
		oController.getView().attachEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, stepCornerTextView.getController().updateSubViewInstancesOnReset.bind(stepCornerTextView.getController()));
		oController.getView().attachEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, stepRequestView.getController().updateSubViewInstancesOnReset.bind(stepRequestView.getController()));
		oController.getView().attachEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, stepFilterMappingView.getController().updateSubViewInstancesOnReset.bind(stepFilterMappingView.getController()));
	...
	function _updateTopNProperties(oController) {
		if (oStep.getTopN()) {
			...
			if (oStep.getTopN().orderby.length === 0) {
				oController.getView().fireEvent(modelerConstants.events.step.SETTOPNPROPERTIES);
	...
	function _setFilterMappingFields(oController) {
		...
		oController.getView().fireEvent(modelerConstants.events.step.SETVISIBILITYOFFILTERMAPPINGFIELDS, {
	...
	return Controller.extend("sap.apf.modeler.ui.controller.step", {
		onInit : function() {
			...
			oStepPropertyMetadataHandler = new (oViewData.StepPropertyMetadataHandler || StepPropertyMetadataHandler)(oCoreApi, oStep);
		...
		handleValidationForNumberOfRecords : function(oEvent) {
			...
			var sValueState = (sValue.indexOf("E") !== -1 || sValue.indexOf("e") !== -1 || sValue.indexOf(".") !== -1 || sValue <= 0 || sValue > 10000) ? ValueState.Error : ValueState.None;
		...
		handleChangeForNoOfRecords : function(oEvent) {
			...
			if (oEvent.getSource().getValueState() === ValueState.Error) {
				return;
			}
