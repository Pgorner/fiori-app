...
sap.ui.define([
	...
	function _retrieveOrCreateFFObject(oController) {
		...
		if (oParams && oParams.arguments && oParams.arguments.facetFilterId) {
			...
			_setRetainListSequence(oFacetFilter);
		}
	...
	//to retain the facet filter selections 
	function _setRetainListSequence(oFacetFilter){
		true;
	}
	...
	function _instantiateSubViews(oController) {
		...
		oVHRView.attachEvent(FacetFilterEvents.UPDATEPROPERTIES, oController.setFFProperty.bind(oController));
		oFRRView.attachEvent(FacetFilterEvents.UPDATEPROPERTIES, oController.setFFProperty.bind(oController));
		//To be fired whenever useSameAsVHR check box is selected/unselected
		oController.getView().attachEvent(FacetFilterEvents.USESAMEASVHR, oFRRView.getController().handleCopy.bind(oFRRView.getController()));
		oController.getView().attachEvent(FacetFilterEvents.ENABLEDISABLEFRRFIELDS, oFRRView.getController().enableOrDisableView.bind(oFRRView.getController()));
		oVHRView.attachEvent(FacetFilterEvents.USESAMEASVHR, oFRRView.getController().handleCopy.bind(oFRRView.getController()));
		//To be fired to update subView facet filter and config editor instances after reset
		oController.getView().attachEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, oVHRView.getController().updateSubViewInstancesOnReset.bind(oVHRView.getController()));
		oController.getView().attachEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, oFRRView.getController().updateSubViewInstancesOnReset.bind(oFRRView.getController()));
		//To be fired whenever facet filter visibility is changed
		oController.getView().attachEvent(FacetFilterEvents.DONOTSHOWATRUNTIME, oVHRView.getController().clearVHRFields.bind(oVHRView.getController()));
		oController.getView().attachEvent(FacetFilterEvents.CLEARVHRFIELDSIFVALUELIST, oVHRView.getController().clearVHRFields.bind(oVHRView.getController()));
		oController.getView().attachEvent(FacetFilterEvents.DONOTSHOWATRUNTIME, oFRRView.getController().clearFRRFields.bind(oFRRView.getController()));
	...
	function _setPreselectionDefaults(oController) {
		...
		if (nullObjectChecker.checkIsNotNullOrUndefinedOrBlank(defaultValueListArray)) {
			defaultValueListArray.forEach(function(defaultValueForToken) {
				oController.byId("idPreselectionDefaults").addToken(new Token({
	...
	function _setConfigValueList(oController) {
		...
		if (nullObjectChecker.checkIsNotNullOrUndefinedOrBlank(valueListArray)) {
			valueListArray.forEach(function(configTextForToken) {
				oController.byId("idConfigListOfValuesMultiInput").addToken(new Token({
	...
	function _setOrRemoveMandatoryForValueHelpRequest(oController, bIsVisible) {
		oController.getView().fireEvent(FacetFilterEvents.ENABLEDISABLEFRRFIELDS);
		_hideOrShowUseVHRAsFRRCheckBox(oController, bIsVisible);
		if (bIsVisible) {
			oController.byId("idVHRVBox").insertItem(oController.byId("idVHRView"));
		} else {
			oController.getView().fireEvent(FacetFilterEvents.CLEARVHRFIELDSIFVALUELIST);
	...
	function _setValueStateForFixedValue(warningMessageKey, oController) {
		oController.byId("idPreselectionDefaults").setValueState(ValueState.Warning);
	...
	function _addTokenForMultiInputField(textForToken, oMultiInputControl) {
		var aPreselectionDefaults = new Token({
			text : textForToken
		});
	...
	return Controller.extend("sap.apf.modeler.ui.controller.facetFilter", {
		...
		onInit : function() {
			...
			viewValidatorForFF = new ViewValidator(oController.getView());
		...
		updateSubViewInstancesOnReset : function(oConfigEditor) {
			...
			oController.getView().fireEvent(modelerConstants.events.UPDATESUBVIEWINSTANCESONRESET, {
				"oConfigurationEditor" : oConfigurationEditor,
				"oParentObject" : oFacetFilter
			});
		},
		...
		validateSelectedValues : function(oController, sSelectedValue, aAllValues) {
			...
			oValidInvalidObj = utils.validateSelectedValues(sSelectedValue, aAllValues);
		...
		handleChangeForVisibilityAtRuntime : function() {
			...
			if (oController.byId("idDoNotShowAtRuntimeCheckBox").getSelected()) {
				...
				oController.getView().fireEvent(FacetFilterEvents.ENABLEDISABLEFRRFIELDS);
				oController.getView().fireEvent(FacetFilterEvents.DONOTSHOWATRUNTIME);
		...
		handleChangeForProperty : function() {
			...
			sFacetFilterProperty = textManipulator.removePrefixText(sFacetFilterProperty, oTextReader(modelerConstants.texts.NOTAVAILABLE));
			...
			if (bSelectedButton === oController.byId("idSingleSelectionMode")) {
				...
				if (oController.byId("idPreselectionDefaultsLabel").getVisible()) {
					...
					if (aPSDefaults && aPSDefaults.length > 1) {
						...
						oDefaultTokenForSingleSelection = new Token({
							text : aPSDefaults[0]
						});
				...
				oController.byId("idPreselectionDefaults").setValueState(ValueState.None);
				oFacetFilter.setMultiSelection(true);
			}
			oConfigurationEditor.setIsUnsaved();
		},
		...
		handleChangeForUseVHRAsFRRCheckBox : function() {
			...
			oController.getView().fireEvent(FacetFilterEvents.ENABLEDISABLEFRRFIELDS);
			oController.getView().fireEvent(FacetFilterEvents.USESAMEASVHR);
			oConfigurationEditor.setIsUnsaved();
		},
