...
sap.ui.define([
...
], function(modelerConstants, RepresentationHandler, uiConstants, coreConstants, nullObjectChecker, optionsValueModelBuilder,
			SortDataHandler, RepresentationBasicDataHandler, StepPropertyMetadataHandler, RepresentationTypesHandler,
			ViewValidator, deepExtend, Button, MvcController, View, ViewType, jQuery
){
	'use strict';
	var oParams,
		oCoreApi,
	...
	function _instantiateCornerTexts(oController) {
		...
		oController.getView().attachEvent(modelerConstants.events.representation.SETCHARTICON, oView.getController().setChartIcon.bind(oView.getController()));
	}
	...
	var constructor = MvcController.extend("sap.apf.modeler.ui.controller.representation", {
		/** @description Implicit parameter injected with the following members
			...
			updateTree:ƒ (),
			StepPropertyMetadataHandler: {sap.apf.modeler.ui.utils.StepPropertyMetadataHandler}
		...
		onInit : function() {
			...
			oController.promiseControllerIsCreated = new Promise(function(resolve) {
				...
				oStepPropertyMetadataHandler = new (oViewData.StepPropertyMetadataHandler || StepPropertyMetadataHandler)(oCoreApi, oParentStep);
		...
		handleChangeForChartType : function(oEvent) {
			...
			if (!oRepresentationTypeHandler.isChartTypeSimilar(sOldRepresentationType, sNewRepresentationType)) {
				oController.getView().fireEvent(modelerConstants.events.REMOVEALLPROPERTIESFROMPARENTOBJECT);
				_clearRepresentationBasicData();
				_setDefaultStateForRepnAsPromise(sNewRepresentationType).done(function() {
					...
					oController.getView().fireEvent(modelerConstants.events.representation.SETCHARTICON);
					oConfigurationEditor.setIsUnsaved();
				});
			} else {
				oRepresentationBasicDataHandler.instantiateBasicDataAsPromise().then(function(){
		...
		_getAnnotatedChartTypes : function(aRepresentationTypes, aChartTypes, aDimensions, aMeasures, oCoreApi){
			...
			var aChartTypesCloned = deepExtend([], aChartTypes);
