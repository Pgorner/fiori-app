...
sap.ui.define([
	...
	'use strict';
	var ConfigurationObjectTypes = modelerConstants.configurationObjectTypes;

	/**
	 * @class
	 * @name sap.apf.modeler.ui.controller.configurationList
	 */
	return Controller.extend("sap.apf.modeler.ui.controller.configurationList", /** @lends sap.apf.modeler.ui.controller.configurationList.prototype */ {
		onInit : function() {
			...
			APFRouter.patternMatch(this);
		},
		/**
		 * Instantiates and injects toolbar view in configurationList view.
		 *
		 * @private
		 */
		...
		/**
		 * Creates instance of custom APFTree control and calls property setters of custom APFTree control.
		 *
		 * @private
		 */
		...
		/**
		 * Instantiates and injects titleand BreadCrumb in configurationList view.
		 *
		 * @private
		 */
		...
		/**
		 * Setter to set title to the configuration list view master content.
		 *
		 * @private
		 * @param {String} sTitle String that has to be set as the title on master content of configuration list view
		 */
		...
		/**
		 * After rendering of the view sets height to the tree.
		 *
		 * @private
		 */
		...
		_setHeightForScrollBar : function() {
			...
			Helper.onResize(function() {//on resize handle height and width accordingly by passing its parent dom reference
				height = jQuery(oViewInstance.byId("configPage").getDomRef()).height();
				oViewInstance.byId("idMasterScroll").setHeight(height+"px");
			});
		},
		/**
		 * Sets texts for page title and save,cancel,export button.
		 *
		 * @private
		 */
		...
		/**
		 * Sets the publish button visible if using cloud foundry and an exit is available.
		 *
		 * @private
		 */
		_setPublishVisible : function() {
			var visible = this.oCoreApi.isUsingCloudFoundryProxy();
			this.byId("idPublishbutton").setVisible(visible);
		},
		/**
		 * Adds style class to the page title, master and detail area.
		 *
		 * @private
		 */
		...
		/**
		 * Sets the height for the subview container,
		 *
		 * @private
		 * @param {sap.ui.core.mvc.View} oSubView Subview instance
		 * @returns {float} Height of the subview container 
		 */
		...
		/**
		 * Sets the subview on navigation change.
		 *
		 * @private
		 * @param {sap.ui.core.mvc.View} oSubView Subview instance 
		 */
		...
		/**
		 * Getter for current subview instance.
		 *
		 * @private
		...
		/**
		 * Injects the appropriate subview instance based on URL context change.
		 *
		 * @private
		 * @param {Object} oParams URL context and a boolean isHierarchicalStep to check if the step is a hierarchy step
		 */
		...
		/**
		 * Clears title and breadCrumb from the detail page.
		 *
		 * @private
		 */
		...
		/**
		 * Creates link control for forming Bread Crumb.
		 *
		 * @private
		 * @param {string} sName - setting text for link
		 * @param {string} id 
		 * @param {string} sPath - helps to navigate to selected node using link
		...
		/**
		 * Creates Label control for forming Bread Crumb.
		 *
		 * @private
		 * @param {string} sValue -setting text for Text Control
		 * @param {string} id
		...
		/**
		 * Creates Icon control for forming Bread Crumb.
		 *
		 * @private
		 * @returns {sap.m.Icon} - Icon Control
		 */
		_createIconForBreadCrumb : function() {
			var breadCrumbIcon = new Icon({
		...
		/**
		 * Helps to navigate to selected node using link.
		 *
		 * @private
		...
		/**
		 * Helps to get all possible node details in hierarchical format.
		 *  Ex:if selected node is representation can get details representation,step,
		 * category,configuration.
		 *
		 * @private
		 * @param {Object} -oParentNode -contains information about current node & parent node
		...
		/**
		 * Helps to create BreadCrumb using link & Text.
		 *
		 * @private
		 * @param {Object} oSelectedNodes - gets all possible nodes 
		 * @param {string} sType - Type of Node (configuration || facetFilter || step etc,.)
		 */
		...
		/**
		 * Updates the title and breadcrumb of the detailpage.
		 *
		 * @private
		...
		updateTitleAndBreadCrumb : function(sTitle) {
			...
			if (sTitle) { // Updated title of node on handlechange from the subview
				...
				if (nodeObjectType) {
					...
					if (nodeObjectType === ConfigurationObjectTypes.SMARTFILTERBAR) {
						sFormTitle = text;
					}
				}
			}
			...
			if (oParentNodeContext) {
				...
				switch (nodeObjectType) { // based on the object type build the breadcrumb value
					case ConfigurationObjectTypes.CONFIGURATION:
						this._breadCrumbCreation(oSelectedNodes);
						break;
					case ConfigurationObjectTypes.SMARTFILTERBAR:
					case ConfigurationObjectTypes.FACETFILTER:
						sType = this.oCoreApi.getText("facetFilter");
						this._breadCrumbCreation(oSelectedNodes, sType);
						break;
					case ConfigurationObjectTypes.CATEGORY:
						sType = this.oCoreApi.getText("category");
						this._breadCrumbCreation(oSelectedNodes, sType);
						break;
					case ConfigurationObjectTypes.STEP:
						sType = this.oCoreApi.getText("category");
						this._breadCrumbCreation(oSelectedNodes, sType);
						break;
					case ConfigurationObjectTypes.REPRESENTATION:
						sType = this.oCoreApi.getText("category");
						this._breadCrumbCreation(oSelectedNodes, sType);
						break;
					case ConfigurationObjectTypes.NAVIGATIONTARGET:
			...
			if (nodeObjectType !== ConfigurationObjectTypes.STEP) {
				this.oTitleBreadCrumbController.setTitleForDetailPage(sFormTitle);
			}
		},
		/**
		 * Sets selection on the tree.
		 *
		 * @private
		...
		/**
		 * Removes all selection from the tree.
		 *
		 * @private
		 */
		...
		/**
		 * Updates the tree when a category assigned to a step is changed.
		 *
		 * @private
		 * @param {Object} objectContexts Contains old and new context of the node which has to be updated in the tree
		 */
		updateConfigTree : function(objectContexts) {//Set of step contexts and categories of steps
			...
			objectContexts.forEach(function(oStepContext) {//For each step context, the step shall be added or removed in the category based on the removeStep boolean
				...
				existingStep = deepExtend({}, self.oModel.getData().aConfigDetails[indexOfConfig].configData[1].categories[indexOfCategory].steps[indexOfStep]);
			...
			if (objectContexts[noOfObjects - 1].changeCategory) {
				UIComponent.getRouterFor(this).navTo(objectContexts[noOfObjects - 1].oldContext.name, objectContexts[noOfObjects - 1].categoryChangeContext.arguments, true);//If the step is being removed from the present category,, navigate to the next category in which the step is present
			}
			this.configEditor.setIsUnsaved();
		},
		/**
		 * Updates node in the tree whenever title of node is changed from any of the detail subviews.
		 *
		 * @private
		 * @param {Object} params Label name and id of the node to be updated
		 * @param {Object} context applicationId and temporary configuration id
		 */
			...
			if (params) {
				...
				if (this.selectedNode) {
					if (params.name && this.selectedNode._getBindingContext() && this.selectedNode._getBindingContext().getObject()) { //update scenario
						...
						if (this.selectedNode.getBindingContext().getObject().type === ConfigurationObjectTypes.REPRESENTATION
								|| this.selectedNode.getBindingContext().getObject().type === ConfigurationObjectTypes.NAVIGATIONTARGET) {
				...
				if ((this.selectedNode.getBindingContext().getObject().type === ConfigurationObjectTypes.CONFIGURATION) && context) {
					UIComponent.getRouterFor(this).navTo(this.selectedNode.getBindingContext().getObject().type, context, true);
		...
		/**
		 * Loads only list of all configurations in the tree.
		 *
		 * @private
		 */
		createConfigList : function() {//Loads only a list of configurations in the tree
			...
			this.configList.forEach(function(config, index) {
				...
				oConfigDetails.type = ConfigurationObjectTypes.CONFIGURATION;
		...
		/**
		 * Displays 'No COnfiguration Selected' text in detail area in case of multiple configurations.
		 *
		 * @private
		 */
		showNoConfigSelectedText : function() {
			...
			var noConfigSelected = new Label().addStyleClass("noConfigSelected");
		...
		/**
		 * In case of only one configuration loads it and sets as selected and in case of multiple configurations none is selected.
		 *
		 * @private
		 */
		updateConfigListView : function() {//In case of only one configuration; To load the configuration and set it as selected and In case of multiple configurations none selected
			...
			if (this.selectedNode === null || this.selectedNode === undefined || this.selectedNode.length == 0) {//only case when first time coming from application list view otherwise there will always a node selected.
				if (this.configList.length > 1) { // if more than one configuration,show a message in detail area,no config selected
					...
					this.toolbarController.enableCopyDeleteButton();
					this._enableDisableExportAndExecuteButton(true);
					UIComponent.getRouterFor(this).navTo(this.oModel.getData().aConfigDetails[0].type, context, true);
				}
			}
		},
		/**
		 * Returns the data which are relevant to the passed node.
		 *
		 * @private
		 * @param {Object} params - id's of ancestor node to traverse through
		 * @param {string} type - define the node type like category, step etc
		 * @returns {Object} 
		 * 	nodeData {
		...
		_getNodeData : function(params, type) {
			...
			var getFilterData = function(id) { //Get the facetFilter index
				var indexOfConfig = getConfigurationData.call(this, params[ConfigurationObjectTypes.CONFIGURATION]).index;
				var node = {};
			...
			var getStepData = function(id) {//Get the step index
				var indexOfConfig = getConfigurationData.call(this, params[ConfigurationObjectTypes.CONFIGURATION]).index;
				var indexOfConfigData = getCategoryData.call(this, params[ConfigurationObjectTypes.CATEGORY]).indexConfigData;
				var indexOfCategory = getCategoryData.call(this, params[ConfigurationObjectTypes.CATEGORY]).index;
				var node = {};
			...
			var getRepresentationData = function(id) {//Get the representation index
				...
				var indexOfConfigData = getCategoryData.call(this, params[ConfigurationObjectTypes.CATEGORY]).indexConfigData;
				var indexOfCategory = getCategoryData.call(this, params[ConfigurationObjectTypes.CATEGORY]).index;
				var indexOfStep = getStepData.call(this, params[ConfigurationObjectTypes.STEP]).index;
				var node = {};
			...
			switch (type) {
				case ConfigurationObjectTypes.CONFIGURATION:
					nodeData = getConfigurationData.call(this, params[ConfigurationObjectTypes.CONFIGURATION]);
					break;
				case ConfigurationObjectTypes.FACETFILTER:
					nodeData = getFilterData.call(this, params[ConfigurationObjectTypes.FACETFILTER]);
					break;
				case ConfigurationObjectTypes.SMARTFILTERBAR:
					nodeData = getFilterData.call(this, params[ConfigurationObjectTypes.SMARTFILTERBAR]);
					break;
				case ConfigurationObjectTypes.CATEGORY:
					nodeData = getCategoryData.call(this, params[ConfigurationObjectTypes.CATEGORY]);
					break;
				case ConfigurationObjectTypes.STEP:
					nodeData = getStepData.call(this, params[ConfigurationObjectTypes.STEP]);
					break;
				case ConfigurationObjectTypes.REPRESENTATION:
					nodeData = getRepresentationData.call(this, params[ConfigurationObjectTypes.REPRESENTATION]);
					break;
				case ConfigurationObjectTypes.NAVIGATIONTARGET:
					nodeData = getNavigationTargetData.call(this, params[ConfigurationObjectTypes.NAVIGATIONTARGET]);
					break;
				default:
					break;
			}
		...
		/**
		 * Retains the expansion state from its previous model if it exists.
		 *
		 * @private
		...
		_retainNodeState : function(params, type) {
			var configIndexInTree = this._getNodeData(params, ConfigurationObjectTypes.CONFIGURATION).index;
		...
		/**
		 * Prepare the model for tree with editor instance passed.
		 * 
		...
		updateTree : function() {
			...
			var oParams = {};
			oParams[ConfigurationObjectTypes.CONFIGURATION] = this.configId;
			var configIndexInTree = self._getNodeData(oParams, ConfigurationObjectTypes.CONFIGURATION).index;
			...
			function pushFilter(oIcon, oFilterObject) {
				...
				if (sFilterOption === ConfigurationObjectTypes.FACETFILTER) {
			...
			if (sFilterOption === ConfigurationObjectTypes.SMARTFILTERBAR) {
				oSmartFilter = this.configEditor.getSmartFilterBar();
				pushFilter("sap-icon://filter", oSmartFilter);
			} else if (sFilterOption === ConfigurationObjectTypes.FACETFILTER) {
			...
			categories.forEach(function(category) {
				oParams[ConfigurationObjectTypes.CATEGORY] = category.getId();
				...
				self.configEditor.getCategoryStepAssignments(sCategoryId).forEach(function(sStepId) {
					...
					oParams[ConfigurationObjectTypes.CATEGORY] = sCategoryId;
					oParams[ConfigurationObjectTypes.STEP] = oStep.getId();
					var stepState = self._retainNodeState(oParams, ConfigurationObjectTypes.STEP);
					var aRepresentations = oStep.getRepresentations();
					var aRepData = aRepresentations.map(function(oRepresentation) {
						oParams[ConfigurationObjectTypes.REPRESENTATION] = oRepresentation.getId();
						var representationState = self._retainNodeState(oParams, ConfigurationObjectTypes.REPRESENTATION);
						return {
				...
				var categoryState = self._retainNodeState(oParams, ConfigurationObjectTypes.CATEGORY);
				//Check if the config editor is loaded then check expansion state of root node 
				if (self.oModel.getData().aConfigDetails[configIndexInTree].bIsLoaded) {
					var categoryRootNode = self._getNodeData(oParams, ConfigurationObjectTypes.CATEGORY);
			...
			var populateNavTargets = function(aSemanticActions, oNavigationTarget) {
				oParams[ConfigurationObjectTypes.NAVIGATIONTARGET] = oNavigationTarget.getId();
				var navigationTargetState = self._retainNodeState(oParams, ConfigurationObjectTypes.NAVIGATIONTARGET);
				//Check if the config editor is loaded then check expansion state of root node 
				if (self.oModel.getData().aConfigDetails[configIndexInTree].bIsLoaded) {
					var nTRootNode = self._getNodeData(oParams, ConfigurationObjectTypes.NAVIGATIONTARGET);
				...
				oPromise.then(function(aSemanticActions) {
					...
					if (aNavigationTarget.length === aNavigationTargets.length) {//Once all the navigation targets are populated push them into allCategoryInConfig
						...
						if (allCategoryInConfig[2]) {
							...
							var currentUrlParams = APFRouter.params;
							if (currentUrlParams.name === "navigationTarget") {
								APFRouter.setCurrentSelectionState(currentUrlParams, self);
							}
						}
					}
				});
		...
		/**
		 * Checks if the navigation target key exists in the given configuration index and return the index of the key in the table.
		 *
		 * @private
		 * @params {string} Key as navigation target ID 
		 * @params {int} Index of the configuration in the tree
		 */
		...
		/**
		 * Checks if the navigation target key exists in the given configuration index and return the value of the key.
		 *
		 * @private
		 * @params {string} Key as navigation target ID 
		 */
		...
		/**
		 * Inserts the key value pair into the navigation target texts table.
		 *
		 ...
		 * @private
		 * @param {object} params 
		 * @param {string} params.key Key as navigation target ID 
		 * @param {string} params.value Text of the navigation target action
		 * @param {any} params.configIndexInTree Text of the navigation target action
		 */
		_setNavigationTargetName : function(params) {
			...
			if (isEmptyObject(this.navTargetTextsTable)) {//If the table is an empty object insert the object configDetails
		...
		/**
		 * On expand stores the binding context of the nodes which could be used later to set the state of the tree.
		 *
		 * @private
		 */
		_handleToggleTreeNode : function(oEvent) { // on expand stores the binding context of the nodes which could be used later to set the state of the tree
			...
			if (selectedObjectType === ConfigurationObjectTypes.CONFIGURATION) { //if configuration is expanded, the child nodes has to be expanded
		...
		/**
		 * Returns sPath of the selected configuration.
		 *
		 * @private
		 * @param {string} configId Configuration Id for which sPath has to be derieved
		...
		/**
		 * Returns a valid sPath from the URL context.
		 *
		 * @private
		 * @params {object} params URL context
		 * @returns {string} Returns a valid sPath from the URL context
		 */
		getSPathFromURL : function(params) {
			...
			this.oModel.getData().aConfigDetails.forEach(function(config, index) {
				if (config.AnalyticalConfiguration === params.arguments.configId) {
					...
					oValidURL.objectType = ConfigurationObjectTypes.CONFIGURATION;
					...
					if (params.arguments.categoryId !== undefined) {
						self.oModel.getData().aConfigDetails[index].configData[1].categories.forEach(function(category, categoryIndex) {
							if (category.id === params.arguments.categoryId) {
								...
								oValidURL.objectType = ConfigurationObjectTypes.CATEGORY;
								...
								if (params.arguments.stepId !== undefined) {
									self.oModel.getData().aConfigDetails[index].configData[1].categories[categoryIndex].steps.forEach(function(step, stepIndex) {
										if (step.id === params.arguments.stepId) {
											...
											oValidURL.objectType = ConfigurationObjectTypes.STEP;
											...
											if (params.arguments.representationId !== undefined) {
												self.oModel.getData().aConfigDetails[index].configData[1].categories[categoryIndex].steps[stepIndex].representations.forEach(function(representation, representationIndex) {
													if (representation.id === params.arguments.representationId) {
														...
														oValidURL.objectType = ConfigurationObjectTypes.REPRESENTATION;
													}
												});
											}
										}
					...
					if (params.arguments.facetFilterId !== undefined) {
						self.oModel.getData().aConfigDetails[index].configData[0].filters.forEach(function(facetFilter, facetFilterIndex) {
							if (facetFilter.id === params.arguments.facetFilterId) {
								...
								oValidURL.objectType = ConfigurationObjectTypes.FACETFILTER;
					...
					if (params.arguments.smartFilterId !== undefined) {
						...
						oValidURL.objectType = ConfigurationObjectTypes.SMARTFILTERBAR;
						self.oModel.getData().aConfigDetails[index].configData[0].expanded = true;
					}
					if (params.arguments.navTargetId !== undefined) {
						self.oModel.getData().aConfigDetails[index].configData[2].navTargets.forEach(function(navTarget, navTargetIndex) {
							if (navTarget.id === params.arguments.navTargetId) {
								...
								oValidURL.objectType = ConfigurationObjectTypes.NAVIGATIONTARGET;
		...
		/**
		 * Set the expansion state of selected node + delete the new node or config.
		 *
		 * @private
		...
		_navHandleExpandDelete : function(node, params, isSwitchConfig) {
			...
			if (params.configId.indexOf(ConfigurationObjectTypes.ISNEWCONFIG) === 0 && bIsDifferntConfig) {
		...
		/**
		 * Check whether the sub view is new or existing.
		 *
		 * @private
		 * @params {object} params Route context 
		 */
		_isNewSubView : function(params) {
			var isNew = false;
			if (params.arguments.configId.indexOf(ConfigurationObjectTypes.ISNEWCONFIG) === 0) { //Check the prefix apf1972- for new 
				isNew = true;
			} else {
				if (params.name === ConfigurationObjectTypes.CONFIGURATION) {
					isNew = (this.configurationHandler.getConfiguration(params.arguments.configId) !== undefined) ? false : true;
				} else if (params.name === ConfigurationObjectTypes.CATEGORY) {
					isNew = (this.configEditor.getCategory(params.arguments.categoryId) !== undefined) ? false : true;
				} else if (params.name === ConfigurationObjectTypes.FACETFILTER) {
					isNew = (this.configEditor.getFacetFilter(params.arguments.facetFilterId) !== undefined) ? false : true;
				} else if (params.name === ConfigurationObjectTypes.STEP) {
					isNew = (this.configEditor.getStep(params.arguments.stepId) !== undefined) ? false : true;
				} else if (params.name === ConfigurationObjectTypes.REPRESENTATION) {
				...
				} else if (params.name === ConfigurationObjectTypes.NAVIGATIONTARGET) {
		...
		/**
		 * Return the configuration tree node.
		 *
		 * @private
		 */
		...
		/**
		 * On mandatory state reset the configuration and prepare new tree structure.
		 *
		 * @private
		...
		_navMandatoryResetState : function(context, callback) {
			...
			if (!isNewView) {
				...
				if (selectedNode) { // If node exists then set it to be selected else make the parent config node as selected cause the selected node has been deleted
					...
					var configObjTypes = ConfigurationObjectTypes;
		...
		/**
		 * Reset the configuration and delete the respective node if its new view.
		 *
		 * @private
		 * @params {Object} configuration list instance
		...
		/**
		 * On tree node selection performs action like navigate to view and performs checks for navigation scenarios.
		 *
		 * @private
		 */
		_onTreeNodeSelection : function(oEvent) {
			...
			if (!this.bProgramaticSelection) {
				...
				if (currNodeId !== prevNodeId || this.bNavDeletionMode) {
					...
					var navigationHandlerInstance = NavigationHandler.getInstance();
					...
					var copyConfigEditor = this.configEditor ? deepExtend({}, this.configEditor) : undefined;
						...
						navigationHandlerInstance.throwLossOfDataPopup(self, {
							...
							no : function() {
								self.configurationHandler.loadConfiguration(self.configId, function(configurationEditor, messageObject) {
									if (messageObject === undefined) {
										self.configEditor = configurationEditor;
										if (self.configId.indexOf(ConfigurationObjectTypes.ISNEWCONFIG) === 0) { //If new confiuration splice from model before navigating 
		...
		navigateToDifferntView : function(oParentNodeDetails, oSelectedNodeDetails, bIsSaved, bIsDifferntConfig) {
			if ((!isEmptyObject(oParentNodeDetails) && (bIsSaved === undefined || bIsSaved)) || (!bIsDifferntConfig)) { //If context is not equal to empty traverse to view
				UIComponent.getRouterFor(this).navTo(oSelectedNodeDetails.nodeObjectType, oParentNodeDetails, true);
			}
		},
		/**
		 * Remove the navigation target texts of the deleted configuration and deferred object associated.
		 *
		 * @private
		 * @params {int} indexOfConfig Index of configuration {Integer} Number of configurations in application
		 * @params {int} numberOfConfigs
		 */
		...
		handleConfirmDeletion : function(selectedNode, selectContext) { //event handler for confirmation dialog from delete button in the toolbar
			...
			switch (selectedTreeNodeDetails.nodeObjectType) {
				...
				case ConfigurationObjectTypes.NAVIGATIONTARGET:
					...
					newContextForCopiedNode = aNewContextForSelectedNode.join("/");
					break;
				case ConfigurationObjectTypes.STEP://Finds the step in all the categories it is present in and deletes it
					...
					newContextForCopiedNode = aNewContextForSelectedNode.join("/");
					break;
				case ConfigurationObjectTypes.REPRESENTATION://Finds the representation in all the categories it is present in and deletes it
					indexOfCategory = aOldContextForSelectedNode[6];
					indexOfStep = aOldContextForSelectedNode[8];
					...
					if (!isNew) {
						this.configurationHandler.removeConfiguration(selectedTreeNodeDetails.nodeAPFId, function(id) {
							...
							if (selectContext) {
								...
								UIComponent.getRouterFor(self).navTo(self.selectedNode._getBindingContext().getObject().type, selectedNodeContext, true);
							} else {
								self.clearTitleAndBreadCrumb();
								self.byId("idConfigDetailData").removeAllContent(); //Remove All Previous Stacked Content from DOM
								if (configListExists) {
									noConfigSelected = new Label().addStyleClass("noConfigSelected");
									...
									noConfigSelected = new Label().addStyleClass("addNewConfig");
			...
			if (selectedTreeNodeDetails.nodeObjectType !== ConfigurationObjectTypes.CONFIGURATION) {
				this.configEditor.setIsUnsaved();
				this.clearTitleAndBreadCrumb();
		...
		/**
		 * Handler for press of export button.
		 *
		 * @private
		 */
			...
			if (this.byId("idConfigDetailData").getContent().length >= 1) {
			 ...
			 var exportAction = function() {
                if (!this.exportConfigurationDialog) {
                    // Use Fragment.load() instead of sap.ui.xmlfragment
                    Fragment.load({
                        id: "idExportConfigurationFragment",
                        name: "sap.apf.modeler.ui.fragment.exportConfiguration",
                        controller: this
                    }).then(function(oDialog) {
                        this.exportConfigurationDialog = oDialog;
                        ...
                        this.configurationHandler.memorizeConfiguration(
                            subViewInstance.getView().getViewData().oParams.arguments.configId
                        );
                        SyncStyleClass("sapUiSizeCompact", this.getView(), this.exportConfigurationDialog);
                        this.exportConfigurationDialog.open();
                    }.bind(this));
                } else {
                    // If dialog already exists, just apply style and open
                    SyncStyleClass("sapUiSizeCompact", this.getView(), this.exportConfigurationDialog);
                    this.exportConfigurationDialog.open();
                }
            };
		...
		/**
		 * Handle for press of publish button.
		 *
		 * @private
		 */
		handlePublishPress : function() {
			if (this.oCoreApi.isUsingCloudFoundryProxy()) {
				UiHandler.showShareDialog(this.oCoreApi, this);
			}
		},
		/**
		...
		/**
		 * Sets text for export dialog.
		 *
		 * @private
		 */
		_setExportConfigDialogText : function() {
			Fragment.byId("idExportConfigurationFragment", "idExportConfigDialog").setTitle(this.oCoreApi.getText("exportConfig"));
			Fragment.byId("idExportConfigurationFragment", "idDownloadConfig").setText(this.oCoreApi.getText("downloadConfig"));
			Fragment.byId("idExportConfigurationFragment", "idDownloadTextProperty").setText(this.oCoreApi.getText("downloadTextProperty"));
			Fragment.byId("idExportConfigurationFragment", "idCloseButton").setText(this.oCoreApi.getText("close"));
		},
		/**
		 * Adds style class to the export configuration dialog.
		 *
		 * @private
		 */
		_addStyleClassForExportDialog : function() {
			Fragment.byId("idExportConfigurationFragment", "idDownloadConfig").addStyleClass("configLink");
			Fragment.byId("idExportConfigurationFragment", "idDownloadTextProperty").addStyleClass("textPropertyLink");
		},
		/**
		 * Handler for close of export configuration dialog.
		 *
		 * @private
		 */
		...
		/**
		 * Handler for press of configuration link in export dialog.
		 *
		 * @private
		 */
		_handleOpenConfigLinkPress : function() {
			this.configurationHandler.exportConfiguration(this.configId, function(configurationString, configurationName) {
				File.save(configurationString, configurationName, 'json', 'application/json', 'utf-8');
			});
		},
		/**
		 * Handler for press of text properties link  in export dialog.
		 *
		 * @private
		 */
		_handleOpenTextsLinkPress : function() {
			...
			File.save(exportedTexts, configurationName, 'properties', 'text/plain', 'utf-8');
		},
		_performExportOrExecute : function(requiredAction) {
			...
			var disableBtnOnNoConfigSelected = function(configListExists) {
				...
				if (configListExists) {
					showText = new Label().addStyleClass("noConfigSelected");
					showText.setText(oController.oCoreApi.getText("noConfigSelected"));
				} else {
					showText = new Label().addStyleClass("addNewConfig");
			...
			onUnsavedState = function() {
				navigationHandlerInstance.throwLossOfDataPopup(oController, {
					...
					no : function() {
						if (oController.configId.indexOf(ConfigurationObjectTypes.ISNEWCONFIG) === 0) { //If new confiuration splice from model before navigating 
		...
		/**
		 * Returns the details of the step to check if it is hierarchical (from APFRouter).
		 *
		 * @param {string} sPath - takes sPath of the step
		 */
