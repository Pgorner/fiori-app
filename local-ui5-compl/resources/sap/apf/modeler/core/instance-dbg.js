...
sap.ui.define([
	...
	"sap/apf/modeler/core/textHandler",
	"sap/apf/modeler/core/textPool",
	"sap/apf/modeler/core/applicationHandler",
	"sap/apf/modeler/core/configurationHandler",
	"sap/apf/modeler/core/configurationEditor",
	"sap/apf/modeler/core/step",
	"sap/apf/modeler/core/hierarchicalStep",
	"sap/apf/modeler/core/smartFilterBar",
	"sap/apf/modeler/core/facetFilter",
	"sap/apf/modeler/core/navigationTarget",
	"sap/apf/modeler/core/elementContainer",
	"sap/apf/modeler/core/representation",
	"sap/apf/modeler/core/configurationObjects",
	"sap/apf/utils/parseTextPropertyFile",
	"sap/apf/modeler/core/lazyLoader",
	"sap/apf/modeler/core/registryWrapper",
	...
	SessionHandler,
	representationTypes,
	ImportedEntityTypeMetadata,
	ImportedConfigurationFactory,
	checkForTimeout,
	CoreFilter,
	uriGenerator,
	ImportedMetadata,
	ImportedMetadataFacade,
	ImportedMetadataProperty,
	messageDefinition,
	ImportedMetadataFactory,
	OdataProxy,
	modelerProxy,
	ImportedAjaxHandler,
	ImportedProxyTextHandlerForLocalTexts,
	ajax,
	odataRequestWrapper,
	modelerMessageDefinition,
	TextHandler,
	TextPool,
	ImportedApplicationHandler,
	ImportedConfigurationHandler,
	ImportedConfigurationEditor,
	ImportedStep,
	ImportedHierarchicalStep,
	ImportedSmartFilterBar,
	ImportedFacetFilter,
	ImportedNavigationTarget,
	ImportedElementContainer,
	ImportedRepresentation,
	ImportedConfigurationObjects,
	parseTextPropertyFile,
	ImportedLazyLoader,
	ImportedRegistryWrapper,
	exportToGlobal,
	ImportedStartParameter,
	ImportedFileExists,
	ImportedAnnotationHandler,
	deepExtend,
	OData,
	jQuery
) {
	 ...
	 * @alias sap.apf.modeler.core.Instance
	 */
	function Instance(persistenceConfiguration, inject) {
		var that = this;
		...
		var datajs;
		//constructors
		ApplicationHandler = (inject && inject.constructors && inject.constructors.ApplicationHandler) || ImportedApplicationHandler;
		ConfigurationHandler = (inject && inject.constructors && inject.constructors.ConfigurationHandler) || ImportedConfigurationHandler;
		ConfigurationEditor = (inject && inject.constructors && inject.constructors.ConfigurationEditor) || ImportedConfigurationEditor;
		ConfigurationObjects = (inject && inject.constructors && inject.constructors.ConfigurationObjects) || ImportedConfigurationObjects;
		ConfigurationFactory = (inject && inject.constructors && inject.constructors.ConfigurationFactory) || ImportedConfigurationFactory;
		ElementContainer = (inject && inject.constructors && inject.constructors.ElementContainer) || ImportedElementContainer;
		Step = (inject && inject.constructors && inject.constructors.Step) || ImportedStep;
		HierarchicalStep = (inject && inject.constructors && inject.constructors.HierarchicalStep) || ImportedHierarchicalStep;
		SmartFilterBar = (inject && inject.constructors && inject.constructors.SmartFilterBar) || ImportedSmartFilterBar;
		FacetFilter = (inject && inject.constructors && inject.constructors.FacetFilter) || ImportedFacetFilter;
		NavigationTarget = (inject && inject.constructors && inject.constructors.NavigationTarget) || ImportedNavigationTarget;
		Representation = (inject && inject.constructors && inject.constructors.Representation) || ImportedRepresentation;
		Hashtable = (inject && inject.constructors && inject.constructors.Hashtable) || ImportedHashtable;
		RegistryProbe = (inject && inject.constructors && inject.constructors.RegistryProbe) || ImportedRegistryWrapper;
		LazyLoader = (inject && inject.constructors && inject.constructors.LazyLoader) || ImportedLazyLoader;
		Metadata = (inject && inject.constructors && inject.constructors.Metadata) || ImportedMetadata;
		EntityTypeMetadata = (inject && inject.constructors && inject.constructors.EntityTypeMetadata) || ImportedEntityTypeMetadata;
		MetadataFacade = (inject && inject.constructors && inject.constructors.MetadataFacade) || ImportedMetadataFacade;
		MetadataProperty = (inject && inject.constructors && inject.constructors.MetadataProperty) || ImportedMetadataProperty.constructor;
		MetadataFactory = (inject && inject.constructors && inject.constructors.MetadataFactory) || ImportedMetadataFactory;
		StartParameter = (inject && inject.constructors && inject.constructors.StartParameter) || ImportedStartParameter;
		AnnotationHandler = (inject && inject.constructors && inject.constructors.AnnotationHandler) || ImportedAnnotationHandler.constructor;
		FileExists = (inject && inject.constructors && inject.constructors.FileExists) || ImportedFileExists;
		ProxyTextHandlerForLocalTexts = (inject && inject.constructors && inject.constructors.ProxyTextHandlerForLocalTexts)
			|| ImportedProxyTextHandlerForLocalTexts;
		AjaxHandler = (inject && inject.constructors && inject.constructors.AjaxHandler || ImportedAjaxHandler);
		//instances
		...
		messageHandler.loadConfig(messageDefinition);
		messageHandler.loadConfig(modelerMessageDefinition);
		...
		if (inject && inject.instances && inject.instances.component) {
			manifests = {}; 
			manifests.baseManifest = Object.getPrototypeOf(inject.instances.component).getMetadata()._getManifest();
			manifests.manifest = deepExtend({}, inject.instances.component.getManifest());
		}
		...
		this.ajax = function(oSettings) {
			var injectForAjax = deepExtend({}, oSettings);
		...
		if (inject && inject.constructors && inject.constructors.PersistenceProxy) {
			...
			if (isUsingCloudFoundryProxy) {
				persistenceProxy = new modelerProxy.ModelerProxy(persistenceConfiguration, injectForFollowUp);
			} else if (manifests && manifests.manifest["sap.apf"] && manifests.manifest["sap.apf"].activateLrep) {
				persistenceProxy = new (inject && inject.constructors && inject.constructors.LayeredRepositoryProxy || LayeredRepositoryProxy)(persistenceConfiguration, injectForFollowUp);
			} else {
				persistenceProxy = new (inject && inject.constructors && inject.constructors.OdataProxy || OdataProxy)(persistenceConfiguration, injectForFollowUp);
			}
		}
		...
		if (inject && inject.constructors && inject.constructors.SessionHandler) {
			...
			sessionHandler = new SessionHandler(injectForFollowUp);
		}
		//core interface
		...
		fnOdataRequestWrapper = (inject && inject.functions && inject.functions.odataRequestWrapper) || odataRequestWrapper;
		/**
		...
		/**
		 * @see sap.apf.core.utils.checkForTimeout
		 */
		this.checkForTimeout = function(oServerResponse) {
			var messageObject = checkForTimeout(oServerResponse);
		...
		function importConfigurationFromLRep(applicationId, configurationId, callbackOverwrite, callbackImport) {
			...
			function readTextsFromVendorLayer () {
				var filterLanguage = new CoreFilter(messageHandler, 'Language', 'eq', coreConstants.developmentLanguage);
				var filterApplication = new CoreFilter(messageHandler, 'Application', 'eq', applicationId);
		...
		function importTexts (textElements, applicationId, callbackImport){
			...
			function loadTexts(existingTexts, metadata, messageObject) {
				...
				if (messageObject) {
					...
					textPool = new TextPool(injectTextPool, applicationId, existingTexts);
		 ...
		 * @param {function} callbackImport Called after import of text file finished. Contains success
		 */
		this.importTexts = function(textFileString, callbackImport) {
			var messageObject;
			var len;
			var i;
			var textFileInformation = parseTextPropertyFile(textFileString, {
				instances : {
					messageHandler : messageHandler
				}
			});
		 ...
		 * @param {function} callbackConfirmOverwrite Called if configuration contains an already existing configurationID
		...
		this.importConfiguration = function(configuration, callbackConfirmOverwrite, callbackImport) {
			...
			function validateGuids(configObject, callbackImport) {
				...
				if (!utils.isValidGuid(configObject.configHeader.Application)) {
				...
				if (!utils.isValidGuid(configObject.configHeader.AnalyticalConfiguration)) {
					messageObjects.push(messageHandler.createMessageObject({
						code : 11038,
				...
				textKeys = ImportedConfigurationObjects.getTextKeysFromConfiguration(configObject);
		...
		this.getRepresentationTypes = function() {
			var origRepresentationTypes = representationTypes();
			var represenationTypes = [];
			deepExtend(represenationTypes, origRepresentationTypes);
		...
		this.navigateToGenericRuntime = function (applicationId, configurationId, navigationMethod){
			var finalUrl;
			if(inject && inject.exits && inject.exits.getRuntimeUrl && typeof inject.exits.getRuntimeUrl === "function"){
	...
	exportToGlobal("sap.apf.modeler.core.Instance", Instance);
