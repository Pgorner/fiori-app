...
sap.ui.define([
	...
	Toolbar.prototype._parsePosition = function(sPosition) {
		return {
			...
			idx: parseInt(sPosition.substr(1, sPosition.length - 1))
		};
	};
	...
	Toolbar.prototype._genSettings = function (oGroupConfig) {
		...
		this._oInitialSettingState = {};
		aAllSettingItems.forEach(function(oSettingItem){
			this._oInitialSettingState[oSettingItem.getName()] = oSettingItem.getSelected();
		}.bind(this));
		this._oLastConfirmedSettingState = Object.assign({}, this._oInitialSettingState);

		// Need set the old setting state on the toolbar instance for reference
		var fnRetoreState = function (aAllSettingItems, oReferenceState) {
			var sSettingKey;
			for (var i = 0; i < aAllSettingItems.length; i++) {
				sSettingKey = aAllSettingItems[i].getName();
				aAllSettingItems[i].setSelected(oReferenceState[sSettingKey]);
			}
		};
		...
		this._oSettingsDialog = new ViewSettingsDialog({
			...
			confirm: function() {
				...
				for (var i = 0; i < aSettingItems.length; i++) {
					...
					this._oLastConfirmedSettingState[aSettingItems[i].getName()] = aSettingItems[i].getSelected();
				}
			...
			cancel: function() {
				...
				fnRetoreState(aAllSettingItems, this._oLastConfirmedSettingState);
			}.bind(this),
			reset: function() {
				fnRetoreState(aAllSettingItems, this._oInitialSettingState);
			}.bind(this)
		});
	...
	Toolbar.prototype._setSettingItemProperties = function(settingItemKey, settingItemStatus) {
		...
		for (var i = 0; i < settingItems.length; i++) {
			if (settingItems[i].getName() === settingItemKey) {
				...
				this._oLastConfirmedSettingState[settingItemKey] = settingItemStatus;
				break;
			}
		}
	};
	...
	Toolbar.prototype._calcOffsetForLegendPopover = function () {
		...
		} else if (Device.browser.name === "cr") {
		...
		if (Core.getConfiguration().getRTL() === true) {
			...
			if (aLegendContent && aLegendContent.length > 0) {
				...
				iOffsetX = Math.round((iConstant - parseInt(oLegend.getWidth())) * devicePxPerCssPx);
			}
		}
		return iOffsetX;
	};
