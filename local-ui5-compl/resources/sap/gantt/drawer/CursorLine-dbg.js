...
sap.ui.define([
	...
	CursorLine.prototype.drawSvg = function (aSvgBodyNode, aSvgHeaderNode, oLocale, oCursorPoint) {
		...
		aSvgHeaderNode.each(function(data, index){
			...
			if (iMarkerHeight !== null){
				that._drawHeaderLabel(aHeaderTopG, aActualDrawingPoints[index], oAvailSvg, parseInt(iMarkerHeight[0]));
			} else {
	...
	CursorLine.prototype._drawHeaderLabel = function (aGroup, oSvgPoint, oAvailSvg, iMarkerHeight) {
		...
		var iHeight;
		var oContainer = oAxisTime._oZoomStrategy && oAxisTime._oZoomStrategy.getParent().getParent(), oTimeStamp;
		if (oContainer && oContainer.isA("sap.gantt.simple.GanttChartContainer")){
			oContainer._timeStamp = !oContainer._bCalcTimeStamp && oContainer.getEnableTimeScrollSync() && oContainer._timeStamp;
			oContainer._bCalcTimeStamp = false;
		}
		// update path
		var getLabel = function(x){
			oTimeStamp = oContainer && oContainer._timeStamp ? oContainer._timeStamp : oAxisTime.viewToTime(x);
		...
		aRect
			.attr("width", function (d) {
				...
				textwidth = GanttUtils.getShapeTextWidth(txt, parseInt(Parameters.get("sapUiFontSmallSize")),"Arial","normal");
		...
		aText.exit().remove();
		if (oContainer && oContainer.isA("sap.gantt.simple.GanttChartContainer")){
			oContainer._timeStamp = oTimeStamp;
		}
	};
	...
	CursorLine.prototype._getTimeLabel = function (oLocalTime, oLocale, oAxisTime) {
		var oZoomStrategy = oAxisTime.getZoomStrategy();

		var sLabel = oLocale.getFormattedDate(oZoomStrategy.getLowerRowFormatter(), oLocalTime);
