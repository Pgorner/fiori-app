...
sap.ui.define([
	...
	/**
	 * Definition string of the SVG 'defs' tag.
	 * @param bRemoveClone Flag to determine 'Clone' text removal form the Id
	 * @returns {string} - Definition string to be rendered by caller.
	 * @private
	 */
	 SvgDefs.prototype._getDefString = function (bRemoveClone) {
		var sRetVal = "<defs id='" + this.getId() + "'>",
			aDefs = this.getDefs();

		if (aDefs && aDefs.length > 0) {
			aDefs.forEach(function(oDef){
				sRetVal += oDef.getDefString(bRemoveClone);
			});
		}
