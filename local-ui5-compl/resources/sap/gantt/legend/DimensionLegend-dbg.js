 ...
 */
sap.ui.define([
	"sap/ui/core/Control", "./LegendBase", "sap/gantt/drawer/Legend", "sap/ui/Device"
], function (Control, LegendBase, LegendDrawer, Device) {
	"use strict";
	...
	DimensionLegend.prototype._drawDimensionText = function (aShape, oShape, oLegendContainerSize, oLegendSize) {
		...
		.attr("y", function(d, i){
			return that.getScaledLegendHeight() * i + that.getLegendSpace() * (i + 1) +
					parseInt(that.getFontSize()) / 2 + that.getScaledLegendHeight() / 2 - 2;
			})
		...
		.attr("text-anchor", Device.browser.name === "ie" && sap.ui.getCore().getConfiguration().getRTL() ? "end" : "start");
