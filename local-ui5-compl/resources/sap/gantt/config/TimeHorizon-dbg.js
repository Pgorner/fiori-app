...
sap.ui.define([
	...
	TimeHorizon.prototype.setStartTime = function (vStartTime, bSurpressInvalidate) {
		this._allowGanttInitialRender(); // we might need to jump to visible horizon
		return this.setProperty("startTime", this._convertTimestamp(vStartTime), bSurpressInvalidate);
	};

	TimeHorizon.prototype.setEndTime = function (vEndTime, bSurpressInvalidate) {
		this._allowGanttInitialRender(); // we might need to jump to visible horizon
		return this.setProperty("endTime", this._convertTimestamp(vEndTime), bSurpressInvalidate);
	};
	...
	TimeHorizon.prototype._allowGanttInitialRender = function () {
		var oParent = this.getParent();
		if (oParent && typeof oParent.getParent === "function") {
			var oSecondParent = oParent.getParent();
			if (oSecondParent && oSecondParent.isA("sap.gantt.simple.GanttChartWithTable")) {
				delete oSecondParent._bPreventInitialRender;
			}
		}
	};
