...
sap.ui.define([
	...
	"sap/ui/core/Element"
], function(jQuery, Core, UtilizationChart, BackSlashPattern, Element){
	"use strict";
	...
	UtilizationLineChart.prototype.renderEmptyDomRefs = function(oRm, bClosedTag) {
		...
		oRm.class("sapGanttUtilizationLine");
		oRm.openEnd();
		if (bClosedTag) {
			oRm.close("g");
		}
	...
	UtilizationLineChart.prototype._getOverageFillPattern = function() {
		...
		if (sPatternId) {
			sPatternId = Element.getElementById(sPatternId).getRefString();
		} else {
	...
	UtilizationLineChart.prototype.renderMiddleLine = function(oRm, mProp) {
		if (this.getShowMiddleLine()) {
			...
			oRm.class("sapGanttUtilizationMiddleLine");
	...
	UtilizationLineChart.prototype.renderTooltips = function(oRm, mProp) {
		...
		oRm.openStart("g");
		oRm.class("ulc-tooltips").openEnd();
	...
	UtilizationLineChart.prototype.renderDimensionPath = function(oRm, oDimension, iRowY, iRowHeight, sIdSuffix) {
		...
		oRm.attr("stroke", oDimension.getDimensionColor());
		oRm.class("sapGanttUlcDimensionPath");
		oRm.openEnd().close("path");
