...
sap.ui.define([ "sap/ui/base/Object" ],function(BaseObject) {
		...
		GanttExtension.prototype.getDomRefs = function() {
			...
			var oHeaderSvg = fnById("-header-svg"),
				oHeader = null,
				oGanttContainerToolbar = null;
			if (oHeaderSvg) {
				oHeader = oHeaderSvg.parentNode;
			}

			if (this.getGantt().getParent()
					&& this.getGantt().getParent() instanceof sap.gantt.simple.GanttChartContainer
					&& this.getGantt().getParent().getToolbar()) {
				oGanttContainerToolbar = window.document.getElementById(this.getGantt().getParent().getToolbar().getId());
			}
			return {
				gantt: fnById("-gantt"),
				ganttSvg: fnById("-svg"),
				...
				ganttChart:  window.document.getElementById(this.getGantt().getId()),
				ganttChartContainerToolbar: oGanttContainerToolbar
			};
		};
