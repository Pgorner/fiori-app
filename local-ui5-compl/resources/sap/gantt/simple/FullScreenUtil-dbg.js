...
sap.ui.define([
	...
	/**
	 * Utility class used by gantt for switching the control to fullscreen
	 */
	var FullScreenUtil = {
			...
			if (bIsFullScreen) {
				...
				if (oContent) {
						...
						oControl._oHTML = new HTML({
							...
							afterRendering: function() {
								if (oControl && oControl._oHTML) {
									oControl._oFullScreenDialog.getDomRef().classList.remove("sapUiPopupWithPadding");
					...
					if (!oControl._oFullScreenDialog) {
						oControl._oFullScreenDialog = new Dialog({
							...
							beforeClose: function() {
								...
								if (oControl && oControl._placeHolder && fnExternalClose) {
									fnExternalClose.call(oControl.getGanttCharts()[0], false, oFullScreenButton);
								}
							},
							content: [
								oControl._oHTML
						...
						oControl._oFullScreenDialog.addStyleClass("sapFullScreenGanttDialog");
				...
				if (oControl.getShowSearchSidePanel()) {
					oControl.setProperty("showSearchSidePanel", false, true);
					window.setTimeout(function(){	// using timeout to make sure Splitter container heights are calculated before adding the side panel
							oControl.setProperty("showSearchSidePanel", true, false);
						}, 0);
				}
