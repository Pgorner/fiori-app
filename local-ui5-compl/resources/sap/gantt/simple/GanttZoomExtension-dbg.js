...
sap.ui.define([
	...
	var GanttZoomHelper = {
		...
		addEventListeners: function(oGantt) {
			...
			if (oGanttSvg) {
				//Get all the StockChartTooltips if present.
				var aStockChartTooltips = document.querySelectorAll('.sc-tooltips');
				if (!oGantt.fnStockChartMouseMoveListener) {
					oGantt.fnStockChartMouseMoveListener = this.fnStockChartMouseMoveHandler.bind(this);
				}
				for (var i = 0; i < aStockChartTooltips.length; i++) {
					//Add MouseMove event Listener to the Container containing all the tooltips.
					aStockChartTooltips[i].removeEventListener('mousemove', oGantt.fnStockChartMouseMoveListener);
					aStockChartTooltips[i].addEventListener('mousemove', oGantt.fnStockChartMouseMoveListener);
				}
			}
		},

		fnStockChartMouseMoveHandler: function(oEvent) {
			//Function to return the Tooltip of all the element at a given mouse point
			function getAllElementsFromPoint(x, y) {
				var oItem = document.elementFromPoint(x, y);
				var sTooltip = "";
				while (oItem && oItem.tagName == "rect") {
					if (oItem.children && oItem.children.length && oItem.children[0].tagName == "title") {
						if (oItem.classList.contains("stockChartTooltip")) {
							oItem.style.pointerEvents = "none";
							oItem = document.elementFromPoint(x, y);
						} else {
							sTooltip = oItem.children[0].innerHTML + "\n" + sTooltip;
							oItem.style.pointerEvents = "none";
							oItem = document.elementFromPoint(x, y);
						}
					} else {
			...
			var oStockChartTooltip = oEvent.currentTarget; //Get current svg element
			var aTooltips = oStockChartTooltip.querySelectorAll(".sc-tooltips rect");

			// Add pointer-events:all
			aTooltips.forEach(function(oTooltip){
				oTooltip.style.pointerEvents = "all";
			});

			// then get all elements at the mouse position
			var aFinalTooltip = getAllElementsFromPoint(oEvent.clientX, oEvent.clientY);

			for (var i = 0; i < oStockChartTooltip.children.length; i++) {
				var oStockChartTooltipRect = oStockChartTooltip.children[i];
				if (oStockChartTooltipRect.classList.contains("stockChartTooltip")) {
					oStockChartTooltipRect.getElementsByTagName("title")[0].innerHTML = aFinalTooltip;
					oStockChartTooltipRect.style.pointerEvents = "all";
					oStockChartTooltipRect.setAttribute("width",oStockChartTooltip.getBoundingClientRect().width);
				}
			}
		},
	...
	GanttZoomExtension.prototype.decideMouseWheelZoom = function(oOriginEvent, iScrollDelta, iMousePositionX, bSuppressSyncEvent) {
		...
		if ((bZoomOut && iCurrentZoomLevel > 0) || (bZoomIn && iCurrentZoomLevel < iMaximalZoomLevel)) {
			...
			// Mousewheel zoom triggers a zooming every 700ms, so the update requests fired with 700ms will be done once
			...
			var iTimeDelay = (!this.iMouseWheelZoomDelayedCallId && (Date.now() - this.iLastMouseWheelZoomTimeInMs > 100)) ? 0 : 700;
			if (iTimeDelay === 0) {
	...
	GanttZoomExtension.prototype.isZoomingRectangleNotExisted = function() {
		...
		return aSvgNode.selectAll(".sapGanttChartTimePeriodZoomRectangle").length === 0;
	};
	...
	GanttZoomExtension.prototype.calculateBirdEyeRange = function(iRowIndex, bBirdEyeTriggered) {
		var oGantt = this.getGantt();
		...
		iRowIndex = iRowIndex ? iRowIndex - aRows[0].getIndex() : iRowIndex;
		var bRowVisible = aRows[iRowIndex] ? true : false;
		if (iRowIndex != null && iRowIndex >= 0 && bRowVisible){
			oHorizonRange = this._getBirdEyeRangeOnRow(iRowIndex);
			oStartTime = oHorizonRange.startTime;
			oEndTime = oHorizonRange.endTime;
		} else if (iRowIndex != null && iRowIndex >= 0 && !bRowVisible) {
			return null;
		} else {
			for (var i = 0; i < aRows.length; i++){
				oHorizonRange = this._getBirdEyeRangeOnRow(i, bBirdEyeTriggered);
	...
	GanttZoomExtension.prototype._getBirdEyeRangeOnRow = function(iRowIndex, bBirdEyeTriggered) {
		var oGantt = this.getGantt();
		var oTable = oGantt.getTable();
		...
		if (oRowContext){
			...
			aShapesInRow.forEach(function(oShape){
				//For BaseConditionalShape use Active Element for BirdEyeView Calculation
				if (oShape instanceof sap.gantt.simple.BaseConditionalShape) {
					oShape = oShape._getActiveShapeElement();
					//For BasseGroup within BaseConditionalShape get Calculate BirdEye for Each Shape
					if (oShape instanceof sap.gantt.simple.BaseGroup) {
						oShape.getShapes().forEach(function(oShape){
							if (oShape.getCountInBirdEye && oShape.getCountInBirdEye()) {//Check for BaseShape
				...
				if (oShape instanceof sap.gantt.simple.BaseGroup && oShape.isPseudoShape && oShape.getCountInBirdEye && oShape.getCountInBirdEye()) {
					var oShapeStartTime = oShape.groupBirdEyeRangeStartTime;
					var oShapeEndTime = oShape.groupBirdEyeRangeEndTime;
					var oResult = that._calculateBirdEyeTimeRange(oStartTime, oEndTime, oShapeStartTime, oShapeEndTime);
					oStartTime = oResult.startTime;
					oEndTime = oResult.endTime;
					return;
				}
				if (oShape && oShape.getCountInBirdEye && oShape.getCountInBirdEye()){//Check for BaseShape
					var oShapeStartTime = oShape.getTime();
					var oShapeEndTime = oShape.getEndTime();
		...
		if (oStartTime && oEndTime){
			...
			oStartTime = bBirdEyeTriggered ? new Date(oStartTime.getTime() - timeRangePerPixel * 5) : oAxisTime.viewToTime(oAxisTime.timeToView(oStartTime) - 5);
			oEndTime = bBirdEyeTriggered ? new Date(oEndTime.getTime() + timeRangePerPixel * 5) :  oAxisTime.viewToTime(oAxisTime.timeToView(oEndTime) + 5);
		}
	...
	GanttZoomExtension.prototype._getShapeInRow = function(oElement, aShapesInRow){
		...
		if (oElement.isA("sap.gantt.simple.GanttRowSettings")){
			...
			var aOverlayList =  AggregationUtils._fetchOverlayAggregation(oElement, !bHasExpandShape || pseudoShapeExpanded, false, oExpandModel.aShapeSchemes);
