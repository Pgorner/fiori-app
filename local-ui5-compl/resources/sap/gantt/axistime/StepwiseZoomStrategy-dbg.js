...
sap.ui.define([
	...
	StepwiseZoomStrategy.prototype.applySettings = function () {
		...
		this.initialSettings = arguments[0];
		return this;
	};
	...
	StepwiseZoomStrategy.prototype.setInitialProperties = function () {
		...
		if (this._getIndexOfTimeLineOption(this.getTimeLineOption()) === -1){
			this.setTimeLineOption(this._aTimeLineOptions[Math.floor((this._getCountOfTimeLineOptions() - 1) / 2)]);
		}
	};
	...
	StepwiseZoomStrategy.prototype.setTotalHorizon = function (oTotalHorizon) {
		//redraw will call syncContent(), in there rate will be updated
		var oLastTotalHorizon = this.getTotalHorizon();
		if (oLastTotalHorizon) {
			this.oLastTotalHorizonStartTime = oLastTotalHorizon.getStartTime();
			this.oLastTotalHorizonEndTime = oLastTotalHorizon.getEndTime();
		}
	...
	StepwiseZoomStrategy.prototype._updateTimeRangeAndViewRange = function (oHorizon) {
		...
		if (oAxisTime && oHorizon) {
			...
			var oDiscontinuousProvider = oAxisTime._discontinuityProviderInstance && oAxisTime._discontinuityProviderInstance.useDiscontinuousScale(oTimeLineOption.smallInterval.unit) ? oAxisTime._discontinuityProviderInstance : null;
			if (oDiscontinuousProvider) {
				nHorizonTimeRange = oDiscontinuousProvider.distance(oHorizonStartTime, oHorizonEndTime);
			}

			//update view range
			oAxisTime.setViewRange([0, Math.ceil((nHorizonTimeRange * this._getTimeLineRange(oTimeLineOption) / nUnitTimeRange) / this._aZoomRate[this.getZoomLevel()])]);
		}
		return this;
	};
	...
	StepwiseZoomStrategy.prototype.updateInitialVisibleHorizon = function (visibleHorizon, initialZoomLevel) {
		this.setProperty("timeLineOption", this._getTimeLineOptionByIndex(initialZoomLevel), true);
		var dMiddleOfVisibleHorizon = this.calMiddleDate(Format.getTimeStampFormatter().parse(visibleHorizon.getStartTime()), Format.getTimeStampFormatter().parse(visibleHorizon.getEndTime()));
		var oNewVisibleHorizon = this.calVisibleHorizonByRate(this._aZoomRate[this.getZoomLevel()], dMiddleOfVisibleHorizon);
		AxisTimeStrategyBase.prototype._setVisibleHorizon.call(this, oNewVisibleHorizon);
	};
	/**
	...
	StepwiseZoomStrategy.prototype.syncContext = function (nClientWidth) {
		...
		if (this.bBirdEyeTriggered) {
            // Update zoom level, zoom rate and time line option when bird eye triggered
            this.bBirdEyeTriggered = false;
			var oAxisTime = this.getAxisTime();

			if (oAxisTime) {
				var oVisibleHorizon = this.getVisibleHorizon();
				var fScale = this.calZoomScaleByDate(Format.abapTimestampToDate(oVisibleHorizon.getStartTime()), Format.getTimeStampFormatter().parse(oVisibleHorizon.getEndTime()),nClientWidth);

				var fZoomRate = this._oZoom.base.scale / fScale;
				var iZoomLevel = 0;

				while (fZoomRate >= this._aZoomRate[iZoomLevel]) {
					iZoomLevel++;
				}
				...
				if (this.iLastZoomLevel !== iZoomLevel) {
					oAxisTime.setZoomRate(this._aZoomRate[iZoomLevel]);
					this.setProperty("zoomLevel", iZoomLevel, true);
					this.setProperty("timeLineOption", this._getTimeLineOptionByIndex(iZoomLevel), true);
					this._updateVisibleHorizon(this.getGanttVisibleWidth());
					this.iLastZoomLevel = this.getZoomLevel();
					oRetVal.axisTimeChanged = true;
				}
			}
		} else if (nClientWidth !== this.getGanttVisibleWidth()) {
			this._updateVisibleHorizon(nClientWidth);
			oRetVal.axisTimeChanged = true;
			AxisTimeStrategyBase.prototype.updateGanttVisibleWidth.apply(this, arguments);

		} else if ((this.iLastZoomLevel === this.getZoomLevel()) && (!this.oLastTotalHorizonStartTime || ((this.oLastTotalHorizonStartTime === oTotalHorizon.getStartTime()) && (this.oLastTotalHorizonEndTime === oTotalHorizon.getEndTime())))) {
			oRetVal.axisTimeChanged  = false;

		} else {
			oRetVal.axisTimeChanged  = true;
			this.iLastZoomLevel = this.getZoomLevel();
			this.oLastTotalHorizonStartTime = oTotalHorizon.getStartTime();
			this.oLastTotalHorizonEndTime = oTotalHorizon.getEndTime();
		}
				...
				if (this._sZoomControlType !== sap.gantt.config.ZoomControlType.Select){
					...
					for (var j in oTimeLineOptions) {
						...
						this._aZoomRate[i] = this._oZoom.base.scale / this.calZoomScale(
								...
								this._getTimeLineRange(oTimeLineOptions[j])
						);
					}
				} else {
					i = 0;
					for (var n in oTimeLineOptions) {
						this._aZoomRate[i] = this._oZoom.base.scale / this.calZoomScale(
								...
								this._getTimeLineRange(oTimeLineOptions[n])
						);
						i++;
					}
				}
	...
	StepwiseZoomStrategy.prototype._updateZoomControlType = function (sZoomControlType) {
		if (sZoomControlType) {
			...
			this._updateZoomRateArray();
			this._updateTimeLineOptions();
			if (this.initialSettings.zoomLevel != null && this.initialSettings.zoomLevel != 0){
				this.setZoomLevel(this.initialSettings.zoomLevel);
			} else {
				this.setZoomLevel(this._getIndexOfTimeLineOption(this.getTimeLineOption(), this.getTimeLineOptions()));
			}
		}
