...
sap.ui.define([
	...
	var AxisTimeStrategyBase = Element.extend("sap.gantt.axistime.AxisTimeStrategyBase", /* @lends sap.gantt.axistime.AxisTimeStrategyBase */ {
		metadata: {
			...
			properties: {
				 ...
				 * <li>range: defines a certain length in pixel. This can be a value, or a function that returns the value. Gantt Chart loops the granularity levels from the beginning and chooses the first granularity
				 ...
				 * 						<li><code>"selector": (requiredRange) => 90 </code> - This granularity level is selected as the current level if 12 hours is the first time span that
				 *                                 consumes more than 90 pixels to be displayed in the chart area. Selector takes precedence over the range for level selection and provides flexibility to adjust range or column size in the timeline</li>
				 * 					</ul>
				 * 				</li>
				 * 				<li>
				 ...
				 * @private since 1.126
				 */
				_discontinuousProvider: { type: "object", multiple: false, visibility: "hidden" }
			},
			aggregations: {
				/**
				 ...
				 * We recommend that you set the type of this argument to <code>sap.gantt.skipTime.SkipPattern</code>.
				 * Otherwise, some properties you set may not function properly.
				 *
				 * Supported in <code>sap.gantt.axistime.StepwiseZoomStrategy</code> from 1.126.
				 * Supported in <code>sap.gantt.axistime.ProportionZoomStrategy</code> from 1.128.
				 * Supported in <code>sap.gantt.axistime.FullScreenStrategy</code> from 1.128.
				 *
				 * The default value for skipTimePattern is null.
				 * @experimental since 1.126
				 */
				skipTimePattern: { type: "sap.gantt.skipTime.SkipPattern", multiple: false },
	 ...
	 * @param {sap.gantt.skipTime.SkipPattern} oSkipTimePattern Skip time pattern to be set
	...
	AxisTimeStrategyBase.prototype.setSkipTimePattern = function (oSkipTimePattern, bSuppressInvalidate) {
		...
		this.setProperty("_discontinuousProvider", oDiscontinuousProvider, bSuppressInvalidate);
	};
	...
	AxisTimeStrategyBase.prototype.exit = function () {
		Localization.detachChange(this._onLocalizationChanged.bind(this));
	...
	AxisTimeStrategyBase.prototype.createAxisTime = function (oLocale) {
		...
		var timeRange = [oHorizonStartTime, oHorizonEndTime];
		var iRange = this._getTimeLineRange(oTimeLineOption);
		var viewRange = [0, Math.ceil(nHorizonTimeRange * iRange / nUnitTimeRange)];
		var oDiscontinuousProvider = this.getProperty("_discontinuousProvider");

		if (oDiscontinuousProvider) {
			var nTotalTradingTimeRange = oDiscontinuousProvider.distance(oHorizonStartTime, oHorizonEndTime);
			viewRange = [0, Math.ceil(nTotalTradingTimeRange * iRange / nUnitTimeRange)];
		}
	...
	*/
	AxisTimeStrategyBase.prototype.getCalendarWeekNumbering = function () {
		...
		return eCalendarWeekNumbering || GanttChartConfigurationUtils.getCalendarWeekNumbering();
	};
	...
	*/
	...
	AxisTimeStrategyBase.prototype.calZoomBase = function () {
		...
		if (oBaseTimeLineOption) {
			var fScale = this.calZoomScale(
					...
					this._getTimeLineRange(oBaseTimeLineOption)
			);
	...
	AxisTimeStrategyBase.prototype.checkFirstDayOfWeek = function (mSettings) {
		if (typeof mSettings.firstDayOfWeek === "undefined") {
			...
			if (oWeekConfig) {
				...
				mSettings.firstDayOfWeek = LocaleData.getInstance(GanttChartConfigurationUtils.getLanguageTag()).getFirstDayOfWeek();
			}

			Localization.attachChange(this._onLocalizationChanged.bind(this));
            Formatting.attachChange(this._onLocalizationChanged.bind(this));
		}
	};

	/**
	 * Utility method to return the timelineOption range from range function with fallback to range property
	 * @param {object} oTimeLineOption timelineOption
	 * @returns {number} returns the range for the timelineOption
	 * @private
	 */
	AxisTimeStrategyBase.prototype._getTimeLineRange = function (oTimeLineOption) {
		return typeof oTimeLineOption.innerInterval.range === "function" ? oTimeLineOption.innerInterval.range() : oTimeLineOption.innerInterval.range;
	};
	...
	AxisTimeStrategyBase.prototype._onLocalizationChanged = function () {
		...
		if (oWeekConfig) {
			...
			this.setFirstDayOfWeek(LocaleData.getInstance(GanttChartConfigurationUtils.getLanguageTag()).getFirstDayOfWeek());
		}
