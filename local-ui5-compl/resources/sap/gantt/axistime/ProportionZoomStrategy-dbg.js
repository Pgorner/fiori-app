...
sap.ui.define([
	...
	var ProportionZoomStrategy = AxisTimeStrategyBase.extend("sap.gantt.axistime.ProportionZoomStrategy", {
		metadata: {
			...
			properties: {
				/**
				 * Flag to enable millisecond support in the Gantt Chart's timeline.
				 * When the flag is enabled, the control accepts ABAP timestamps in the format of <code>YYYYMMDDHHMMSSLLL</code> or JS Date with milliseconds bound to the total horizon and visible horizon.
				 * Additionally, the control appends a millisecond offset to ABAP timestamps bound to the total horizon and visible horizon in the following format: <code>YYYYMMDDHHMMSSLLL</code>.
				 * @since 1.123
				 */
				enableMillisecondSupport: { type: "boolean", defaultValue: false, bindable: false }
			}
		}
	});
	...
	/**
	 * @private
	 * @param {object} mSettings input settings
	 * @returns {ProportionZoomStrategy} zoom strategy aggregation
	 */
	...
	/**
		...
		* @since 1.123
		* @public
		* @param {boolean} bValue value to enable or disable
		* @returns {ProportionZoomStrategy} zoom strategy aggregation
	 */
	ProportionZoomStrategy.prototype.setEnableMillisecondSupport = function (bValue) {
		this.setProperty("enableMillisecondSupport", bValue);
		Format._setEnableMillisecondSupport(bValue);
		return this;
	};
	 ...
	 * @private
	 */
	ProportionZoomStrategy.prototype._applyMissingSettings = function () {
		if (this.getEnableMillisecondSupport()) {
			Format._setEnableMillisecondSupport(true);
		}
	};
	...
	ProportionZoomStrategy.prototype.setZoomLevel = function (iZoomLevel) {
		...
		performance.mark("ProportionZoomStrategy.setZoomLevel--end");
		return this;
	};
		...
		if (this._oZoom && this._oZoom.base) {
			...
			return {
					minRate:  this._oZoom.base.scale / this.calZoomScale(
							...
							this._getTimeLineRange(oCoarsestTimeLineOption)
					),
					maxRate: this._oZoom.base.scale / this.calZoomScale(
							...
							this._getTimeLineRange(oFinestTimeLineOption) * 4
					)
			};
		}
	};
	...
	ProportionZoomStrategy.prototype._updateTimeLineOption = function(){
		...
		if (oAxisTime) {
			var start = oAxisTime.timeToView(startTime, undefined, oAxisTime.continuousScale);

			for (i in oTimeLineOptions) {
				var interval = oTimeLineOptions[i].innerInterval;
				var end = oAxisTime.timeToView(ObjectPath.get(interval.unit).offset(startTime, interval.span), undefined, oAxisTime.continuousScale);
				var r = Math.abs(Math.ceil((end - start)));
				var bValue = typeof interval.selector === "function" ? interval.selector(r) : r >= this._getTimeLineRange(oTimeLineOptions[i]);
