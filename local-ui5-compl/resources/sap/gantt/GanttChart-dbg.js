...
function (
	...
	GanttChart.prototype.init = function () {
		...
		this._oColumn = new sap.ui.table.Column({
			template: new sap.ui.core.HTML({
				content: "<div tabIndex = \" 0 \" class=\" sapGanttCellTransparent \">" + oRb.getText("XCEL_GANTT_CELL") + "</div>"
			})
		});
	 ...
	 * @return {this} GanttChart instance for chaining
	 */
	 ...
	 * @param {float} fTimeZoomRate
	 * @returns {this} Reference to this to allow method chaining
	 */
	...
	GanttChart.prototype._getDraggingDomObjs = function (oEvent) {
		...
		if (oParentDom && oParentDom.getAttribute("class")) {
			...
			} else if (sap.ui.getCore().byId(sTopShapeClass) instanceof sap.gantt.shape.Text){
				result.groupDom = oParentDom.parentElement;
				result.contentDoms = oParentDom.parentElement.childNodes;
			}
		} else {
		...
		if (dx > 3 || dy > 3) {
			...
			if (this._oDraggingData === undefined) {
				...
				if (aDragDiv.empty() || dragDiv === null) {
					...
					this._resetShadowRelativePosition(oClonedDom, undefined, undefined, d3.select(this._oDraggingData.groupDom).data());
	...
	GanttChart.prototype._resetShadowRelativePosition =  function (oClonedDom, relativeX, relativeY, oShapeData) {
		var that = this;
			...
			if (!relativeX) {
				aChildNodes = aChildNodes.filter(function(index, oNode) {
					...
					if (oNode.getAttribute("class")) {
						oShape = sap.ui.getCore().byId(oNode.getAttribute("class").split(" ")[0]);
					}
					if (oJQueryNode.attr("x") == undefined || oJQueryNode.attr("y") == undefined || oShape == null || oShape.getTime(oShapeData[0]) == "") {
						return false;
					}
					return true;
				});
	...
	GanttChart.prototype._collectDraggingShapeData = function (oDraggingSource, oEvent) {
		...
		if (Core.getConfiguration().getRTL() === true) {
			...
			sNewStartTime = new Date(sNewEndTime - iShapeDuration).getTime();
		} else {
			sNewStartTime = this.getAxisTime().viewToTime(sShapeCurrentStartX).getTime();
			sNewEndTime = new Date(sNewStartTime + iShapeDuration).getTime();
		}
		/*
	...
	GanttChart.prototype._getScrollRateByScrollLeft = function (iScrollLeft){
		...
		if (nScrollWidth !== nVisibleWidth){
			...
			if (Core.getConfiguration().getRTL() === true && Device.browser.edge) {
	...
	GanttChart.prototype._getHSBScrollLeftByScrollRate = function (fScrollRate){
		...
		if (Core.getConfiguration().getRTL() && Device.browser.edge) {
	...
	GanttChart.prototype._drawHeader = function () {
		...
		if (Device.browser.edge && Core.getConfiguration().getRTL()) {
		...
		var sPathData = "";
		for (i = 0; i < aLabelList[1].length; i++) {
			oLabel = aLabelList[1][i];
			if (oLabel) {
				sPathData +=
					" M" +
					" " + (oLabel.value - 1 / 2) +
		...
		// remove first visible large interval label before creating paths
		var oAxisTime = this.getAxisTime();
		if (oAxisTime.largeIntervalLabel && !oAxisTime.largeIntervalPath) {
			for (i = 0; i < aLabelList[0].length; i++) {
