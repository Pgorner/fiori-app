...
sap.ui.define([
	 ...
	 * @returns {this} A reference to the GanttChartWithTable control, which can be used for chaining
	 * @public
	 * @deprecated Since version 1.50.7. We recommend use setTableProperties function instead
	 */
	 ...
	 * @public
	 * @deprecated Since version 1.50.7. We recommend use getTableProperties function instead
	 */
	 ...
	 *
	 * @param {sap.ui.table.plugins.SelectionPlugin} oSelectionPlugin Selection plugin to use.
	 * Passing an empty value disables any selection plugin that was used previously.
	 * @public
	 * @since 1.76
	 */
	GanttChartWithTable.prototype.useSelectionPlugin = function (oSelectionPlugin) {
		var oGanttChart = this.getAggregation("_chart");
		var oLeftTable = this.getAggregation("_selectionPanel");
		var oRightTable = oGanttChart.getAggregation("_treeTable");

		if (this._oSelectionPlugin) {
			// remove old selection plugin first
			this._oSelectionPlugin.detachSelectionChange(oGanttChart._onRowSelectionChange, oGanttChart);
			oRightTable.removePlugin(this._oSelectionPlugin);
			oLeftTable.removePlugin(this._oSelectionPlugin);

			// this is needed because removing the plugin from a table does not work if it doesn't have it in the plugins aggregation
			delete oRightTable._oSelectionPlugin;
			delete oLeftTable._oSelectionPlugin;

			this._oSelectionPlugin = null;
		}

		if (oSelectionPlugin) {
			oSelectionPlugin.attachSelectionChange(oGanttChart._onRowSelectionChange, oGanttChart);
			oRightTable.addPlugin(oSelectionPlugin);
			oLeftTable.addPlugin(oSelectionPlugin);

			// this is needed because adding the same plugin to a 2nd table removes it from the plugins aggregation of the the 1st table
			oRightTable._oSelectionPlugin = oSelectionPlugin;
			oLeftTable._oSelectionPlugin = oSelectionPlugin;
	...
	GanttChartWithTable.prototype.onAfterRendering = function () {
		this._adjustChartHeaderHeight();//Adjust the GantChart header height before the Row Count Calculation
		this._attachEvents();
	};
	...
	GanttChartWithTable.prototype._syncGanttTablesDomEvents = function(oEvent) {
		...
		this._adjustChartHeaderHeight(); //Adjust the GantChart header height after the Table rendering is completed.
