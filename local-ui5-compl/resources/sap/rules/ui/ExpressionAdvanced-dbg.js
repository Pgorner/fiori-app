...
sap.ui.define([
...
], function(Library, ExpressionAdvancedRenderer, Log, Dialog, Control, Button, TextArea, Text, MessageBox, HorizontalLayout, Utils, Popup, jQ, ExpressionBase, MetadataAnalyser,
	ValueHelpProvider, LocaleData, Constants, Formatting, DateFormat,
	coreLibrary, JSONModel, Calendar, ODataModel, ValueHelpDialog, SmartFilterBar,
	ControlConfiguration, Column, Label, Filter, FilterOperator, coreLib, BusyIndicator, Element/*DecisionTableCellExpressionAdvanced*/) {
	"use strict";
	/**
	...
	var ExpressionAdvanced = ExpressionBase.extend("sap.rules.ui.ExpressionAdvanced", {
		...
		metadata: {
			properties: {
				...
				type: {
					...
					defaultValue: Library.ExpressionType.All,
					bindable: "bindable"
				},
		...
		renderer: ExpressionAdvancedRenderer
	});

	ExpressionAdvanced.prototype.init = function() {
		ExpressionBase.prototype.init.apply(this, arguments);
		var codeMirrorCSS = sap.ui.require.toUrl("sap/rules/ui/codemirror/lib") + "/codemirror.css";
		var showHintCSS = sap.ui.require.toUrl("sap/rules/ui/codemirror/addon/hint") + "/show-hint.css";
		sap.ui.require([
                        "sap/ui/dom/includeStylesheet"
                      ], (includeStyleSheet) => {
                            includeStyleSheet(codeMirrorCSS);
                            includeStyleSheet(showHintCSS);
                      });
	...
	ExpressionAdvanced.prototype._processValidationResult = function(result) {
		if (result.status === Library.ValidationStatus.Error) {
	...
	ExpressionAdvanced.prototype.validateExpression = function(sInput) {
		...
		var result = {};
		var incomingdata = {};
		var oEL = Element.getElementById(this.getExpressionLanguage());
		if (oEL) {
	...
	ExpressionAdvanced.prototype._showPopUp = function() {
		...
		pop.attachClosed(function() {
			jQuery(document.getElementById(thisId)).remove();
		});
		var dock = Popup.Dock;
		var cssClass = 'sapMInputBaseMessage' + errorState + ' sapMFocus';
		var T = 'sapMValueStateMessageError sapMText';
		var r = coreLib.getResourceBundleFor('sap.m');

		if (errorState === coreLibrary.ValueState.Success) {
	...
	ExpressionAdvanced.prototype.onAliasLinkPress = function(aliasName) {
		var expressionLanguage = Element.getElementById(this.getExpressionLanguage());
	...
	ExpressionAdvanced.prototype.aliasClickCancel = function(oEvent) {
		...
		this.fireDialog({
			dialogStatus: library.ValueListDialogMode.Close
		});
	};
	...
	ExpressionAdvanced.prototype._createValueHelpProvider = function(valueHelpInfo, bReplaceWord) {
		...
		this.oMetadataAnalyzer = new MetadataAnalyser(oModel);
	...
	ExpressionAdvanced.prototype._createCpValueHelp = function(valueHelpInfo, curPos, bReplaceWord, bValuehelpLinkPress) {
        ...
        var basePath = Element.getElementById(this.getExpressionLanguage()).getModel().sServiceUrl;
    ...
    ExpressionAdvanced.prototype._addValueHelpColumn = function(oColumName, oTable) {
        var oColumn = new Column().setLabel(new Label({
            text : oColumName
        }));
    ...
    ExpressionAdvanced.prototype._fetchFilterParams = function(valueHelpInfo) {
        var that = this;
        var aSearchParams = [];
        var aFilterParams = [ new Filter("AttributeId", FilterOperator.EQ, valueHelpInfo.metadata.attributeId),
                new Filter("DataObjectId", FilterOperator.EQ, valueHelpInfo.metadata.dataObjectId),
                new Filter("VocabularyId", FilterOperator.EQ, valueHelpInfo.metadata.vocabularyId) ];
	...
	ExpressionAdvanced.prototype._setCodeMirror = function() {
		...
		function autoComplete(cm, commandName, timeout) {
			...
			if ((oAdvancedExpressionEditor.getEditable() !== false) && Element.getElementById(oAdvancedExpressionEditor.getExpressionLanguage())) {
		...
		function ordinaryKeyCallback(cm) {
			...
			if (oAdvancedExpressionEditor.getEditable() !== false) {
				...
				oAdvancedExpressionEditor.timer = window.setTimeout(function() {
					if (oAdvancedExpressionEditor.needAutoComplete && Element.getElementById(oAdvancedExpressionEditor.getExpressionLanguage())) {
	...
	*!/
	...
	ExpressionAdvanced.prototype._handleChange = function() {
		this.codeMirror.on("change", function(cm, change) {
			...
			var oEL = Element.getElementById(oAdvancedExpressionEditor.getExpressionLanguage());
			if (oEL) {
