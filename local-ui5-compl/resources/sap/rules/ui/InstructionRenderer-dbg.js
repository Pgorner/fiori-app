sap.ui.define([
	...
	"sap/ui/core/library",
], function(Control, CoreItem, Select, Input, Button, SelectDialog, StandardListItem, Text, DatePicker,
    TypeFloat, TypeDate, TypeTime, TypeDateTime, HTML, JSONModel, Filter, FilterOperator, Utils, DateTimePicker,
    TimePicker, ValueHelpProvider, ValueHelpDialogProvider, LocaleData, Constants, Popover, Formatting, String, MessageBox, DateFormat, coreLibrary) {
    "use strict";
    ...
    return Control.extend("sap.rules.ui.InstructionRenderer", {
			...
			render: function (oRM, oControl) {
	            ...
	            if (controls) {
	                if (oControl._getIndent()) {
	                    oControl._setTableStart(oRM, oControl)
	                }
	                ...
	                for (var i = 0, len = controls.length; i < len; ++i) {
	                    ...
	                    if (isLineBreakStart && oControl._getIndent()) {
	                        oControl._setLogicalOperatorStart(oRM, oControl);
	                    }

	                    oRM.renderControl(controls[i]);

	                    if (isLineBreakEnd && oControl._getIndent()) {
	                        oControl._setLogicalOperatorEnd(oRM, oControl);
	                    }
	                }

	                if (oControl._getIndent()) {
	                    oControl._setTableEnd(oRM, oControl);
	                }
	            }
        ...
        _createValueListControl: function(instruction, nextCreateFunction, localPath, fullPath) {
            ...
            if (oValueListAnnotation) {
                ...
                if (oPopover instanceof Popover) {
        ...
        _createSelectionControl: function(instruction, nextCreateFunction, localPath, fullPath) {
            ...
            var ctrl = new Select({
                ...
                enabled: (oParent.bReadOnly && oParent.isA("sap.rules.ui.DecisionTableCellExpressionBasics")) ? false : true
            });
        ...
        _setTableStart: function(oRM, oControl) {
        	...
        	oRM.openStart("td", oControl);
        	oRM.openEnd();
        	oRm.class("sapRULExpressionBasicVerticalAlign");
        	oRM.openEnd();
        },

        _setTableEnd: function(oRM, oControl) {
        	oRm.close("td");
        	oRm.close("tr");
        	oRm.close("table");
        },

        _setLogicalOperatorStart: function(oRM, oControl) {
        	oRM.openStart("div", oControl);
        	oRm.class(this.LINE_GAP_CLASS);
        	oRM.openEnd();
        	oRm.close("td");
        	oRm.close("tr");
        ...
        _setLogicalOperatorEnd: function(oRM, oControl) {
        	oRm.close("td");
        	oRM.openStart("td", oControl);
        	oRm.class("sapRULExpressionBasicVerticalAlign");
        	oRM.openEnd();
        ...
        _getTableStart: function() {
            return '<table><td></td><td class="sapRULExpressionBasicVerticalAlign">';
        },
        ...
        _getLogicalOperatorStart: function() {
            return '<div class="' + this.LINE_GAP_CLASS + '"></td></tr><tr><td class="sapRULExpressionBasicVerticalAlign">';
        },

        _getLogicalOperatorEnd: function() {
            return '</td><td class="sapRULExpressionBasicVerticalAlign">';
        },
