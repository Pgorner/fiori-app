...
sap.ui.define([
    ...
    "./DecisionTableRenderer"
    
], function (Log, jQuery, library, DecisionTableCell, RuleBase, Utils, Table, Column, OverflowToolbar, Popover, Menu, Dialog, MenuButton, Button,
             ToolbarSpacer, Text, Input, ObjectIdentifier, Link, Label, BusyIndicator, DisplayListItem, MenuItem, FlexBox, MessageStrip,
             DecisionTableSettingsOld, DecisionTableSettings, DecisionTableCellFormatter, ConstantUtils, AstExpressionLanguage, AstUtil, Constants, LocaleData, JSONModel,
             MessageToast, Title, mobileLibrary, /*Context,*/ coreLibrary,
             TableLibrary, EventBus, Event, Sorter,
             Formatting, DateFormat, coreLib, Element, Context, RowmodesFixed, DecisionTableRenderer) {
    ...
    var oDecisionTable = RuleBase.extend("sap.rules.ui.DecisionTable", {
            ...
            associations: {
                ...
                */
				...
				ariaRoleDescription: {
					type: "sap.ui.core.Control",
                	multiple: true,
                	singularName: "ariaRoleDescription"
				}
            }
        },
        ...
        _addNewRow: function (selectedRow, bFirst) {
            ...
            oTable.setSelectedIndex(oRowData.Sequence -1);
            this._clearErrorMessages();
            this._saveWorkAround([],this.oBundle.getText("addRowSuccess"));
        },
        ...
        _addTable: function () {
			var oTable = new Table({
				...
				rowMode:new RowmodesFixed({ rowCount:this.VISIBLEROWCOUNT, rowContentHeight:50 }),
				selectionMode: {
					parts: [{
			...
			oTable.attachRowsUpdated(function() {
                for (const oRow of oTable.getRows()) {
                    for (const oCell of oRow.getCells()) {
                        this._updateTableCell(oCell, oRow.getBindingContext())
                    }
                }
            })
		...
		tooltip:this.oBundle.getText("decisionTable")
            });
        ...
        _bindRows: function () {
            ...
            var oSorter = new Sorter("Sequence");
	...
	_closePopover: function () {
        ...
        _columnFactory: function (sId, oContext) {
            ...
            var _colIfThenHeader = this._createColIfThenHeader(oContext);
            var _colDescriptionHeader = this._createColDescriptionHeader(oContext, this);
            var column = new Column(sId, {
                width: this.columnWidth,
                minWidth : 128,
                multiLabels: [
                	_colIfThenHeader, // The order of the labels is important!
                	_colDescriptionHeader
                ],
                template: this._createCell(oContext,_colDescriptionHeader.getId()) //, headerSpan: columnType === sap.rules.ui.DecisionTableColumn.Condition ? [this.iNumOfCondition, 1] : [this.iNumOfResults, 1]
            });

            relativePath = this._getBindModelName();
            this.firstResultColumnBound = oContext.getProperty(relativePath + "Type") === library.DecisionTableColumn.Result;
            if (columnType === library.DecisionTableColumn.Result) {
        ...
        _createCell: function (oContext, sAriaLabelledId) {
            ...
            var oCell = new DecisionTableCell({
                ...
                ariaLabelledBy: sAriaLabelledId,
        ...
        _createColDescriptionHeader: function (oContext, oDtControl) {
            ...
            if (columnType === library.DecisionTableColumn.Condition) {
                sPath = oContext.getModel().createKey("/DecisionTableColumnConditions", oHeaderKey);
                sDTColPath = oContext.getModel().createKey("/DecisionTableColumns", oHeaderKey);
                oAstContext = new Context(oContext.getModel(), sPath);
                if (this.getAstExpressionLanguage()) {
                ...
                oText.bindText({
                    parts: [{
                        ...
                        path: sDTColPath + "/Label"
                    }, {
                        path: headerPath + "/FixedOperator"
                    }],
                    formatter: function (sExpression, sLabel, sFixedOperator) {
                        //bug RULES-5278
                        var titleText = "";
                        if(that.getAstExpressionLanguage()){
                        	titleText = !sLabel ? displayExpression : sLabel; 
                        } else {
                        	titleText = !sLabel ? sExpression : sLabel;
                        }
                        if (sFixedOperator) {
                ...
                //Result header text
            } else if (columnType === library.DecisionTableColumn.Result) {
            ...
            return new Label({
                text: {
                    ...
                    formatter: function (sType, iSequence) {
                        if (iSequence === 1) {
                            return oBundle.getText("title_if");
                        } else if (sType === library.DecisionTableColumn.Result) {
                            return oBundle.getText("title_then");
                        } else {
                            return "";
                        }
                    }
        ...
        _createNewDecisionTableSettings: function () {
            ...
            var oDecisionTableSettings = new DecisionTableSettings({
        ...
        _cutCopyRow: function (operation) {
            ...
            MessageToast.show(this.oBundle.getText(operation + "RowSuccess"));
        },
        ...
        _decideSelectionMode: function (editable) {
            return editable ? TableLibrary.SelectionMode.MultiToggle : TableLibrary.SelectionMode.None;
        },
        ...
        _deleteRow: function () {
            ...
            MessageToast.show(this.oBundle.getText("deleteRowSuccess"));
        },
                ...
                cellArr.results.forEach(function (cell) {
                    if (cell.parserResults && cell.parserResults.status === "Success") {
                        ...
                        try {
                            ...
                            cell.Content = "";
                            Log.error("failed to pars AST cell in ColId " + cell.ColId + " RowId " + cell.RowId +
        ...
        _getOutputColumnDescription: function (oName, oLabel) {
			var expLanguage;
			if (this.getExpressionLanguage()) {
				expLanguage = Element.getElementById(this.getExpressionLanguage());
        ...
        _handleHeaderSpan: function () {
            if (!this.multiHeaderFlag) {
                ...
                for (i = 0; i < aColumns.length; i++) {
                    if (aColumns[i].Type === library.DecisionTableColumn.Condition) {
                        this.iNumOfCondition++;
                    } else if (aColumns[i].Type === library.DecisionTableColumn.Result) {
                        this.iNumOfResults++;
                    }
                }
                var oTable = this.getAggregation('_table');
        ...
        _initDataBucket: function () {
            ...
            var oEL;
            if (this.getAstExpressionLanguage()) {
                oEL = Element.getElementById(this.getAstExpressionLanguage());
            } else {
                oEL = Element.getElementById(this.getExpressionLanguage());
            }
        ...
        _initDisplayModel: function () {
            this._displayModel = new JSONModel();
        ...
        _markSelection: function (bMarkSelection) {
            ...
            if (rowPath) {
                ...
                if ((rowIndex - firstVisibleRow > -1) && (rowIndex - firstVisibleRow < oTable.getRowMode().getRowCount())) {
        ...
        _openTableSettings: function () {
            ...
            oDialog.attachBeforeClose(function (oData) {
                ...
                if (dialogState === coreLibrary.ValueState.Success) {
                ...
                if (validateResults.output.status === "Error") {
                    ...
                    for (var cell in columnErrorsMap) {
                        if (columnErrorsMap.hasOwnProperty(cell) && (typeof columnErrorsMap[cell] === "string") && columnErrorsMap[cell] !== "null") {
                            cellValueStateMap[cell] = coreLibrary.ValueState.Error;
                        }
                    }
        ...
        _rowActionWorkAround: function (operation) {
            ...
            if (operation === "insertBefore" || operation === "insertAfter") {
                MessageToast.show(this.oBundle.getText("addRowSuccess"));
            } 
            if (operation === "paste") {
                MessageToast.show(this.oBundle.getText("pasteSuccess"));
            }
        },
        ...
        _setTableRows: function () {
            ...
            var currVisibleRowCount = oTable.getRowMode().getRowCount();
        ...
        _updateTableCell: function (oCell, oRowContext) {
            ...
            if (oRowContext) {
                ...
                var oCellContext = new Context(oModel, cellBindingPath);
                ...
                case "RESULT":
                    ...
                    if (Element.getElementById(oParent.getAstExpressionLanguage())) {
                        ...
                        oCell.setAttributeInfoPath("/" + sResultDataObjectId + "/" + that.getParent()._formatAttributeId(sAttributeId));
                    } else {
        ...
        */
        ...
        renderer: DecisionTableRenderer
     ...
     */
    oDecisionTable.prototype._saveWorkAround = function (mParameters, toastMessage) {
        var oModel = this._getModel();
        oModel.submitChanges(mParameters);
        MessageToast.show(toastMessage);
    };
    
    oDecisionTable.prototype.includes = function (parent, str) {
	...
	oDecisionTable.prototype._getLocaleData = function () {
            var oLocale = Formatting.getLanguageTag();
            return LocaleData.getInstance(oLocale);
    };

    oDecisionTable.prototype._getDateFormatter = function () {
    ...
    oDecisionTable.prototype._getDateTimeFormatter = function () {
            var oLocaleData = this._getLocaleData();
            var datePattern = oLocaleData.getDatePattern('medium');
    ...
    oDecisionTable.prototype.VISIBLEROWCOUNT = 30;

	oDecisionTable.prototype.THRESHOLD = 30;
