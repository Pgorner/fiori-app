...
sap.ui.define(["sap/base/Log","sap/ui/thirdparty/jquery",
	...
	], function (Log, jQuery, library, Label, RuleBase, Panel, Title, Form, Toolbar, ToolbarSpacer, Text, Button, Grid, FormContainer, FormElement,
		ExpressionAdvanced, Link, FlexBox, Dialog, TextRuleSettings, AstYamlConverter, Constants, AstExpressionBasic, ExpressionLanguage, AstExpressionLanguage, coreLib, JSONModel, VerticalLayout, Element, EventBus, MessageToast, mobileLibrary,
		coreLibrary, IconPool, ResponsiveGridLayout, Context, HorizontalLayout, GridData, TextRuleRenderer) {
		"use strict";
		...
		var TextRule = RuleBase.extend("sap.rules.ui.TextRule", {
			...
			_addConditionBlock: function (oEvent, sType) {
				...
				mParameters.error = function () {
					Log.info("Error creating " + sKeytext + "entity");
				};
				this._isAddCallInProgress = true;
			...
			_addConditionSuccess: function (newCondition, that, data) {
				...
				var oContext = new Context(oDataModel, sPath);
				...
				oDataModel.read(sPath, {
					...
					success: function (response) {
						...
						if (response.Type === that.typeElse) {
							...
							var oElsePanel = that._createElseFormLayout(oContext, that.oBundle.getText("title_else"), true);
							oVerticalLayout.removeContent(data.nIndex);
							oVerticalLayout.insertContent(oElsePanel, data.nIndex);
							oElsePanel.addEventDelegate({
							    onAfterRendering: function(){
							    	if (data && data.bfirst && !bConditionCreated && oElsePanel.getContent().length > 0 && oElsePanel.getContent()[0].getFormContainers() && 
							    			oElsePanel.getContent()[0].getFormContainers().length > 0 && oElsePanel.getContent()[0].getFormContainers()[0].getFormElements() &&
							    			oElsePanel.getContent()[0].getFormContainers()[0].getFormElements().length > 0 && oElsePanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields() &&
							    			oElsePanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields().length > 0 && 
							    			oElsePanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields()[0]._input) {
							    		oElsePanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields()[0]._input.focus();
										bConditionCreated = true;
									}
							    }
							});
							...
							var oElseIfPanel = that._createFormLayout(oContext, that.oBundle.getText("title_elseIf") + sNumbering, true);
							oVerticalLayout.insertContent(oElseIfPanel, nNewIndex);
							that._adjustElseIfTitle(data.verticalLayout, nNewIndex, true);
							oElseIfPanel.addEventDelegate({
							    onAfterRendering: function(){
							    	if (data && data.bfirst && !bConditionCreated && oElseIfPanel.getContent().length > 0 && oElseIfPanel.getContent()[0].getFormContainers() && 
							    			oElseIfPanel.getContent()[0].getFormContainers().length > 0 && oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements() &&
							    			oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements().length > 0 && oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields() &&
							    			oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields().length > 0 && 
							    			oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields()[0]._input) {
							    		oElseIfPanel.getContent()[0].getFormContainers()[0].getFormElements()[0].getFields()[0]._input.focus();
										bConditionCreated = true;
									}
							    }
							});
						}
						that._isAddCallInProgress = false;
						that._updateBusyState(false);
						MessageToast.show(that.oBundle.getText("branchCreatedSuccess"));
					},
					error: function () {
						Log.info("Error reading TextRuleResultExpressions");
						that._isAddCallInProgress = false;
					}
				});
			...
			_adjustElseIfTitle: function (oVerticalLayout, nNewIndex, bAdded) {
				...
				var sElseIf = this.oBundle.getText("title_elseIf");
				var iterator;
				if (bAdded) {
			...
			_bindRule: function () {
				...
				if (sBindingPath && oModel) {
					...
					oModel.submitChanges({
						...
						error: function () {
							Log.info("Error reading TextRule data from backend");
						}
					});
				}
			...
			_createFormLayout: function (oContext, sTitle, bExpanded) {
				...
				var oPanel = new Panel({
					...
					content: [new Form({
                        ...
                        formContainers: [
                            // If form container
                            // Then form container
                            that._createThenFormContainer(oContext, this.oBundle.getText("title_then"))
                        ]
                    })]
				...
				if (sTitle !== this.typeIf) {
					...
					var oAddButton = new Button({
						visible: "{TextRuleModel>/editable}",
					...
					var oDeleteButton = new Button({
						...
						visible: "{TextRuleModel>/editable}",
						press: function (oEvent) {
							that.showDeleteConfirmationDialogMessage(oEvent.getSource());
						}
					}).setBindingContext(oContext);
			...
			showDeleteConfirmationDialogMessage: function(oSource) {
				var that = this;
				var oDialog = new Dialog({	
					title: this.oBundle.getText("warning_title"),
					type: DialogType.Message,
					state: ValueState.Warning,
					content: new Text({	
						text: this.oBundle.getText("condition_delete_confirmation_message")
					}),	
					afterClose: function() {	
						oDialog.destroy();	
					}	
				...
				oDialog.setBeginButton(new Button({	
					text: this.oBundle.getText("textrule_confirmation_dialog_delete"),
					press: function() {	
						that._deleteConditionBlock(oSource)	
						oDialog.close();
					}
				}));	
				oDialog.setEndButton(new Button({	
					text: this.oBundle.getText("textrule_confirmation_dialog_cancel"),
					press: function() {
						oDialog.close();
					}
				}));	
					...
					if (sConditionId === oTextRuleConditions[i].Id) {
						...
						if (oResultExpressions) {
							for (var j = 0; j < oResultExpressions.length; j++) {
								...
								var oExpressionContext = new Context(this._getModel(), sPath);
			...
			_createTextRuleSettings: function () {
				...
				var oTextRuleSettings = new TextRuleSettings({
					...
					enableSettingResult: this.getEnableSettingResult()
				});
				if (this.getAstExpressionLanguage()) {
				...
				var settingModel = new JSONModel(settingModelData);
			 ...
			 */
			_deleteConditionBlock: function (oSource) {
				var that = this;
				var oDataModel = this._getModel();
				...
				var sKeyText;
				var initialConditionCount = oVerticalLayout.getContent().length;
				var bConditionDeleted = false;
				if (sType === that.typeElse) {
					sKeyText = "/TextRuleDefaultBranches";
				...
				var _deleteSuccess = function () {
					...
					oVerticalLayout.addEventDelegate({
					    onAfterRendering: function(){
					    	if(!bConditionDeleted) {
					    		if (initialConditionCount === 3 && (nIndex === 1 || nIndex === 2)){
					    			oVerticalLayout.getContent()[nIndex].getContent()[0].focus();
					    		} else if(nIndex === initialConditionCount-2 && oVerticalLayout.getContent().length > 0 && oVerticalLayout.getContent()[nIndex] && 
					    				oVerticalLayout.getContent()[nIndex].getContent().length > 0 && oVerticalLayout.getContent()[nIndex].getContent()[0].getMetadata().getName() 
					    				=== "sap.m.Button") {
					    			oVerticalLayout.getContent()[initialConditionCount-2].getContent()[0].focus()
					    		} else if(oVerticalLayout.getContent().length > 0 && oVerticalLayout.getContent()[nIndex] && 
					    				oVerticalLayout.getContent()[nIndex].getContent().length > 0 && oVerticalLayout.getContent()[nIndex].getContent()[0].getFormContainers().length > 0) {
					    			oVerticalLayout.getContent()[nIndex].getContent()[0].getFormContainers()[0].getFormElements()[0].getFields()[0]._input.focus();
					    		}
					    		bConditionDeleted = true;
					    	}
					    }
					});
					MessageToast.show(that.oBundle.getText("branchDeletedSuccess"));
				};
				...
				oDataModel.remove(sPath, {
					...
					error: function () {
						Log.info("Error deleting " + sKeyText + "entity");
					}
				});
			},
			...
			_addTextRuleResultExpression: function (oEvent) {
                ...
                mParameters.success = function (newTextRuleExpression) {
                    ...
                    var oExpressionContext = new Context(that._getModel(), sPath);
                    ...
                    for (var i = 0; i < oTextRuleConditions.length; i++) {
                        if (sConditionId === oTextRuleConditions[i].Id && oTextRuleConditions[i].TextRuleResultExpressions) {
                            ...
                            oElement.getParent().insertFormElement(that._formElementsFactory(that.oBundle.getText("title_then") + "result" + nIndex,
                ...
                mParameters.error = function () {
                	Log.info(that.oBundle.getText("errorCreating") + sKeytext + that.oBundle.getText("entity"));
                };
                oDataModel.createEntry(sKeytext, mParameters);
            },
            ...
            _deleteTextRuleResultExpression: function (oEvent) {
                ...
                oDataModel.remove(sPath, {
                    ...
                    error: function () {
                    	Log.info(that.oBundle.getText("errorDeleting") + sKeytext + that.oBundle.getText("entity"));
                    }
                });
			...
			_formElementsFactory: function (sId, oContext) {
                ...
                var oExpressionControl = this._getExpressionAdvancedText(oContext, expression, businessDataType, sAttributeId, false, oLabel.getId());
				...
				var formElement = new FormElement({
					...
					fields: [oExpressionControl, new HorizontalLayout({
			...
			_getElseButton: function () {
				var that = this;
				if (!Element.getElementById("_elseButton")) {
					this.oElseButton = new Button({
					id: "_elseButton",
			...
			_getElseIfButton: function () {
				var that = this;
				if (!Element.getElementById("_elseIfButton")) {
					this.oElseIfButton = new Button({
					id: "_elseIfButton",
			...
			_getElseIfPanel: function () {
				var sTitle = this.oBundle.getText("title_elseIf");
				var oControls = [];
				...
				if (oElseIfData.length > 0) {
					for (var i = 0; i < oElseIfData.length; i++) {
						...
						var oContext = new Context(this._getModel(), sPath);
			...
			_getElsePanel: function () {
				var sTitle = this.oBundle.getText("title_else");
				var oControls = [];
			 ...
			 */
			_getExpressionAdvancedText: function (oContext, expression, businessDataType, attributeId, bConditionContext, sAriaValue) {
				var that = this;
				var oExpressionLanguage = null;
				var attributeInfo = "";
				var sType = businessDataType ? businessDataType : library.ExpressionType.BooleanEnhanced;
				...
				if (this.getAstExpressionLanguage()) {
					...
					if (attributeId) {
                        attributeInfo = "/" + resultDataObjectId + "/" + this._formatAttributeId(attributeId);
					}
					...
					return new AstExpressionBasic({
						...
						change: function (oEvent) {
							...
							if (astNodes && astNodes.length > 0 && astNodes[0].Type === "I" && astNodes[0].Value != "" && astNodes[0].Value != undefined) {
						...
						change: function (oEvent) {
							...
							if (parserResults.status !== "Error") {
								...
								try {
									...
									Log.error("Exception while converting ast for expression" + oSource.getValue() + " :" + e.message);
								}
			...
			_formatAttributeId: function (attributeId) {
			...
			_getExpressionFromAstNodes: function (oContext) {
				var that = this;
				var oExpressionLanguage = Element.getElementById(this.getAstExpressionLanguage());
			...
			_getIfPanel: function () {
				var sTitle = this.oBundle.getText("title_if");
			...
			_initDisplayModel: function () {
				...
				this._displayModel = new JSONModel(data);
			...
			_initInternalModel: function () {
				...
				this._internalModel = new JSONModel(data);
				this.setModel(this._internalModel, "TextRuleModel");
				var ruleBuilderModel = this.getModel("ruleBuilderInternalModel");
				var textRuleModel = this.getModel("TextRuleModel");
				if (ruleBuilderModel && textRuleModel) {
					var bEnableSettings = ruleBuilderModel && ruleBuilderModel.getObject("/textRuleConfiguration") ? ruleBuilderModel.getObject("/textRuleConfiguration").enableSettings : true;
					textRuleModel.setProperty("/enableSettings", bEnableSettings);
				}
			},
			...
			_openTextRuleSettings: function () {
				...
				oDialog.attachBeforeClose(function (oData) {
					...
					if (dialogState === ValueState.Success) {
			...
			_resetControl: function () {
				...
				var oContext = new Context(oModel, bindingContextPath);
			...
			_segregateTextRuleData: function (conditionData) {
				...
				if(conditionData.length>0){
					conditionData[0].Type=this.typeIf;
				}
			...
			_removeExistingAstNodes: function (sPath, oContext, astNodes, expressionConverted, oModel) {
				...
				} else if (that.includes(sPath, "TextRuleResultExpression")) {
			...
			setEnableSettingResult: function (value) {
				//value = true;
				this.setProperty("enableSettingResult", value, true);
				return this;
			},
			...
			setExpressionLanguage: function (value) {
				...
				var expressionLanguage = (value instanceof Object) ? value : Element.getElementById(value);
			...
			setAstExpressionLanguage: function (value) {
				...
				var astExpressionLanguage = (value instanceof Object) ? value : Element.getElementById(value);
