sap.ui.define([
	...
	"sap/ui/core/Lib",
	"sap/ui/core/Element",
	"./AstExpressionBasicRenderer"

], function (Log, jQuery, library, Control, ResponsivePopover, List, JSONModel, mobileLibrary, StandardListItem, CustomData, Sorter,
	...
	SelectFunctionDialog, TermsProvider, astBundleLibrary, LocaleData, LoopFunctionDialog, UtilsBase, EventBus, Formatting, DateFormat, Device, coreLib, Element, AstExpressionBasicRenderer) {
	"use strict";
	...
	var AstExpressionBasic = ExpressionBase.extend("sap.rules.ui.AstExpressionBasic", {
		metadata: {
			properties: {
				...
				//property to allow the Reference Context
				isReferenceContext: {
					type: "boolean",
					defaultValue: false
				},
				enableAggregateFunctionAttribute: {
					type: "boolean",
					defaultValue: false
				}
			},
		...
		init: function () {
			...
			this.loopFunctionCallBack = jQuery.proxy(this._updateLoopFunction, this);
		...
		onBeforeRendering: function () {
			...
			this._ariaLabelledById = this.getAriaLabelledBy();
			this._enableAggregateFunctionAttribute = this.getEnableAggregateFunctionAttribute();
		},
		...
		onAfterRendering: function () {
			...
			if (this.getValue() && this.getValue().trim()) {
				...
				if (!this.bTypingFlow && this._selectedSpans[0] && !this._selectedSpans[0].textContent.endsWith(Constants.DOT) && this._selectedSpans[
					...
					Constants.UNDEFINED && !this.dotInOperationsContext && !(this._enableAggregateFunctionVocabulary &&
						this._jsonArray.length === 0) && !this._isFirstTokenUpdateOrAppendOperation()) {
					this._createSpaceSpanItem();
				}
				this.bTypingFlow = false;
				if(!this.dataObjectInfo && this.getIsAttributeContext()) {
                    this.dataObjectInfo = this.getDataObjectInfo() + (this.relString ? this.relString.replace(".", "") : "");
                }
			...
			if (this._ariaLabelledById) {
				this._input[0].setAttribute("aria-labelledby",this._ariaLabelledById + " " + this._input[0].id);
			}
			this._input[0].setAttribute("aria-haspopup","list");
			// TR Condition does not have any label. Note: aria-labelledby will override aria-label
			this._input[0].setAttribute("aria-label",this._input[0].textContent);
		},
		...
		_convertInputToUiModel: function (inputText) {
			if (inputText) {
				...
				try {
					...
					Log.error("Error in parsing string to JSON: " + e.message);
				}
		...
		_getSuggestionsForTheGivenInput: function (inputText) {
			...
			var _referenceContext = this.getIsReferenceContext();
			...
			if (this._enableAggregateFunctionVocabulary) {
				_attributeContext = false;
				_associationContext = true;
				_referenceContext = true;
			}
			...
			if (this.getOperationsContext() && inputText !== "") {
				...
				} else if (this._isFirstTokenUpdateOrAppendOperation() && this._selectedSpans[0].textContent === " " + Constants.DOT) {
                    _attributeContext = true;
			...
			suggestionContext.ReferenceContext = _referenceContext;
			var suggestions = this._filterSuggestionBasedOnCardinality(uiModel, suggestionContext);
			...
			//Close the AutoSuggestion for the Vocabulary Input in the Aggregate Function Dialog when no suggestion to display.
			if(this._enableAggregateFunctionVocabulary && suggestions && suggestions.autoComplete && suggestions.autoComplete.categories && suggestions.autoComplete.categories.terms
					&& suggestions.autoComplete.categories.terms.length === 0) {
				this._oAutoSuggestionPopUp.close();
			}
			return suggestions;
		},
		
		_filterSuggestionBasedOnCardinality: function (uiModel, suggestionContext) {
		    var suggestions = this._oAstExpressionLanguage.getSuggesstions(uiModel, suggestionContext);
		    //filter suggestion to allow only 1..n cardinality for Attributes Suggestion in Aggregate Function Dialog.
            if ((this._enableAggregateFunctionAttribute || this._enableAggregateFunctionWhereClause) && suggestions && suggestions.autoComplete && suggestions.autoComplete.categories && suggestions.autoComplete.categories.terms) {
                for(var index = 0; index<suggestions.autoComplete.categories.terms.length; index++){
                	if((suggestions.autoComplete.categories.terms[index].cardinality && suggestions.autoComplete.categories.terms[index].cardinality === Constants.ONE_TO_N)
                    		|| (suggestions.autoComplete.categories.terms[index].type && 
                    				suggestions.autoComplete.categories.terms[index].type === "R" 
                    			&& suggestions.autoComplete.categories.terms[index].isDataObjectTable)) {
                        suggestions.autoComplete.categories.terms.splice(index,1);
                    }
                }
            }
            return suggestions;
		...
		_addDotToMiscellaneousSuggestions: function () {
			if (this.relString && this.relString.trim().toUpperCase().indexOf(Constants.UPDATE) === 0) {
				...
				if (lastTermDOType === Constants.ASSOCIATIONDOTYPE || lastTermDOType === Constants.REFERENCEDOTYPE) {
					return true;
		...
		_constructExpandedRelString: function (jsonData) {
			...
			if (jsonData.function === Constants.AVG || jsonData.function === Constants.SUM || jsonData.function === Constants.MIN || jsonData.function ===
				Constants.MAX) {
				attribute = (typeof jsonData.vocabulary === "string" || jsonData.vocabulary instanceof String) ? jsonData.attribute : this._fetchAttributesSelectFunction(
						jsonData);
			...
			} else if (jsonData.function === Constants.FOREACH) {
				return jsonData.function + "(" + this.getTable(jsonData) + " , " + this._fetchForEachOperations(jsonData.actions) + ")";
			}
		},
		
		_fetchForEachOperations: function (aOperations) {
			var sOperations = "";
			for (var iterator = 0; iterator < aOperations.length - 1; iterator++) {
	            if (aOperations[iterator] !== "") {
	            	sOperations = sOperations + aOperations[iterator] + " , ";
	            }
	        }
			sOperations = sOperations + aOperations[iterator];
			return sOperations;
		...
		_fetchAttributesSelectFunction: function (jsonData) {
				if (this._hasAssociationId(jsonData.vocabulary, jsonData.function)
						|| this._hasReferenceId(jsonData.vocabulary, jsonData.function)) {
					return this._returnAttributeBasedOnType(jsonData.vocabulary);
				} else if (jsonData.vocabulary.attributes && Object.keys(jsonData.vocabulary.attributes).length === 1) {
					for (var key in jsonData.vocabulary.attributes) {
						return key;
					}
				}
			},
		...
		getTable: function (jsonData) {
			var dataObjectInfo = (typeof jsonData.vocabulary === "string" || jsonData.vocabulary instanceof String) ? jsonData.vocabulary :
					this._getDataObjectInfoBaseOnVocaulary(jsonData);
		...
		_getDataObjectInfoBaseOnVocaulary: function(jsonData) {
				var dataObjectInfo = jsonData.dataObject;
				if (this._hasAssociationId(jsonData.vocabulary, jsonData.function)
						|| this._hasReferenceId(jsonData.vocabulary, jsonData.function)) {
					dataObjectInfo = this._returnDataObjectBasedOnType(jsonData.vocabulary);
				}
				return dataObjectInfo;
		},
		...
		_hasReferenceId: function (vocabId, functionSelected) {
			var vocabularyId = "";
			if ((typeof vocabId === "object" || vocabId instanceof Object)) {
				if (vocabId && vocabId.attributes) {
					for (var key in vocabId.attributes) {
		...
		_returnAttributeBasedOnType: function (vocabId) {
			var vocabularyId = "";
			if ((typeof vocabId === "object" || vocabId instanceof Object)) {
				if (vocabId && vocabId.attributes) {
					for (var key in vocabId.attributes) {
			...
			if (vocabularyId) {
				...
				if (aSplitArray.length > 2) {
					for (var iterator = 2; iterator < aSplitArray.length; iterator++) {
						...
						if (term && term._dataObjectType === "AO" && term._cardinality === "1..n") {
							index = iterator;
						} else if (term && term._dataObjectType === "R" && term._isDataObjectTable) {
	                        index = iterator;
	                    }
					}
					if (index > 0) {
						for (var iterator = index + 1; iterator < aSplitArray.length; iterator++) {
							attributeId  += "/" + aSplitArray[iterator];
						}
					}
					if (index === 0) {
						for (var iterator = 2; iterator < aSplitArray.length; iterator++) {
				...
				if (attributeId) {
					return Constants.DOT + attributeId ;
				} else {
		...
		_returnDataObjectBasedOnType: function (vocabId) {
			var vocabularyId = "";
			if ((typeof vocabId === "object" || vocabId instanceof Object)) {
				if (vocabId && vocabId.attributes) {
					for (var key in vocabId.attributes) {
				...
				if (aSplitArray.length > 2) {
					...
					if (index > 0) {
						for (var iterator = 1; iterator <= index; iterator++) {
							dataObjectId += "/" + aSplitArray[iterator];
						}
					}
				}
				if (dataObjectId) {
		...
		_createSpanItem: function (oItem) {
			...
			if (oItem.tokenType === Constants.TERM) {
				...
				if (objectFromReference) {
					if ("dataObjectType" in oItem && oItem.dataObjectType != Constants.Element && !this._enableAggregateFunctionVocabulary && !this.ruleWithElementDO) {
						...
						if (this._addDotToReferenceText(oItem)) {
							...
							Constants.TERM && oItem.dataObjectType != Constants.Element && !this.ruleWithElementDO && !this._enableAggregateFunctionVocabulary)) {
						oItem.text = objectFromReference;
						if (this._addDotToReferenceText(oItem)) {
							oItem.text = objectFromReference + Constants.DOT;
						}
			...
			if (oItem.tokenType === Constants.DATEBUSINESSDATATYPE || oItem.tokenType === Constants.TIMEBUSINESSDATATYPE) {
                ...
                var text = oItem.text.replace(/\'/g, "");
                var sDateFormatted = formatter.format(formatter.parse(text))
                if (sDateFormatted !== "") {
                    oItem.text = "'" + sDateFormatted + "'";
                }
			}
		...
		_isFirstTokenUpdateOrAppendOperation: function () {
			var bOperation = false
			if (this.relString && (this.relString.trim().toUpperCase().indexOf(Constants.UPDATE) === 0 || 
					this.relString.trim().toUpperCase().indexOf(Constants.APPEND) === 0)) {
				bOperation = true;
			}
			return bOperation;
		},
		...
		_addDotToReferenceText: function (oItem) {
			if (this._uiModel && this.nIndexOfToken && this._uiModel[this.nIndexOfToken] && oItem && 'startIndex' in oItem && 
				this._uiModel[this.nIndexOfToken].startIndex !== oItem.startIndex) {
                return false;
            }
			var bOperationDot = this._isFirstTokenUpdateOrAppendOperation() && this.dotInOperationsContext;
			if (bOperationDot || (!bOperationDot && this._attributeContext) || (!bOperationDot && oItem.dataObjectType === "AO") ||
					(!bOperationDot && oItem.dataObjectType === Constants.REFERENCEDOTYPE) || 
					(!this._isFirstTokenUpdateOrAppendOperation() && (oItem.dataObjectType === "S" || oItem.dataObjectType === "T"))) {
		...
		_setUpEventHandlers: function () {
			...
			this._input.on('keydown', function (event) {
				if(event.key != "Tab"){
					that.aKeyTracker.push({key:event.key, keyCode:event.keyCode, pressed: true});
				}
				...
				if (event.keyCode === 8 || event.keyCode == 46) { // BackSpace Keyup event handling 
				...
				} else if (isMac && event.key === "Alt") {
					that.optionPressed = true;
				} else if ((that.ctrlPressed && (event.keyCode === 32 || event.key === " ")) || (isMac && that.optionPressed && (event.keyCode === 32 || event.key === " ")) ) {
					that._initializeCursorSpan();
					that.ctrlPressed = false;
					that.optionPressed = false;
					...
					var iIndex = that.aKeyTracker.findIndex(function (oItem) { return oItem.pressed === true; });
					if ( iIndex !== -1) {
						var e = jQuery.Event('keyup');
						e.which = that.aKeyTracker[iIndex];
						e.key = that.aKeyTracker[iIndex].key;
						e.keyCode = that.aKeyTracker[iIndex].keyCode;
						$this.trigger(e);
					}
			...
			this._input.on("click", function (event) {
				...
				if (that._selectedSpans && that._selectedSpans[0] && that._selectedSpans[0].getAttribute(Constants.JSON)) {
					...
					try {
						...
						if (json.function === Constants.FOREACH) {
							that.loopFunctionDialog._createLoopFunctionDialog(_suggestionsList.autoComplete.categories.functions, that.loopFunctionCallBack,
								that._oAstExpressionLanguage, that._isDialogOpenedCallbackReference, json);
						} else if (json.function === Constants.TOP || json.function === Constants.BOTTOM || json.function === Constants.SELECT || json.function === Constants.FIRST) {
			...
			this._input.on('keyup', function (event) {
				if (event.key != "Tab") {
					var iKeydownIndex = that.aKeyTracker.findIndex(function(oItem){return oItem.keyCode == event.keyCode});
					that.aKeyTracker.splice(iKeydownIndex,1);
				}
				...
				if (event.key === "Control" || (Device.os.name === "mac" && event.key === "Alt") || event.keyCode === 35 || event.keyCode === 36 || event.keyCode === 37 || event.keyCode === 39 || event.keyCode === 20 || event.keyCode === 144) {
					return;
				}
				...
				if (currentText !== lastText) {
					if(lastText? lastText.trim() == "" : true && currentText.trim() == ""){
						this.textContent = "";
						return;
					}
		...
		_detachEventHandlers: function () {
			if (this._input) {
				...
				this._input.off('focusout');
				this._input.off('focus');
			}
		},
		...
		_handleBackSpaceAndDeleteKey: function (event) {
			...
			if (that._selectedSpans[0].className === "sapAstObjectClass") {
				...
				if ((that.filterTextLength > 0 && !isDeleteKey) || isDeleteKey) {
                    ...
                    if(referenceId) {
                    	term = termsProvider.getTermByTermId(referenceId);
                    }
                    if (term && (term._dataObjectType === "AO" || term._dataObjectType === Constants.REFERENCEDOTYPE)) {
                        reference = refArray[0] === Constants.DOT ? Constants.DOT + reference : reference;
                        this.isDotRequired = false;
                    } else if(term && (term._dataObjectType === "T" || term._dataObjectType === "S")) {
                        this.isDotRequired = false;
                    } else {
		...
		_handlePopupFocusOut: function (event) {
			...
			if (relatedTarget && currentTarget !== relatedTarget.id && !this._isAutoSuggestionPopover(relatedTarget)) {
				if (this._oAutoSuggestionPopUp.isOpen()) {
					this._closeAutoSuggestionPopup();
				}
				if (!this.isPopoverOpening){
		...
		_isAutoSuggestionPopover: function (element) {
			var els = [];
			var bIsAutoSuggestionPopover = false
			while (element) {
				if (element.id && element.id.indexOf("autoCompletePopover") !== -1) {
					bIsAutoSuggestionPopover = true;
					break;
				}
				els.unshift(element);
				element = element.parentNode;
			}
			return bIsAutoSuggestionPopover;
		},
		...
		_handleKeyUpEvent: function (currentText, event, bPrintable) {
			...
			var tokens = this._oAstExpressionLanguage.getTokensForGivenStringInput(this.relString);
			...
			var span = this._selectedSpans ? this._selectedSpans[0] : null;

			if (this.bEditInBetween && event.key === "'" && span &&
				span.nextSibling && 'getAttribute' in span.nextSibling &&
				(span.nextSibling.getAttribute("tokenType") === 'U' || span.nextSibling.getAttribute("tokenType") === 'N')) {
			...
			} else if (this.bEditInBetween && event.key === "'" && span &&
				span.nextSibling && 'getAttribute' in span.nextSibling && span.nextSibling.className === 'sapAstLiteralClass') {
				this._setCaretPosition({
					spanIndex: this.nIndexOfToken + 1,
			...
			if (currentText !== "" && event.key !== 'Backspace') {
				...
				if (this._selectedSpans[0] && this._selectedSpans[0].className !== "" && this._selectedSpans[0].className !== "sapAstLiteralClass" ||
					(this._selectedSpans[0] && this._selectedSpans[0].getAttribute(Constants.TOKEN_TYPE) ===
		...
		_updateUIModelForFreeFlow: function (sChar) {
			...
			if (this._uiModel.length === 0) {
				...
				if (sChar === Constants.DOT && this.getOperationsContext()) {
					...
					if (span.textContent.startsWith(sChar) && this.bEditInBetween) {
		...
		_filterSuggestionList: function (sText) {
			...
			if (this._suggestionsList.autoComplete.valuehelp) {
				aFilteredSuggestions.autoComplete.valuehelp = this._suggestionsList.autoComplete.valuehelp;
			}
            ...
            var createFilteredList = function () {
                ...
                if (oSuggestions.functions) { //Functions
                    ...
                    var loopFunctions = oSuggestions.functions.loop;
					if(loopFunctions && loopFunctions.length > 0){
						var aList = fnCheckMatchesAndAdd(loopFunctions);
                        if (aList && aList.length > 0) {
                            aFilteredSuggestions.autoComplete.categories.functions.loop = aList;
                        }
					}
					
					var defaultOperations = oSuggestions.functions.operations;
					if(defaultOperations && defaultOperations.length > 0){
						var aList = fnCheckMatchesAndAdd(defaultOperations);
                        if (aList && aList.length > 0) {
                            aFilteredSuggestions.autoComplete.categories.functions.operations = aList;
                        }
					}
                }
		...
		_fireChange: function (textContent) {
			...
			if (this._getValueAfterReplacingSpace(this.getValue()) !== this._getValueAfterReplacingSpace(this.relString)) {
		...
		//below function replaces space that don't occur between single quotes
		_getValueAfterReplacingSpace: function(valueToBeReplaced){
			if(valueToBeReplaced){
				return valueToBeReplaced.replace(/([^']+)|('(?:[^'\\]|\\.)+')/g, function($0, $1, $2) {
					if ($1) {
		...
		//Updates json for select link span after click and apply
		_updateLoopFunction: function (oEvent) {
			this.isDialogOpen = false;
			this._selectedSpans[0].setAttribute(Constants.JSON, JSON.stringify(oEvent.getSource().mProperties.jsonData));
			this._selectedSpans[0].textContent = oEvent.getSource().mProperties.value;
			this._updateUiModel();
		...
		_showAutoSuggestion: function (suggestionContent, bExpand) {
			...
			var that = this;
			setTimeout(function(){
				if (that._oAutoSuggestionPopUp && !that._oAutoSuggestionPopUp.isOpen() && that._focusedOut == false) {
					that._oAutoSuggestionPopUp.destroyContent();
					that._oAutoSuggestionPopUp.addContent(new AutoCompleteSuggestionContent({
					...
					that._oAutoSuggestionPopUp.setOffsetX(xPos);
					that._oAutoSuggestionPopUp.setOffsetY(yPos);
					that._oAutoSuggestionPopUp.openBy(that.getDomRef("input"));
				} else {
					that._oAutoSuggestionPopUp.destroyContent();
					that._oAutoSuggestionPopUp.addContent(new AutoCompleteSuggestionContent({
						content: suggestionContent.autoComplete,
				...
				if (!that._oAutoSuggestionPopUp.isOpen() && that._focusedOut == false) {
					that._oAutoSuggestionPopUp.setOffsetX(xPos);
					that._oAutoSuggestionPopUp.setOffsetY(yPos);
					that._oAutoSuggestionPopUp.openBy(that.getDomRef("input"));
				}
		...
		_isLastTokenSpaceItem: function () {
			...
			if (oLastItem && oLastItem.tokenType !== Constants.WS && oLastItem.dataObjectType && oLastItem.dataObjectType !== "S" && oLastItem.dataObjectType !==
				"T" && oLastItem.dataObjectType !== "AO" && oLastItem.dataObjectType !== Constants.REFERENCEDOTYPE && (oLastItem.tokenType === Constants.TERM && !oLastItem.getText().endsWith(Constants.DOT))
			) {
				...
				isLastTokenSpace = false;
			} else if (oLastItem && oLastItem.tokenType !== Constants.WS && this._isFirstTokenUpdateOrAppendOperation() && !this._attributeContext) { // last token has update add space 
				isLastTokenSpace = false;
		...
		_addItemToEmptyInput: function (oEvent, newSpan, spanId, item) {
			...
			this._uiModelToSpan(this._uiModel);
			this._selectedSpans = newSpan;
			if (this.getIsAttributeContext() && !this.isAssociation && !this.isReference &&
	...
	// add single quotes to the input text if the context is an attribute of string type
	_getInputStringFormattedBasedOnContext: function(oEvent){

		// return if already starts with a quote
		if(oEvent.getParameter("newValue")[0] == "'" || oEvent.getParameter("newValue")[0] == "\""){
			return;
		}
		var sHeaderInfo = this.getHeaderInfo() && this.getHeaderInfo().headerValue? this.getHeaderInfo() && this.getHeaderInfo().headerValue : null;
		var sAttributeInfo = this.getAttributeInfo();
		var sContextAttrId = "";

		if(this.relString){
			sContextAttrId = this.relString;
		}
		else if(sHeaderInfo){
			sContextAttrId = sHeaderInfo;
		}
		else if(sAttributeInfo){
			sContextAttrId = sAttributeInfo;
		}

		if(oEvent.mParameters && oEvent.mParameters.value){

			if(sContextAttrId){
				var sTargetIds = null;
				var aSplitValues = sContextAttrId.split(" ");
				for(var i=aSplitValues.length-1; i>=0; i--){
					if(aSplitValues[i].startsWith("/")){
						sTargetIds = aSplitValues[i];
						break;
					}
				}

				if(sTargetIds){
					sContextAttrId = sTargetIds.substring(1, sContextAttrId.length);
					sContextAttrId = sContextAttrId.replaceAll("/", ".");
					var oTerm = this._oAstExpressionLanguage._astBunldeInstance.TermsProvider.TermsProvider.getTermByTermId(sContextAttrId);
					if(oTerm && oTerm.getBusinessDataType()){
						if(oTerm.getBusinessDataType() == "S"){
							oEvent.mParameters.newValue = "\'" + oEvent.mParameters.newValue + "\'";
						}
					}
				}
		...
		_setTextOnCursorPosition: function (oEvent) {
			...
			if(oEvent.getParameter("newValue")){
				this._getInputStringFormattedBasedOnContext(oEvent);
			}
	    ...
	    if(oEvent.getParameter("newValue")){
		item.text = oEvent.mParameters.newValue;
	    }
			...
			if (this._uiModel.length === 0) {
				...
				if (this.isAssociation || this.isReference) {
	                this._cursorPostion += 1;
	            }
			} else {
		...
		_getItemFromSuggestionModel: function (oEvent, previousId) {
			...
			if (oSource && oSource.getBindingContext()) {
				...
				if (oBindingContextObject.ResultDataObjectId) {
					...
					if (this.getDataObjectInfo() && this.isDotRequired && !oBindingContextObject.isParentEntity) {
						item.reference = oBindingContextObject.id ? "./" + oBindingContextObject.id : "";
					} else {
						this.isDotRequired = true;
						item.reference = oBindingContextObject.id ? "/" + oBindingContextObject.id : "";
			...
			if (this.getDataObjectInfo() && !this.bEditInBetween) {
				...
				} else if (this.getIsAttributeContext() && item.text == ".") {
		...
		_createObjectSpanItem: function (item, previousId, oBindingContextObject) {
			...
			item.text = (oBindingContextObject.label !== null && oBindingContextObject.label !== "") ? oBindingContextObject.label :
				oBindingContextObject.name;
			if (!(oBindingContextObject.type !== Constants.ASSOCIATIONDOTYPE || oBindingContextObject.type !== Constants.REFERENCEDOTYPE)) {
				item.reference = oBindingContextObject.id ? "/" + oBindingContextObject.id : "";
				if (previousId && previousId.indexOf("-") > 0) {
					previousId = previousId.substring(previousId.indexOf("-") + 1)
				}
				...
				} else if (oBindingContextObject.type === Constants.REFERENCEDOTYPE) {
					this.isReference = true;
				}
				//Add the ./ to Associations if it has a relative path
				if (this.getDataObjectInfo() && this.isDotRequired && oBindingContextObject.type !== 'S' && oBindingContextObject.type !== 'T') {
					item.reference = oBindingContextObject.id ? "./" + oBindingContextObject.id : "";
					this.isDotRequired = false;
				} else {
					item.reference = oBindingContextObject.id ? "/" + oBindingContextObject.id : "";
			...
			item.bAddSpaceAutoMatically = false;
			if (this._isFirstTokenUpdateOrAppendOperation() && this.getOperationsContext() && !this.getConditionContext()) {
				this._attributeContext = false;
				item.bAddDotAutomatically = false;
				item.bAddSpaceAutoMatically = false;
				this.isDotRequired = false;
		...
		_createPopver: function () {
			...
			if (!this._oAutoSuggestionPopUp) {
				this._oAutoSuggestionPopUp = new ResponsivePopover("autoCompletePopover_" + this._createUUID(),{
					content: new AutoCompleteSuggestionContent(),
					showArrow: false,
					placement: PlacementType.Vertical,
					horizontalScrolling: true,
					showHeader: true,
					contentWidth: "400px",
					title: this.oBundle.getText("astSuggestionDialogTitle"),
					initialFocus: that, //To keep focus on input even after popup is opened
					afterOpen: function () { //Needed to set cursor for text rule
						// focus should be set on the popover after its opened
						this.focus();
						that.isPopoverOpening = false;
                        var position;
                        if (that.bEditInBetween) {
		...
		_updateUiModel: function () {
			...
			if (this.getDomRef("input")) {
				...
				for (var iterator = 0; iterator < children.length; iterator++) {
					...
					if(!(this.getDomRef("input").children && this.getDomRef("input").children.length == 1 && this.getDomRef("input").children[0].outerHTML == "<br>")){
						this._uiModel.push(item);
					}
				}
 	   ...
 	   renderer: AstExpressionBasicRenderer
	});
