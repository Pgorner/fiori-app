...
sap.ui.define([
    ...
    function (jQuery, library, DecisionTableCellExpressionAdvanced, DecisionTableCellFormatter, DecisionTableCellExpressionBasic, Popover,
        Constants, ConstantUtils, AstExpressionLanguage, AstUtil, LocaleData, Control, Input, Context, coreLibrary, Formatting,
        DateFormat, EventBus, coreLib, Element, Fragment, DecisionTableCellAstExpressionBasic, DecisionTableCellRenderer) {
        "use strict";
         ...
         */
        var DecisionTableCell = Control.extend("sap.rules.ui.DecisionTableCell", {
            metadata: {
            ...
            renderer: DecisionTableCellRenderer
        });
        ...
        DecisionTableCell.prototype._createStaticControl = function () {
            ...
            if(bEditable) {
            	oDisplayedControl.addStyleClass("sapRULDecisionTableSCellEditable");
            } else {
            	oDisplayedControl.removeStyleClass("sapRULDecisionTableSCellEditable");
            }
            ...
            var oCellContext = new Context(this.getModel(), sPath);
            ...
            if (valueStatePath && valueStatePath !== "null" && !this.getAstExpressionLanguage()) {
                oDisplayedControl.bindProperty("valueState", {
                    path: valueStatePath,
                    formatter: function (sValue) {
                        var valueState = coreLibrary.ValueState.None;
                        if (sValue === coreLibrary.ValueState.Error) {
                            valueState = coreLibrary.ValueState.Error;
                        }
                        return valueState;
                    }.bind(this)
                });
        ...
        DecisionTableCell.prototype.onFocusIn = async function () {
            ...
            var fnAfterPopOverClose = function (oEvent) {
                if (this._oPopover) {
                    ...
                    if (oDecisionTableCellExpression.isA("sap.rules.ui.ExpressionAdvanced")) { //after choose hint popover close, we fix it with focus in DecisionTableCellExpressionAdvanced so we need to copy the value from the code mirror to the expression advanced value
                    ...
                    } else if (oDecisionTableCellExpression.isA("sap.rules.ui.AstExpressionBasic")) {
            ...
            // if pop over is still open but user clicks on other cell, reference to older popover remains. Popover binding has to reset each time.
	        this._oPopover = null;
            if (!this._oPopover) {
                ...
                if (Element.getElementById("popover")) {
                    Element.getElementById("popover").destroy();
                }
                ...
                if (this.getAstExpressionLanguage()) {
                    this._oPopover = await Fragment.load({
											name: 'sap.rules.ui.fragments.DecisionTableCellAstExpressionBasic',
                                            controller: this
                                           })
                    ...
                    oDecisionTableCellExpression.attachChange(DecisionTableCellAstExpressionBasic.prototype._changeValue);
                } else {
                    if (sdecisionTableFormat === library.DecisionTableFormat.RuleFormat) {
                        if (sValueRuleFormat.toUpperCase() === library.RuleFormat.Advanced) {
                            this._oPopover = await Fragment.load({
													name: 'sap.rules.ui.fragments.DecisionTableCellExpressionAdvanced',
                                                    controller: this
                                                    })
                        } else {
                            this._oPopover = await Fragment.load({
                                                    name: 'sap.rules.ui.fragments.DecisionTableCellExpressionBasic',
                                                    controller: this
                                                    })
                        }
                ...
                var num = jQuery(document.getElementById(this.getId())).closest('td').width() * 0.93;
                var sWidth = num + "px";
        ...
        DecisionTableCell.prototype.setDecisionTableCellType = function (value) {
            if(value != null && value.isA("sap.rules.ui.type.DecisionTableCell")) {
                this.setProperty("decisionTableCellType", value);
            }
        };
